...
| `bank_id_template` | — | 用于动态生成内存库名称的模板。支持占位符：`{profile}`、`{workspace}`、`{platform}`、`{user}`、`{session}`。例如：`hermes-{profile}` 可按当前活跃的 Hermes 用户配置隔离不同内存库。若所有占位符均为空，则最终名称简化为 `hermes` |
| `bank_mission` | — | 用于指定推理时的任务类型（身份/框架设定）。通过 Banks API 实现该功能 |
| `bank_retain_mission` | — | 决定哪些信息会被保留。同样通过 Banks API 控制 |

### 回忆功能设置

| 键值 | 默认值 | 描述 |
|------|--------|------|
| `recall_budget` | `mid` | 回忆的详细程度：`low`/`mid`/`high` |
| `recall_prefetch_method` | `recall` | 自动回忆方式：`recall`（直接返回原始事实）或 `reflect`（通过大语言模型进行综合生成） |
| `recall_max_tokens` | `4096` | 回忆结果的最大 token 数 |
| `recall_max_input_chars` | `800` | 自动回忆时输入查询的最大字符数 |
| `recall_prompt_preamble` | — | 为回忆出的记忆内容添加的定制前缀 |
| `recall_tags` | — | 用于筛选记忆内容的标签 |
| `recall_tags_match` | `any` | 标签匹配模式：`any`/`all`/`any_strict`/`all_strict` |
| `recall_types` | `observation` | 回忆过程中可获取的事实类型（包括自动回忆及 `hindsight_recall` 工具生成的结果）。支持逗号分隔的字符串或 JSON 列表。**默认值已限制为仅 `observation` 类型**（详见下文“行为变更”部分）。如需同时包含原始事实，可设置为 `observation,world,experience` |
| `auto_recall` | `true` | 在每轮对话开始前自动回忆相关记忆 |

> **行为变更 —— `recall_types` 的默认值现已改为仅 `observation`。**
>
> 之前版本中，回忆功能会返回所有三种类型的事实；而现在仅返回观察结果。
>
> 根据 [Hindsight 的文档](https://hindsight.vectorize.io/developer/observations)，观察结果是基于原始事实构建的**整合型**知识层：这些知识经过去重处理，以证据为依据，并会随着新事实的加入而持续优化，同时还会标注证据数量和新鲜度。而原始的 `world`/`experience` 类型事实则是构成观察结果的各个独立证据。在每轮对话中注入上下文时，观察结果单位 token 所承载的信息更丰富，且无需向模型提供已被单个观察结果概括的多个原始事实。
>
> 如需恢复原有的广泛回忆功能，可在 `~/.hermes/hindsight/config.json` 中将 `recall_types` 设置为 `"observation,world,experience"`（字符串或 JSON 列表形式）。此设置同时适用于自动回忆功能及 `hindsight_recall` 工具——因为两者读取的都是相同的 `recall_types` 设置（该工具的架构中不存在针对单次调用的 `types` 参数），因此默认值的限制会同时影响这两种功能。
...
| `retain_source` | — | 可选字段，用于为保留的记忆内容附加 `metadata.source` 标识 |
...
| 键值 | 默认值 | 描述 |
|------|--------|------|
| `memory_mode` | `hybrid` | 内存如何集成到智能体中的方式 |

**`memory_mode` 的可选值：**
- `hybrid` —— 同时实现自动上下文注入，并向大语言模型提供相关工具
- `context` —— 仅自动注入上下文，不向模型提供任何工具
- `tools` —— 仅提供工具功能，不进行自动上下文注入
...
| `llm_model` | 各提供商对应的具体模型名称 | 例如 `gpt-4o-mini`、`qwen/qwen3.5-9b` 等 |
| `llm_base_url` | — | `openai_compatible` 类型模型对应的端点地址，例如 `http://192.168.1.10:8080/v1` |
...
| 工具名称 | 描述 |
|----------|------|
| `hindsight_retain` | 支持自动实体提取，用于存储信息；允许为每次调用指定可选的标签 |
| `hindsight_recall` | 采用多策略搜索方式（语义搜索 + 实体图搜索） |
| `hindsight_reflect` | 基于大语言模型实现跨内存库的综合生成功能 |
...
| `HINDSIGHT_LLM_API_KEY` | 本地模式使用的大语言模型 API 密钥 |
| `HINDSIGHT_API_LLM_BASE_URL` | 本地模式下大语言模型的基础地址（如 OpenRouter 的地址） |
| `HINDSIGHT_API_URL` | 可用于覆盖默认的 API 端点地址 |
| `HINDSIGHT_BANK_ID` | 可用于覆盖内存库名称 |
| `HINDSIGHT_BUDGET` | 可用于覆盖回忆功能的详细程度设置 |
| `HINDSIGHT_MODE` | 可用于覆盖当前运行模式（`cloud`、`local_embedded`、`local_external`） |

## 客户端版本要求

该插件需要 `hindsight-client >= 0.4.22` 版本。若检测到安装的版本较旧，插件会在会话启动时自动升级。
