...
> 使用 `hermes honcho setup` 也能实现相同功能，但前提是 **Honcho** 已成为当前活跃的内存提供方——因为 `honcho` 子命令仅对活跃的提供方有效。在全新安装时，请使用 `hermes memory setup honcho`。

## 架构概览

### 双层上下文注入机制

为保留提示词缓存，上下文会在 API 调用时注入到**用户消息**中（而非系统提示词）中。仅静态模式的相关头部信息会出现在系统提示词中。注入的上下文内容会被包裹在 `<memory-context>` 标签内，并附有系统说明，表明其为背景数据而非新的用户输入。

该机制包含两个独立层，各自按照不同的节奏更新：
...
会对用户进行多轮 `.chat()` 推理，这些结果会附加在基础上下文之后。

这两层内容会被合并，随后通过 `_truncate_to_budget` 函数根据 `contextTokens` 限制进行截断处理（每个标记对应4个字符，且会保留单词边界）。

### 最新消息查询重写功能（可选）

当 `queryRewrite: true` 时，第一轮辩证推理会首先使用共享的 `memory_query_rewrite` 辅助任务，将最新消息转化为一个简洁的记忆检索问题。该重写后的问题会被用于后续的辩证推理；而基础上下文检索则仍直接使用原始消息作为搜索查询。如果重写过程超时或返回无效结果，插件将会回退到下方的传统冷/热提示词。启用此功能后，通用的辩证预处理步骤将被跳过，以避免其干扰用户的第一个消息。

**默认为关闭状态**——该重写功能每个辩证周期只会增加一次辅助模型调用（而非每轮推理都调用）。建议在 `hermes model` -> auxiliary models -> **Memory query rewrite** 中选择一种快速且成本较低的模型；其在 config.yaml 文件中的请求超时时间由 `auxiliary.memory_query_rewrite.timeout` 指定（默认为8秒）。该任务及对应模块（`plugins/memory/query_rewrite.py`）与具体内存提供方无关，任何内存提供方均可重复使用。周期运行的频率仍由 `dialecticCadence` 控制。

### 冷启动与会话热状态提示词的区别

当无法使用最新消息重写功能时，第一轮辩证推理会自动根据会话状态选择对应的备用提示词：

- **冷状态**（未缓存基础上下文）： “这个人是谁？他的偏好、目标和工作风格是什么？请提供有助于 AI 助手立即发挥作用的事实信息。”
- **热状态**（已存在基础上下文）： “结合本次会话中已讨论的内容，哪些关于该用户的背景信息与当前对话最为相关？请优先考虑当前对话中涉及的最新信息，而非仅依赖个人履历信息。”

此设置不可配置，系统会自动判定。

### 辩证推理深度（多轮推理）

`dialecticDepth` 参数的取值范围为1–3，最终值会被限制在指定范围内，用于控制每个辩证周期内会触发多少次 `.chat()` 调用：

| 深度 | 推理轮数 | 行为说明 |
|------|----------|----------|
| 1 | 单次 `.chat()` 调用 | 仅执行基础查询（无论处于冷状态还是热状态） |
| 2 | 审核 + 综合 | 第0轮的结果会先经过自我审核，第1轮则进行针对性综合处理。如果第0轮已输出内容较长（超过300字符），或结构复杂且包含超过100字符的列表/分节内容，则会触发条件性跳过该轮 |
| 3 | 审核 + 综合 + 协调统一 | 第2轮会将前几轮推理中出现的矛盾点进行协调，最终形成综合结论 |

### 比例式推理层级设置

当未设置 `dialecticDepthLevels` 时，每轮推理都会使用相对于“基础层级” `dialecticReasoningLevel` 的比例级数：
...
可通过 `dialecticDepthLevels` 参数进行手动覆盖，即为每轮推理指定明确的层级字符串数组。

### 基于查询长度的自适应推理层级

系统会自动根据查询长度调整 `dialecticReasoningLevel` 的强度：查询长度≥120字符时提升1级，≥400字符时提升2级，最终层级不会超过 `reasoningLevelCap` 的限制（默认为“高”）。若将 `reasoningHeuristic` 设置为 `false`，则可强制所有自动调整都使用 `dialecticReasoningLevel` 这一固定层级。

### 三个相互独立的辩证调控参数

| 参数名 | 控制内容 | 类型 |
|--------|----------|------|
| `dialecticCadence` | 辩证推理的触发频率——即两次推理之间至少需要经过的轮数 | 整数 |
| `dialecticDepth` | 每次触发时进行的推理轮数（1–3） | 整数 |
| `dialecticReasoningLevel` | 每次 `.chat()` 调用的推理强度上限 | 字符串 |

### 输入内容净化处理

在处理用户输入之前，`run_conversation` 函数会先移除其中泄露的 `<memory-context>` 标签内容。但如果 `saveMessages` 功能将包含已注入上下文的对话轮次保存下来，这些内容仍可能通过消息历史记录出现在后续对话中。此时净化函数会同时移除 `<memory-context>` 标签及其相关的系统说明文字。
...
| 工具名称 | 是否调用大语言模型 | 功能描述 |
|----------|------------------|----------|
| `honcho_profile` | 否 | 显示用户信息卡片——即用户关键事实的快照 |
| `honcho_search` | 否 | 支持跨会话的消息检索（结合语义分析和关键词搜索，返回按相关性排序的摘录；默认检索长度为800个标记，最大为2000个） |
| `honcho_context` | 否 | 提供完整的会话上下文：包括摘要、用户画像、信息卡片以及所有对话记录 |
| `honcho_reasoning` | 是 | 通过多轮辩证式的 `.chat()` 推理，由大语言模型生成答案 |
| `honcho_conclude` | 否 | 用于创建、查看/搜索或删除持久化的结论内容（列表中会显示需要删除的结论条目ID） |

工具的可见性取决于 `recallMode` 设置：在 `context` 模式下这些工具会被隐藏，而在 `tools` 和 `hybrid` 模式下则始终可见。
...
| 键名 | 类型 | 默认值 | 描述 |
|------|------|---------|------|
| `apiKey` | 字符串 | — | API 密钥。若未提供，则会回退使用 `HONCHO_API_KEY` 环境变量中的值。通过 OAuth 方式连接时，该字段将存储自动刷新的访问令牌 | 
| `oauth` | 对象 | — | 包含 OAuth 授权相关信息的对象（如刷新令牌、过期时间、客户端信息以及令牌获取端点）。这些信息由连接/登录流程自动生成并定期轮换，无需手动修改。可选：仅提供 API 密钥也可正常使用 |
| `baseUrl` | 字符串 | — | 自托管 Honcho 服务的基地址。本地地址则无需进行 API 密钥验证 |
| `environment` | 字符串 | `"production"` | 用于映射 SDK 运行环境 |
| `enabled` | 布尔值 | 自动判断 | 总开关。只要存在 `apiKey` 或 `baseUrl`，该开关就会自动处于开启状态 |
| `workspace` | 字符串 | 主机标识符 | Honcho 工作空间 ID。所有处于同一工作空间中的配置文件可以共享相同的用户身份及相关记忆信息 |
| `peerName` | 字符串 | — | 用户端对应的身份名称 |
| `aiPeer` | 字符串 | 主机标识符 | AI 端对应的身份名称 |

### 身份映射功能（网关多用户场景）

在基于网关的部署环境中（如 Telegram、Discord、Slack 等平台），每个用户都会拥有该平台自带的运行时标识符（如 Telegram 的 UID、Discord 的 snowflake ID、Slack 的用户 ID）。上述三个键用于控制如何将这些运行时标识符映射到 Honcho 中的用户身份。该映射过程由配置文件决定，且结果具有确定性——系统不会自动合并不同平台的用户身份，也不会根据运行时信息进行推断。

| 键名 | 类型 | 默认值 | 描述 |
|------|------|---------|------|
| `pinUserPeer` | 布尔值 | `false` | 当该参数设置为 `true` 时，所有通过网关接入的用户都会被统一映射到 `peerName` 所指定的身份。适用于需要让所有平台用户以及其他用户共享同一身份的单操作员部署场景 |
| `userPeerAliases` | 对象 | `{}` | 用于将运行时标识符映射到对应用户身份的映射表（格式为 `{"7654321": "alice"}`）。推荐的使用模式是一对多映射，即将所有运行时标识符都映射到同一个用户身份名称。系统不支持一对一映射，即一个运行时标识符只能对应一个用户身份 |
| `runtimePeerPrefix` | 字符串 | `""` | 用于为那些没有匹配到别名的运行时标识符添加前缀，以实现命名空间隔离（例如添加前缀 `"telegram_"` 后，标识符会变为 `telegram_7654321`）。仅在没有匹配到别名时才会使用该参数，旨在避免不同平台因运行时标识符结构相似而导致的冲突 |

> **已废弃**：`pinPeerName` 是 `pinUserPeer` 的旧版别名，目前仍会被读取以保持向后兼容性（当两者都设置时，`pinUserPeer` 的值会优先生效）。`hermes honcho setup` 工具在首次运行时会自动将旧别名转换为 `pinUserPeer`，并且不会再保存旧格式的配置。
...
在同一个共享环境（`hermes`）中，这两种配置文件均会使用同一用户（`yourname`），但每个 AI 对等体都会独立生成自身的观察结果、结论及行为模式。编码者的记忆侧重于代码相关内容，而主代理的记忆则更为广泛。

主机密钥源自当前激活的 Hermes 配置文件：默认为 `hermes`，或者为 `hermes_<profile>` 格式（例如 `hermes -p coder` 时，主机密钥为 `hermes_coder`）。为保持兼容性，系统仍会读取旧版的 `hermes.<profile>` 格式主机配置文件；而当 CLI 写入针对特定配置文件的 Honcho 配置后，这些旧配置将会被迁移。

### 辩证推理机制

| 键名 | 类型 | 默认值 | 描述 |
|------|------|---------|-------------|
| `dialecticDepth` | int | `1` | 每个辩证循环的迭代次数（范围为 1–3，会进行限制）。1 表示单次查询，2 表示审核+综合分析，3 表示审核+综合分析+对齐处理 |
| `dialecticDepthLevels` | array | — | 每次迭代的推理级别字符串数组，为可选配置。可覆盖按比例设定的默认值。例如：`["minimal", "low", "medium"]` |
| `dialecticReasoningLevel` | string | `"low"` | `.chat()` 函数的默认推理级别：`"minimal"`、`"low"`、`"medium"`、`"high"`、`"max"` |
| `dialecticDynamic` | bool | `true` | 当该值为 `true` 时，模型可通过 `honcho_reasoning` 工具在每次调用时自行调整推理级别。若为 `false`，则始终使用 `dialecticReasoningLevel` 所设定的级别 |
| `dialecticMaxChars` | int | `600` | 自动注入的辩证补充内容的最大字符数。此限制仅适用于自动注入的内容——通过 `honcho_reasoning` 工具明确生成的输出则会完整返回 |
| `dialecticMaxInputChars` | int | `10000` | 传递给 `.chat()` 函数的辩证查询输入的最大字符数。Honcho 云服务的限制也为 10k |
| `reasoningHeuristic` | bool | `true` | 支持根据查询内容动态调整：会根据查询长度自动提升自动注入的辩证内容的级别（查询长度≥120 字符时升 1 级，≥400 字符时升 2 级），最终级别会受 `reasoningLevelCap` 的限制。若该值为 `false`，则所有自动调用都将固定使用 `dialecticReasoningLevel` 所设定的级别 |
| `reasoningLevelCap` | string | `"high"` | `reasoningHeuristic` 动态调整的上限值：`"minimal"`、`"low"`、`"medium"`、`"high"`、`"max"` |
...
| `messageMaxChars` | int | `25000` | 通过 `add_messages()` 函数发送的每条消息的最大字符数。超过此限制后，消息将被分割并添加 `[continued]` 标记。Honcho 云服务的限制也为 25k |
...
| `dialecticCadence` | int | `1` | 每次触发辩证推理的 `.chat()` 调用的最小间隔轮次数 |
| `injectionFrequency` | string | `"every-turn"` | 可设置为 `"every-turn"` 或 `"first-turn"`：前者表示在每一轮对话中都注入基础上下文；后者则仅在用户发送的第一条消息时注入基础上下文，而辩证补充内容则保持其自身的间隔规则 |
| `queryRewrite` | bool | `false` | 在进行辩证推理之前，将最新的消息重写为检索查询（每轮会多触发一次辅助 LLM 调用） |
| `firstTurnBaseWait` | float | `3.0` | 第一轮对话等待基础上下文或会话初始化完成的最大秒数。设置为 `0` 表示禁用该等待机制（完全异步处理，上下文会在后续轮次中提供）。第 2 轮及以后的对话不会因初始化延迟而等待 |
| `firstTurnDialecticWait` | float | `2.0` | 第一轮对话等待辩证推理结果完成的最大秒数。设置为 `0` 表示禁用该等待机制 |

### 细粒度观察功能

该功能与 Honcho 的每对等体独立配置的 `SessionPeerConfig` 一一对应。当相关配置存在时，会覆盖原有的 `observationMode` 预设值。
