...
请使用 `SOUL.md` 文件来定义持久的智能体个性特征，而 `AGENTS.md` 则用于存储针对特定项目的操作指南。

### .cursorrules 兼容性

如果您已有 `.cursorrules` 或 `.cursor/rules/*.mdc` 文件，Hermes 也能读取它们。无需重复设置编码规范——这些文件会自动从工作目录中加载。

### 检索机制

在会话开始时，Hermes 会首先加载当前工作目录中的顶层 `AGENTS.md` 文件。对于子目录中的 `AGENTS.md` 文件，则会在调用工具时（通过 `subdirectory_hints.py`）动态检索，并将其结果嵌入到工具输出中——它们不会提前加载到系统提示语中。
...
**内存**用于存储事实信息：您的环境设置、偏好选项、项目路径，以及智能体了解到的关于您的信息。而**技能**则用于存储操作流程：多步骤工作流、针对特定工具的指令，以及可重复使用的方案。对于“是什么”的问题，使用内存；对于“怎么做”的问题，则使用技能。

### 何时创建技能

如果您发现某项任务需要5个以上步骤且会反复执行，可以让智能体为其创建一个技能。例如，您可以要求它“将你刚才的操作保存为一个名为 `deploy-staging` 的技能”。下次只需输入 `/deploy-staging`，智能体就会自动加载完整的操作流程。

### 管理内存容量

内存容量是有意限制的（`MEMORY.md` 最多为约2,200个字符，`USER.md` 最多为约1,375个字符）。当内存占满时，智能体会对相关条目进行整合。您可以通过要求它“清理内存”或“替换旧的Python 3.9相关说明——我们现在使用的是3.12版本”来协助管理。

### 让智能体记住信息

在高效的会话结束后，您可以说“把重点记下来，下次再用”，智能体就会保存关键内容。您也可以具体说明，例如：“请将‘我们的CI流程使用GitHub Actions及`deploy.yml`工作流’这条信息保存到内存中。”

:::警告
内存是一个静态快照——在当前会话期间所做的更改，要等到下一次会话开始时才会显示在系统提示语中。智能体会立即将数据写入磁盘，但提示语缓存不会在会话进行中失效。
:::
...
大多数大型语言模型服务提供商都会对系统提示语的前缀部分进行缓存。如果保持系统提示语的稳定性（相同的上下文文件、相同的内存设置），则同一会话中的后续消息就能实现**缓存命中**，从而大幅降低成本。请避免在会话进行中更换模型或系统提示语。

### 在达到限制前使用 /compress 功能

长时间的会话会导致令牌积累。当您发现响应变慢或内容被截断时，可以运行 `/compress` 命令。该命令会总结对话历史，在保留关键上下文的同时大幅减少令牌数量。您可以使用 `/usage` 命令查看当前的令牌使用情况。
...
无需逐一执行终端命令，您可以让智能体编写一个脚本一次性完成所有操作。例如，要求它“写一个Python脚本将所有 `.jpeg` 文件重命名为 `.jpg`，然后运行该脚本”，这种方式比逐个重命名更高效快捷。

### 选择合适的模型

您可以在会话进行中通过 `/model` 命令切换模型。对于复杂的推理和架构设计任务，可使用前沿模型（如Claude Sonnet/Opus、GPT-4o）；而对于格式化、重命名或生成样板代码等简单任务，则可选择速度更快的模型。

:::提示
请定期运行 `/usage` 命令查看令牌消耗情况。如需了解过去30天的使用模式，可运行 `/insights` 命令。
:::
...
在您常用的Telegram或Discord聊天频道中使用 `/sethome` 命令将其设为主频道。定时任务的结果和计划任务的输出都会发送到这里。如果没有设置主频道，智能体就无处发送主动消息。

### 使用 /title 对会话进行分类

通过 `/title auth-refactor` 或 `/title research-llm-quantization` 等命令为会话命名。带有名称的会话可以通过 `hermes sessions list` 命令轻松查找，也可以使用 `hermes -r "auth-refactor"` 命令继续处理。未命名的会话则会越来越多，难以区分。

### 通过私信配对实现团队访问控制

无需手动收集用户ID来建立白名单，只需启用私信配对功能即可。当团队成员向机器人发送私信时，他们会收到一个一次性配对码。您可以通过 `hermes pairing approve telegram XKGH5N7P` 命令进行批准——这种方式既简单又安全。

### 工具执行进度显示模式

使用 `/verbose` 命令可以控制您看到的工具执行信息量。在消息平台中，通常“少即是多”——将显示模式设置为“new”即可仅查看新的工具调用记录。而在命令行界面中，设置为“all”则能实时查看智能体的所有操作。

:::提示
在消息平台上，会话会在空闲一段时间后（默认为24小时）或每天凌晨4点自动重置。如果需要更长的会话时间，可在 `~/.hermes/config.yaml` 文件中根据不同平台进行相应设置。
:::
