...
使用 `SOUL.md` 文件来设定持久的智能体性格特征，而 `AGENTS.md` 则用于记录针对特定项目的操作指南。

### .cursorrules 兼容性

如果您已有 `.cursorrules` 或 `.cursor/rules/*.mdc` 文件，Hermes 也会读取这些文件。无需重复定义编码规范——系统会自动从当前工作目录加载这些规则。

### 工具发现机制

在会话启动时，Hermes 会首先加载当前工作目录中的顶层 `AGENTS.md` 文件。对于子目录中的 `AGENTS.md` 文件，则会在调用工具时（通过 `subdirectory_hints.py`）动态检测并将其结果整合到工具输出中——它们不会被提前加载到系统提示语中。

:::提示
请确保上下文文件简洁明了。由于这些内容会嵌入到每一条消息中，因此每个字符都会计入您的令牌预算。
:::
...
**内存**用于存储事实性信息，如您的环境设置、偏好选项、项目路径以及智能体了解到的关于您的信息。而**技能**则用于存储流程指令，包括多步骤工作流、针对特定工具的操作指南以及可复用的模板。对于“是什么”的问题，使用内存；对于“怎么做”的问题，则使用技能。

### 何时创建技能

如果发现某项任务需要5个以上步骤且会重复执行，可以请智能体为其创建一个技能。例如，您可以说：“把你刚才的操作保存为一个名为 `deploy-staging` 的技能。”下次只需输入 `/deploy-staging`，智能体就会自动加载完整的操作流程。

### 管理内存容量

内存容量是有限制的（`MEMORY.md` 最多为约2,200字符，`USER.md` 最多为约1,375字符）。当内存占满时，智能体会对相关条目进行整合。您可以通过说“清理你的内存”或“替换掉那条关于Python 3.9的说明——我们现在使用的是3.12版本”来协助管理。

### 让智能体记住信息

在高效的会话结束后，可以说“请记住这些内容，以便下次使用”，智能体就会保存关键要点。您也可以更具体地指定：“请将‘我们的CI流程使用GitHub Actions及`deploy.yml`工作流’这条信息保存到内存中。”

:::警告
内存中的内容是静态快照——在当前会话期间所做的更改要等到下一次会话开始时才会显示在系统提示语中。虽然智能体会立即将数据写入磁盘，但提示语缓存不会在会话进行中失效。
:::
...
大多数大型语言模型服务提供商都会对对话开头部分（系统提示语+历史记录）进行缓存。如果保持系统提示语不变（即上下文文件和内存内容不变），则同一会话中的后续消息就能触发**缓存命中**，从而大幅降低成本。缓存是按模型和账户来区分的——因此，如果显式地切换模型、使用[自动提供商回退功能](../user-guide/features/fallback-providers.md)或[凭证池轮换功能](../user-guide/features/credential-pools.md)，都会迫使系统在下一轮响应时重新读取整个对话内容，从而按全额价格计费。偶尔切换没有问题，但在长时间会话中频繁切换会大幅增加成本。

### 在达到限制前使用 /compress 功能

长时间的会话会导致令牌数量不断累积。当您发现响应变慢或内容被截断时，可以运行 `/compress` 命令。该命令会总结对话历史，在保留关键上下文的同时大幅减少令牌用量。您可以使用 `/usage` 命令查看当前的令牌使用情况。

### 通过任务委派实现并行处理

需要同时研究三个主题？可以让智能体使用 `delegate_task` 功能分配多个并行子任务。每个子智能体会在独立的上下文中运行，最终只返回汇总结果——这能极大降低主对话的令牌消耗。

### 使用 execute_code 处理批量操作

无需逐个执行终端命令，可以让智能体编写一个脚本一次性完成所有操作。例如，让智能体“写一个Python脚本将所有 `.jpeg` 文件重命名为 `.jpg`，然后运行该脚本”，这种方式比逐个重命名文件更高效、成本更低。

### 选择合适的模型

可以使用 `/model` 命令在会话中进行模型切换。对于复杂的推理和架构设计任务，可使用前沿模型（如Claude Sonnet/Opus、GPT-4o）；而对于格式化、重命名或生成模板等简单任务，则可选择速度更快的模型。请注意，每次模型切换都会重置提示语缓存（见上文），因此在长时间会话中，直接使用另一款模型开始新会话通常比频繁切换更为经济。

:::提示
定期运行 `/usage` 命令查看您的令牌消耗情况。如需了解过去30天的详细使用模式，可运行 `/insights` 命令。
:::
...
可以通过 `/title auth-refactor` 或 `/title research-llm-quantization` 等方式为会话命名。带有名称的会话可以通过 `hermes sessions list` 命令轻松查找，也可以通过 `hermes -r "auth-refactor"` 命令恢复。未命名的会话则会越来越多，最终难以区分。

### 通过私信配对实现团队访问控制

无需手动收集用户ID来建立白名单，只需启用私信配对功能即可。当团队成员向机器人发送私信时，他们会收到一个一次性配对码。您只需使用 `hermes pairing approve telegram XKGH5N7P` 命令进行批准——既简单又安全。

### 工具执行进度显示模式

可以使用 `/verbose` 命令控制您看到的工具执行信息量。在消息平台中，通常越简洁越好——将显示模式设置为“new”即可仅查看新的工具调用记录。在命令行界面中，将模式设置为“all”则可以实时查看智能体的所有操作。

:::提示
默认情况下，消息会话不会自动重置——上下文信息会一直保留，直到您执行 `/reset` 命令或触发压缩功能为止。如果您希望会话在闲置一段时间后或每天固定时间自动重置，可以通过 `~/.hermes/config.yaml` 文件中的 `session_reset` 配置项来开启该功能。
:::
