...
| `email` | `config.yaml` 中已配置 SMTP 服务 |
| `sms` | 已配置 SMS 提供商 |
| `local` | 具有 `~/.hermes/cron/output/` 目录的写入权限 |
| `origin` | 将内容发送至创建任务时的聊天窗口 |
...
即使交付失败，任务仍会正常运行——只是不会向任何地方发送内容。您可以查看 `hermes cron list` 中的 `last_error` 字段（如有显示）以获取最新错误信息。

### 检查项 2：检查 `[SILENT]` 的使用情况

如果您的定时任务没有输出，交付功能将会被抑制。此外，若代理响应中包含定时任务专用标记 `[SILENT]`，交付也会被抑制。这是为监控任务设计的默认行为——但请确保您的提示语不会意外导致所有内容都被抑制。

建议使用类似“若无变化，则仅回复 [SILENT]”这样的提示语。避免要求代理在较长的解释文本中包含 `[SILENT]`，因为定时任务系统会将该标记视为抑制信号。

### 检查项 3：平台令牌权限

每个消息平台上的机器人都需要特定的权限才能接收消息。若交付失败且无任何提示：
...
默认情况下，定时任务的响应会带有页眉和页脚（通过 `config.yaml` 中的 `cron.wrap_response: true` 参数启用）。某些平台或集成可能无法正确处理这种格式。如需禁用该功能：
...
在将技能添加到定时任务之前，必须先将其安装。如果某个技能缺失，请使用 `hermes skills install <skill-name>` 命令或通过 CLI 中的 `/skills` 功能进行安装。

### 检查 2：确认技能名称与技能文件夹名称是否一致
...
定时任务运行时，`cronjob`、`messaging` 和 `clarify` 这些工具集会被禁用。这是为了防止出现递归创建定时任务、直接发送消息（消息传递由调度器处理）以及交互式提示框的情况。如果某个技能依赖于这些工具集，那么它在定时任务环境中将无法正常工作。请查阅该技能的文档，确认其是否支持在非交互式（无界面）模式下运行。
...
**任务执行时出现“未找到对应技能”错误**
该技能必须安装在运行调度器的机器上。如果在不同机器之间切换，技能信息不会自动同步——请使用 `hermes skills install <skill-name>` 命令重新安装。

**任务虽已运行但无输出结果**
可能是交付目标存在问题（参见上文“交付失败”部分），或者没有生成任何输出，又或者是响应中包含了 cron 的静默标记 `[SILENT]`。

**任务挂起或超时**
调度器采用基于无操作状态的超时机制（默认为 600 秒，可通过 `HERMES_CRON_TIMEOUT` 环境变量进行配置，设置为 `0` 表示无限制）。只要代理持续主动调用工具，它就可以继续运行——只有在长时间无操作后才会触发超时。对于耗时较长的任务，建议使用脚本来处理数据收集工作，并仅输出最终结果。

### 检查项 3：锁竞争问题

调度器通过基于文件的锁定机制来避免多个任务同时执行。如果存在两个正在运行的网关实例（或者 CLI 会话与某个网关发生冲突），可能会导致任务被延迟或跳过。
