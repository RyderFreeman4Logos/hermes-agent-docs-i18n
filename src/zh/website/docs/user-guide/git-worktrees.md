...
对于并行 Agent，可打开多个终端并在每个终端中运行 `hermes -w` —— 每次调用都会自动创建独立的工作树和分支。

## 整合运用

- 使用 **git worktrees** 为每个 Hermes 会话提供独立的干净代码检出环境。
- 利用 **分支** 记录实验的整体历史进展。
- 通过 **检查点 + `/rollback`** 功能，在每个工作树中快速纠正错误。

这样的组合能够带来以下优势：

- 确保不同的 Agent 和实验互不干扰。
- 实现快速的迭代周期，并能轻松恢复因错误编辑造成的问题。
- 生成整洁、便于审查的拉取请求。
