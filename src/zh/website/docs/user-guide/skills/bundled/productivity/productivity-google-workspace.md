...
可通过 gws CLI 或 Python 操作 Gmail、Calendar、Drive、Docs、Sheets 等功能。
...
通过 Hermes 管理的 OAuth 机制及轻量级 CLI 封装层，可操作 Gmail、Calendar、Drive、Contacts、Sheets 和 Docs 等功能。若已安装 `gws`，该技能将使用其作为执行后端以实现更全面的 Google Workspace 功能支持；否则则回退至内置的 Python 客户端实现。

## 参考资料

- `references/gmail-search-syntax.md` — Gmail 搜索操作符（如 is:unread、from:、newer_than: 等）

## 脚本文件

- `scripts/setup.py` — OAuth2 配置文件（需运行一次以完成授权）
- `scripts/google_api.py` — 兼容性封装 CLI。在可用时优先使用 `gws` 执行操作，同时保持与 Hermes 现有的 JSON 输出格式一致。

## 首次设置指南

整个设置过程完全无需交互——您只需按步骤操作，即可在 CLI、Telegram、Discord 或任何其他平台上使用该技能。
