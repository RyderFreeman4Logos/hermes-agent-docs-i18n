...
从PDF及扫描件中提取文本（使用pymupdf、marker-pdf库）。
...
:::info
以下为Hermes在触发该技能时加载的完整技能定义。当该技能处于激活状态时，智能体将依据此内容执行操作。
:::

# PDF与文档提取功能

对于DOCX格式文件：可参考`docx`技能（用于创建/编辑），或使用`python-docx`库进行结构化读取。
对于PPTX格式文件：可参考`powerpoint`技能（支持完整的创建/读取/编辑功能）。
对于PDF文件的各类操作（合并、拆分、处理表单、添加水印、创建新文件等）：请参考`pdf`技能。
该技能主要实现**从PDF及扫描文档中提取文本**的功能。

> **收到“`read_file` 提取覆盖率警告”？** `read_file` 功能虽能自动转换本地 PDF 文件，但仅读取文本层；警告信息中会列出未检测到文本的页面（即扫描图片）。对于少量页面，使用“渲染+视觉分析”方式效率最高：先执行 `pdftoppm -jpeg -r 150 -f N -l N file.pdf /tmp/page` 将文件转换为图像，再对每张图像调用 `vision_analyze` 函数。若需批量处理大量页面的 OCR 任务，请使用下方的 `marker-pdf` 方法（步骤 2）。

## 步骤 1：是否有远程 URL？

如果文档存在 URL，**请始终优先尝试使用 `web_extract` 方法**：
...
仅在以下情况才使用本地提取功能：文件位于本地、web_extract 失败，或需要批量处理时。
...
> “该文档需要通过 OCR 或高级提取功能（marker-pdf）进行处理，这需要约 5GB 的空间用于存储 PyTorch 及相关模型。您的系统目前仅有 [X]GB 的空闲空间。可选方案：释放更多空间，提供文件 URL 以便我使用 web_extract 进行提取，或者尝试使用 pymupdf——它虽能处理文本型 PDF，但不支持扫描版文档和公式识别。”
...
- pymupdf 是最稳妥的默认选择：响应迅速、无需模型支持、可在任何环境中运行。
- marker-pdf 适用于 OCR 处理、扫描文档、公式以及复杂布局的文档——仅在必要时安装即可。
- 这两个辅助脚本均支持通过 `--help` 参数查看完整用法说明。
- 首次使用时，marker-pdf 会自动将约 2.5GB 的模型下载到 `~/.cache/huggingface/` 目录中。
