...
通过 OpenStreetMap/OSRM 实现地理编码、兴趣点查询、路线规划以及时区查询等功能。
...
利用免费的开放数据源提供位置相关服务。包含 8 条命令、44 类兴趣点，无任何依赖（仅使用 Python 标准库），也无需 API 密钥。

数据来源：OpenStreetMap/Nominatim、Overpass API、OSRM、TimeAPI.io。

该技能已取代旧版的 `find-nearby` 技能——`find-nearby` 的所有功能都已被下方的 `nearby` 命令覆盖，且支持相同的 `--near "<地点名>"` 快捷方式以及多类别查询功能。

## 适用场景

- 用户发送 Telegram 位置标记（消息中包含纬度/经度）→ 使用 `nearby` 命令
- 用户需要某个地名的坐标 → 使用 `search` 命令
- 用户已知坐标但需要对应地址 → 使用 `reverse` 命令
- 用户想查找附近的餐厅、医院、药店、酒店等 → 使用 `nearby` 命令
- 用户需要了解驾车/步行/骑行距离或行程时间 → 使用 `distance` 命令
- 用户需要获取两个地点之间的逐段导航路线 → 使用 `directions` 命令
- 用户需要了解某个地点的时区信息 → 使用 `timezone` 命令
