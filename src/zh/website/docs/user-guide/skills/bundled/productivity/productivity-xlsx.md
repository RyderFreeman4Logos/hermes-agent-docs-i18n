...
用于创建、读取和编辑 Excel .xlsx 工作簿以及 CSV 文件。
...
| 相关技能 | [`docx`](/docs/user-guide/skills/bundled/productivity/productivity-docx)、[`pdf`](/docs/user-guide/skills/bundled/productivity/productivity-pdf)、[`powerpoint`](/docs/user-guide/skills/bundled/productivity/productivity-powerpoint) |
...
利用 Python 和 openpyxl 库处理 Excel .xlsx 工作簿：可创建包含公式与图表的多工作表格式化工作簿，查看或导出现有文件，编辑单元格及结构，还可实现与 CSV 格式的相互转换。所有辅助脚本均为基于 argparse 的命令行工具，会以 JSON 格式输出结果，并采用显式的 UTF-8 输入输出方式。

## 适用场景

- 创建.xlsx报告：支持多工作表、数字格式设置、样式应用、合并单元格、冻结窗格、自动筛选、条件格式、图表展示、数据验证下拉列表、原生Excel表格、定义名称、超链接、单元格备注以及工作表保护功能。  
- 读取工作簿：可查看工作表清单，将数据导出为JSON或CSV格式，区分公式与缓存值、备注、定义名称及表格信息。  
- 编辑现有文件：可设置单元格内容、追加行、插入/删除行或列（通过`xlsx_restructure.py`实现基于引用的操作），还可复制/重命名工作表、表格、定义名称、备注及保护设置。  
- 通过LibreOffice在无界面模式下重新计算公式（使用`xlsx_recalc.py`）。  
- 支持带类型推断功能的CSV格式互操作，以及非UTF-8编码处理。  
- 不支持旧版的.xls二进制格式（需先使用LibreOffice转换：`soffice --headless --convert-to xlsx old.xls`）。  

## 先决条件

- Python 3.10及以上版本，并已安装`openpyxl`（可通过`pip install openpyxl`安装）。无需其他第三方库，其余功能均依赖标准库。  
- 可选：如需在无界面模式下重新计算或转换格式，可安装LibreOffice（命令为`soffice`）。  

## 运行方式

从该技能对应的`scripts/`目录中使用`terminal`工具运行辅助脚本（所有脚本均支持`--help`参数）：
