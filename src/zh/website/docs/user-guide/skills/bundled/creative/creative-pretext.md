...
7. **绕过特定行与调整宽度**：在沿着形状排列内容时，如果某行的通道过窄而无法容纳一行文本，应选择*跳过该行*（即执行 `y += lineHeight; continue;`），而非将极小的 `maxWidth` 值传递给 `layoutNextLineRange`——否则会导致每行仅显示一个字符，看起来十分不完整。

8. **打造出色的演示效果**：默认的初始渲染效果往往类似教程级别。建议添加渐变边框、细微的扫描线效果、空闲状态下的自动动画，以及精心设计的交互响应（拖动、悬停、滚动、点击）。缺少这些元素的话，“酷炫的 Pretext 演示”只会沦为“README 文件里的实习生作品”。

## 验证清单

- [ ] 演示文件为独立的 `.html` 文件——可通过双击或 `python3 -m http.server` 打开  
- [ ] 已通过 `esm.sh` 导入 `@chenglou/pretext`，且版本已固定  
- [ ] 用于训练的文本为真实散文，而非伪随机文本，并与演示主题相符  
- [ ] 传递给 `prepare` 函数的字体字符串与 CSS 中定义的字体完全一致  
- [ ] `prepare()` / `prepareWithSegments()` 函数仅调用一次，而非每帧都调用  
- [ ] 使用深色背景搭配精心挑选的配色方案——而非默认的白色画布  
- [ ] 至少包含一种交互响应（拖动/悬停/滚动/点击）或空闲状态下的自动动画  
- [ ] 已使用 `python3 -m http.server` 在本地进行测试，确认无控制台错误  
