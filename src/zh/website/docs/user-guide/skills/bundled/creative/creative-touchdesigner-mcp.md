...
具备上下文感知能力（能识别所选的操作及当前网络环境）。
Hub健康检查：执行`GET http://localhost:40404/mcp`命令，即可获取包含实例PID、项目名称以及TD版本的JSON格式数据。
...
**备用方案：** 使用 `rgba32float` 格式的固定 TOP 值（8 位数值会被限制在 0-1 范围内，从而导致着色器无法正常运行）。

**反馈 TOP 值设置：** 应通过 `top` 参数进行引用，而非直接输入数据线。首次编译后，“源数据不足”的问题即可解决；出现“编译依赖循环”警告属于正常现象。

**分辨率限制：** 非商业用途的分辨率上限为 1280×1280，此时需使用 `outputresolution = 'custom'` 参数。

**大型着色器处理：** 先将 GLSL 代码写入 `/tmp/file.glsl` 文件，再通过 `td_write_dat` 或 `td_execute_python` 命令将其加载进来。

**顶点/点数据访问（TD 2025.32 版本）：** 应使用 `point.P[0]`、`point.P[1]`、`point.P[2]` 这种格式，而非 `.x`、`.y`、`.z`。

**扩展功能：** 在 CONSTANT 模式下，`ext0object` 格式的写法为 `"op('./datName').module.ClassName(me)"`。使用 `td_write_dat` 编辑完扩展代码后，需调用 `td_reinit_extension` 函数以重新初始化该扩展。

**脚本回调函数：** 无论何时都必须通过 `me.parent()` 或 `scriptOp.parent()` 来使用相对路径。
