...
**备用方案**：始终使用 `rgba32float` 格式的常量 TOP 值（8 位数值将被限制在 0-1 范围内，从而导致着色器无法正常运行）。

**反馈 TOP 值**：应使用 `top` 参数引用，而非直接输入数据线。“源数据不足”的问题在首次处理后会得到解决，出现“处理依赖循环”警告属于正常现象。

**分辨率限制**：非商业用途的渲染分辨率上限为 1280×1280，此时需设置 `outputresolution = 'custom'`。

**大型着色器**：先将 GLSL 代码写入 `/tmp/file.glsl` 文件，再通过 `td_write_dat` 或 `td_execute_python` 命令将其加载进来。

**顶点/点数据访问（TD 2025.32 版本）**：应使用 `point.P[0]`、`point.P[1]`、`point.P[2]` 这种格式，而非 `.x`、`.y`、`.z`。

**扩展功能**：在 CONSTANT 模式下，`ext0object` 格式的代码应为 `"op('./datName').module.ClassName(me)"`。使用 `td_write_dat` 编辑完扩展代码后，需调用 `td_reinit_extension` 函数进行重新初始化。
