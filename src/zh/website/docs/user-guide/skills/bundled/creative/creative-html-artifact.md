...
{/* 此页面由 website/scripts/generate-skill-docs.py 根据技能的 SKILL.md 自动生成。请修改源文件 SKILL.md，而非此页面。 */}

# HTML 文档生成

用于创建自包含的 HTML 文件，以实现说明、规划或评审功能。
...
|---|---|
| 来源 | 已内置（默认安装） |
| 路径 | `skills/creative/html-artifact` |
| 版本 | `1.0.0` |
| 开发者 | Anthropic（html-effectiveness 图库，MIT 许可），为 Hermes Agent 优化 |
| 许可证 | MIT |
| 支持平台 | linux、macos、windows |
| 标签 | `html`、`artifact`、`explainer`、`plan`、`report`、`code-review`、`diagram`、`svg`、`design`、`prototype`、`editor` |
| 相关技能 | [`claude-design`](/docs/user-guide/skills/bundled/creative/creative-claude-design)、[`popular-web-designs`](/docs/user-guide/skills/bundled/creative/creative-popular-web-designs)、[`design-md`](/docs/user-guide/skills/bundled/creative/creative-design-md)、[`excalidraw`](/docs/user-guide/skills/bundled/creative/creative-excalidraw)、[`p5js`](/docs/user-guide/skills/bundled/creative/creative-p5js) |
...
每当需要生成适合人类*阅读、分享或交互*的内容时——如概念说明、实施计划、状态/故障报告、代码审查指南、技术或教育类图表、多组设计方案，或是能将结果导出给用户的简易编辑器——该技能均可直接生成一个自包含的 `.html` 文件，无需构建步骤、无需依赖项，也无需使用 CDN。

当文档需要颜色、布局、图表、表格、代码或交互功能时，HTML 的表现力优于 Markdown。它可在任何浏览器中打开，可通过链接分享，即使内容超过 100 行仍保持可读性，还能承载 Markdown 无法处理的 SVG 图表和动态控件。当用户要求“生成 HTML 文件/文档”，或请求*解释某事物的工作原理*、*撰写计划/拉取请求/报告*、*绘制图表*、*对比不同选项*，或*设计交互原型*时——即便用户没有明确提到“HTML”，也应默认使用 HTML 文档生成功能。

## 该技能的诞生原因（以及它取代了哪些技能）

该技能**取代了**之前的三项技能：`sketch`（用于生成多版本简易 HTML 原型）、`architecture-diagram`（用于生成暗色风格的基础设施 SVG 图表）以及 `concept-diagrams`（用于生成教育类 SVG 图表）。将它们整合为一个技能是有明确原因的：这三项技能最终生成的*输出结果完全相同*——均为包含内联 CSS/SVG 的自包含 HTML 文件——且功能高度重叠（三项“图表”技能、两条“对比方案”路径，且没有统一的标记系统）。将其合并为一个可通过模式切换的技能，既消除了“该用哪个技能”的困惑，确保所有输出都采用统一的风格，同时保留了每项技能的独特优势：`sketch` 的精确度调节功能与验证机制、`architecture-diagram` 的暗色基础设施风格，以及 `concept-diagrams` 的九级渐进式教育系统与原型库。

此次整合并未增加额外负担：该技能**没有任何依赖项**（无需 Node、FFmpeg、Chromium 或 pip 包——它直接生成纯 HTML/CSS/SVG 文件），因此即便它以*内置方式*提供（默认启用），而 `concept-diagrams` 曾是可选功能，其唯一始终存在的开销仅为一行描述文字。所有参考资料、模板和示例图库都会按需加载。`concept-diagrams` 曾是可选功能，是因为它的应用场景较为小众，而非因为安装成本较高——将其升级为通用型、无依赖的内置技能，才是最适合它的定位。那些需要实际安装组件（如 `hyperframes`：需要 Node + FFmpeg + Chromium）的图表类功能，则仍保持可选状态，不会被合并进来。

在以下情况下请使用其他技能：需要匹配特定品牌的外观风格 → `popular-web-designs`；需要正式的设计标记规范文件 → `design-md`；需要定制化且视觉设计为核心目标的文档 → `claude-design`；需要手绘/白板风格的 `.excalidraw` 文件 → `excalidraw`；需要生成动态画布艺术作品 → `p5js`。该技能适用于所有其他可生成为可阅读、可分享的 HTML 页面的场景。

## 参考文件（按需加载）

- `references/house-style.md` —— 标准的 `:root` 标记块、类型系统，以及卡片/表格/提示框/代码块等模板格式。**在创建任何文档之前请先阅读此文件。**
- `references/examples.md` —— 20 个完整的参考 HTML 文件（来自 Anthropic 的 html-effectiveness 图库，MIT 许可），按不同模式分类，并附有加载这些文件的脚本。请根据任务选择合适的示例，通过完整案例来调整文档风格。
- `references/svg-diagrams.md` —— 手动编写的内联 SVG 代码：箭头标记、节点组、决策菱形、边的语义规则、坐标网格规范。如需创建流程图/架构图/概念图，请阅读此文件。
- `references/concept-archetypes.md` —— 九级渐进式教育颜色系统，以及一系列图表原型库（时间轴、树状图、象限图、分层堆叠图、前后对比图、中心辐射图、横截面图）。用于创建教育类或非软件相关的可视化内容时请参考此文件。
- `references/dark-tech.md` —— 暗色“基础设施”风格标记变体（保留了旧版 `architecture-diagram` 的风格）。用于创建云服务/基础设施/系统架构图时请阅读此文件。
- `references/throwaway-editors.md` —— 简易编辑器的实现方案，以及可在 `file://` 协议下使用的复制到剪贴板导出功能。当需要生成能将状态导回提示词的交互式控件时，请阅读此文件。
- `references/fidelity-and-verify.md` —— 用于调节简易版与正式版文档精确度的滑块、多版本对比布局，以及强制性的浏览器视觉验证流程。

## 模板

- `templates/base.html` —— 包含统一风格 `<style>` 块的文档框架。
- `templates/diagram.html` —— 双模式图表容器（包含轻量级教育风格与暗色基础设施风格的 CSS、箭头标记、节点/边类）。请在指定位置插入您的 SVG 文件。
- `templates/editor.html` —— 简易编辑器的框架结构（状态处理 → 渲染 → 导出）。

可通过 `skill_view(name="html-artifact", file_path="templates/base.html")` 加载相应模板。

## 工作流程

1. **选择模式**。根据需求匹配对应的文档类型——说明文、计划书、报告、代码审查内容、图表、多版本方案或编辑器。模式决定了应使用哪种模板、参考哪些资料，以及采用哪个示例。
2. **每次都先阅读匹配的示例**。html-effectiveness 图库中的 20 个文件是该技能设计的基准；虽然文字描述能对这些文件进行说明，但完整示例所呈现的内容密度、排版间距和结构都是摘要无法替代的。在开始编写任何内容之前，请先阅读这些示例。
   ```
