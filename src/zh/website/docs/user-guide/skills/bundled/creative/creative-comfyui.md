...
| 标签 | `comfyui`、`图像生成`、`stable-diffusion`、`flux`、`sd3`、`wan-video`、`hunyuan-video`、`创意工具`、`生成式AI`、`视频生成` |
...
利用官方的 `comfy-cli` 进行设置与生命周期管理，再通过直接的 REST/WebSocket API 执行工作流，从而借助 ComfyUI 生成图像、视频、音频及 3D 内容。
...
7. **视频/音频工作流的超时设置**——当输出节点为 `VHS_VideoCombine`、`SaveVideo` 等类型时会自动检测该参数；默认值从 300 秒调整为 900 秒。如需手动修改，可使用 `--timeout 1800` 指定新值。

8. **输出文件名中的路径遍历防护**——服务器提供的文件名会经过 `safe_path_join` 处理，从而阻止任何试图绕过 `--output-dir` 限制的路径。请保持此防护机制启用，因为使用自定义保存节点的工作流可能会生成任意路径。

9. **工作流 JSON 即为任意代码**——由于自定义节点会运行 Python 代码，因此提交未知工作流时的信任级别与执行 `eval` 语句相同。在运行前，请务必检查来自不可信来源的工作流内容。

10. **自动随机化种子值**——在 `--args` 参数中传入 `seed: -1`（或使用 `--randomize-seed` 选项而不指定具体种子值），即可让每次运行都生成新的种子值。实际生成的种子值会记录到标准错误流中。

11. **`tracking` 提示**——首次运行 `comfy` 时可能会出现关于数据统计的提示。如需跳过该交互式提示，可使用 `comfy --skip-prompt tracking disable` 命令。`comfyui_setup.sh` 脚本已自动处理此功能。

## 验证清单

可使用 `python3 scripts/health_check.py` 一次性运行所有验证项。也可手动逐一检查。
