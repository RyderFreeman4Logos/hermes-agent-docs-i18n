...
适用于所有格式的ASCII艺术视频制作流程。可将视频/音频/图片或生成式输入数据转换为带颜色的ASCII字符视频输出（MP4、GIF、图像序列）。功能涵盖：视频转ASCII转换、随音频变化的音乐可视化效果、生成式ASCII艺术动画、视频与音频的混合动态效果、文本/歌词叠加功能，以及实时终端渲染功能。

## 创作标准

这属于视觉艺术形式。ASCII字符是其表现媒介，而电影艺术则是其参照标准。

在编写任何一行代码之前，先明确创意概念。整体氛围应是怎样的？它要讲述怎样的视觉故事？为何这个项目会与其他所有 ASCII 视频有所不同？用户的提示仅是起点——应以富有创意的视角去解读它，而非机械地照搬文字内容。

首次渲染的结果必须堪称卓越，无需经过多次修改即可呈现出令人震撼的视觉效果。如果作品显得千篇一律、缺乏层次，或是有着典型的“AI 生成的 ASCII 艺术”风格，那就说明思路有误——在最终交付之前，务必重新审视创意概念。

不要局限于参考资料中的现有元素。参考资料中的效果目录、着色器预设以及调色板库仅可作为基础素材。针对每个项目，应将这些元素进行组合、改造，甚至创造全新的图案。那些参考资料不过是绘画的颜料调色板——真正的作品由你亲手打造。

要主动发挥创造力。当项目需求超出现有功能范围时，应拓展相关技能的适用范围。如果参考资料中没有符合创意构想的内容，那就自行开发。至少要加入一个用户虽未提出要求但一定会欣赏的视觉元素——比如转场效果、特殊特效，或是能提升整部作品格调的色彩选择。

相比技术上的完美，更为重要的是整体风格的统一性。视频中的所有场景都应通过统一的视觉语言相互关联——相同的色温、相近的角色配色方案、一致的运动表现风格。即便每个场景在技术上都很准确，但如果每个场景都使用截然不同的特效，那依然属于美学上的失败。

**层次丰富、构思精巧。** 每一帧都值得细细品味，绝无单调的纯黑色背景，始终采用多网格构图，每个场景都有独特变化，色彩运用也处处经过精心设计。

## 模式

| 模式 | 输入 | 输出 | 参考文档 |
|------|-------|--------|-----------|
| **视频转ASCII** | 视频文件 | 对原始视频的ASCII风格再现 | `references/inputs.md` § 视频采样 |
| **音频驱动** | 音频文件 | 基于音频特征生成的视觉效果 | `references/inputs.md` § 音频分析 |
| **生成模式** | 无（或种子参数） | 过程化生成的ASCII动画 | `references/effects.md` |
| **混合模式** | 视频 + 音频 | 含音频驱动叠加层的ASCII视频 | 参考上述两种输入方式 |
| **歌词/文本模式** | 音频 + 文本/SRT文件 | 带有视觉效果的时间同步文本 | `references/inputs.md` § 文本/歌词处理 |
| **TTS旁白模式** | 文本引语 + TTS接口 | 带有打字效果的旁白或引语视频 | `references/inputs.md` § TTS集成 |
...
| 层级 | 工具 | 用途 |
|-------|------|---------|
| 核心层 | Python 3.10+、NumPy | 数学运算、数组操作、向量化处理 |
| 信号处理层 | SciPy | FFT变换、峰值检测（音频模式） |
| 图像处理层 | Pillow (PIL) | 字体光栅化、帧解码、图像读写 |
| 视频输入输出层 | ffmpeg（命令行工具） | 解码输入视频、编码输出视频、音频混合 |
| 并行处理层 | concurrent.futures | 通过多个工作线程实现批量/片段渲染 |
| 文本转语音层 | ElevenLabs API（可选）| 生成旁白片段 |
| 可选组件 | OpenCV | 视频帧采样、边缘检测 |
