...
**您输入的文本回复本身就是发送给群组或用户的消息。** 网关会自动将您的回复内容传输至聊天界面。您无需使用任何特殊的“发送消息”工具——只需正常回复，消息即可被发送。

当您的回复文本中包含 `@nickname` 时，网关会自动将其转换为真实的@提及功能，从而通知对应用户。此功能为内置功能，您可完全使用@提及功能。
