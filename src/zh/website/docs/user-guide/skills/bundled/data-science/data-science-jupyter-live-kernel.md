...
:::info
以下是当触发该技能时 Hermes 加载的完整技能定义。技能处于激活状态时，代理程序会将此内容视为操作指令。
:::

# Jupyter 实时内核（hamelnb）

通过实时 Jupyter 内核为您提供**带状态管理的 Python REPL 环境**。变量会在多次执行之间保持不变。当您需要逐步构建状态、测试 API、查看 DataFrame 或对复杂代码进行迭代开发时，可选用此功能而非 `execute_code`。
