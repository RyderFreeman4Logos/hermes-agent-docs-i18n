...
通过 xurl CLI 操作 X/Twitter：发布内容、搜索、发送私信、处理媒体文件以及调用 v2 API。
...
`xurl` 是 X 开发者平台专为 X API 提供的官方 CLI 工具。它既支持针对常见操作的快捷命令，也允许以原始的 curl 风格访问任何 v2 端点。所有命令都会将结果以 JSON 格式输出到标准输出流。
...
- 发送私信
- 上传媒体文件（图片和视频）
- 以原始方式访问任何 X API v2 端点
- 多应用/多账户工作流

该技能已取代旧版的 `xitter` 技能（后者基于第三方 Python CLI 开发）。`xurl` 由 X 开发者平台团队维护，支持带自动刷新功能的 OAuth 2.0 PKCE 认证，且支持的 API 功能范围更为广泛。

---

## 密钥安全（强制要求）

在 Agent/LLM 会话中使用时必须遵守的严格规则：

- **绝不可**读取、打印、解析、总结、上传或向 LLM 上下文发送 `~/.xurl` 文件中的内容。
- **绝不可**要求用户在聊天界面中粘贴凭证/令牌。
- 用户必须在自己的设备上手动在 `~/.xurl` 文件中填写密钥信息。在 Docker 环境下，该文件路径应为 Hermes 工具子进程所能识别的 `~` 路径；具体说明请参见下方的 Docker 使用注意事项。
- **绝不可**在 Agent 会话中推荐或执行包含内联密钥的认证命令。
- **绝不可**在 Agent 会话中使用 `--verbose` / `-v` 参数——这可能会泄露认证头信息/令牌。
- 如需验证凭证是否存在，仅可使用命令：`xurl auth status`。

Agent 命令中禁止使用的参数（这些参数会接收内联密钥）：
`--bearer-token`, `--consumer-key`, `--consumer-secret`, `--access-token`, `--token-secret`, `--client-id`, `--client-secret`

应用凭证的注册及轮换操作必须由用户在 Agent 会话之外手动完成。完成凭证注册后，用户还需通过 `xurl auth oauth2` 命令进行认证——同样需在 Agent 会话之外操作。令牌会以 YAML 格式存储在 `~/.xurl` 文件中，每个应用拥有独立的令牌，且 OAuth 2.0 令牌会自动刷新。

---

## 安装

请选择一种安装方式。在 Linux 系统上，使用 shell 脚本或 `go install` 命令最为便捷。
