...
通过 CDP 查看实时的 Hermes 桌面端 DOM/CSS 结构。
...
在开发 `apps/desktop` 类型应用且用户正在运行该应用（通过 `hgui` 或 `npm run dev`）时，你可以查看用户当前所查看窗口的**实时渲染 DOM**——包括计算后的样式、几何信息、实际生效的 CSS 规则以及控制台输出——而无需仅依据 `.tsx` 文件进行推断，从而避免出错。

开发服务器会自动在 `127.0.0.1:9222` 端口上开启 Chrome DevTools Protocol 接口。由于渲染器基于 Chromium 页面，因此 DevTools 能读取的所有内容，脚本也同样可以获取。

**但这并不能替代人工查看。** CDP 能回答那些基于事实的问题（例如“计算后的内边距是多少”、“该元素是否已成功渲染”、“哪个选择器匹配”），却无法判断视觉效果是否良好。色彩平衡、间距感受以及“设计是否美观”等问题仍需依靠用户肉眼或截图来判断。用 CDP 回答事实性问题，而将审美判断留给用户。

## 适用场景

- 验证 UI 变更是否已在运行中的应用程序中生效
- 当遇到“为什么这个元素仍然是 X 值？”的情况时——在修改任何内容之前先找出决定其属性的规则
- 为即将修改的组件寻找稳定的选择器
- 在真实节点上查看设计令牌的计算值
- 查看用户提及但无法复制的渲染器控制台错误信息

**不建议用于：** 性能分析或堆内存调试（如 `node-inspect-debugger`、`debugging-hermes-desktop`），以及那些核心问题是“视觉效果是否正确”的场景。

## 端口设置

所有运行的开发服务器都会在 `127.0.0.1:9222` 端口开启该接口。仅在以下两种情况下会关闭该端口（位于 `apps/desktop/electron/dev-cdp.ts` 文件中）：

- **打包后的应用程序**——始终关闭，且任何环境变量都无法改变这一设置；
- **未设置 `HERMES_DESKTOP_DEV_SERVER`**——对于未打包的 `electron .` 应用程序，若其针对 `dist/` 目录运行，则会被视为打包应用进行功能测试，因此行为也与打包应用一致。

`HERMES_DESKTOP_CDP_PORT` 可用于更改端口号（值为 `9333`）或直接禁用该功能（值为 `off`）。
