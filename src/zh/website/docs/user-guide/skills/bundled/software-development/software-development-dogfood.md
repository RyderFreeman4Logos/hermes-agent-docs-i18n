...
本技能将指导您使用浏览器工具集对 Web 应用程序进行系统化的探索性质量检测。您需要浏览应用程序、与界面元素交互、收集问题相关证据，并生成结构化的缺陷报告。
...
由用户提供：
1. **目标网址**——测试的入口点  
2. **范围**——需要重点测试的区域或功能（如需全面测试，则选择“整个站点”）  
3. **输出目录**（可选）——用于保存截图和报告的路径（默认值为 `./dogfood-output`）
