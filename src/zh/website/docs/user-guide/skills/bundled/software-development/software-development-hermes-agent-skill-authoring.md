...
在代码库中编写 SKILL.md：包括 frontmatter、验证规则及结构要求。
...
:::info
以下是当触发该技能时 Hermes 会加载的完整技能定义。技能处于激活状态时，代理程序会将其视为操作指令。
:::

# 在代码库中编写 Hermes-Agent 技能

## 概述

SKILL.md 文件可以存储在两个位置：

1. **用户本地**：`~/.hermes/skills/<可选分类>/<名称>/SKILL.md` —— 为个人使用，不会共享。可通过 `skill_manage(action='create')` 创建。
2. **代码库中（适用于当前项目）**：`/home/bb/hermes-agent/skills/<分类>/<名称>/SKILL.md` —— 会被提交并随软件包一起分发。需使用 `write_file` 和 `git add` 操作。`skill_manage(action='create')` 不适用于此目录结构。

## 适用场景

- 用户要求你在“当前分支/代码库/提交版本”中添加技能
- 你要提交的可复用工作流需要随 hermes-agent 一同分发
- 你需要编辑 `/home/bb/hermes-agent/skills/` 下的现有技能（小范围修改可使用 `patch`，整体重写可使用 `write_file`；对于代码库中的技能，`skill_manage` 仍可用于执行 patch 操作，但不支持创建操作）

## 必需的 frontmatter 要求

验证规则来源：`tools/skill_manager_tool.py::_validate_frontmatter`。必须满足以下要求：

- 文件开头以 `---` 开头（不能有前置空行）
- 内容之前以 `\n---\n` 结尾
- 文件格式需为 YAML 映射结构
- 必须包含 `name` 字段
- 必须包含 `description` 字段，长度不超过 **1024 字符**（即 `MAX_DESCRIPTION_LENGTH` 的限制）
- 在结尾的 `---` 之后必须有非空内容

`skills/software-development/` 下的所有技能均遵循此统一结构规范：
