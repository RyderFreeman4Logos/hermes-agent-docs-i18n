...
| 版本 | `2.0.0` |
| 开发者 | Francesco Bonacci (f-trycua)、Hermes Agent |
| 许可协议 | MIT |
...
# 计算机操作（通用型，支持所有模型，跨平台）

您拥有一个 `computer_use` 工具，可在**后台**控制用户的桌面——您的操作不会移动用户的光标、抢占键盘焦点，也不会切换虚拟桌面或屏幕分区。用户可以在编辑器中继续输入，而您则可在另一个窗口的浏览器中进行操作。这与 pyautogui 风格的自动化方式截然不同。

此处介绍的所有功能均适用于任何具备工具调用能力的模型——无论是 Claude、GPT、Gemini，还是本地 OpenAI 兼容端点上的开源模型。无需学习 Anthropic 特有的架构规范。

Hermes 在底层驱动 [cua-driver](https://github.com/trycua/cua) 的运行。  
该封装技能会向 Hermes 教授 `computer_use` 工作流以及相应的动作词汇表。请使用下文所列的动作，而非直接调用 cua-driver 的 MCP 工具。如需了解驱动的内部实现及特定平台的行为特性，请参考通过 `cua-driver skills install` 安装的 Cua 技能。Hermes 的自动检测功能是 cua-driver 的后续计划，因此目前请将 Hermes 指向生成的 `~/.cua-driver/skills/cua-driver` 目录，或将其符号链接到您的技能空间中。

## 标准工作流

**第一步——先进行信息采集。** 几乎所有任务都是从这一步开始的：
