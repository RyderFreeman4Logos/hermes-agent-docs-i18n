...
通过 Claude Code CLI 授权代码编写任务（功能、PR 等操作）。
...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。当技能处于激活状态时，智能体将以此作为操作指令。
:::

# Claude Code — Hermes 编排指南

通过 Hermes 终端将代码编写任务委托给 [Claude Code](https://code.claude.com/docs/en/cli-reference)（Anthropic 的自主编码智能体 CLI）。Claude Code v2.x 能够自主读取文件、编写代码、执行 shell 命令、创建子智能体以及管理 git 工作流。

## 先决条件

- **安装：** `npm install -g @anthropic-ai/claude-code`  
- **认证：** 运行一次 `claude` 即可完成登录（Pro/Max 版通过浏览器 OAuth 认证，或设置 `ANTHROPIC_API_KEY`）  
- **控制台认证：** 使用 `claude auth login --console` 进行 API 密钥相关操作  
- **SSO 认证：** 企业版用户可使用 `claude auth login --sso` 登录  
- **查看状态：** 使用 `claude auth status`（返回 JSON 格式）或 `claude auth status --text`（以易读文本形式显示）  
- **健康检查：** 运行 `claude doctor` 可检查自动更新器及安装状态  
- **版本查询：** 使用 `claude --version` 查看版本信息（需使用 v2.x+ 版本）  
- **更新：** 使用 `claude update` 或 `claude upgrade` 进行升级  

## 两种编排模式

Hermes 与 Claude Code 的交互方式存在本质区别，可根据具体任务选择合适的模式。  

### 模式 1：打印模式（`-p`）——非交互式（适用于大多数任务）  

打印模式用于执行一次性任务，处理完成后立即退出，无需伪终端，也不会出现交互式提示符。这是最为简洁的集成方式。
...
- `●` 标记的行 = Claude 正在主动使用工具（读取、写入或执行命令）
...
- **> 85%** — 出现幻觉的风险显著上升，建议使用 `/compact` 或 `/clear` 命令
...
| `MAX_THINKING_TOKENS` | 设置思考令牌的上限（设为 `0` 可完全禁用思考功能） |
| `MAX_MCP_OUTPUT_TOKENS` | 限制 MCP 服务器的输出长度（默认值因版本而异，可设置为例如 `50000`） |
