...
将编码任务委托给 Claude Code CLI（支持功能、PR 处理等）。
...
通过 Hermes 终端将编码任务委托给 [Claude Code](https://code.claude.com/docs/en/cli-reference)（Anthropic 的自主编码智能体 CLI）。Claude Code v2.x 能够自主读取文件、编写代码、执行 Shell 命令、创建子智能体以及管理 Git 工作流。

## 先决条件

- **安装：** `npm install -g @anthropic-ai/claude-code`
- **认证：** 运行一次 `claude` 进行登录（Pro/Max 版通过浏览器 OAuth 登录，或设置 `ANTHROPIC_API_KEY`）
- **控制台认证：** 使用 API 密钥计费时，运行 `claude auth login --console`
- **SSO 认证：** 企业版用户可使用 `claude auth login --sso` 登录
- **检查状态：** 使用 `claude auth status`（输出 JSON 格式）或 `claude auth status --text`（人类可读格式）
- **健康检查：** 运行 `claude doctor` 可检查自动更新器及安装状态的健康状况
- **版本检查：** 运行 `claude --version`（需使用 v2.x 及以上版本）
- **更新：** 使用 `claude update` 或 `claude upgrade` 进行更新

## 两种编排模式

Hermes 与 Claude Code 的交互存在两种根本不同的方式。可根据具体任务选择合适模式。

### 模式 1：打印模式（`-p`）——非交互式（大多数任务的首选）

打印模式用于执行一次性任务，返回结果后便退出。无需 PTY，也不会有交互式提示。这是最简洁的集成方式。
...
请注意以下指示符号：
- 底部显示的 `❯` = 正在等待你的输入（Claude 已完成操作或正在提问）
- 显示为 `●` 的行 = Claude 正在主动使用工具（读取、写入或执行命令）
- `⏵⏵ bypass permissions on` = 状态栏显示的权限模式
- `◐ medium · /effort` = 状态栏中显示的当前工作强度等级
- `ctrl+o to expand` = 工具输出被截断（可交互式展开查看）

### 上下文窗口状态
在交互模式下使用 `/context` 命令，即可查看上下文使用情况的彩色网格图。关键阈值如下：
- **< 70%** — 正常运行，保持完整精度
- **70-85%** — 精度开始下降，建议使用 `/compact` 命令压缩上下文
- **> 85%** — 出现幻觉的风险显著上升，应立即使用 `/compact` 或 `/clear` 清理上下文
...
| `CLAUDE_CODE_EFFORT_LEVEL` | 默认工作强度：`low`、`medium`、`high`、`max` 或 `auto` |
| `MAX_THINKING_TOKENS` | 设置思考token的上限（设为 `0` 可完全禁用思考功能） |
| `MAX_MCP_OUTPUT_TOKENS` | 限制来自 MCP 服务器的输出长度（默认值因版本而异，可自行设置，例如 `50000`） |
| `CLAUDE_CODE_NO_FLICKER=1` | 启用备用屏幕渲染模式，消除终端闪烁现象 |
| `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB` | 为保障安全，从子进程中移除敏感凭证 |

## 成本与性能优化建议

1. 在打印模式下使用 `--max-turns` 参数，可防止对话陷入无限循环。对于大多数任务，初始值可设置为 5-10。
2. 使用 `--max-budget-usd` 设置成本上限。注意：仅创建系统提示词缓存就需要至少约 0.05 美元。
3. 对于简单任务，使用 `--effort low` 参数，既能提升速度又能降低成本；对于复杂推理任务，则可使用 `high` 或 `max`。
4. 在 CI/脚本环境中使用 `--bare` 参数，可跳过插件和钩子的检测过程，提高效率。
5. 使用 `--allowedTools` 参数，仅允许 Claude 使用完成任务所需的工具（例如在代码审查时仅允许使用“读取”工具）。
6. 当上下文内容过多时，在交互会话中使用 `/compact` 命令压缩上下文。
7. 如果只需分析已知内容，可直接通过管道传输输入数据，无需让 Claude 读取文件。
8. 对于简单任务，使用 `--model haiku` 可降低成本；而对于复杂的多步骤任务，则建议使用 `--model opus`。
9. 在打印模式下使用 `--fallback-model haiku`，可在模型负载过重时实现平滑切换。
10. 不同任务请开启新的会话——单个会话时长为 5 小时，使用全新上下文能提升效率。
11. 在 CI 环境中使用 `--no-session-persistence` 参数，可避免已保存的会话在磁盘上不断累积。
...
3. **`--max-budget-usd` 的最低值为约 0.05 美元** —— 仅创建系统提示词缓存就需要这么多成本。设置低于此值的参数会立即引发错误。
4. **`--max-turns` 仅适用于打印模式** —— 在交互会话中该参数会被忽略。
5. **Claude 可能会使用 `python` 而非 `python3`** —— 在没有 `python` 符号链接的系统中，Claude 的 bash 命令首次执行时会失败，但它会自动进行修正。
6. **恢复会话时需要处于相同的目录下** —— `--continue` 参数会查找当前工作目录下的最新会话。
7. **使用 `--json-schema` 时需要足够的 `--max-turns` 参数** —— Claude 需要先读取相关文件才能生成结构化输出，这一过程需要多轮对话。
8. **“信任此对话”选项每个目录仅会出现一次** —— 仅首次出现时会弹出，之后会直接从缓存中调用。
9. **后台运行的 tmux 会话会持续存在** —— 使用完成后务必通过 `tmux kill-session -t <name>` 命令终止这些会话，避免资源泄漏。
10. **斜杠命令（如 `/commit`）仅在交互模式下有效** —— 在打印模式（`-p`）下，需用自然语言描述任务。
11. **`--bare` 参数会跳过 OAuth 验证** —— 此时必须设置 `ANTHROPIC_API_KEY` 环境变量，或是在设置中配置 `apiKeyHelper`。
12. **上下文容量过高确实会影响输出质量** —— 当上下文窗口使用率超过 70% 时，AI 的输出质量会明显下降。建议使用 `/context` 命令监控状态，并及时通过 `/compact` 压缩上下文。

## Hermes Agent 使用规则

1. 对于单次任务，优先使用打印模式（`-p`）—— 输出更整洁，无需处理对话框，且结果为结构化格式。
2. 多轮交互式任务请务必使用 tmux —— 这是管理 TUI 应用任务的唯一可靠方式。
3. 始终设置 `workdir` 参数 —— 确保 Claude 专注于正确的项目目录。
4. 在打印模式下设置 `--max-turns` 参数 —— 可防止无限循环和成本失控。
5. 定期监控 tmux 会话状态 —— 可使用 `tmux capture-pane -t <session> -p -S -50` 命令查看任务进度。
6. 注意观察 `❯` 指示符 —— 它表示 Claude 正在等待用户输入（已完成操作或正在提问）。
7. 使用完成后及时清理 tmux 会话 —— 终止会话可避免资源浪费。
8. 向用户汇报任务结果 —— 任务完成后，需总结 Claude 的操作内容以及产生的变化。
9. 不要强制终止运行缓慢的会话 —— Claude 可能正在进行多步骤处理，建议先查看进度。
10. 使用 `--allowedTools` 参数 —— 仅授予任务所需的权限，避免功能冗余。
