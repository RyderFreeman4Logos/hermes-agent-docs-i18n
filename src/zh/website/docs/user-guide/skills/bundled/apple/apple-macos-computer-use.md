...
可在后台控制 macOS 桌面——包括截图、鼠标操作、键盘输入、滚动、拖动等——且不会抢占用户的光标、键盘焦点或屏幕空间。该技能适用于所有具备工具调用功能的模型。只要系统中存在 `computer_use` 工具，即可加载此技能。
...
:::info
以下是 Hermes 在触发此技能时加载的完整技能定义。当该技能处于激活状态时，智能体看到的指令即为内容。
:::

# macOS 电脑操作（通用，适用于所有模型）

您拥有一个可在**后台**控制 Mac 的 `computer_use` 工具。您的操作不会移动用户的光标、抢占键盘焦点或切换屏幕空间。即便您在另一个屏幕空间中使用 Safari 浏览网页，用户仍可继续在编辑器中输入内容。这与 pyautogui 风格的自动化方式截然不同。

此处介绍的所有功能均适用于所有具备工具调用功能的模型——无论是 Claude、GPT、Gemini，还是通过本地 OpenAI 兼容端点运行的开源模型。无需学习 Anthropic 特有的架构规范。

## 标准工作流程

**第一步——先进行截图。** 几乎所有任务都是从这一步开始的：
