...
通过 arXiv 提供的免费 REST API 搜索并获取学术论文。无需 API 密钥，也无需额外依赖，只需使用 curl 即可。
...
该 API 会返回 Atom XML 格式的数据。可使用 `grep`/`sed` 进行解析，或通过 `python3` 处理以获得整洁的输出结果。
...
在获取某篇论文的元数据后，会自动生成对应的 BibTeX 条目：
