...
通过存档快照与备用读取方式来恢复被屏蔽、付费墙限制或 WAF 阻拦的页面。当 web_extract 功能或浏览器遇到 403/429 错误、挑战页面、付费墙内容，或是机器人检测弹窗时，可选用此功能。
...
当页面无法正常获取——出现 403/429 错误、Cloudflare 的“稍候…”提示、付费墙限制，或是机器人检测弹窗——请不要放弃，也无需反复尝试同一网址。第三方服务通常会保存该页面的**副本**。可按成本从低到高的顺序，依次尝试以下方法。
