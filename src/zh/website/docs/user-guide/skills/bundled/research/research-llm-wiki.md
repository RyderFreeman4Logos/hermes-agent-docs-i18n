...
通过相互关联的 Markdown 文件来构建并维护一个持久且不断扩充的知识库。
该功能基于 [Andrej Karpathy 的 LLM Wiki 模式](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)。

与传统 RAG（每次查询都需要从头重新检索知识）不同，该 Wiki 会一次性整合所有知识并保持其最新状态。其中的交叉引用早已存在，矛盾之处也会被标记出来，最终的综合结果会反映所有已获取的信息。

**分工协作：** 由人类负责筛选资料并指导分析方向；智能体则负责总结内容、建立交叉引用、整理文件并确保一致性。

## 何时使用该技能

当用户出现以下情况时，可使用此技能：
- 要求创建、构建或启动一个 Wiki 或知识库
- 要求将某份资料导入、添加到 Wiki 中或对其进行处理
- 提出问题且配置路径下已存在相关 Wiki
- 要求对 Wiki 进行代码检查、审计或健康状况检测
- 在研究过程中提及自己的 Wiki、知识库或“笔记”

## Wiki 的存储位置

**位置：** 通过 `WIKI_PATH` 环境变量设置（例如在 `${HERMES_HOME:-~/.hermes}/.env` 文件中）。

如果未设置该变量，则默认路径为 `~/wiki`。
