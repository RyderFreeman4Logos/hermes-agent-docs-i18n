...
通过相互关联的 Markdown 文件来构建并维护一个持久、不断扩充的知识库。
该方案基于 [Andrej Karpathy 的 LLM Wiki 模式](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)。

与传统 RAG（每次查询都需要从头重新检索知识）不同，该 Wiki 仅需构建一次知识库即可保持其最新状态。其中的交叉引用早已设置完成，矛盾之处也会被标记出来，最终的综合结果会涵盖所有已获取的信息。

**分工协作：** 人类负责筛选资料并指导分析方向；智能体则负责总结内容、建立交叉引用、整理文件并确保信息一致性。

## 该技能的触发场景

当用户出现以下情况时，可使用此技能：
- 要求创建、搭建或启动维基页面或知识库
- 要求将某个来源内容导入、添加到或处理其维基页面中
- 提出问题且配置路径下已存在相关维基页面
- 要求对维基页面进行代码检查、审计或健康状况检测
- 在研究场景中提及自己的维基页面、知识库或“笔记”
...
该维基实际上只是一个包含 Markdown 文件的目录——您可以在 Obsidian、VS Code 或任何文本编辑器中打开它。无需数据库，也不需要任何特殊工具。
...
## 恢复现有的维基内容（非常重要——请在每次会话前执行）

如果用户已有现有维基内容，**在采取任何操作之前务必先了解以下信息**：

① **阅读 `SCHEMA.md`** — 了解相关领域、编写规范以及标签分类体系。
② **阅读 `index.md`** — 了解现有的页面及其概要。
③ **查看最近的 `log.md` 记录** — 阅读最近 20-30 条记录，以掌握近期活动情况。
