...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。技能运行时，智能体看到的指令即为内容。
:::

# 有依据的引用

所有来自外部来源的表述都会附带行内编号引用，以及类似 Perplexity 风格的 `Sources:` 列表。专门的脚本负责管理 `url → [n]` 的映射关系，因此编号与网址均来自检索结果，而非模型内存——模型仅会输出其接收到的小整数。

在处理高重要性的任务时，同一份日志还会充当事实核查链条的角色：每个信息来源都会附带原文引用（除非该内容确实出现在获取的页面文本中，否则会被拒绝）；模型生成的结论会标记为 `[unverified]`；而对于那些引用的来源缺乏相关证据的草稿，`verify --evidence` 命令会将其判定为无效。

该技能可处理聊天回复、书面文档（Markdown、PDF、DOCX、幻灯片）以及研究报告中的内容。但它不适用于学术领域的 BibTeX 处理流程——如需处理会议论文，请使用 `arxiv` 技能，而该技能会为 `arxiv` 提供数据支持（详见 `references/citation-formats.md`）。

## 适用场景

凡是答案或输出内容基于你获取的信息而非已有知识时，均可使用此技能：
- 研究、对比分析、新闻摘要，以及“X 的当前状况如何”这类问题；
- 任何需要写入磁盘的文档，只要其中包含对外部事实的引用、改写或汇报，如报告、简报、文档、演示文稿、维基页面等；
- 需要用户对结果进行事实核查的场景；
- 需要综合多源信息且必须明确标注不同来源的场合。

如果信息检索只是作为其他任务的附带操作——比如在编码过程中快速查找语法或版本信息、闲聊，或是进行创意写作时——则无需使用内联引用。只有当用户很可能需要该链接时，才应提供网址。

## 先决条件

除标准工具集外无需其他额外条件。`scripts/sources.py` 仅为基于 Python 3 标准库编写的文件。信息检索功能可通过所配置的方式实现，包括 `web_search`、`web_extract`、`browser_navigate` 或 `terminal`（如 curl、各类命令行工具）。

账本存储位置：` $HERMES_HOME/cache/citations/ledger.json`（支持根据配置文件自动调整）。如需为特定任务指定不同路径，可使用 `--ledger <路径>` 或 `HERMES_CITATION_LEDGER` 参数进行覆盖。
