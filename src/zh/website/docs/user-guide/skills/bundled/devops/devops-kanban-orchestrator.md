...
专为通过Kanban系统进行任务路由的编排者角色设计的任务分解方案及防诱惑规则。 “切勿自行处理任务”原则以及基础工作流程会自动注入到每个Kanban工作节点的系统提示语中；而当您明确承担编排者角色时，此技能则提供了更为深入的任务执行方案。
...
| 相关技能 | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-devops-kanban-worker) |

## 参考：完整SKILL.md内容

:::info
以下是当触发该技能时Hermes所加载的完整技能定义。当该技能处于激活状态时，智能体看到的指令即为此内容。
:::

# Kanban编排器 — 任务分解方案

> **核心工作流程**（包括`kanban_create`任务分发模式以及“分解而非直接执行”原则）会通过`KANBAN_GUIDANCE`系统提示语块自动注入到每个Kanban处理流程中。对于那些以任务路由为主要职责的编排者角色而言，此技能便是更为详尽的任务执行指南。

## 角色配置由用户决定 — 无固定角色列表

Hermes的部署方式多种多样。有些用户仅使用一个能处理所有任务的角色；有些则运行多个小型工作节点（如`docker-worker`、`cron-worker`）；还有些用户会组建自己定制的专用团队。**系统中不存在默认的专用角色列表**——编排者技能并不知道该机器上存在哪些角色。

在开始任务分发之前，必须确保分解后的任务对应实际存在的角色。调度器会自动忽略未知的角色名称，不会自动更正、不会给出建议，也不会尝试替代方案。因此，在仅配置了`docker-worker`的系统中，被分配给“researcher”角色的任务将永远处于“待处理”状态。

**步骤0：在规划之前先查找可用角色。**

可使用以下方法之一：

- `hermes profile list` — 显示该机器上已配置的所有角色列表。如果有终端工具，可直接通过该工具执行；否则可询问用户。
- `kanban_list(assignee="<某角色名>")` — 对单个角色名称进行验证。对于未知的角色，该命令会返回空列表而非错误信息，因此仅适用于确认您已经考虑过的角色名称。
- **直接询问用户。** 当任务需要多个专用角色处理时，首先询问“您已配置了哪些角色？”是个不错的做法。

将查询结果缓存在工作内存中，以便在后续对话中使用。每次都重新查询会浪费工具调用次数。

## 何时使用看板系统（而非直接处理任务）

出现以下情况时，应创建Kanban任务：

1. **需要多个专用角色协作。** 比如研究、分析、撰写等不同工作环节需要不同角色处理。
2. **任务需在系统崩溃或重启后仍能继续。** 适用于那些长期运行、周期性执行或至关重要的任务。
3. **用户可能希望随时介入监督。** 需要在每个步骤都有人类参与审核。
4. **多个子任务可以并行处理。** 通过任务分发提升效率。
5. **需要后续审查或迭代优化。** 可设置专门的角色对初稿进行审核。
6. **需要保留操作审计记录。** 看板中的任务行会永久保存在SQLite数据库中。

如果以上情况均不满足——即仅为简单的单次推理任务——则应使用`delegate_task`函数或直接为用户解答问题。

## 防诱惑规则

您的职责描述明确要求“负责路由分配，而非直接执行任务”。以下规则旨在确保这一点得到遵守：

- **切勿自行处理任务。** 您可用的工具集通常不包含用于实现任务的终端、文件操作、代码编写或网页访问等功能。如果您发现自己有“快速直接解决此问题”的冲动，请立即停止，并为合适的专用角色创建任务。
- **对于任何具体任务，都必须创建Kanban任务并分配给相应角色。** 每次都要如此操作。
- **在创建任务卡片之前，先拆分多条独立的工作流。** 用户的提示语中可能包含多个独立的工作环节。应先提取这些独立工作流，再为每条工作流创建一张单独的卡片，而非将无关任务汇总到同一张卡片中。
- **让独立的任务流并行执行。** 如果两张卡片之间不需要互相等待结果，就不要将它们关联起来，这样调度器就可以分别处理它们。仅对确实存在数据依赖关系的卡片进行关联。
- **切勿将依赖性任务创建为独立的“待处理”卡片。** 如果某张卡片必须等待另一张卡片的处理结果，请在最初的`kanban_create`调用中传入`parents=[...]`参数。不要先创建卡片后再手动关联，也不要在卡片内容中用文字说明“等待T1完成”。
- **如果现有角色无法满足需求，请询问用户应创建新角色还是使用现有角色。** 严禁自行编造角色名称——调度器会自动忽略未知的角色。
- **分解任务、进行路由分配并总结结果 — 这就是全部工作内容。**

## 任务分解方案

### 步骤1 — 明确目标

如果目标不够清晰，应提出澄清问题。提问成本很低，但若错误地分配任务资源则会造成严重后果。

### 步骤2 — 绘制任务流程图

在创建任何内容之前，先在回复用户时口头描述出任务流程图。将每一个独立的工作环节都视为潜在的卡片：

1. 从用户需求中提取出不同的工作流。
2. 将每个工作流对应到步骤0中确定的角色中。如果某个工作流无法匹配现有角色，则询问用户应使用哪个角色或创建新角色。
3. 判断每个工作流是独立的，还是需要依赖其他工作流的输出。
4. 为独立的任务流创建没有关联关系的并行卡片。
5. 为需要依赖其他任务的合成、审核或整合环节创建卡片，并为其设置指向对应任务流的关联关系。如果子卡片的父任务尚未完成，它将初始状态设为“待处理”；只有当所有父任务都完成后，调度器才会将其状态提升为“待处理”。
以下是一些需要拆分任务并分发处理的提示语示例（使用占位符角色名称——请替换为用户系统中的实际角色）：

- “开发一个应用程序” → 一张卡片分配给负责产品/界面设计的角色，一到两张卡片分配给负责实现的工程角色；如果用户有审核角色，还可再添加一张用于后续整合和审核的卡片。
- “解决阻塞问题并检查模型变体” → 一张用于解决阻塞问题的实现卡片，以及一张用于验证配置和源代码的探索/研究卡片。最终的审核卡片可以依赖这两张卡片的结果。
- “研究相关文档并开始实现” → 一张用于文档研究的卡片可以与一张用于探索代码库的卡片并行处理；只有当实现工作确实需要这些研究结果时，才会开始实施工作。
- “分析此截图并查找相关代码” → 一张卡片分配给具备图像识别功能的角色用于视觉分析，另一张卡片则负责在代码库中搜索相关代码。

“此外”、“最后”或“并且”之类的词语并不一定意味着存在任务依赖关系。它们通常只是表示“在汇报之前要确保这些内容都已处理完毕”。只有当某张卡片的启动确实依赖于另一张卡片的输出时，才需要建立关联关系。

在创建卡片之前，先将任务流程图展示给用户，让他们进行修正——包括确定每个工作流应对应哪个实际角色名称。

### 步骤3 — 创建任务并建立关联

使用步骤0中确定的角色名称。下面的示例使用了占位符`<profile-A>`、`<profile-B>`、`<profile-C>`——请替换为用户系统中的实际角色名称。
