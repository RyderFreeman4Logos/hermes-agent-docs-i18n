...
使用 mcporter CLI 可以直接列出、配置、认证以及调用 MCP 服务器/工具（通过 HTTP 或标准输入输出方式），支持临时服务器的创建、配置文件的修改以及 CLI/类型生成等功能。
...
使用 `mcporter` 可以直接从终端发现、调用并管理 [MCP（模型上下文协议）](https://modelcontextprotocol.io/) 服务器及工具。
