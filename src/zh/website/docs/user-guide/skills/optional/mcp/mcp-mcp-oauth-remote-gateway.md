...
| 相关技能 | `native-mcp`, [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
Hermes 内置的 MCP OAuth 客户端会在 Hermes 进程内部的 `127.0.0.1:<port>` 地址上启动一个一次性运行的 HTTP 监听器，并将该回环地址注册为 OAuth 的 `redirect_uri`。对于用户本机上的本地 CLI 来说，这种方式运行得非常完美。但当 Hermes 作为远程网关（容器、VPS、消息机器人等）运行时，这种方法就会彻底失效——因为用户的浏览器会将 `127.0.0.1` 解析为用户自己的笔记本电脑地址，而非远程容器地址，因此授权码永远无法送达 Hermes。

该技能会手动完成 OAuth 授权流程，并将生成的令牌写入 Hermes 令牌存储系统所期望的特定文件中，这样后续执行 `/reload-mcp` 指令时就能找到缓存的令牌，从而完全跳过浏览器端的授权流程。

## 适用场景

当同时满足以下所有条件时，可使用此技能：

1. 用户希望添加一个需要 OAuth 授权的远程 HTTP MCP 服务器（而非使用静态 Bearer 令牌）。
2. Hermes 正在作为**远程网关**运行（容器、VPS、Docker、托管服务等），而非用户笔记本电脑上的本地 CLI。
3. 该服务器支持带有 PKCE 机制的 OAuth 2.1 以及 RFC 7591 规定的动态客户端注册功能（大多数现代 MCP 服务器都支持，如 Better Stack、Linear、Cloudflare、Datadog 等）。如果服务器不支持 DCR 功能（GitHub 是明显的例外），则此技能不适用——此时应使用预先注册的 OAuth 应用或个人访问令牌。

**以下场景请勿使用此技能：**
- **本地 CLI Hermes 环境** — 仅需在 `mcp_servers.<name>` 中设置 `auth: oauth`，然后执行 `/reload-mcp` 即可。内置的授权流程会自动打开浏览器，并在本地回环地址上捕获回调信息，运行效果非常好。
- **支持静态 Bearer 令牌（API 密钥）的服务器** — 如果用户愿意，始终优先使用 `headers.Authorization: "Bearer <token>"` 的方式，这种方式更简单，且无需处理令牌刷新问题。
- **GitHub Copilot MCP**（`api.githubcopilot.com/mcp/`）—— GitHub 不提供 DCR 功能，应使用个人访问令牌或预先注册的 OAuth 应用（参见注意事项 12）。

## 为什么内置 OAuth 流程在远程网关上会失败

Hermes 内置的 MCP OAuth 客户端（`tools/mcp_oauth.py`）的工作流程如下：

1. 选择一个空闲的本地端口 `P`。
2. 向认证服务端注册一个动态 OAuth 客户端，同时指定 `redirect_uri = http://127.0.0.1:P/callback`。
3. 在 Hermes 进程内部于 `127.0.0.1:P` 地址启动一个 HTTP 服务器。
4. 打印出授权 URL，并在其本地端点等待接收授权码。

当 Hermes 在远程环境中运行时，`redirect_uri` 中的 `127.0.0.1` 实际上是远程容器的回环地址，而非用户的本机地址。用户完成授权后，浏览器会尝试访问 `http://127.0.0.1:P/callback?code=...`，但该地址会被解析为用户自己的笔记本电脑地址，因此无法建立连接。回调请求永远无法送达 Hermes 进程，整个流程会超时，执行 `/reload-mcp` 后会返回“没有可用的 MCP 工具”这样的提示，且不会提供任何详细信息。

可识别的故障症状包括：在 hermes 用户下出现 `[xdg-open] <defunct>` 类型的进程，`$
