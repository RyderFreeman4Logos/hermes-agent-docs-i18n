...
| | |
|---|---|
| 来源 | 可选——通过 `hermes skills install official/mlops/outlines` 命令安装 |
...
- **需要自动依据 JSON Schema 进行内容生成**
- **希望在语法层面控制令牌采样过程**

**GitHub 星标数**：12,000+ | **来源**：dottxt.ai（原 .txt）

> **API说明（Outlines 1.x版本）：** 该技能针对当前v1版本的API设计。  
> 1.0版本之前的辅助函数（如`outlines.models.transformers(...)`, `outlines.generate.json/choice/regex/...`等）**已被移除**。在v1版本中，您需要通过`outlines.from_transformers(...)`（或`from_vllm`, `from_llamacpp`, `from_openai`）来创建模型，随后直接使用`model(prompt, output_type)`格式调用该模型，并指定输出类型。JSON/Pydantic格式的输出将以**JSON字符串**的形式返回——建议使用`YourModel.model_validate_json(result)`进行验证。
