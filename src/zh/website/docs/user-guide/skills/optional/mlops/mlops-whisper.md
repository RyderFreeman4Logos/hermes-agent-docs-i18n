...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。技能处于激活状态时，智能体将依据此内容执行相应操作。
:::
...
9. **使用 faster-whisper 模型** – 速度比 openai-whisper 快 4 倍  
...
1. **幻觉现象** – 可能会重复或编造内容  
2. **长音频处理精度** – 长度超过 30 分钟的音频效果较差  
3. **说话人识别功能缺失** – 不支持语音分段  
4. **口音处理效果不一** – 不同口音的生成质量差异较大  
5. **背景噪音影响** – 可能降低生成准确性  
6. **实时延迟** – 不适合用于实时字幕生成  
