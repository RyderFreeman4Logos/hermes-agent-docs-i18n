...
**核心优势：**
- **拥有 23,000 多个 GitHub 星标**
- 具备 GPT-4V 级别的能力（目标定位）
- 采用 Apache 2.0 许可协议
- 提供多种模型规模选择（7B-34B 参数量）

**可选替代方案：**
- **GPT-4V**：基于 API 的最高质量模型  
- **CLIP**：简单的零样本分类功能  
- **BLIP-2**：更适用于文本标注任务  
- **Flamingo**：处于研究阶段，尚未开源  
...
2. **采用4位量化**——可显著降低显存需求  
3. **需配备GPU**——使用CPU推理速度极慢  
4. **明确提示词**——具体的问题能获得更优质的回答  
5. **多轮对话**——有助于保持对话上下文  
...
| 模型 | FP16格式显存需求 | 4位量化显存需求 | 每秒生成Token数 |
|------|------------------|------------------|----------------|
| 7B   | 约14 GB           | 约4 GB            | 约20            |
...
1. **幻觉现象**——可能会描述图片中不存在的内容  
2. **空间推理能力较弱**——难以准确判断位置关系  
3. **小字体识别困难**——难以看清细微文字  
4. **物体计数不精准**——对多个物体的统计往往有误差  
5. **显存需求高**——需要性能强大的GPU  
6. **推理速度较慢**——低于CLIP模型  
