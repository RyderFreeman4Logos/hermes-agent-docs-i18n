...
SAM：通过点、框和掩码实现零样本图像分割。

## 技能元数据

| | |
|---|---|
| 来源 | 可选 —— 使用 `hermes skills install official/mlops/segment-anything-model` 安装 |
...
本指南介绍了如何使用 Meta AI 的 Segment Anything 模型实现零样本图像分割。
...
**核心功能：**
- **零样本分割**：无需微调即可处理任意图像领域的数据  
- **灵活的提示方式**：支持使用点、边界框或之前的掩码作为输入  
- **自动分割**：可自动生成所有物体掩码  
- **高精度输出**：基于1100万张图片中的11亿个掩码进行训练  
- **多种模型规格**：ViT-B（速度最快）、ViT-L、ViT-H（精度最高）  
- **ONNX格式导出**：可在浏览器及边缘设备上部署  

**可选替代方案：**  
- **YOLO/Detectron2**：适用于需要分类的实时物体检测  
- **Mask2Former**：适用于需分类的语义/全景分割任务  
- **GroundingDINO + SAM**：适用于基于文本提示的分割功能  
- **SAM 2**：适用于视频分割任务  
...
| 分割掩码质量差 | 尝试不同的提示词，同时使用框和点的方式 |
| 边缘区域出现伪影 | 使用 stability_score 进行过滤 |
| 小物体被遗漏 | 提高 points_per_side 的数值 |

## 参考资料

- **[高级用法](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/models\segment-anything-model/references/advanced-usage.md)** - 批量处理、微调及集成方法
- **[故障排除](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/mlops/models\segment-anything-model/references/troubleshooting.md)** - 常见问题及解决方案

## 资源链接

- **GitHub 仓库**：https://github.com/facebookresearch/segment-anything
...
- **HuggingFace 页面**：https://huggingface.co/facebook/sam-vit-huge
