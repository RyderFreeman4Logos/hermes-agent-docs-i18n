...
可将静态网站发布到 &#123;slug&#125;.here.now，同时将私有文件存储在云盘 Drives 中，以便在不同智能体之间传递。
...
here.now 允许智能体发布网站，并将私有文件存储在云盘 Drives 中。

该技能可用于完成两项任务：

- **网站发布**：将网站及文件发布到 `{slug}.here.now` 地址。
- **文件存储**：将智能体的私有文件存储在云盘文件夹中。
...
**如果文档内容与实际 API 行为存在差异，请以实际 API 行为为准。**

若文档加载失败或超时，则继续依据本地的技能说明以及实际 API/脚本的输出结果进行操作。对于正在进行的操作，建议优先遵循实际 API 的行为规范。
