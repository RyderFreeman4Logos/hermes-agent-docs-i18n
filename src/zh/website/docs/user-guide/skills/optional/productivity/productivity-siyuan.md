...
通过 curl 调用 [SiYuan](https://github.com/siyuan-note/siyuan) 核心 API，即可对自托管知识库中的块和文档进行搜索、读取、创建、更新及删除操作。无需额外工具——只需 curl 和 API 令牌即可。
...
{"code": 0, "msg": "", "data": { ... }}
```
`code: 0` 表示操作成功。其他数值则表示出现错误——请查看 `msg` 字段以获取详细信息。
...
此外还提供以下接口：`/api/block/prependBlock`（参数相同，用于在块开头插入内容）以及 `/api/block/insertBlock`（通过 `previousID` 而非 `parentID` 来在指定块之后插入内容）。
...
- **ID 格式验证**：ID 必须符合 `YYYYMMDDHHmmss-xxxxxxx` 的格式，其他格式的 ID 均会被拒绝。
