...
配置 Twilio 号码、短信/MMS 以及人工智能外呼功能。
...
| 路径 | `optional-skills/productivity\telephony` |
| 版本 | `1.0.0` |
| 开发者 | Nous Research |
| 许可协议 | MIT |
...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。技能处于激活状态时，智能体将看到这些指令作为操作指南。
:::

# 电话服务 —— 无需修改核心工具即可实现号码、通话与短信功能

这款可选技能为 Hermes 增加了实用的电话功能，同时不会将其纳入核心工具列表中。

它附带了一个辅助脚本 `scripts/telephony.py`，可用于：
- 将提供商凭证保存至 `${HERMES_HOME:-~/.hermes}/.env` 文件中  
- 搜索并购买 Twilio 电话号码  
- 在后续会话中记住该号码以便重复使用  
- 通过该号码发送短信或彩信  
- 无需 webhook 服务器即可轮询该号码的来电短信  
- 使用 TwiML 的 `<Say>` 或 `<Play>` 标签直接发起 Twilio 呼叫  
- 将已拥有的 Twilio 号码导入 Vapi 系统  
...
- 查看后续发送到该号码的消息  
- 在不同会话之间保留该号码及相关标识信息  
- 为来电短信轮询及其他自动化场景提供面向未来的电话身份解决方案  

需要注意的是，它并不会将 Hermes 转变为实时的来电电话网关。来电短信功能是通过轮询 Twilio REST API 来实现的。对于包括通知发送及部分一次性验证码获取在内的许多工作流而言，这种方式已足够使用，且无需额外搭建核心 webhook 基础设施。  
...
仅显示在上一个检查点之后收到的消息，阅读完毕后再将检查点向前推进：
