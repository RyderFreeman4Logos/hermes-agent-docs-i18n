...
无需修改核心工具即可为 Hermes 添加电话功能。该功能可配置并持久保存 Twilio 号码，实现短信/MMS 的收发、直接拨打电话，还能通过 Bland.ai 或 Vapi 发起由人工智能驱动的外呼。
...
- 使用 TwiML 的 `<Say>` 或 `<Play>` 功能直接拨打 Twilio 电话
- 将已拥有的 Twilio 号码导入 Vapi
- 通过 Bland.ai 或 Vapi 发起人工智能驱动的外呼

## 解决的问题

该技能旨在覆盖用户实际需要的各类电话操作：
- 外拨电话
- 发送短信
- 拥有一个可重复使用的 Agent 号码
- 查看发送到该号码的消息
- 在不同会话之间保留该号码及相关标识符
- 为未来接收来电短信及其他自动化场景提供完善的电话身份标识

需要注意的是，它**不会**将 Hermes 转变为实时的来电电话网关。来电短信功能是通过轮询 Twilio REST API 来实现的。对于许多工作流而言，这种方式已经足够，比如发送通知或获取一次性验证码，且无需额外搭建核心 webhook 基础设施。

## 安全规则 — 必须遵守

1. 每次拨打电话或发送短信前务必确认。
2. 绝不对紧急电话号码进行拨打。
3. 绝不可利用电话功能进行骚扰、垃圾信息发送、冒充他人或从事任何非法活动。
4. 应将第三方电话号码视为敏感的操作数据：
   - 不要将其保存在 Hermes 的内存中
   - 除非用户明确要求，否则不要在技能文档、摘要或后续记录中提及这些号码
5. 可以持久保存**Agent 所拥有的 Twilio 号码**，因为这是用户配置的一部分。
6. VoIP 号码**无法保证**适用于所有第三方双重身份验证流程。请谨慎使用，并向用户明确说明相关限制。

## 决策树 — 该选择哪种服务？

建议使用以下逻辑而非硬编码的服务提供商路由方式来做出决策：
...
原因：
- Twilio 可让 Agent 拥有专属号码
- Vapi 能提供更出色的对话式 AI 通话质量以及更多语音/模型选项
...
### 4) “我想使用自定义预录语音消息拨打电话”
使用带有公共音频 URL 的 **Twilio 直接通话**功能。

原因：
- 播放自定义 MP3 文件最为简单
- 可与 Hermes 的 `text_to_speech` 功能结合使用，再搭配公共文件托管服务或隧道传输工具
...
用于存储仅在技能层面、需在会话之间保留的状态信息，例如：
- 记住的默认 Twilio 号码/SID
- 记住的 Vapi 电话号码 ID
- 用于轮询收件箱的上一条来电消息的 SID/日期

这意味着：
- 下次加载该技能时，`diagnose` 功能可以告知当前已配置的号码信息
- `twilio-inbox --since-last --mark-seen` 命令可以从上一个检查点继续工作

## 查找辅助脚本

安装该技能后，可通过以下方式找到对应的脚本：
