...
以只读方式访问 Canvas LMS，用于查看课程与作业列表。
...
- 首次使用时，需通过运行 `$CANVAS list_courses` 命令来验证授权状态；如果返回 401 错误，则需指导用户完成设置
- Canvas 的请求频率限制为每 10 分钟约 700 次请求；若达到限制，请检查 `X-Rate-Limit-Remaining` 请求头中的数值

## 故障排除

| 问题 | 解决方案 |
|---------|----------|
| 401 权限不足 | 令牌无效或已过期——需在 Canvas 设置中重新生成令牌 |
| 403 禁止访问 | 该令牌缺乏处理当前课程的权限 |
| 课程列表为空 | 可尝试使用 `--enrollment-state active` 参数，或省略该参数以查看所有状态下的课程 |
| 所属机构错误 | 请确认 `CANVAS_BASE_URL` 与浏览器中显示的地址一致 |
| 超时错误 | 检查与 Canvas 服务器的网络连接状况 |
