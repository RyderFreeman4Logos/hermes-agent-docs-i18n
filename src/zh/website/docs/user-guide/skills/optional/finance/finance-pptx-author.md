...
该技能基于 Anthropic 在 [anthropics/financial-services](https://github.com/anthropics/financial-services) 中发布的 `pptx-author` 和 `pitch-deck` 技能改编而来。原版本中的 MCP / Office-JS 相关功能已被移除，因此适用于无界面 Python 环境。

对于已正式发布的、功能更为全面的 PowerPoint 编辑技能（包括幻灯片、演讲者备注、嵌入内容以及媒体文件处理），请参考内置的 `powerpoint` 技能。该技能采用更为轻量化的设计，专为需要基于模型生成演示文稿的场景而优化——例如商业计划书、管理层备忘录和财报说明等，这类文档中的所有数据都必须能够追溯到对应的源工作表。

## 输出规范

- 将结果写入 `./out/<名称>.pptx` 文件中。如果 `./out/` 目录不存在，请先创建该目录。
- 在最终回复中需提供相应的相对路径。
...
### 所有数据均需追溯至模型来源
若幻灯片上的数据源自 `./out/model.xlsx`，请在脚注中注明对应的表格及单元格位置。
...
在图表精度至关重要的场景下（即要求模型的图表样式与演示文稿完全一致），应从原始工作簿中将图表渲染为PNG格式并嵌入其中。而原生的`pptx.chart`格式图表较为脆弱，且往往无法符合企业的规范要求。
...
### 无外部发送功能
该智能体仅负责生成文件，不会发送电子邮件、上传内容或发布信息。具体的内容投递工作由编排层负责处理。
...
在读取工作簿之前，请务必先重新计算其数值——只有当已有程序计算过该工作表的内容时，openpyxl才能读取到这些计算后的数值。建议首先使用 `excel-author` 智能体中的重新计算工具，或通过真实的 Excel 会话来打开/保存文件。

## 商业计划书演示文稿的幻灯片结构清单

典型的银行类商业计划书通常采用以下结构。虽非强制要求，但可作为搭建框架的参考：
...
- 非财务类的演示文稿（如季度全员会议材料、营销演示文稿）——建议使用更通用的 `powerpoint` 智能体。
- 包含大量动画效果、过渡效果或演讲者备注的演示文稿——同样建议使用更通用的 `powerpoint` 智能体。
