...
:::info
以下是当触发该技能时Hermes所加载的完整技能定义。技能启用后，智能体将依据此内容执行操作。
:::

## 环境要求

该技能依赖于**无界面版 openpyxl**——您需要将结果保存为 .xlsx 格式的文件。在处理单元格着色、公式、命名范围以及敏感性分析表时，请遵循 `excel-author` 技能的相关规范。在交付前务必重新计算：`python /path/to/excel-author/scripts/recalc.py ./out/model.xlsx`。

# DCF 模型构建器

## 概述

该技能依据投资银行的标准，为股票估值创建符合专业水准的 DCF 模型。每次分析都会生成一份详细的 Excel 模型（DCF 表格底部还会包含敏感性分析内容）。

## 工具

- 默认会使用用户提供的所有信息以及 MCP 服务器可提供的数据源。

## 重要约束——请先仔细阅读

以下约束适用于所有 DCF 模型构建过程。在开始之前请务必查看：

**必须使用公式而非硬编码值（绝对不可违反）：**
- 所有的预测值、利润率、折现率、现值以及敏感性分析单元格都必须是 Excel 中的动态公式——绝不能是直接用 Python 计算后以数值形式写入的内容
- 使用 openpyxl 时：`ws["D20"] = "=D19*(1+$B $8)"` 是正确的写法；而 `ws["D20"] = calculated_revenue` 则是错误的
- 唯一允许的硬编码数值包括：(1) 原始历史数据，(2) 假设参数（增长率、WACC 输入值、终值增长率），(3) 当前市场数据（股价、债务余额）
- 如果发现自己用 Python 计算出结果后再直接写入——请立即停止。当用户更改假设时，模型必须能够灵活调整。

**需与用户逐步骤核对（切勿一次性构建完整模型）：**
...
1. **WACC与终端增长率**——显示企业价值对折现率及永久增长率的敏感度；
2. **营收增长率与EBIT利润率**——反映营业收入增长及运营杠杆的影响；
3. **贝塔值与无风险利率**——展示对企业价值中权益成本组成部分的敏感度。
...
本部分列出了在构建DCF模型时需避免的所有错误做法。

### 错误做法：使用简化的敏感度表格近似值或占位文本
...
**该功能会使用`xlsx`技能来处理所有电子表格操作。** `xlsx`技能具备以下功能：  
- 标准化的公式构建规则  
- 数字格式设定规范  
- 通过`recalc.py`脚本实现公式自动重算  
- 全面的错误检测与验证功能  

该功能生成的所有Excel文件都必须符合`xlsx`技能的要求，包括杜绝公式错误并确保能够正确重算。  

## 质量评估标准  

每个DCF模型都需做到以下几点：  
1. 基于历史业绩，设定**合理且真实的营收与利润率假设**  
2. 采用正确的CAPM方法，进行**恰当的资本成本计算**  
3. 提供**全面的敏感性分析**，明确估值区间  
4. 附带合理依据，进行**清晰的终值计算**  
...
### 文档编写
### 文档编写规范
1. **记录所有假设**：阐明关键输入参数的推导依据  
2. **标注数据来源**：明确每项数据的具体出处  
3. **说明方法论**：详细描述任何非标准化的分析思路  
4. **标示不确定性**：突出信息透明度较低的领域  

### 质量控制措施
1. **交叉验证计算结果**：通过多种方式核验数学运算的准确性  
2. **压力测试假设条件**：通过敏感性分析确保模型稳定性  
3. **同行评审**：请他人核查公式与推导过程  
4. **版本控制**：在项目推进过程中保存不同版本的文档  
...
### 多业务板块型企业
- 为每个业务单元分别建立DCF估值模型  
- 不同业务板块采用差异化的增长率与利润率假设  
- 采用分部加总法进行估值  
- 需评估业务协同效应  

## 故障排除指南

**若遇到计算错误或不合理结果，请查阅 [TROUBLESHOOTING.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md) 以获取详细的调试指导。**
...
3. **根据本技能中详述的DCF方法开始构建模型**

### 在模型构建过程中

1. **使用openpyxl库创建Excel模型**，公式需动态生成（而非硬编码数值）
2. **遵循xlsx技能规定的公式编写与格式设置规范**
...
3. **检查结果**：
   - 若`status`显示为“success”→进入步骤4
   - 若`status`显示为“errors_found”→查看`error_summary`内容，并参阅[TROUBLESHOOTING.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md)获取调试指导
...
5. **抽查公式正确性**：
   ...
   - 确认收入计算公式引用的是合并列（而非嵌套的IF公式）

6. **交付模型**

### 可用的数据来源

- **MCP服务器**：如已配置可使用（Daloopa用于获取历史财务数据）
- **网络搜索/抓取**：用于获取当前股票价格、贝塔系数及市场数据
- **用户提供的数据**：历史财务数据、市场一致预期估值
...
**必填项：**
- 运行命令 `python recalc.py model.xlsx 30`，直至状态显示为“success”（即无公式错误）。
...
- 终值增长率须低于WACC。
- 税率应在21%至28%之间。
- 文件命名格式为：`[Ticker]_DCF_Model_[Date].xlsx`

## 数据来源 —— 首选MCP，网页作为备用

下文多处提到“使用S&P Kensho MCP / Daloopa MCP / FactSet MCP”。这些均为源自原有Cowork插件环境中的商业金融数据MCP服务。在Hermes中：
- **如果您已配置了任何结构化的金融数据MCP**（Hermes支持MCP功能，参见`native-mcp`技能），则优先使用它们来获取特定时间点的对比数据、历史交易案例及监管文件信息。
- **否则**，可采取以下替代方案：
  - 对于美国公司的监管文件，可通过SEC EDGAR网站（`https://www.sec.gov/cgi-bin/browse-edgar`）使用`web_search`/`web_extract`功能进行检索；
  - 从公司官网的投资者关系页面获取新闻稿和业绩展示材料；
  - 使用`browser_navigate`功能访问交互式数据平台；
  - 采用用户提供的数据（当上下文信息不足时需主动询问用户）。
- **严禁编造数据**。如果无法找到倍数、历史案例或监管文件编号，应将该单元格标记为`[UNSOURCED]`，并将其展示给用户。
