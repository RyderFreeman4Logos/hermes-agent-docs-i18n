...
只读型 EVM 客户端：支持 8 条区块链上的钱包、代币及Gas费用查询。
...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。技能运行时，代理程序会依据此内容执行相应操作。
:::

# EVM 区块链技能

可查询 8 条支持 EVM 的区块链数据，并提供以美元计价的费用信息。
包含 14 种命令：钱包资产组合查询、代币信息查询、交易查询、活动状态查询、Gas费用追踪、
网络统计信息查询、价格查询、多链扫描、大额资金转移检测、ENS域名解析、授权状态检查、合约分析以及交易解码。

支持 8 条区块链：Ethereum、BNB Chain（BSC）、Base、Arbitrum One、Polygon、
Optimism、Avalanche（C-Chain）、zkSync Era。

无需 API 密钥，也无任何外部依赖——仅使用 Python 标准库（urllib、json、argparse、threading）。

> **已取代独立的 `base` 技能。** 之前位于 `optional-skills/blockchain/base/` 下的 Base 特有代币（AERO、DEGEN、TOSHI、BRETT、WELL、cbETH、cbBTC、wstETH、rETH）以及所有 Base RPC 功能，均已整合到此技能中。如需使用 Base 相关功能，可在任意命令后添加 `--chain base` 参数。

---

## 适用场景
- 用户希望查询任意 EVM 区块链上的钱包余额或资产组合
- 用户希望一次性查看所有区块链上同一钱包的账户状态
- 用户希望通过交易哈希来查看交易详情（或解码其操作内容）
- 用户需要了解 ERC-20 代币的元数据、价格、供应量或市值信息
- 用户希望获取某个地址的最新交易历史
- 用户希望了解当前 Gas 费用，或比较不同区块链间的手续费差异
- 用户希望查找最近区块中发生的大额资金转移记录
- 用户希望解析 ENS 域名（如 vitalik.eth）或反向查询地址信息
- 用户希望检查某个合约是否存在危险的代币授权设置
- 用户希望分析智能合约的类型（代理合约？ERC-20？ERC-721？字节码大小如何？）
- 用户希望在执行交易前比较不同区块链间的 Gas 费用

---

## 前提条件
仅需 Python 3.8 及以上版本的标准库，无需通过 pip 安装任何额外包。
数据来源：CoinGecko 免费 API（有速率限制，约每分钟 10-30 次请求）；ENS 数据来自 ensideas.com 公开 API；交易解码功能依赖 4byte.directory 公开 API。
如需自定义 RPC 接口地址，请设置 `export EVM_RPC_URL=https://your-rpc.com`。
