...
查询带有美元价格标注的 Solana 区块链数据——包括钱包余额、带数值的代币组合、交易详情、NFT信息、大额持仓者检测以及实时网络统计信息。该功能基于 Solana RPC 与 CoinGecko 数据源，无需 API 密钥。

## 技能元数据

| | |
|---|---|
| 来源 | 可选——可通过 `hermes skills install official/blockchain/solana` 安装 |
...
:::info
以下是当触发该技能时 Hermes 所加载的完整技能定义。技能运行时，Agent 就会依据这些指令来执行操作。
:::

# Solana 区块链技能

通过 CoinGecko 获取带有美元价格标注的 Solana 在链数据。
提供 8 种查询命令：钱包资产组合、代币信息、交易记录、活动动态、NFT信息、
大额持仓者检测、网络统计以及价格查询。
...
- 用户想要获取某个地址的最新交易历史
- 用户想了解某个钱包所拥有的 NFT
- 用户希望检测大规模的 SOL 转账行为（即大额持仓者检测）
- 用户想要了解 Solana 网络的健康状况、每秒交易处理量、当前时代号或 SOL 价格
...
价格数据来自 CoinGecko 的免费 API（无需密钥，但每分钟请求限制在约 10-30 次）。如需更快查询速度，可使用 `--no-prices` 参数。
...
如需自定义地址，请设置：export SOLANA_RPC_URL=https://your-private-rpc.com
