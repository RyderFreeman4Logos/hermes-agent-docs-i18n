...
通过 Stripe Link 实现代理支付——支持卡片、共享支付令牌（SPT）及审批流程。
...
:::info
以下是当触发该技能时 Hermes 加载的完整技能定义。技能处于激活状态时，代理将依据此内容执行操作。
:::

# Stripe Link CLI 技能

该技能封装了 [@stripe/link-cli](https://github.com/stripe/link-cli) 功能，使 Hermes 能够使用一次性虚拟卡片或共享支付令牌（SPT）代表用户完成购买。每次消费都需通过 Link 移动端/网页应用内的审批流程——Hermes 无法自行批准。
...
如果用户需要发起付费 API 调用（出现 HTTP 402 错误且无结账表单），则说明使用的“卡片”路径有误——应通过此技能使用 SPT，或转交给 `mpp-agent` 技能处理。

## 先决条件

- 系统 `PATH` 路径中已安装 Node.js 20+ 版本（可通过 `node --version` 查验）
- 用户需位于美国地区（需拥有 Link 账户）

在 Hermes 尝试付款之前，无需预先设置 Link 账户、支付方式或消费审批应用——CLI 在首次运行时会引导用户完成相关设置：

- 一个 Link 账户，地址为 https://app.link.com，可在首次使用 `link-cli` 进行身份验证时创建/关联
- 至少一种支付方式，可在首次访问 https://app.link.com/wallet 时添加
- Link 移动端/网页应用，用于在收到首次消费请求时进行审批

无需配置环境变量——认证状态由 CLI 存储在其自身的配置目录中。
