...
- **难度级别** — 宁静/简单/普通/困难？
- **游戏模式** — 生存/创造/冒险？
- **在线模式** — 开启（需 Mojang 认证，使用正规账号）或关闭（适合局域网或破解版玩家）？
- **预期玩家数量** — 预计会有多少玩家？（会影响内存分配和视野距离设置）
- **内存分配方式** — 由用户指定，还是让智能体根据模组数量和可用内存自动决定？
- **视野距离/模拟距离** — 由用户指定，还是让智能体根据玩家数量和硬件配置自动选择？
- **PvP 模式** — 开启还是关闭？
- **访问权限设置** — 开放访问还是仅限白名单用户？
- **备份功能** — 是否需要自动备份？备份频率如何？

如果用户未作特殊要求，可选用合理的默认值，但在生成配置文件之前务必先征得用户同意。
