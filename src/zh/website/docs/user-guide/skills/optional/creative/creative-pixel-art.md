...
支持使用特定时代风格的调色板（NES、Game Boy、PICO-8）进行像素艺术创作。
...
可将任意图像转换为复古风格的像素艺术，还可根据需求为其添加动画效果，
生成带有时代特色特效（雨滴、萤火虫、雪花、余烬等）的短 MP4 或 GIF 文件。

该技能附带两个脚本：

- `scripts/pixel_art.py` — 将照片转换为像素艺术 PNG 图像（采用 Floyd-Steinberg 点阵算法）
- `scripts/pixel_art_video.py` — 将像素艺术 PNG 转换为动画 MP4 文件（也可生成 GIF）

这两个脚本均可直接导入或运行。若需还原特定时代的色彩效果（如 NES、Game Boy、PICO-8 等平台的风格），可使用预设的硬件调色板；若希望呈现街机或 SNES 风格，则可选用自适应的 N 色量化功能。

## 适用场景

- 用户希望从源图像生成复古像素艺术风格的作品  
- 用户要求采用NES、Game Boy、PICO-8、C64、街机或SNES等游戏机的视觉风格  
- 用户需要简短的循环动画（如雨景、夜空、下雪场景等）  
- 海报、专辑封面、社交平台帖子、精灵图、角色形象、头像  

## 工作流程

在生成内容之前，需先与用户确认风格偏好。不同的预设会带来截然不同的输出结果，重新生成则成本较高。  

### 第一步 — 提供风格选项

调用`clarify`函数并传入4种具有代表性的预设风格。根据用户的实际需求选择合适的风格组合——无需一次性展示全部14种风格。  
...
- 非常小的图像源（宽度小于100像素）在8-10像素的块尺寸下会被压缩。若图像过小，建议先进行放大处理。
- 分数形式的 `block` 或 `palette` 值会导致量化错误——请确保使用正整数。
- 动画粒子数量是根据约640x480分辨率的画布优化的。对于超大尺寸的图像，可能需要使用不同的种子值进行二次处理以调整密度。
- `mono_green` / `mono_amber` 选项会强制将颜色饱和度设为 `0.0`（即去饱和）。若尝试覆盖此设置并保留色度，双色调色板可能在平滑区域产生条纹。
...
- 输出路径中会生成PNG文件
- 在预设的块尺寸下能清晰看到方形的像素块
- 颜色数量与预设值一致（可通过肉眼观察图像或运行 `Image.open(p).getcolors()` 查验）
- 视频为有效的MP4格式（可使用 `ffprobe` 打开），且文件大小非零

## 出处说明

`pixel_art_video.py` 中的命名硬件调色板以及程序化动画循环均源自 [pixel-art-studio](https://github.com/Synero/pixel-art-studio)（MIT许可证）。详细信息请参阅该技能目录下的 `ATTRIBUTION.md` 文件。
