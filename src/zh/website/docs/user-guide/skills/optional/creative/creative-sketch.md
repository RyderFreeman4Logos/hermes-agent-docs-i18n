...
临时 HTML 原型：提供 2-3 种设计版本供用户对比选择。

## 技能元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `hermes skills install official/creative/sketch` 安装 |
...
当用户希望在**最终确定设计方向之前**先预览不同方案时，可使用此技能 — 通过生成临时 HTML 原型来探索各种 UI/UX 构思。其目的是创建 2-3 种交互式版本，便于用户直观对比不同设计方向，而非输出可直接使用的代码。
