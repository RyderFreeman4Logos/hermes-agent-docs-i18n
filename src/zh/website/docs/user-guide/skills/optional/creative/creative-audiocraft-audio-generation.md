...
AudioCraft：通过 MusicGen 实现文本转音乐，通过 AudioGen 实现文本转声音。
...
| 支持平台 | linux、macos |
| 标签 | `多模态`、`音频生成`、`文本转音乐`、`文本转声音`、`MusicGen` |
| 相关技能 | [`heartmula`](/docs/user-guide/skills/optional/creative/creative-heartmula)、[`songwriting-and-ai-music`](/docs/user-guide/skills/bundled/creative/creative-songwriting-and-ai-music) |
...
本指南介绍如何使用 Meta 的 AudioCraft，结合 MusicGen、AudioGen 以及 EnCodec 实现文本转音乐和文本转声音的功能。

## 何时使用 AudioCraft

**在以下情况下可使用 AudioCraft：**
- 需要根据文本描述生成音乐  
- 制作音效与环境音频  
- 开发音乐生成应用  
- 需要基于旋律条件的音乐生成  
...
- **多种模型规模**：从小型（300M参数）到大型（3.3B参数）  
- **立体声支持**：可生成完整的立体声音频  
- **风格控制**：MusicGen-Style功能可实现基于参考样本的生成  
...
| `musicgen-stereo-*` | 不固定 | 立体声输出 | 立体声音乐生成 |
...
- **[高级用法](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/creative\audiocraft-audio-generation/references/advanced-usage.md)** - 训练、微调与部署指南
- **[故障排除](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/creative\audiocraft-audio-generation/references/troubleshooting.md)** - 常见问题及解决方案
...
- **演示示例**：https://huggingface.co/spaces/facebook/MusicGen
