...
| 路径 | `optional-skills/creative\tlddraw-offline` |
| 版本 | `1.0.0` |
| 开发者 | Teknium + Hermes Agent |
| 许可证 | MIT |
...
用于操作 Tldraw 离线桌面应用（offline.tldraw.com）：读取已打开的画布、进行编辑，以及编写**文档脚本**——即嵌入在 `.tldraw` 文件中的 JavaScript 代码，这类代码会在文件加载时执行，从而赋予文件持久化功能。该应用会运行一个**本地 HTTP API**（默认地址为 `localhost:7236`），编程智能体可通过终端使用简单的 `curl` 命令来操控该 API——这正是该应用官网演示页面（实时编辑画布的功能）的实现方式。智能体不会使用鼠标操作或 GUI 点击功能，也不会直接手动编辑 `.tldraw` 文件。请在操作期间保持 Tldraw 离线应用处于打开状态。

## 适用场景

- 用户已打开 tldraw 离线版，并要求你创建或修改画布（图表、线框图、布局等）。
- 你希望通过嵌入的文档脚本为绘图内容添加动态交互功能（响应式形状、交互式按钮、动画效果以及连接逻辑）。

切勿手动绘制形状来模拟绘图效果——而应编写生成这些形状的代码。相比直接在画布上绘图，智能体在编写画布脚本方面要高效得多。

## 先决条件

- **tldraw offline 已安装并正在运行，且当前有文档打开**。相关版本可在此获取：
  https://github.com/tldraw/tldraw-offline/releases/latest（提供 macOS DMG、Windows x64/Arm64、Linux `x86_64`/`arm64` AppImage 以及 amd64/arm64 `.deb` 格式安装包）。
- **在应用中安装 Agent skills**：通过“开发 → 安装 Agent Skills”即可完成。该应用会将自身的 tldraw skill 文件写入 `~/.codex/skills/`、`~/.claude/skills/`、`~/.cursor/skills/` 和 `~/.gemini/skills/` 目录中，从而向对应 Agent 教授下述的 `curl` 使用方法。（此 Hermes skill 的实现方式与 Hermes 官方指南中的指导一致。）
- **本地控制 API**：应用启动时会将包含端口信息（默认为 7236）、令牌、进程 ID 以及启动时间的 `server.json` 文件写入配置目录（Linux 下为 `~/.config/tldraw/`，macOS 下为 `~/Library/Application Support/tldraw/`，Windows 下为 `%APPDATA%\tldraw\`）。除 `GET /` 请求外，所有其他请求均需在请求头中添加 `Authorization: Bearer <token>`。若正常退出，该文件会被删除；若文件存在但对应端口无响应，则表示应用已非正常退出，应视为其未处于运行状态。
- **每次终端调用时都需重新读取端口与令牌**：由于每次打开终端都会生成新的 shell 环境，因此通过 `export` 设置的令牌不会持久保留——若尝试“仅设置一次后重复使用”，将会发送空令牌从而导致 401 错误。因此应在每次调用开头直接读取相关值：`PORT=$(jq -r .port <server.json>); TOKEN=$(jq -r .token <server.json>)`。
- 进行本地编辑无需账号或网络连接。
