...
借助 Hermes Kanban 功能，规划、搭建并监控多智能体视频制作流程。当用户需要制作各类视频——无论是叙事电影、产品/营销视频、音乐视频、解说视频、ASCII/终端艺术作品、抽象/生成循环视频、漫画、3D 视频、实时艺术或装置艺术——且此类工作需要拆分为由不同角色（编剧、设计师、动画师、渲染师、配音员、剪辑师等）承担的任务，并通过看板进行协调时，即可使用此工具。该技能会通过自适应方式明确任务范围，根据所需风格组建合适的团队，生成用于创建 Hermes 智能体配置文件及初始看板任务的设置脚本，进而协助监控任务执行情况，在任务停滞或失败时及时介入处理。它会根据不同场景的需求，将相应任务分配给合适的 Hermes 渲染/音频/设计智能体（如 `ascii-video`、`manim-video`、`p5js`、`comfyui`、`touchdesigner-mcp`、`blender-mcp`、`pixel-art`、`baoyu-comic`、`claude-design`、`excalidraw`、`songsee`、`heartmula` 等），必要时还会结合外部 API 实现文本转语音、图像生成及图像转视频等功能。
...
无论是对 15 秒的产品预告片、5 分钟的叙事短片、音乐视频还是 ASCII 循环动画等任何类型的视频制作需求，均可通过 Hermes Kanban 流程进行处理，该流程会将任务拆分并分配给不同的专业智能体。

此技能本身并不负责任何渲染工作。它实际上是一个元流程，具体功能包括：

1. 通过精准的检测来明确任务范围；
2. 根据视频风格确定合适的团队配置（包括各角色及其所需工具）；
3. 生成设置脚本，用于创建 Hermes 智能体配置文件、项目工作空间以及初始看板任务；
4. 将任务交由对应的负责人智能体处理，由其通过看板进一步拆分任务；
5. 监控任务执行情况，在任务停滞或失败时提供协助。

实际的渲染工作则会在看板流程启动后由合适的现有智能体及工具来完成，这些工具可能包括 `ascii-video`、`manim-video`、`p5js`、`comfyui`、`touchdesigner-mcp`、`blender-mcp`、`songwriting-and-ai-music`、`heartmula`，或是结合外部 API，甚至是使用带有 PIL 和 ffmpeg 功能的普通 Python 代码。

## 何时不宜使用此技能

- 若视频项目为连续式的流程化任务，且无需专业分工，可直接编写代码完成制作；
- 若用户仅需快速转换文件格式（例如“将此 MP4 文件转换为 GIF”），可直接使用 ffmpeg；
- 若输出结果仅为静态图片、GIF 或纯音频文件，应使用相应的专用技能（如 `ascii-art`、`gifs`、`meme-generation`、`songwriting-and-ai-music`）；
- 若任务恰好适合某一个现有的智能体来完成（例如纯粹的 ASCII 视频制作，可直接使用 `ascii-video`）。
...
接下来由“导演”角色接管工作，它通过看板工具集将任务拆分并分配给相应的专业角色。

### 第6步 — 监控与干预

需保持关注——虽然看板系统可以自主运行，但遇到卡住的任务或不良输出时，仍需人工（或AI）进行判断。

监控方式包括：定期查询 `kanban list`，使用 `kanban show <id>` 检查那些耗时超过预期的“运行中”任务，并监测任务的心跳状态。当某个工作者的输出未通过审核时，可采取的标准干预措施包括：

1. 使用 `kanban_comment` 在该工作者的任务上留下具体反馈；
2. 创建一个以原任务为父任务的重新执行任务；
3. 调整任务要求的范围，再由“导演”角色重新拆分任务。

有关诊断方案、干预流程以及“任务卡住”处理指南，请参阅 **[references/monitoring.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**。

## 参考：实际案例

文中提供了六个涵盖不同视频类型的实际流程示例——叙事电影、产品/营销视频、音乐视频、数学/算法讲解视频、ASCII艺术视频以及实时装置艺术视频——展示了同一工作流如何针对不同的项目生成各具特色的团队架构与任务结构。详情请见 **[references/examples.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**。

## 重要规则

1. **先调研，后行动。** 在开始创建任务描述或团队之前，务必先回答至少三个基础问题。一个糟糕的任务描述会影响到整个流程的顺利进行。

2. **匹配团队与视频类型。** 不要每次都使用相同的四角色配置。例如，没有节奏分析角色的音乐视频很可能会效果不佳；而没有编剧角色的叙事电影则会导致场景逻辑混乱。详情请参阅 `references/role-archetypes.md`。

3. **每个项目对应一个工作空间。** 同一视频项目的所有角色共享同一个 `dir:` 工作空间。任务通过共享文件系统及结构化的交接方式传递成果。**每次**调用 `kanban_create` 时，都会传入 `workspace_kind="dir"` 和 `workspace_path="<绝对项目路径>"` 参数。

4. **为每个项目设置独立租户。** 应使用特定于项目的租户标识（`--tenant <project-slug>`）。这样既能保证控制台的整洁性，又能避免与其他正在进行的看板任务相互干扰。

5. **充分利用现有技能。** 如果某个场景适合现有的某项技能，相应的渲染器应通过在其任务上添加 `--skill <name>` 参数，或在其角色配置中设置 `always_load`，来加载该技能。无需重复实现技能已具备的功能。

6. **“导演”角色不得直接执行任务。** 即使拥有完整的“看板 + 终端 + 文件”工具集，“导演”角色的 `SOUL.md` 规则也禁止其直接执行任务。它的职责仅限于拆分任务并分配给相应角色——每一个具体任务都会转化为对某个专业角色的 `hermes kanban create` 调用。`kanban-orchestrator` 技能对此有更详细的说明。

7. **避免过度拆分任务。** 一个30秒的产品视频并不需要20个任务。应力求构建出最小的任务结构，同时确保任务能够有效并行处理，并设置适当的人工审核节点。

8. **在启动任务前验证API密钥。** 外部API（如文本转语音、图像生成、图像转视频等）需要从 `~/.hermes/.env` 文件或用户的密钥存储库中获取相应的密钥。如果工作者因密钥缺失而出错，就会浪费一个任务处理机会。设置脚本中的 `check_key` 函数可在检测到缺失必要密钥时立即终止流程。
