...
借助 Hermes Kanban 功能，规划、搭建并监控多智能体视频制作流程。当用户需要制作各类视频——无论是叙事电影、产品/营销视频、音乐视频、解说视频、ASCII/终端艺术作品、抽象/生成循环视频、漫画、3D 视频，还是实时/装置艺术作品——且此类工作需要拆分为由不同角色（编剧、设计师、动画师、渲染师、配音员、剪辑师等）通过看板协同完成时，即可使用此工具。该工具会通过自适应分析来确定项目需求范围，根据目标风格匹配合适的团队组合，生成用于创建 Hermes 智能体配置文件及初始看板任务的设置脚本，同时还能够监控任务执行进度，在任务停滞或失败时及时介入处理。它会根据不同场景的需求，将相应任务分配给合适的 Hermes 渲染/音频/设计智能体（如 `ascii-video`、`manim-video`、`p5js`、`comfyui`、`touchdesigner-mcp`、`blender-mcp`、`pixel-art`、`baoyu-comic`、`claude-design`、`excalidraw`、`songsee`、`heartmula` 等），必要时还会结合外部 API 实现文本转语音、图像生成及图像转视频等功能。»
...
无论是需要15秒的产品预告片、5分钟的叙事短片、音乐视频，还是ASCII循环动画，均可通过此工具将各类视频制作需求纳入Hermes Kanban流程中，进而拆分任务并分配给相应的专业智能体处理。

该智能体本身并不负责任何渲染工作。它实际上是一个元流程，其功能包括：

1. 通过精准分析来确定项目需求范围；
2. 根据目标风格匹配合适的团队组合（包括各角色的职责及所需工具）；
3. 生成设置脚本，用于创建Hermes智能体配置文件、项目工作空间以及初始看板任务；
4. 将任务移交至对应的负责人智能体，由其通过看板进一步拆分任务；
5. 监控任务执行进度，在任务停滞或失败时提供协助。
...
- 当视频项目属于连续式的流程化任务，无需专业分工时——可直接编写代码完成制作。
- 当用户仅需进行快速的单次转换（例如“将此MP4文件转换为GIF”）时——建议直接使用ffmpeg。
- 当输出结果为静态图片、GIF或纯音频文件时——应使用对应的专用智能体（如 `ascii-art`、`gifs`、`meme-generation`、`songwriting-and-ai-music`）。
- 当任务完全适合某个现有的单一智能体处理时（例如纯粹的ASCII视频——可直接使用 `ascii-video`）。
...
从这里开始，由“导演”角色接管工作，它通过看板工具集将任务拆分并分配给相应的专业角色。

### 第6步 — 监控与干预

保持关注——看板系统可自主运行，但遇到卡住的任务或不良输出时，仍需人工（或AI）进行判断。

监控方式包括：定期查询 `kanban list`，使用 `kanban show <id>` 检查那些耗时超过预期的“运行中”任务，并查看任务的心跳状态。当某个工作者的输出无法通过审核时，可采取的标准干预措施包括：

1. 在该工作者的任务上留下具体反馈（使用 `kanban_comment`）；
2. 创建一个新任务并让原任务作为其父任务；
3. 调整任务要求的范围，再由“导演”角色重新拆分任务。
...
这里提供了六个涵盖不同视频风格的真实案例——故事片、产品/营销视频、音乐视频、数学/算法讲解视频、ASCII艺术视频以及实时装置艺术视频——展示了如何通过相同的工作流生成截然不同的团队配置和任务结构。详情请见 **[references/examples.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**。

## 重要规则

1. **先探索，后行动。** 在开始创建任务描述或团队之前，务必先回答至少三个基础问题。一个糟糕的任务描述会影响到整个工作流程。

2. **让团队与视频风格相匹配。** 不要每次都使用相同的四角色配置。缺乏节奏分析角色的音乐视频肯定无法达到预期效果；没有编剧角色的故事片则会导致场景逻辑混乱。详情请参阅 `references/role-archetypes.md`。

3. **每个项目对应一个工作空间。** 某个视频的所有相关角色都共享同一个 `dir:` 工作空间。任务通过共享文件系统及结构化的交接方式传递成果。**每一次**调用 `kanban_create` 时，都会传入 `workspace_kind="dir"` 和 `workspace_path="<绝对项目路径>"` 参数。

4. **为每个项目设置独立租户。** 使用特定于项目的租户标识（`--tenant <project-slug>`）。这样可以限制控制台的数据范围，避免与其他正在进行的看板任务相互干扰。

5. **尊重现有的技能配置。** 如果某个场景适合现有的技能，相应的渲染器应通过在其任务上使用 `--skill <name>` 参数，或在其角色配置中设置 `always_load`，来加载该技能。无需重复实现技能已具备的功能。

6. **“导演”角色绝不直接执行任务。** 即使拥有完整的“看板 + 终端 + 文件”工具集，“导演”角色的 `SOUL.md` 规则也禁止其直接执行任务。它的职责仅限于拆分任务并分配给相应角色——每一个具体任务都会转化为对某个专业角色的 `hermes kanban create` 调用。`kanban-orchestrator` 技能对此有更详细的说明。

7. **避免过度拆分任务。** 一个30秒的产品视频并不需要20个任务。应尽量构建最小的任务结构，同时确保任务能够有效并行处理，并设置合适的人工审核节点。

8. **在启动任务前验证API密钥。** 外部API（如文本转语音、图像生成、图像转视频等功能）需要从 `${HERMES_HOME:-~/.hermes}/.env` 文件或用户的密钥存储中获取密钥。如果工作者因密钥缺失而出错，就会浪费一个任务处理名额。设置脚本中的 `check_key` 函数会在检测到缺少必要密钥时立即终止流程。
