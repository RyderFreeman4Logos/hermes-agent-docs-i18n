...
基于 Hermes Kanban 技术，规划、搭建并监控多智能体视频制作流程。当用户需要制作各类视频——无论是叙事电影、产品/营销视频、音乐视频、解说视频、ASCII/终端艺术作品、抽象/生成循环动画、漫画、3D 视频、实时艺术或装置艺术——且此类工作需要拆分为由不同角色（编剧、设计师、动画师、渲染师、配音员、剪辑师等）承担的任务，并通过看板进行协调时，即可使用此工具。该工具会通过智能分析确定项目范围，根据所需风格组建合适的团队，生成用于创建 Hermes 智能体配置文件及初始看板任务的设置脚本，同时还能够监控任务执行进度，在任务停滞或失败时及时介入处理。它会根据不同场景的需求，将相应任务分配给合适的 Hermes 渲染/音频/设计智能体（如 `ascii-video`、`manim-video`、`p5js`、`comfyui`、`touchdesigner-mcp`、`blender-mcp`、`pixel-art`、`baoyu-comic`、`claude-design`、`excalidraw`、`songsee`、`heartmula` 等），必要时还会结合外部 API 实现文本转语音、图像生成及图像转视频等功能。»
...
无论是需要15秒的产品预告片、5分钟的叙事短片、音乐视频，还是ASCII循环动画，只要是有视频制作需求的任务，都可以通过 Hermes Kanban 流程进行处理——该流程会将工作拆分并分配给不同的专业智能体角色。

需要注意的是，此技能本身并不负责任何内容的渲染工作。它实际上是一个元流程，主要功能包括：

1. 通过智能分析确定任务范围；
2. 根据视频风格为项目组建合适的团队（包括所需角色及每个角色应使用的工具）；
3. 生成设置脚本，用于创建 Hermes 智能体配置文件、项目工作空间以及初始看板任务；
4. 将任务交由对应的负责人智能体处理，由其通过看板进一步拆分任务；
5. 监控任务执行进度，在任务停滞或失败时及时提供协助。

实际的渲染工作则会在看板流程启动后由合适的现有智能体及工具来完成，这些工具可能包括 `ascii-video`、`manim-video`、`p5js`、`comfyui`、`touchdesigner-mcp`、`blender-mcp`、`songwriting-and-ai-music`、`heartmula`，或是外部 API，甚至是结合 PIL 和 ffmpeg 的普通 Python 代码。

## 何时不宜使用此技能

- 当视频属于单一的连续式项目，无需专业分工时——可直接编写代码完成制作。
- 当用户只需要进行快速的单次转换（例如“将此 MP4 文件转换为 GIF”）时——建议直接使用 ffmpeg 工具。
- 当输出结果为静态图片、GIF 或仅包含音频的内容时——应使用对应的专用技能（如 `ascii-art`、`gifs`、`meme-generation`、`songwriting-and-ai-music`）。
- 当某项工作能够被某个现有的单一技能完美处理时（例如纯粹的 ASCII 视频——可直接使用 `ascii-video`）。
