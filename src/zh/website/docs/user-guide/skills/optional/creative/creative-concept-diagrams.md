...
该技能可生成简洁扁平、支持明暗模式的 SVG 图表，并将其作为独立的 HTML 文件输出。它采用统一的教育类视觉设计语言，包含 9 种语义化颜色渐变、句首大写的排版风格以及自动切换的深色模式。非常适合用于教育类及非软件相关的可视化场景——如物理实验装置、化学反应机制、数学曲线、实物模型（飞机、涡轮机、智能手机、机械表）、人体解剖结构、平面图、剖面图、叙事流程（X 的生命周期、Y 的处理过程）、中心辐射式系统集成（智慧城市、物联网）以及分解式层视图等。如果针对特定主题存在更专业的技能（如专门的软件/云架构设计、手绘草图、动画演示等），建议优先使用那些专业技能；否则，该技能也可作为通用的 SVG 图表备选方案，具备简洁的教育风格。随工具包附带 15 个示例图表。
...
| 版本 | `0.1.0` |
| 开发者 | v1k22（初始提交者），后移植至 hermes-agent |
| 许可协议 | MIT |
...
该技能可利用统一的扁平化、极简设计体系生成专业级 SVG 图表。输出结果为一个独立的 HTML 文件，可在任何现代浏览器中以相同效果呈现，并具备自动切换的明暗模式。

## 适用范围

**最适合用于：**
- 物理实验装置、化学反应机制、数学曲线、生物学内容
- 实物模型（飞机、涡轮机、智能手机、机械表、细胞等）
- 人体解剖结构、剖面图、分解式层视图
- 平面图、建筑结构转换图
- 叙事流程（X 的生命周期、Y 的处理过程）
- 中心辐射式系统集成（智慧城市、物联网网络、电力网格）
- 任何领域的教育类/教科书风格可视化内容
- 定量图表（分组柱状图、能量分布图）

**如需以下功能，请优先选择其他技能：**
- 具有暗黑科技风格的专用软件/云基础设施架构设计（如有 `architecture-diagram` 技能，可考虑使用）
- 手绘式白板草图（如有 `excalidraw` 技能，可考虑使用）
- 动画演示或视频输出（可考虑使用动画类技能）

如果针对特定主题存在更专业的技能，建议优先使用。若没有合适的技能，该技能仍可作为通用的 SVG 图表备选方案——其输出将具备下文所述的简洁教育风格，几乎适用于所有主题，是较为理想的默认选择。

## 工作流程

1. 确定图表类型（见下文的图表类型说明）。
2. 按照设计体系规则布局各组成部分。
3. 以 `templates/template.html` 作为模板编写完整的 HTML 页面——将 SVG 文件粘贴到模板中标记为 `<!-- PASTE SVG HERE -->` 的位置。
4. 将文件保存为独立的 `.html` 文件（例如 `~/my-diagram.html` 或 `./my-diagram.html`）。
5. 用户可直接在浏览器中打开该文件——无需服务器，也无需额外依赖。

可选：如果用户需要查看多个图表的浏览式画廊，请参考文末的“本地预览服务器”相关说明。
