...
| | |
|---|---|
| 来源 | 可选 — 通过 `hermes skills install official/creative/touchdesigner-mcp` 安装 |
| 路径 | `optional-skills/creative\touchdesigner-mcp` |
| 版本 | `1.1.0` |
| 开发者 | kshitijk4poor |
| 许可协议 | MIT |
| 支持平台 | linux、macos、windows |
| 标签 | `TouchDesigner`、`MCP`、`twozero`、`creative-coding`、`real-time-visuals`、`generative-art`、`audio-reactive`、`VJ`、`installation`、`GLSL` |
| 相关技能 | [`ascii-video`](/docs/user-guide/skills/bundled/creative/creative-ascii-video)、[`manim-video`](/docs/user-guide/skills/bundled/creative/creative-manim-video) |
...
1. **绝不要猜测参数名称。** 首先调用 `td_get_par_info` 获取操作类型的参数信息。您所使用的训练数据并不适用于 TD 2025.32 版本。
2. **如果出现 `tdAttributeError` 异常，请立即停止。** 在继续执行之前，先对出错的节点调用 `td_get_operator_info`。
3. **切勿在脚本回调中硬编码绝对路径。** 应使用 `me.parent()` 或 `scriptOp.parent()` 来获取路径。
4. **优先使用原生 MCP 工具，而非 `td_execute_python`。** 建议使用 `td_create_operator`、`td_set_operator_pars`、`td_get_errors` 等函数；仅当需要处理复杂的多步骤逻辑时才考虑使用 `td_execute_python`。
5. **在构建操作之前先调用 `td_get_hints`。** 该函数会返回与您正在处理的操作类型相关的特定模式。
...
36种原生工具。免费插件（无需付费或许可证——预计2026年4月正式生效）。具备上下文感知能力（能识别已选操作及当前网络环境）。中心节点健康检查：通过`GET http://localhost:40404/mcp`可获取包含实例PID、项目名称及TD版本的JSON数据。
...
**备用方案：** 使用固定值的 `rgba32float` 格式数据（8位数值将被限制在0-1之间，从而导致着色器无法正常运行）。

**反馈值 TOP 的设置方式：** 应参考 `top` 参数，而非直接输入数据线。首次处理完成后，“源数据不足”的问题即会得到解决；出现“处理依赖循环”警告属于正常现象。

**分辨率限制：** 非商业用途的渲染分辨率上限为1280×1280。此时需使用 `outputresolution = 'custom'` 参数。

**大型着色器的处理方法：** 先将GLSL代码写入 `/tmp/file.glsl` 文件，再通过 `td_write_dat` 或 `td_execute_python` 命令来加载该文件。

**顶点/点数据的访问方式（TD 2025.32版本）：** 应使用 `point.P[0]`、`point.P[1]`、`point.P[2]` 这种格式，而非 `.x`、`.y`、`.z`。

**扩展功能的实现方式：** 在CONSTANT模式下，`ext0object` 格式的代码应为 `"op('./datName').module.ClassName(me)"`。使用 `td_write_dat` 编辑完扩展代码后，必须调用 `td_reinit_extension` 函数以重新初始化扩展功能。

**脚本回调函数的路径指定：** 无论何种情况，都必须通过 `me.parent()` 或 `scriptOp.parent()` 来指定相对路径。

**节点清理操作：** 在遍历节点之前，务必先使用 `list(root.children)` 获取所有子节点列表，并同时检查每个节点的 `valid` 属性是否为真。
