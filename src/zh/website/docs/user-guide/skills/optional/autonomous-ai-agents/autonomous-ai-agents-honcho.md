...
在 `hybrid` 和 `context` 模式下，基础上下文（用户描述 + 身份卡 + 会话摘要）会在每一轮对话之前自动注入。因此无需重新获取已注入的内容。仅在以下情况才需调用工具：
- 需要基础上下文中未包含的信息
- 用户明确要求你调取或检查记忆内容
- 需要针对新内容撰写结论时
...
在工具端使用 `honcho_reasoning` 所消耗的成本与自动注入机制相同。在明确调用工具之后，自动注入的节奏会重置——从而避免在同一轮对话中重复计费。
