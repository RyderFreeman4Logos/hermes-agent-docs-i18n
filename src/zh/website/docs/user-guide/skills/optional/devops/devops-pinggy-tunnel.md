...
利用 Pinggy 的 SSH 反向隧道，将本地服务（开发服务器、Webhook 接收端、MCP 端点、演示程序）暴露到公共互联网上。无需安装任何守护进程——用户只需使用现有的 SSH 客户端连接到 `a.pinggy.io:443`，Pinggy 便会返回一个公开的 HTTP/HTTPS 地址。

免费套餐：支持最长 60 分钟的隧道连接，分配随机子域名，无需注册。专业套餐（每月 3 美元）需手动选择并获取令牌方可使用。

## 适用场景

- 用户希望“在本地公开此服务”、“共享我的开发服务器”、“将此 URL 设为公开地址”、“建立 N 端口的隧道”或“为 webhook 获取公开 URL”。  
- 需要在执行本地任务时接收 webhook 回调（如来自 Stripe、GitHub、Discord 或 AgentMail 的请求）。  
- 需要与远程方共享一次性的 HTTP 演示内容（如 MCP 服务器、Ollama/vLLM 接口或控制面板）。  
- 主机已安装 SSH，但没有 `cloudflared`/`ngrok` 程序，而安装这些工具又显得有些过度。  

如果主机已配置好 `cloudflared`，建议使用 `cloudflared-quick-tunnel` 功能——Cloudflare 的快速隧道不会在 60 分钟后失效。  

## 先决条件

- PATH 环境变量中包含 `ssh` 命令（可通过 `ssh -V` 测试）。Linux、macOS 及 Windows 10 及以上系统默认已具备该功能，无需额外安装。  
- 在建立隧道之前，本地服务需已在 `127.0.0.1:<port>` 地址上运行。Pinggy 可生成相关 URL，但在本地服务启动前这些 URL 会返回 502 错误。  

可选配置：

- 若需使用付费的 Pro 版功能（如永久子域名、自定义域名、多条隧道以及无 60 分钟时间限制），需设置 `PINGGY_TOKEN` 环境变量。免费版本则无需任何凭证。  
