...
**无任何依赖项，无需 API 密钥，可在 Linux、macOS 和 Windows 上运行。**
...
| “这个网站安全吗？” | `web_search` | 信誉评估需要网页相关背景信息 |
...
- **DNS 查询**的 MX/NS/TXT 记录查询通过 Google DoH（HTTPS）实现，更易突破防火墙限制
- **SSL 检查**需通过端口 443 连接到目标主机，是唯一的“主动”操作

## 数据来源

所有查询均为**被动式**操作——不涉及端口扫描或漏洞检测：
...
- 部分 WHOIS 服务器会根据 GDPR 法规隐去注册人信息，需告知用户这一情况
- 对于非常流行的域名（拥有数千份证书），crt.sh 的查询速度可能会较慢，建议用户做好相应预期
- 域名可用性检测基于启发式方法（通过 3 种被动信号判断），其准确性不及注册商 API
