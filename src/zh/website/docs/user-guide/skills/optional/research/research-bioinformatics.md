...
当被询问有关生物信息学、基因组学、测序、变异调用、基因表达、单细胞分析、蛋白质结构、药物基因组学、宏基因组学、系统发育学或任何计算生物学任务的问题时，可使用此技能。

该技能可作为两个开源生物信息学技能库的入口。它不会直接整合数百个领域专用技能，而是对它们进行索引，以便按需获取所需功能。

## 来源

◆ **bioSkills** — 385 个参考技能（代码模板、参数指南、决策树）
  代码仓库：https://github.com/GPTomics/bioSkills
  格式：每个主题对应一个包含代码示例的 SKILL.md 文件，支持 Python/R/CLI 语言。

◆ **ClawBio** — 33个可运行的分析流程技能（包含可执行脚本及可复现性包）
  代码仓库地址：https://github.com/ClawBio/ClawBio
  文件格式：附带演示示例的Python脚本。每次分析完成后会生成report.md、commands.sh以及environment.yml文件。

## 如何获取并使用这些技能

1. 从下方的索引中确定对应的领域及技能名称。
2. 克隆相应的代码仓库（为节省时间，建议使用浅克隆）：
   ```bash
