...
协助完成面向 NeurIPS/ICML/ICLR 的机器学习论文撰写：从构思到投稿的全流程服务。

## 技能元数据

| | |
|---|---|
| 来源 | 可选 — 通过命令 `hermes skills install official/research/research-paper-writing` 安装 |
...
| 相关技能 | [`arxiv`](/docs/user-guide/skills/bundled/research/research-arxiv), [`subagent-driven-development`](/docs/user-guide/skills/optional/software-development/software-development-subagent-driven-development) |

## 参考：完整 SKILL.md 内容

:::info
以下为 Hermes 在触发该技能时加载的完整技能定义。当技能处于激活状态时，智能体将依据此内容执行相应操作。
:::

# 研究论文撰写工作流

专为面向**NeurIPS、ICML、ICLR、ACL、AAAI以及COLM**等顶级会议发表的机器学习/人工智能研究论文而设计的端到端处理流程。该技能涵盖了整个研究生命周期：实验设计、执行、监控、分析、论文撰写、审稿、修改以及投稿。

这并非一个线性的流程，而是一个循环往复的迭代过程。实验结果会推动新的实验开展，审稿意见又会触发进一步的分析工作。智能体需能够处理这些反馈循环。
```
┌─────────────────────────────────────────────────────────────┐
...
如需完整的API文档及`CitationManager`类的全部实现，可参阅[references/citation-workflow.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/citation-workflow.md)。

### 步骤1.4：整理相关研究

按研究方法对论文进行分类，而非逐篇处理：

**正确示例**：“有一类研究采用了X的假设[refs]，而我们采用Y的假设，原因是……”
...
| 论点 | 实验 | 预期证据 |
|-------|-----------|-------------------|
| “我们的方法优于基线模型” | 主要对比实验（表1） | 胜率、统计显著性 |
| “在较弱模型上效果更显著” | 模型规模扩展研究 | 单调改进曲线 |
| “收敛需要范围约束” | 有约束与无约束条件对比 | 收敛速度对比 |

**规则**：如果某个实验无法对应到任何论点，则不应执行该实验。

### 步骤2.2：设计基线模型

强大的基线模型是区分被接受论文与被拒论文的关键。审稿人往往会询问：“他们是否与X进行了对比？”
...
如需了解完整的设计模式、定时监控以及错误恢复方案，请参阅 [references/experiment-patterns.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/experiment-patterns.md)。

### 第 2.5 步：设计人工评估（如适用）

许多自然语言处理、人机交互以及对齐相关的研究论文都需要以人工评估作为主要或补充证据。在开展自动化实验之前就应先设计好人工评估方案——因为人工评估往往需要更长的准备时间（如伦理委员会审批、标注员招募等）。

**何时需要进行人工评估：**
- 自动化指标无法反映您关注的关键要素（流畅性、实用性、安全性）
- 您的研究成果侧重于面向用户的特性（可读性、用户偏好、信任度）
...
| **评分方式** | 利克特量表（1-5 分）、成对比较、排序 | 对于大型语言模型生成的文本，成对比较法比利克特量表更可靠 |
| **样本量** | 每位标注员的评估数量及总评估项数 | 可通过功效分析确定样本量，建议至少 100 个评估项，由 3 名以上标注员完成 |
| **一致性度量指标** | 科恩卡帕值、克里彭多夫阿尔法值、ICC 值 | 当标注员人数超过 2 人时使用克里彭多夫阿尔法值；同时报告原始一致性数值 |
...
**报告要求**（审核人员需核查所有内容）：
- 注释员人数及其资质
- 不同注释员之间的一致性，需提供具体指标与数值
- 报酬明细（金额、预计时薪）
- 注释界面描述或截图（附于文档末尾）
- 总注释时长

如需包含针对人工评估数据的统计检验方法、众包质量控制策略以及机构审查委员会相关指导的完整指南，请参阅 [references/human-evaluation.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/human-evaluation.md)。
...
- **效应量**：用于体现实际意义的可恩 d 值或 h 值

关于 McNemar 检验、自助法置信区间以及可恩 h 值的完整实现方式，请参阅 [references/experiment-patterns.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/experiment-patterns.md) 文档。

### 第 4.3 步：明确研究核心内容

分析完成后，需明确回答以下问题：
1. **主要研究发现是什么？** 用一句话概括。
2. **有什么令你感到意外？** 出乎意料的结果往往能造就最优秀的论文。
3. **哪些实验失败了？** 失败的实验往往能提供最多启示。如实报告失败情况有助于提升论文质量。
4. **还需要进行哪些后续实验？** 实验结果常常会引发新的问题。

#### 处理阴性结果或无效结果

当假设不成立或实验结果无显著性时，你有三种处理方式：
...
| 单次迭代效果下降 | 基线分数高于迭代后的输出结果 | 改为单次迭代；模型可能过弱 |
| 过拟合（代码） | 公开测试通过率高，但私有测试通过率低 | 采用结构化分析，而非仅依赖测试反馈 |
| 评分器故障 | 解析失败导致评审小组人数少于3人 | 继续之前需先修复解析器 |

如需完整的提示词、博尔达评分规则、模型选择指南、范围约束设计模式以及计算预算参考信息，请参阅 [references/autoreason-methodology.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/autoreason-methodology.md)。

---

## 第5阶段：论文初稿撰写

完整的初稿撰写流程（各章节顺序、LaTeX框架、图表规范、摘要与引言中的公式格式、相关研究定位等）均记载在 `references/phase5-paper-drafting.md` 中——进入该阶段后请使用 `read_file` 函数加载该文件。同时可结合 `references/writing-guide.md` 了解正文层面的风格规范。
...
每位审稿人在查看元审评意见后，可进一步优化自己的审稿内容。可使用提前终止机制：若审稿人回复“我已完成审稿”（即没有修改意见），则停止迭代。

**用于审稿的模型选择**：即便论文最初是用性能较低的模型撰写的，也建议使用性能最强的模型来进行审稿。审稿模型应与撰写模型分开选择。

**少样本校准**：如果条件允许，可提供1-2篇来自目标期刊的真实已发表审稿意见作为示例。这能显著提升评分的准确性。相关示例可参阅[references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/reviewer-guidelines.md)。

### 步骤6.1b：视觉审稿环节（VLM）

仅依靠文本审稿会遗漏一类问题：图表质量、排版问题以及视觉一致性。如果您拥有具备视觉处理能力的模型，可以对编译后的PDF文件进行单独的**视觉审稿**。
...
各会议均要求填写特定的检查清单。请务必仔细填写——若清单不完整，论文可能会被直接拒收。

相关清单详见 [references/checklists.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/checklists.md)，内容包括：
- NeurIPS论文的16项检查清单
- ICML关于更大影响力与可复现性的要求
- ICLR关于大语言模型相关信息的披露政策
- ACL要求的限制条件部分
- 通用投稿前检查清单
...
上述主要流程针对实证性机器学习论文。其他类型的论文则需要不同的结构与证据标准。有关各类论文的详细指导，请参阅[references/paper-types.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/paper-types.md)。
...
**与实证性论文的主要区别**：
- 研究贡献体现为定理、界限或反例——而非实验数据
- “方法”部分被“基础知识”和“主要结果”替代
- 证据为证明而非实验（尽管对理论的实证验证也是可接受的）
- 正文提供证明概要，附录呈现完整证明，这是标准做法
- 实验部分为可选内容，但若能验证理论预测，则能增强论文的说服力
...
- 为假设编号，并在定理中引用它们：“在假设1-3的前提下，……”

### 综述/教程类论文

**结构**：引言 → 分类/架构 → 详细内容覆盖 → 待解决课题 → 结论
...
如需了解详细的指导原则、常见疑问及反驳策略，请参阅 [references/reviewer-guidelines.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/references/reviewer-guidelines.md)。  
...
`templates/`目录中提供了适用于**NeurIPS 2025**、**ICML 2026**、**ICLR 2026**、**ACL**、**AAAI 2026**以及**COLM 2025**的模板。

有关编译说明，请参阅[templates/README.md](https://github.com/NousResearch/hermes-agent/blob/main/optional-skills/research\research-paper-writing/templates/README.md)。
...
**会议相关资源：** [NeurIPS](https://neurips.cc/Conferences/2025/PaperInformation/StyleFiles) | [ICML](https://icml.cc/Conferences/2025/AuthorInstructions) | [ICLR](https://iclr.cc/Conferences/2026/AuthorGuide) | [ACL](https://github.com/acl-org/acl-style-files)
