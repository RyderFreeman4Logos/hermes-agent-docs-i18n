...
通过 delegate_task 子代理执行计划（两阶段审查机制）。
...
通过为每个任务分配全新的子代理，并结合系统化的两阶段审查机制来执行实现计划。

**核心原则：** 每个任务使用独立的子代理 + 两阶段审查（先审查需求规格，再审查代码质量）= 高质量与快速迭代。

## 适用场景

在以下情况下可使用此技能：
- 已有实现计划（来自 `plan` 技能或用户需求）
- 任务之间大多相互独立
- 对代码质量和规范合规性要求较高
- 希望在任务之间实现自动审查
...
读取计划文件，提前提取所有任务及其完整文本内容和上下文信息，进而生成待办清单：
