...
通过真实浏览器访问网站并将其网络请求记录为 HAR 文件，进而提取出该网站的私有 JSON API，从而允许用户使用普通的 HTTP 请求直接调用该 API —— 这种方式比每次请求都由浏览器控制页面要更加高效且成本低廉。该技术源自 Jared Longster 的创意，后由 Dax (thdxr) 推广普及。此工具仅用于捕获和回放请求，无法绕过身份验证、破解验证码或规避机器人检测；如果网站需要登录会话，用户只需继续使用原有的请求头和 Cookie，而无需伪造它们。

该工具的脚本基于标准库及 Playwright：捕获请求时需要 Playwright，API 提取过程完全依赖标准库，而回放请求则仅需 `requests`/`httpx`（或 `curl`）即可。

该工具涵盖了**所有Hermes浏览器捕获路径**：默认的本地`browser_navigate`后端，以及云端/远程后端（Browserbase、Browser-Use、Firecrawl），还有任何通过 `/browser connect` 连接的CDP端点。由于两种情况下的HAR记录方式存在差异，因此提供了两种捕获脚本——一种用于直接启动浏览器，另一种用于通过CDP连接浏览器（具体操作请参见“如何运行”部分）。

## 适用场景

- 需要为“<网站>”构建CLI或客户端时——可直接提取该网站的API，而无需手动编写点击脚本。
- 某网站没有公开API，但页面明显会加载JSON数据时。
- 需要反复对同一查询执行`browser_navigate`操作时——建议先停止操作，一次性提取目标端点。
- 需要对自动补全、搜索、内容源或结账相关的XHR请求进行逆向分析时。
- 已在云端后端（Browserbase / Browser-Use / Firecrawl）或通过 `/browser connect` 方式捕获了会话，且希望在不重新租用浏览器的情况下获取对应API时。

## 前提条件

- Playwright及对应的浏览器二进制文件（仅用于捕获阶段）：
  - 先执行 `pip install playwright`，再运行 `playwright install chromium`
  - （如果系统已通过 `~/.cache/ms-playwright` 安装了Playwright浏览器，可直接复用。）
- 用于回放阶段的 `requests` 或 `httpx` 库（标准库中的 `urllib` 也可使用）。
- 不需要API密钥。客户端所需的任何密钥或令牌均可从HAR文件中获取。
- 对于CDP路径（`har_capture_cdp.py`）：需有一个可访问的CDP端点。在Hermes中，可通过运行 `/browser connect` 查看当前活跃的端点，或直接读取配置文件中的 `BROWSER_CDP_URL`/`browser.cdp_url`。云端后端则通过 `cdpUrl`/`connectUrl` 属性暴露该端点。

## 如何运行

该技能下的 `scripts/` 目录中所存放的脚本，可通过 `terminal` 工具来调用。
**按路径选择捕获工具**——这是最容易让人困惑的部分：

| 浏览器路径 | Hermes 的访问方式 | 捕获工具 |
|---|---|---|
| 本地 `browser_navigate`（默认，agent-browser/Playwright） | 在本地启动浏览器 | `har_capture.py` |
| Camofox（已设置 `CAMOFOX_URL`） | 本地 REST/CDP 接口 | 若该工具支持 CDP，则使用 `har_capture_cdp.py`，否则需自行实现捕获逻辑 |
...
通用规则：**如果是由 Hermes 直接启动浏览器，使用 `har_capture.py`；如果是通过 CDP 连接到已有浏览器，则使用 `har_capture_cdp.py`。** `har_capture.py` 依赖 Playwright 的 `record_har_path` 函数，而该函数仅能在本地运行的环境中使用。`har_capture_cdp.py` 则通过 `connect_over_cdp()` 方法建立连接，并从 `page.on("request"/"response")` 事件中收集 HAR 数据，因为已连接的浏览器上无法使用 `record_har_path`。

无论采用哪种路径，都需要使用以下工具：
- `har_to_client.py`——用于筛选 HAR 数据中的 XHR/fetch/JSON 请求，按端点进行分组，并输出参数、请求头、请求体以及重放相关提示信息（如 User-Agent、Cookie、认证信息）。

所有路径的解析均基于该技能对应的目录结构。标准处理流程如下：
