...
**在执行其他任何操作之前**，请先确认 Sherlock 是否可用：
...
**如果用户在消息中明确说明了用户名，则直接从中提取。**

以下情况**无需**使用“澄清”功能：
- “查找 nasa 的账户” → 用户名为 `nasa`
- “搜索 johndoe123” → 用户名为 `johndoe123`
- “查看 alice 是否存在于社交媒体上” → 用户名为 `alice`
- “在社交网络中查找用户 bob” → 用户名为 `bob`

**仅在以下情况下才使用“澄清”功能：**
- 提到了多个可能的用户名（如“搜索 alice 或 bob”）
- 表述含糊不清（如仅说“搜索我的用户名”但未明确具体名称）
- 完全未提及用户名（如“进行一次开源情报搜索”）

提取时需使用用户所给出的**确切**用户名——保留其大小写、数字、下划线等格式。

### 3. 构建命令

**默认命令**（除非用户有特殊要求，否则使用此命令）：
```bash
...
通过 `terminal` 工具来运行搜索。根据网络状况及网站数量的不同，该操作通常需要 30 至 120 秒的时间。
...
### Tor 配置
使用 `--tor` 功能需先运行 Tor 守护进程。如果用户希望保持匿名性但无法使用 Tor，建议：
- 安装 Tor 服务
- 使用 `--proxy` 指定其他代理服务器

### 错误检测
由于某些网站的响应结构原因，可能会始终显示“已找到”结果。应对这些异常结果进行人工核实。
...
### Linux软件包
该工具可在Debian 13+、Ubuntu 22.10+、Homebrew、Kali及BlackArch系统中使用。

## 合规使用指南

本工具仅限用于合法的OSINT收集与研究目的。请提醒用户遵守以下规定：
- 仅搜索自己拥有权限或经许可可调查的用户名；
- 遵守相关平台的服务条款；
- 禁止将此工具用于骚扰、跟踪或任何非法活动；
- 在分享查询结果前，务必考虑其对隐私可能造成的影响。

## 结果验证

运行sherlock工具后，请进行以下验证：
1. 查看输出内容，确认其中列出了包含网址的网站；
2. 若选择文件输出模式，应能生成`<username>.txt`格式的文件（为默认输出形式）；
3. 若使用了`--print-found`参数，输出结果中应仅显示表示匹配项的`[+]`行。
