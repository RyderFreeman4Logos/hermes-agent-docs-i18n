...
| 版本 | `1.0.0` |
| 开发者 | arceus77-7，经 Hermes Agent 改进 |
| 许可协议 | MIT |
...
- 为在 Hermes 终端调用期间保持稳定登录状态，需安装 `tmux`（仅适用于桌面应用流程）

## 使用场景

- 安装或配置 1Password CLI
- 使用 `op signin` 登录账户
- 读取诸如 `op://Vault/Item/field` 这样的机密信息引用
...
请在 `${HERMES_HOME:-~/.hermes}/.env` 文件中设置 `OP_SERVICE_ACCOUNT_TOKEN`（该技能在首次加载时会提示您输入此值）。无需使用桌面应用程序，支持 `op read`、`op inject` 以及 `op run` 命令。
