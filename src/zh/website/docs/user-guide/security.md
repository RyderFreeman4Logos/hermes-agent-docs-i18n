...
当 YOLO 模式处于激活状态时，Hermes 会显示两个持续的视觉提示，以便用户不会忘记当前正在绕过所有审批提示：

- 当 YOLO 已激活时，在会话开始时会显示一条红色横幅：`⚠ YOLO 模式 —— 所有审批提示均被绕过`。当 YOLO 模式关闭时，此提示会隐藏，从而保持默认横幅的整洁。
- 在状态栏中显示 `⚠ YOLO` 字样，其宽度可适应不同界面层级，并且会在您开启或关闭 YOLO 模式时实时更新（支持富文本渲染和纯文本显示）。

:::danger
YOLO 模式会**完全**禁用该会话中的所有危险命令安全检查——**硬性拦截列表除外**（详见下文）。仅当您完全信任正在执行的命令时才应使用此模式（例如，在一次性环境中运行的经过充分测试的自动化脚本）。
:::

对于具有破坏性的会话级命令（如 `/clear`、`/new` / `/reset`、`/undo`、`/quit --delete`，其中 `/exit --delete` 是其别名），CLI 在执行这些命令前也会要求用户确认。详情请参阅 [Slash Commands — 破坏性命令的确认提示](../reference/slash-commands.md#confirmation-prompts-for-destructive-commands)。
...
- `--yolo` / `/yolo` 参数处于开启状态
- `approvals.mode: off` 设置
- 在无界面模式的 Cron 作业中运行
- 用户明确选择“始终允许”

该拦截列表位于 `--yolo` 模式之下，甚至在审批层看到命令之前就会触发拦截，且不存在任何可绕过的标志。目前覆盖的匹配模式如下（并非全部，内容会与 `tools/approval.py::UNRECOVERABLE_BLOCKLIST` 保持同步）：
...
| 在根目录层级将不可信的 URL 通过管道传递给 `sh` 命令 | 这属于远程代码执行攻击，风险过高，无法批准 |

如果命令被拦截，工具调用会向智能体返回说明性错误信息，且该命令不会被执行。如果某个合法的工作流确实需要使用这些命令（例如，您是负责系统清除与重装流程的操作人员），则应在智能体外部直接执行这些命令。

### 审批超时设置

当出现危险命令提示时，用户会有一个可配置的响应时间。如果在超时时间内没有收到任何响应，默认情况下该命令将被**拒绝**（即采取失败即终止策略）。

您可以在 `~/.hermes/config.yaml` 文件中配置此超时时间：
...
- 默认值为 `pair`：对于未经授权的私信，系统会回复一个配对码。  
- `ignore` 模式：会静默忽略所有未经授权的私信。  
- 各平台的相关设置可覆盖全局默认值，因此您可以在 Telegram 上保持配对功能正常使用，同时让 WhatsApp 的私信被静默处理。  

**安全特性**（基于 OWASP 及 NIST SP 800-63-4 的指导原则）：  

| 特性 | 详情 |
|------|------|
| 密码格式 | 由 32 个字符组成的字母表中选取的 8 位字符（不含 0、O、1、I） |
| 随机性 | 采用加密级随机生成方式（`secrets.choice()`） |
| 密码有效期 | 1 小时 |
| 请求频率限制 | 每 10 分钟每个用户最多发送 1 次请求 |
| 待处理密码数量限制 | 每个平台最多有 3 个待处理的配对码 |
| 冒用防护 | 若连续 5 次尝试验证失败，系统将锁定 1 小时 |
| 文件安全 | 所有配对数据文件均设置为 `chmod 0600` 权限 |
...
**存储：** 配对数据存储在 `~/.hermes/pairing/` 目录下，以针对不同平台的 JSON 文件形式存在：
- `{platform}-pending.json` — 待处理的配对请求
- `{platform}-approved.json` — 已通过验证的用户
- `_rate_limits.json` — 流量限制与封禁状态追踪
...
每个容器均会以这些标志运行（定义于 `tools/environments/docker.py` 文件中）：
