...
这将使群组/频道恢复为每个房间一个共享会话的模式，既能保留对话上下文，也能共享令牌成本、中断状态以及上下文数据量。

### 会话重置策略

**默认情况下，网关会话永远不会自动重置**（`mode: none`）。您可以通过 `config.yaml` 中的 `session_reset` 配置项来启用自动重置功能：

- **none** — 永不自动重置（默认值；上下文由 `/reset` 功能和压缩机制管理）
- **idle** — 在 N 分钟无操作后重置
- **daily** — 每天在固定时间重置
- **both** — 以先满足的条件为准（无操作或到日）

在会话自动重置之前，智能体会有机会保存对话中的重要记忆或技能信息。

无论采用何种策略，**存在活跃后台进程的会话也绝不会自动重置**。

## 存储位置

| 内容 | 路径 | 说明 |
|------|------|------|
| SQLite 数据库 | `~/.hermes/state.db` | 所有会话元数据及使用 FTS5 索引存储的消息内容 |
| 网关消息 | `~/.hermes/state.db` | SQLite — 所有会话消息的标准化存储介质 |
...
`~/.hermes/sessions/sessions.json` 是**网关路由索引**——它将消息会话键（`agent:main:<platform>:...`）映射到活跃的会话 ID。该文件仅包含网关/消息相关的条目，因此如果您运行的是消息平台，其中只会显示相关条目（例如 `agent:main:whatsapp:dm:...`）。

这是**正常现象**，并不表示您的 CLI 会话丢失了。`hermes sessions list`、`/sessions` 命令以及控制面板均读取 `state.db` 文件，而该文件中保存着**所有**会话（包括 CLI、TUI 和网关会话）。`~/.hermes/sessions/saved/*.json` 下的 `/save` 快照仅用于方便导出数据，并非索引文件。

如果 CLI 会话确实未出现在 `hermes sessions list` 中，原因可能是 `state.db` 未收到这些会话数据——请运行 `hermes sessions repair` 命令，并留意 CLI 启动时是否出现“⚠ Session store unavailable”的警告，这表明此次运行时 SQLite 持久化操作失败了。
:::

:::note 旧版的 JSONL 转录文件
在 `state.db` 成为标准存储格式之前创建的会话，可能在 `~/.hermes/sessions/` 目录下留下一些 `*.jsonl` 文件。Hermes 已不再读取或写入这些文件。在确认对应的会话确实存在于 `state.db` 中后，可以安全地将其删除。
:::
...
- **sessions** — 会话元数据（id、来源、user_id、模型类型、标题、时间戳、令牌计数）。标题字段有唯一索引（允许为空，但非空值必须唯一）。
...
- 可选的自动清理功能：当 `sessions.auto_prune` 设为 `true` 时，那些已结束且超过 `sessions.retention_days`（默认为 90 天）的会话，会在 CLI 或网关启动时被自动删除
- 在实际删除了相关行之后，系统会对 `state.db` 执行 `VACUUM` 操作以释放磁盘空间（SQLite 在普通 DELETE 操作后不会自动缩小文件大小）
- 自动清理操作最多每 `sessions.min_interval_hours` 小时执行一次（默认为 24 小时）；上次执行的时间戳会存储在 `state.db` 文件中，因此同一 `HERMES_HOME` 目录下的所有 Hermes 进程都能共享这一信息。
