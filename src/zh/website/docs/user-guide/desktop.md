...
桌面版应用以聊天界面为核心，左侧设有导航侧边栏。它旨在支持同时管理多个智能体对话、配置消息发送服务、创建输出文件、浏览项目文件夹结构，以及同时处理多个项目。
...
- **会话级YOLO开关**：仅为当前会话开启或关闭YOLO模式（与TUI功能一致）。YOLO模式会跳过危险命令的确认提示，但请注意其潜在风险——详情请参阅[安全设置 → YOLO模式](./security.md#yolo-mode)。

若要与另一台机器上的Hermes实例聊天，而非使用内置的本地后端？请参阅下文的[连接远程后端](#connecting-to-a-remote-backend)；如需了解远程托管控制台连接的完整机制（包括认证网关、/api/ws聊天套接字以及WebSocket关闭码处理方式），请参阅[Web控制台 → 将Hermes桌面版连接到远程后端](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend)。

#### 模型选择

模型选择器位于**提示词编辑器**中，就在麦克风按钮的左侧。点击该选项即可通过下拉菜单切换模型、推理强度及快速模式。

- **提示词编辑器中的选择器仅保存当前界面状态，不会修改您的默认设置**。它会本地（按设备）保存选择，并在新的聊天会话或重启后继续保留该设置，而不会恢复为默认值——只需选择一次模型，下次按下`Cmd/Ctrl+N`时就会自动使用该模型。在实时聊天中，切换模型仅影响当前对话；无论哪种情况，所选设置都会在创建或切换会话时随之保留，且**绝不会**写入个人配置文件的默认设置。（切换[配置文件](#sessions--profiles)则会使设置恢复为对应配置文件的默认值。）
- **可在设置 → 模型中设置默认模型**。该“主”模型即为您的**按配置文件划分的全球默认模型**——所有新对话、定时任务、子智能体及辅助任务都将以此为起点，且只有此处可修改默认模型。每个[配置文件](#sessions--profiles)都拥有自己的默认模型。
- **各模型的推理强度/快速模式预设**：每个模型都会在桌面版应用中记住自身的推理强度和快速模式设置，每次选择该模型时，这些设置都会被应用到当前会话中。这些预设仅是为了方便桌面端使用，并不会影响定时任务或子智能体的设置。
...
可与Hermes进行语音对话并听到回复，支持与其他地方相同的[语音模式](./features/voice-mode.md)。在macOS系统中，操作系统会提示一次是否允许访问麦克风。

### 设置与引导流程

无需编辑YAML文件，即可通过直观的界面管理服务提供方、模型、工具及凭证。首次运行时的引导流程可在几秒钟内让您发送第一条消息。设置面板涵盖服务提供方/密钥管理、模型选择、工具集配置、MCP服务器、网关以及会话管理等功能。

- **服务提供方设置面板**：专门用于管理推理服务提供方，提供“账户/API密钥”界面，用于登录并按服务提供方存储凭证。
- **菜单中列出的所有服务提供方和模型**：GUI界面会展示完整的 service provider 列表以及`hermes model`所识别的所有模型，因此您可以从与CLI相同的目录中选择模型，而非仅能看到经过筛选的子集。
- **xAI Grok OAuth支持**：Grok是启动器中的一等OAuth服务提供方，您可通过浏览器流程像其他OAuth提供方一样进行登录。
- **通过GUI安装工具后端**：可直接在应用内运行工具后端的安装步骤，无需切换到终端操作。
- **辅助模型警告功能**：如果您将主模型切换到新的服务提供方，而某些辅助任务（如标题生成、摘要生成等）仍绑定在另一个服务提供方上，应用会发出警告，防止您在不知情的情况下将工作分配到两个不同的服务提供方。
...
- **定时任务管理**：查看及管理[ scheduled jobs](../reference/cli-commands.md#hermes-cron)。
- **配置文件管理**：在[Hermes配置文件](./profiles.md)之间切换（每个配置文件拥有独立的配置、技能和会话）。
- **消息发送设置**：配置网关通道。
- **智能体与命令中心**：用于多智能体协作的管理界面。

### 键盘快捷键与导航

- **命令面板**：按下**Cmd+K**（Windows/Linux系统为Ctrl+K），即可通过键盘快速访问各种操作并导航应用。
...
- **会话列表优化**：重新设计的会话列表具备归档功能，并采取了整体优化措施，以便在会话数量增加时仍能保持列表的可管理性。
- **按ID搜索会话**：可直接通过会话ID查找特定会话。
- **多配置文件并发会话**：可在多个[配置文件](./profiles.md)之间同时运行会话，并可通过跨配置文件的`@session`链接引用其他配置文件中的会话。

## 更新功能

应用会在后台检查更新，一旦有新版本可用，便会提供一键更新选项。

[手动更新流程](https://hermes-agent.nousresearch.com/docs/getting-started/updating)同样适用于通过GUI进行更新。
...
应用会先关闭，随后完成清理工作（在退出后会运行清理脚本，以移除正在运行的应用包及其独立的虚拟环境）。如果未安装本地智能体（例如仅使用连接远程后端的“轻量级”GUI客户端），则智能体相关的卸载选项会自动隐藏。

您也可以通过终端执行相同操作——使用`hermes uninstall --gui`仅卸载GUI部分，或使用`hermes uninstall`/`hermes uninstall --full`同时卸载智能体。

:::note
从**源代码检出版本**（即`hermes desktop`的开发者构建版本）运行`hermes uninstall --gui`命令时，还会同时删除工作区中的`node_modules`目录以及`apps/desktop/{dist,release}`目录下的构建输出文件，因为这些都是GUI版本的构建产物。这些文件可通过`hermes desktop`工具（或`npm install`后再重新构建）恢复——但如果您正在对桌面版应用进行开发，可能需要之后重新安装依赖项。
:::

## CLI参考：`hermes desktop`

如需通过CLI启动该应用，只需运行`hermes desktop`即可。默认情况下，它会先安装工作区的Node依赖项，然后构建当前操作系统版本的解压版Electron应用，最后启动该打包后的应用。

| 标志参数                 | 描述                                                                 |
| -------------------- | ----------------------------------------------------------------------- |
| `--skip-build`       | 跳过npm安装/打包步骤，直接启动`apps/desktop/release`目录中已有的解压版应用 |
| `--force-build`      | 即使内容版本戳相同，也强制重新构建整个应用                                   |
| `--build-only`       | 仅构建桌面版应用，不立即启动它（用于`hermes update`命令）                 |
| `--source`           | 直接使用`apps/desktop/dist`目录中的文件通过`electron .`命令启动应用，而非使用打包后的应用 |
| `--cwd PATH`         | 设置桌面版聊天会话的初始项目目录（相当于设置`HERMES_DESKTOP_CWD`环境变量） |
| `--hermes-root PATH` | 覆盖应用所使用的Hermes源代码根目录路径（相当于设置`HERMES_DESKTOP_HERMES_ROOT`环境变量） |
| `--ignore-existing`  | 在解析后端时，强制让应用忽略`PATH`环境变量中已存在的任何`hermes` CLI命令 |
| `--fake-boot`        | 启用确定的启动延迟时间，用于验证启动界面功能                                 |

## 工作原理

打包后的应用仅包含Electron外壳程序。首次启动时，它会将Hermes Agent运行时环境安装到`HERMES_HOME`目录中（即`~/.hermes`，在Windows系统中为`%LOCALAPPDATA%\hermes`）——这一路径与CLI安装使用的路径相同，这也是两者可以互相替代的原因。React渲染器通过标准的网关API与`hermes dashboard`后端进行通信，并复用已有的智能体功能，而非重新实现。安装、后端解析以及自我更新等功能均由Electron主进程负责处理。

## 连接远程后端

默认情况下，该应用会启动并管理自己的**本地**后端。您也可以将其配置为连接到运行在另一台机器上的Hermes后端——可以是VPS、家用服务器，或是通过Tailscale连接的Mini设备。

:::info 远程后端是指正在运行的`hermes dashboard`进程
“远程后端”指的是运行在远程机器上的**`hermes dashboard`**服务器——桌面版应用正是连接到该服务器的。除非该dashboard实际处于运行状态且可访问，否则本节中的任何功能都无法正常工作。桌面版应用不会自动为您启动该服务；您（或某个`systemd`服务）需要在远程主机上保持`hermes dashboard`持续运行，然后应用才会连接到它。如果您还使用消息通道（如Telegram、Discord等），则**网关**是另一个需要您单独启动的长期运行的进程——详情请参阅设置步骤后的说明。
:::该连接分为两个部分：在后端，你需要使用**身份验证提供方**来保护控制面板；而在应用程序中，则需输入后端的 URL 并进行登录。将控制面板绑定到非回环地址后，其身份验证网关便会自动启用，而你所配置的提供方则决定了桌面应用能否成功接入。

**请根据后端所在位置选择合适的提供方：**

- **OAuth（Nous Portal）——适用于任何无法从本地机器直接访问的后端。**登录过程会通过你的 Nous 账户进行验证，因此非常适合用于 VPS、公共主机或任何远程后端。你可以使用 `hermes dashboard register` 命令（或 Portal 的 [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) 页面）为控制面板注册，从而配置其 OAuth 客户端，随后在应用程序中通过“使用 Nous Research 登录”完成认证。如果你自行搭建了身份验证提供方，自托管的 OIDC 提供方也能实现相同功能。
- **用户名/密码——仅限本地或可信网络环境使用。**当后端位于同一可信局域网内，或只能通过 VPN（如 Tailscale）访问时，这是最简单的方案。它无需外部身份验证提供方，仅通过单一共享凭证进行保护，**因此绝对不可用于面向公共互联网的控制面板**——此类场景应优先选择 OAuth 方案。

本节其余内容会介绍用户名/密码登录方式，因为它在可信网络环境中最为快捷；关于 OAuth 登录方式，请参阅[Web 控制面板 → 默认提供方：Nous Research](./features/web-dashboard.md#default-provider-nous-research)。

### 后端设置（远程机器）

首先设置用户名和密码，然后让控制面板绑定到可访问的地址并启动运行。这些凭证存储在 `~/.hermes/.env` 文件中（即密钥文件，权限为 0600）：
