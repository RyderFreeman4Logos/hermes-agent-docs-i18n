...
- **主模型**——智能体用于思考的核心模型。每一条用户消息、每一次工具调用循环以及每条流式响应都会经过该模型处理。
- **辅助模型**——智能体用于处理的分支任务。包括上下文压缩、视觉分析（图像处理）、网页摘要生成、审批评分、MCP 工具路由、会话标题生成以及技能搜索等功能。每种辅助模型都有独立的槽位，可单独进行配置覆盖。

本页面介绍如何通过控制面板配置这两种模型。如果您更喜欢使用配置文件或命令行工具，请跳转到文末的[其他方法](#alternative-methods)。

:::提示 最快捷方式：Nous Portal
[Nous Portal](/user-guide/features/tool-gateway) 允许用户通过一个订阅账户获取 300 多种模型。在首次安装时，运行 `hermes setup --portal` 即可登录，并通过一条命令将 Nous 设置为默认提供商。使用 `hermes portal info` 可查看当前已连接的模型。 
- 使用 Portal 订阅的用户还能享受**按令牌计费提供商 10% 的折扣**。
:::

:::注意 `model:` 结构——空字符串与映射格式
在全新安装时，默认配置中的 `model:` 字段值为 `""`（即空字符串，表示“尚未配置”）。首次运行 `hermes setup` 或 `hermes model` 后，该字段会自动升级为包含 `provider`、`default`、`base_url` 和 `api_mode` 子键的映射格式——这与本页面以及 [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md) 中的格式一致。如果您在 `config.yaml` 文件中看到空字符串，只需运行 `hermes model`（或点击控制面板中的**更改**按钮），Hermes 会自动将其转换为映射格式。
:::
...
最顶部的卡片即为**模型设置**面板。主行始终显示智能体在新建会话时会使用的模型。点击**更改**即可打开模型选择器。
...
- **左侧**——已认证的提供商。此处仅显示您已配置的提供商（已设置 API 密钥、已完成 OAuth 认证或被定义为自定义端点的提供商）。如果缺少某个提供商，请前往**密钥**选项卡添加相应的凭证。
- **右侧**——所选提供商的精选模型列表。这些是 Hermes 为该提供商推荐的智能体模型，而非 `/models` 目录中完整的模型列表（在 OpenRouter 中，该目录包含 400 多种模型，涵盖文本转语音、图像生成及重排序等功能）。

在过滤框中输入内容，可按提供商名称、标识符或模型 ID 进行筛选。

选定某个模型后点击**切换**，Hermes 会将其写入 `~/.hermes/config.yaml` 文件的 `model` 部分。**此设置仅适用于新建会话**——您已打开的现有聊天窗口将继续使用初始设置的模型。若要即时更换当前聊天的模型，可在该聊天窗口中使用 `/model` 命令。
...
所有辅助任务的默认值为 `auto`——这意味着 Hermes 也会尝试使用主模型来处理这些任务。如果主模型不可用或出现容量限制类故障，系统会依次尝试特定任务的 `auxiliary.<task>.fallback_chain` 配置、主模型的 `fallback_providers` / `fallback_model` 配置，最后再尝试 Hermes 内置的辅助模型发现机制。如果您希望为某些分支任务使用成本更低或速度更快的模型，可对特定任务进行覆盖配置。

### 常见覆盖配置方式

| 任务类型 | 何时需要覆盖配置 |
|---|---|
| **标题生成** | 几乎总是需要。有些仅需 0.10 美元/分钟的快速模型即可同时完成会话标题生成和 Opus 内容处理功能。在 OpenRouter 上，默认配置为此类模型为 `google/gemini-3-flash-preview`。 |
| **视觉分析** | 当主模型不支持视觉功能时。可选用 `google/gemini-2.5-flash` 或 `gpt-4o-mini` 等模型。 |
| **上下文压缩** | 当您仅为总结上下文内容而耗费大量推理令牌在 Opus/M2.7 模型上时。使用快速聊天模型即可以 1/50 的成本完成相同任务。 |
| **审批功能** | 对于 `approval_mode: smart` 模式——可使用快速且成本低廉的模型（如 haiku、flash、gpt-5-mini）自动判断低风险命令是否需要批准。使用昂贵模型反而会造成浪费。 |
| **网页提取** | 当您频繁使用 `web_extract` 功能时。逻辑与上下文压缩类似——摘要生成无需复杂推理。 |
| **技能中心** | `hermes skills search` 功能会使用此模型。通常默认的 `auto` 设置即可满足需求。 |
| **MCP 工具路由** | 用于 MCP 工具的路由分配。通常默认的 `auto` 设置即可。 |
| **任务分类指定器** | 用于处理 Kanban 任务分类功能（`hermes kanban specify`），可将简略的描述转换为具体的任务规范。使用性能良好且成本较低的模型即可。 |
| **Kanban 任务分解器** | 用于分解 Kanban 任务——将分类后的任务拆解为适合不同专业角色的子任务结构。 |
| **角色描述生成器** | 用于生成角色描述内容（`hermes profile describe --auto` 或控制面板的自动生成按钮）。属于短时间、低成本的调用操作。 |
| **模型审核器** | 用于执行模型使用情况审核功能。在基于推理能力的模型上运行该功能可能需要数分钟，因此使用成本较低的辅助模型通常更为高效。 |

### 单个任务覆盖配置

点击任意辅助任务行上的**更改**按钮。系统会打开相同的模型选择器，操作流程一致：选定提供商和模型后点击**切换**。该行状态将更新为显示 `provider · model`，而非 `auto (使用主模型)`。

### 全部恢复为自动模式

如果您进行了过度优化后希望重新恢复默认设置，可点击辅助模型部分顶部的**全部恢复为自动模式**按钮。所有槽位将再次使用主模型。

## “用作”快捷功能

页面上的每个模型卡片都配有**用作**下拉菜单。这是最快捷的配置方式——只需选择在分析数据中出现的某个模型，点击**用作**，即可一键将其分配到主模型槽位或任意特定的辅助任务中：
...
- **所有辅助任务**——将该模型同时分配到全部 11 个辅助槽位。适用于希望所有分支任务都使用快速模型场景。
- **单个任务选项**——包括视觉分析、网页提取、上下文压缩等任务类型。当前已分配给各任务的模型会标记为 `current`。

当前被分配了模型的卡片会显示 `main` 或 `aux · <任务类型>` 标签，这样您就能一目了然地了解历史配置中哪些模型被用于哪些功能。

## 保存到 `config.yaml` 的内容

通过控制面板保存设置时，Hermes 会将配置内容写入 `~/.hermes/config.yaml` 文件，具体内容如下：
