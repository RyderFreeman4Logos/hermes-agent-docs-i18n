...
该功能优先在独立线程中运行，且内容仅限于任务发起的原始聊天窗口：

- **支持线程的平台**（Telegram主题、Discord/Slack线程）：每次消息发送都会开启一个专属线程，任务指令会被嵌入该线程的会话中，因此在此线程内的回复能够保留完整上下文。对于周期性任务（例如每日任务），每次执行都会新建一个线程，从而确保每轮消息的后续讨论彼此独立。
- **仅支持私信的平台**（WhatsApp、Signal、短信）：由于不存在线程，任务指令会被复制到原始私信会话中——私信本身即作为对话延续的载体。

系统始终只操作原始聊天窗口：不会向其他渠道或群组进行消息扩散/广播（如“全部”或指定其他聊天的发送），因此无法实现跨渠道的对话延续。复制的消息会以带标签的用户发言形式呈现（`[Cron delivery: <任务名称>]`），从而确保在所有模型提供商中都能保持对话历史的有序切换。

#### 单通道直接延续模式（Slack）

上述优先使用线程的功能会在每次消息发送时创建独立线程。如果您希望任务指令**直接显示在频道时间线中**而不生成新线程，只需将Slack的**对话延续模式**设置为`in_channel`即可：
...
在 `in_channel` 模式下，任务指令会以普通的主题频道消息形式发送（不会创建新线程），而您的回复则通过该频道的共享会话来继续处理任务。三个设置共同决定了这一行为：

- **`cron_continuable_surface: in_channel`** —— 在发送时跳过创建线程的步骤。
- **`reply_in_thread: false`**（必填）—— 使机器人以“平铺式”方式在频道中直接回复您的消息，并将其绑定到最初发送任务指令的同一整个频道会话中。若不设置此选项，虽然仍可继续处理任务，但回复会出现在新线程中（系统会安全地回退到线程式回复方式，绝不会丢失回复——网关会在启动时记录警告，以便您发现这种不一致情况）。
- **`require_mention: false`**（或将该频道添加到 `free_response_channels` 中）—— 这样您就可以直接发送普通消息进行回复；否则，机器人只有在您每次回复时都 `@` 提及它时才会响应。

由于任务处理依赖**整个频道**的会话，因此该会话是共享的：频道中的其他用户以及另一个可继续处理的频道内任务，都可以参与同一场持续对话。这正是“频道内平铺式回复”模式所固有的特性，也是用户选择 `reply_in_thread: false` 时已接受的权衡；如果您希望每次任务的后续处理都相互独立，则应使用默认的“线程”模式。

目前这是 Slack 才具备的功能。其他平台虽然也支持该键值，但会回退到“线程”模式（因为它们的任务延续机制有所不同）；具体选择需根据各个平台进行设置，可在对应平台的配置中调整。这是一个网关端的配置选项——通过执行 `/restart` 命令即可应用更改，无需重新安装 Slack 应用。

:::注意：1:1 私信
`cron_continuable_surface` 是一个**频道**专用设置——1:1 私信本身就没有线程与时间线之分（私信本身就是平铺式的），因此该键值在私信中不起作用。决定私信中的定时任务是否可延续处理的，是另一个独立的预设选项 **`slack.dm_top_level_threads_as_sessions`**：

- **`false`** —— 所有顶层私信共享同一个持续对话会话，因此可延续处理的定时任务指令及您的回复都会出现在**同一个**会话中，任务可以在上下文连贯的情况下继续处理。这正是您在私信中实现定时任务延续处理所期望的方式。
- **`true`**（默认值）—— 每条顶层私信消息都拥有独立的会话，因此对已发送的任务指令的回复会启动一个全新的会话，该会话中不保留任何关于原任务的信息。在这种模式下，无论是定时任务还是其他平铺式发送的内容，都无法实现延续处理。

因此，若要将可延续处理的定时任务发送到 1:1 私信中，请将 `slack.dm_top_level_threads_as_sessions` 设置为 `false`。对于私信而言，`cron_continuable_surface` 并非必需项（即使设置也会被忽略）。
:::

### 静默抑制

如果机器人的最终回复中包含 `[SILENT]`，则该任务将完全被抑制不发送。相关输出仍会保存在本地以便审计（路径为 `~/.hermes/cron/output/`），但不会向目标地址发送任何消息。

这对于监控那些仅在出现异常时才需要反馈的任务来说非常有用：
...
无论是否存在 `[SILENT]` 标记，失败的任务始终会生成报告——唯有成功的任务才能被设置为静默模式。对于需要安静监控的任务，可在没有需要报告的内容时，指示代理仅回复 `[SILENT]`。

## 脚本超时时间

通过 `script` 参数传入的预运行脚本默认超时时间为 3600 秒（1 小时）。此限制仅适用于脚本本身——基于技能或大型语言模型的任务拥有独立的无响应时间额度，不受该数值的限制。如果您的脚本需要不同的超时设置，可以对其进行修改：
...
或者设置 `HERMES_CRON_SCRIPT_TIMEOUT` 环境变量。优先级顺序为：环境变量 → config.yaml 文件 → 默认的 3600 秒。

## 无代理模式（仅脚本任务）

对于那些不需要大语言模型进行推理的周期性任务——例如传统的看门狗任务、磁盘/内存监控、心跳检测以及 CI 系统的轮询——可在创建任务时设置 `no_agent=True`。调度器会按预定时间运行您的脚本，并直接输出其标准输出结果，从而完全跳过代理环节：
