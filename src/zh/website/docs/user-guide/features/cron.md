...
您可以使用自然语言或 Cron 表达式来安排任务自动执行。Hermes 通过一个统一的 `cronjob` 工具来实现 Cron 任务管理，该工具采用操作式界面，无需单独的调度、列表或删除工具。
...
- 暂停、恢复、编辑、触发及删除任务
- 为单个任务绑定零个、一个或多个技能
- 将执行结果返回至原始聊天界面、本地文件或已配置的平台目标
- 在带有常规静态工具列表的新的智能体会话中运行
- 以**无智能体模式**运行——即按计划执行脚本，其标准输出将原样输出，完全不涉及大语言模型（详见下文的[无智能体模式](#no-agent-mode-script-only-jobs)部分）

所有这些功能都可通过 `cronjob` 工具在 Hermes 中实现，因此您只需用自然语言下达指令即可创建、暂停、编辑或删除任务，无需使用命令行界面。

:::提示
Cron 任务会使用 `hermes model` 所选择的提供者。由于 OAuth 刷新是自动完成的，`hermes setup --portal` 是实现无人值守运行的最简便方式。请参阅 [Nous Portal](/integrations/nous-portal)。
:::

:::警告
通过 Cron 运行的会话无法递归创建更多 Cron 任务。为防止出现无限循环的调度情况，Hermes 会在 Cron 执行过程中禁用相关的管理工具。
:::
...
如果智能体的最终响应中包含 `[SILENT]`，则将完全停止消息的发送。为便于审计，相关输出仍会保存在本地（路径为 `~/.hermes/cron/output/`），但不会向目标地址发送任何消息。

此功能非常适合用于监控那些仅在出现异常时才需要报告状态的作业。
