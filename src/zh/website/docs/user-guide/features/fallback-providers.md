...
此切换过程无缝衔接——您的对话历史、工具调用及上下文都会被保留。智能体将从之前的中断点继续工作，只是使用了不同的模型。

:::warning 回退机制会重置提示词缓存
提示词缓存是以处理请求的模型（在大多数提供者中还包括账户）为键进行管理的。当触发回退时，新的提供者/模型没有该对话的缓存前缀，因此后续请求需要以全额输入令牌费用重新读取整个历史记录，而无法享受约 75–90% 的折扣缓存费率。当回合结束并恢复使用主模型时也是如此——首次使用主模型处理的请求同样需要重新完整读取（除非主模型的缓存有效期尚未到期）。这是不可避免的——这是确保在故障期间仍能正常运行的代价——但这也解释了为何在多个提供者之间频繁切换的长时间对话，其成本会明显高于始终使用同一提供者的对话。
:::

:::info 每次回合独立处理，而非整个会话
回退机制是**按回合生效**的：每条新的用户消息都会以恢复主模型为起点。如果主模型在当前回合中发生故障，仅该回合会触发回退。收到下一条消息后，Hermes 会再次尝试使用主模型。在单个回合内，回退机制最多触发一次——如果回退也失败，则会进入常规错误处理流程（先尝试重试，再显示错误信息）。这样既能避免在单个回合内出现级联故障，又能让主模型在每个回合都有机会重新工作。
:::
