...
`fallback_providers`（复数形式，以列表呈现）是当前的配置格式，支持按顺序尝试多个备用选项。而 `fallback_model`（单数形式）则是旧版的单一备用键——Hermes 为保持向后兼容仍会支持该键，但当使用 `hermes fallback` 命令写入配置时，将会使用新的 `fallback_providers` 键，并在写入时迁移旧版配置。若同时设置了这两个键，则以 `fallback_providers` 的优先级为准。
:::
...
| Anthropic | `anthropic` | 需提供 `ANTHROPIC_API_KEY` 或 Claude Code 的认证凭证 |
...
| Google Gemini（OAuth 方式） | `google-gemini-cli` | 需通过 `hermes model` 命令配置 Google 的 OAuth 访问权限（可选：`HERMES_GEMINI_PROJECT_ID`） |
...
| Qwen Portal（OAuth 方式） | `qwen-oauth` | 需通过 `hermes model` 命令配置 Qwen Portal 的 OAuth 访问权限（可选：`HERMES_QWEN_BASE_URL`） |
| MiniMax（OAuth 方式） | `minimax-oauth` | 需通过 `hermes model` 命令配置 MiniMax 平台的 OAuth 访问权限 |
...
| LM Studio（本地运行） | `lmstudio` | 需提供 `LM_API_KEY`（本地运行时可无需该参数）及 `LM_BASE_URL` |
| Hugging Face | `huggingface` | 需提供 `HF_TOKEN` |
| 自定义端点 | `custom` | 需提供 `base_url` 及可选的 `key_env` 参数（详情见下文） |

### 自定义端点回退机制
对于自定义的兼容 OpenAI 的端点，只需添加 `base_url` 参数，如需使用密钥则可再添加 `key_env` 参数：
