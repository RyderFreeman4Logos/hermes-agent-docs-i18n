...
- `role="orchestrator"`：子代理保留`delegation`工具集。其功能受`delegation.max_spawn_depth`限制（默认值为**1**，即扁平结构，因此在默认设置下`role="orchestrator"`并无实际作用）。将`max_spawn_depth`设置为2可允许 orchestrator类型的子代理生成leaf类型的孙代理；设置为3及以上则可支持更深的层级结构。该参数没有上限——实际成本才是限制因素。  
- `delegation.orchestrator_enabled: false`：这是一个全局开关，无论子代理的`role`参数为何值，都会强制将其变为`leaf`类型。

**成本警告**：当`max_spawn_depth: 3`且`max_concurrent_children: 3`时，该结构最多可同时运行3×3×3=27个leaf类型的代理。每增加一个层级，资源消耗都会呈倍数增长——请谨慎提升`max_spawn_depth`值。
...
- 若父代理被中断（用户发送新消息、执行 `/stop` 或 `/new` 命令），所有正在运行的子代理都会被取消，并返回`status="interrupted"`状态。它们正在进行中的任务将会丢失。  
- 子代理在父代理的轮次结束后不会继续运行。  
- 被取消的子代理会返回结构化结果（`status="interrupted"`，`exit_reason="interrupted"`），但由于父代理也被中断，这些结果往往无法出现在用户可见的回复中。

对于那些必须能够抵御中断或持续运行超过当前轮次的**长期任务**，建议使用以下方式：

- `cronjob`（action=`create`）——用于安排独立的代理运行任务，不会受到父代理轮次中断的影响。  
- `terminal(background=True, notify_on_complete=True)`——用于运行长时间运行的shell命令，可在代理执行其他任务时继续运行。  
:::

## 主要特性

- 每个子代理拥有**独立的终端会话**（与父代理分离）。  
- **嵌套委托为可选功能**——只有`role="orchestrator"`类型的子代理才能进一步委托任务，且仅当`max_spawn_depth`的值高于默认的1（扁平结构）时才可行。可通过设置`orchestrator_enabled: false`来全局禁用此功能。  
- leaf类型的子代理**无法**调用`delegate_task`、`clarify`、`memory`和`execute_code`函数。Orchestrator类型的子代理虽保留`delegate_task`功能，但仍无法使用其余三个函数。  
- **中断传播机制**——中断父代理会同时中断所有正在运行的子代理（包括orchestrator类型子代理下的孙代理）。  
- 只有最终汇总结果会被纳入父代理的上下文，从而有效控制令牌使用量。  
...
| **工具访问权限** | 可使用所有非阻塞型工具，并支持推理功能 | 通过RPC调用7种工具，但不支持推理功能 |
| **并行能力** | 默认可同时运行3个子代理（可配置） | 仅单个脚本运行 |
| **适用场景** | 需要推理、判断或多步骤解决的复杂任务 | 需要机械式数据处理或按脚本执行的流程 |
| **令牌消耗** | 较高（需完整执行LLM推理循环） | 较低（仅返回标准输出） |
| **用户交互** | 不支持（子代理无法进行澄清操作） | 不支持 |

**经验法则**：当子任务需要推理、判断或多步骤问题解决能力时，使用`delegate_task`；而当需要执行机械式的数据处理或基于脚本的工作流时，则选用`execute_code`。
