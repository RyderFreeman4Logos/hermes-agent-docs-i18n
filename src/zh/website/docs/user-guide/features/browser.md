...
:::提示
对于简单的信息检索，建议使用 `web_search` 或 `web_extract` —— 它们速度更快且成本更低。当需要与页面进行**交互**时（如点击按钮、填写表单、处理动态内容），则应使用浏览器工具。
:::

### `browser_snapshot`

获取当前页面无障碍结构树的文本格式快照。该功能会返回带有 `@e1`、`@e2` 等 ref ID 的交互元素，以便后续与 `browser_click` 和 `browser_type` 功能配合使用。

- **`full=false`**（默认值）：仅显示交互元素的简洁视图
- **`full=true`**：显示完整的页面内容

长度超过 8000 字符的快照会由大型语言模型自动进行总结处理。

### `browser_click`

根据快照中提供的 ref ID 点击对应的元素。
...
当当前会话处于 CDP 监控器激活状态时（通常是指针对具备 CDP 功能的后端执行了 `browser_navigate` 操作的任何会话），评估操作将通过该监控器的持久 WebSocket 进行，无需启动子进程，从而避免额外开销。否则，则会采用标准的代理-浏览器 CLI 路径进行处理。两种方式的行为表现完全一致，仅有延迟会有所不同。

### `browser_cdp`

该功能可直接传输原始的 Chrome DevTools 协议数据，专为处理其他工具无法覆盖的浏览器操作而设计。可用于原生对话框处理、iframe 内容评估、Cookie/网络控制，以及代理所需的任何 CDP 指令。

**仅当在会话启动时能够访问 CDP 端点时才可用**——即 `/browser connect` 已连接到正在运行的 Chrome、Brave、Chromium 或 Edge 浏览器，或者已在 `config.yaml` 中设置了 `browser.cdp_url`。默认的本地代理-浏览器模式 Camofox 以及各类云服务提供商（Browserbase、Browser Use、Firecrawl）目前均不向该工具提供 CDP 接口——虽然云服务提供商提供了每会话独立的 CDP URL，但实时会话路由功能仍在后续开发中。

**CDP 方法参考文档：** https://chromedevtools.github.io/devtools-protocol/ —— 代理可通过 `web_extract` 功能提取特定方法的页面内容，以此查询参数信息并了解其结构。
...
启用该功能后，系统会在首次执行 `browser_navigate` 指令时自动开始录制，并在会话结束时将录像保存至 `~/.hermes/browser_recordings/` 目录中。该功能同时支持本地模式和云端（Browserbase）模式。超过72小时的录像会自动被清理。

## 隐私保护功能

Browserbase 提供了多种自动隐私保护功能：

| 功能 | 默认设置 | 说明 |
|---------|---------|------|
| 基础隐私保护 | 永久开启 | 通过随机指纹、视口随机化及验证码自动解决来提升隐蔽性 |
| 居民级代理 | 开启 | 经由居民IP路由，以获得更好的访问体验 |
| 高级隐私保护 | 关闭 | 需要使用定制版Chromium浏览器，且需订阅高级套餐 |
| 连接保持 | 开启 | 在网络故障时自动重新建立会话连接 |

:::note
如果您的套餐未包含这些付费功能，Hermes会自动降级处理——首先关闭“连接保持”功能，再关闭代理服务——以确保免费套餐用户仍能正常浏览。
:::

## 会话管理

- 每个任务都会通过Browserbase分配独立的浏览器会话
- 会话在长时间无操作后会被自动清理（默认时间为2分钟）
- 后台线程会每30秒检查一次过期的会话
- 在进程退出时还会进行紧急清理，以防止出现孤立会话
- 会话可通过Browserbase API以 `REQUEST_RELEASE` 状态被释放

## 局限性

- **基于文本的交互**——依赖无障碍访问树而非像素坐标
- **快照大小限制**——页面内容过长时可能会被截断，或在对内容进行总结后长度限制为8000个字符
- **会话超时**——云端会话的有效期取决于您所选服务提供商的套餐设置
- **成本问题**——云端会话会消耗服务提供商的积分；对话结束或长时间无操作后，会话会自动被清理。如需免费进行本地浏览，请使用 `/browser connect` 命令。
- **不支持文件下载**——无法从浏览器中下载文件
