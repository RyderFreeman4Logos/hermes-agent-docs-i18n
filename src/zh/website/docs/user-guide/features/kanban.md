...
这些键仅是一种约定，并非架构强制要求。其优势在于，每个工作节点都会留下足够的证据，以便后续处理者能够快速回答以下四个问题：
...
请将机密信息、原始日志、令牌、OAuth相关资料以及无关的对话记录移出`metadata`字段，转而存储指针和摘要。如果某个任务没有相关文件或测试用例，请在`summary`中明确说明，而将现有的证据（如源代码链接、问题编号或人工审核步骤）存放在`metadata`中。

### 工作节点的生命周期

所有用于处理看板任务的智能体配置都会自动包含工作节点生命周期机制——该机制会在智能体启动时被注入到其系统提示语中（即`KANBAN_GUIDANCE`模块），因此**无需进行任何安装或配置操作**。该机制通过**工具调用**而非CLI命令来指导智能体完成整个生命周期：

1. 启动时，调用`kanban_show()`函数以获取任务标题、内容、上级任务关联信息、之前的尝试记录以及完整的评论线程。
2. 通过终端工具执行`cd $HERMES_KANBAN_WORKSPACE`命令，然后在相应工作空间中执行任务操作。
3. 在执行耗时较长的操作时，每隔几分钟调用一次`kanban_heartbeat(note="...")`函数。**如果任务可能持续运行超过1小时，建议至少每小时调用一次`kanban_heartbeat`**——调度器会在过去1小时内未收到心跳信号的、已运行超过`kanban.dispatch_stale_timeout_seconds`（默认为4小时）的任务自动回收处理，因为系统会认为该智能体已崩溃且未进行清理。虽然任务会被重新标记为`ready`状态以供再次分配，且失败计数不会增加，但您将丢失当前任务的进度。
4. 任务完成后，调用`kanban_complete(summary="...", metadata={...})`函数；如果遇到阻塞，则调用`kanban_block(reason="...")`函数。

最终的`kanban_complete`/`kanban_block`调用属于工作节点协议的一部分。如果智能体在任务仍处于`running`状态时以状态0退出，调度器会将其视为协议违规行为，触发`protocol_violation`事件，并在下一个时间周期自动阻塞该任务，而不会让其重新进入循环处理。这通常意味着模型仅生成了纯文本答案，且未使用看板工具功能即退出。

生命周期机制以及相关的关键参考信息（如工作空间类型、交付成果`artifacts`、创建的任务卡片等）都包含在那个系统提示语模块中，因此无论智能体运行在何种配置下，都能获取这些信息——无需为不同配置单独设置技能。

### 将额外技能绑定到特定任务

有时，单个任务需要特定的专业背景知识，而这些知识并非分配给该任务的智能体默认具备的——例如翻译任务需要`translation`技能，审核任务需要`github-code-review`技能，安全审计任务则需要`security-pr-audit`技能。无需每次都修改负责人的配置，可以直接将所需技能绑定到任务上。

**对于调度智能体**（即通常情况下的情景：一个智能体将任务分配给另一个智能体处理），可通过`kanban_create`工具的`skills`数组来实现这一功能：
...
默认情况下，每个工作节点仅有一次处理其对应卡片的机会——完成工作后调用 `kanban_complete`/`kanban_block` 即可退出。若希望让该工作节点进入**目标循环**模式（即采用与 `/goal` 接口相同的 Ralph 式引擎），则需通过 CLI 参数 `--goal` 或在 `kanban_create` 工具/控制面板中设置 `goal_mode=True`。在这种模式下，每轮处理结束后，辅助裁判会将该工作节点的输出结果与卡片中的标题及内容（视为验收标准）进行比对；如果工作尚未完成且当前轮次时间尚未用完，工作节点将在**同一会话**中继续处理，直到裁判确认合格、工作节点主动终止任务，或时间预算耗尽——此时卡片会被**锁定**以供人工审核，而不会默默退出。
...
- **看板**标签页：每个状态对应一列，包括`triage`、`todo`、`ready`、`running`、`blocked`、`done`（开启相应开关后还会显示`archived`列）。
  - `triage`列用于暂存初步构思。在默认设置（`kanban.auto_decompose: true`）下，调度器会自动对放入此列的任务运行**分解器**。内置的分解器会使用`auxiliary.kanban_decomposer`模型路径，读取您的个人资料列表及其中描述的职责信息，然后将任务拆解为一系列子任务，按最适合处理该任务的专家进行分配。原始任务作为所有子任务的父任务依然存在，因此其负责人（`kanban.orchestrator_profile`，若未设置则使用当前默认负责人）可在所有任务处理完成后重新启动，判断任务是否完成。您可以通过页面顶部的**Orchestration: Auto/Manual**切换按钮（绿色代表自动模式，灰暗灰色代表手动模式）或直接编辑`config.yaml`来切换模式。这两种模式均可与`hermes kanban specify`功能共存——当您不希望任务拆分时，仍可使用该功能对单个任务进行配置。
- 任务卡片会显示任务ID、标题、优先级标签、租户标签、分配的负责人、评论/链接数量、**进度指示条**（若任务存在依赖关系，则显示“已完成N/M个子任务”），以及“创建于N前”。每张卡片都有一个复选框，支持多选操作。
- **Running列内的按负责人分组功能**——通过工具栏中的复选框，可按负责人对Running列进行子分组。
- **通过WebSocket实现实时更新**——该插件会以较短的时间间隔轮询只读的`task_events`表；一旦有任何个人资料（通过CLI、网关或其他仪表板标签页）对任务进行操作，看板就会立即反映变化。为避免大量事件同时触发导致频繁刷新，系统采用了去抖处理机制。
- 可通过拖放卡片在列之间移动来更改任务状态。拖放操作会发送`PATCH /api/plugins/kanban/tasks/:id`请求，该请求会经过与CLI相同的`kanban_db`代码处理——因此三种操作界面始终保持一致。当任务被移动到具有破坏性意义的状态（如`done`、`archived`、`blocked`）时，系统会要求用户确认操作。触摸设备则提供基于指针的替代操作方式，确保可在平板电脑上正常使用看板。
- **内联创建功能**——点击任意列标题处的“+”号，即可输入任务标题、负责人、优先级，还可从现有任务的下拉列表中选择父任务。按回车键可创建任务，按Shift+回车键可在标题字段中插入换行符，按Esc键可取消操作。从Triage列创建的任务会自动被暂存到该列中。
- **多选及批量操作功能**——按住Shift/Ctrl键点击卡片或勾选其复选框即可将其加入选择列表。顶部会出现批量操作栏，支持批量更改任务状态、归档任务或重新分配任务（可通过负责人下拉列表选择，或选择“(unassign)”）。对于批量操作，系统会先要求用户确认。即使部分任务处理失败，其余任务仍会继续执行。
- **点击卡片**（无需按住Shift/Ctrl键）即可打开侧边抽屉（按Esc键或点击页面外部可关闭），其中包含：
  - **可编辑的标题**——点击标题即可重命名。
  - **可编辑的负责人/优先级**——点击元数据行即可修改。
  - **可编辑的描述**——默认以Markdown格式显示（支持标题、加粗、斜体、内联代码、代码块、`http(s)` / `mailto:`链接、项目列表等），还有一个“编辑”按钮，点击后会切换为文本输入框。该Markdown渲染器经过特殊处理，具备XSS防护功能——所有替换操作都会在经过HTML转义的输入数据上执行，仅允许`http(s)` / `mailto:`链接通过，且始终会设置`target="_blank"` + `rel="noopener noreferrer"`属性。
  - **依赖关系编辑器**——以芯片状列表展示父任务和子任务，每个项目旁边都有一个“×”按钮可用于断开关联；此外还有其他所有任务的下拉列表，可用于添加新的父任务或子任务。若尝试形成循环依赖，服务器端会拒绝该操作并给出明确提示。
  - **状态操作行**（包含→ triage / → ready / → running / block / unblock / complete / archive等选项），对于具有破坏性意义的状态转换，系统会要求用户确认。对于位于**Triage**列的卡片，该行还提供两个由大语言模型驱动的操作：**⚗ Decompose**功能会根据任务描述将任务拆解为一系列子任务，并按最适合处理该任务的专家进行分配；**✨ Specify**功能则会对单个任务进行配置重写。若大语言模型判断任务无需拆分，它会自动回退到Specify风格的升级方式，因此Decompose实际上是Specify功能的严格超集。这两种操作均可通过CLI（`hermes kanban decompose <id>` / `specify <id>` / `--all`）、任何网关平台（`/kanban decompose <id>`）以及通过`POST /api/plugins/kanban/tasks/:id/decompose`和`…/specify`接口以编程方式执行。您可以在`config.yaml`的`auxiliary.kanban_decomposer`和`auxiliary.triage_specifier`字段中配置相关模型。
  - 结果区域（同样以Markdown格式显示）、评论区（按回车键可提交评论）以及最近20条事件记录。
- **工具栏过滤器**——支持全文搜索、租户下拉选择（默认值为`config.yaml`中指定的`dashboard.kanban.default_tenant`）、负责人下拉选择、“显示已归档任务”切换开关、“按负责人分组”切换开关，还有一个**Nudge dispatcher**按钮，让您无需等待60秒的时间间隔即可手动触发操作。

在视觉设计上，该看板采用大家熟悉的Linear/Fusion布局风格：深色主题，列标题旁显示数量信息，状态用彩色圆点表示，优先级和租户信息则通过彩色芯片展示。该插件仅读取主题CSS变量（如`--color-*`、`--radius`、`--font-mono`等），因此会自动根据当前激活的仪表板主题调整外观。

### 自动调度与手动调度

对于您放入Triage列的任务，看板有两种处理方式：

**自动模式（默认）**——`kanban.auto_decompose: true`。嵌入在网关中的调度器会每隔一定时间运行一次**分解器**，其运行频率受`kanban.auto_decompose_per_tick`参数限制（默认为每轮间隔处理3个任务），这样即可避免大量Triage列任务同时涌入，导致辅助大语言模型负担过重。分解器会使用内置的分解提示词以及`auxiliary.kanban_decomposer`模型路径，读取您已安装的个人资料及其描述，然后让大语言模型生成一个JSON格式的任务图谱：明确哪些任务需要创建、这些任务应分配给谁、以及哪些任务之间存在依赖关系。原始的Triage列任务会成为任务图谱中所有子任务的父任务，因此会一直存在，直到整个图谱中的任务全部处理完成——之后它会被提升回`ready`状态，其负责人（`kanban.orchestrator_profile`，若未设置则使用当前默认负责人）便可判断任务是否完成，若工作尚未结束，则可继续添加新任务。这就是所谓的“输入简要描述后即可离开”的操作流程。

**手动模式**——`kanban.auto_decompose: false`。Triage列中的任务会一直停留在该列中，直到您主动处理。您可以点击卡片上的**⚗ Decompose**按钮，运行`hermes kanban decompose <id>`命令（或使用`--all`参数），或者通过聊天界面输入`/kanban decompose <id>`来触发分解操作。此模式与分解器出现之前的看板行为一致，适用于您需要完全掌控任务处理流程的场景。

您可以通过看板页面顶部的**Orchestration: Auto/Manual**切换按钮（绿色代表自动模式，灰暗灰色代表手动模式）或直接编辑`config.yaml`来在两种模式之间切换。这两种模式均可与`hermes kanban specify`功能共存——当您不希望任务拆分时，仍可使用该功能对单个任务进行配置。

分解器的任务分配决策取决于个人资料描述，这是一种按负责人分类的标记方式，您可以通过`hermes profile create --description "..."`、`hermes profile describe <name> --text "..."`、`hermes profile describe <name> --auto`（由大语言模型根据该个人资料已安装的技能和模型自动生成描述）命令，或通过展开后的**Orchestration settings**面板中的个人资料编辑器来设置。没有描述的个人资料仍会出现在列表中——它们可以通过名称进行分配，但分配的精确度较低。分解器绝不会将子任务的`assignee`字段设置为`None`：当大语言模型无法识别某个个人资料时，该子任务会被分配给`kanban.default_assignee`（若未设置则使用当前默认负责人）。

`kanban.orchestrator_profile`参数并不会将对应个人资料的提示词、技能或自定义逻辑加载到分解操作中。它的作用是确定任务拆分后的根任务/调度任务的负责人。如需更改分解器使用的模型或提供方，可配置`auxiliary.kanban_decomposer`参数。如需使用某个个人资料的自定义任务拆分逻辑而非内置分解器，则需切换到手动模式，并让该个人资料明确负责创建或分解任务。

相关配置选项（均位于`~/.hermes/config.yaml`文件中的`kanban:`部分）如下：

| 参数名 | 默认值 | 作用 |
|---|---|---|
| `auto_decompose` | `true` | 调度器会每隔一定时间自动运行分解器。 |
| `auto_decompose_per_tick` | `3` | 每次调度器轮询时允许分解的任务数量上限。超出此限制的任务将推迟到下一次处理。 |
| `orchestrator_profile` | `""` | 任务拆分后的根任务/调度任务的负责人。留空则回退到当前默认负责人。 |
| `default_assignee` | `""` | 当大语言模型无法识别某个个人资料时，子任务将被分配给该负责人。留空则回退到当前默认负责人。 |
| `auto_subscribe_on_create` | `true` | 当工作者在具有持久消息传递通道的会话中（如消息网关或TUI）调用`kanban_create`函数时，发起请求的会话会自动订阅新任务的完成/阻塞事件。调度器仍负责消息传递——此参数仅影响发起请求方的聊天记录是否会被添加到通知订阅表中。将其设置为`false`则要求为每个任务单独调用`kanban_notify-subscribe`函数。 |
...
该GUI层仅是一个**通过数据库读取数据 + 通过kanban_db写入数据**的层，本身不包含任何业务逻辑：
