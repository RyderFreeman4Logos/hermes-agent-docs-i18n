...
在聊天会话中，使用 `/bundles` 命令即可查看所有已安装的插件包及其包含的技能。

### 行为特点

- **当技能名称冲突时，插件包优先于单个技能生效**。如果你将一个插件包命名为 `research`，同时系统中也存在名为 `research` 的技能，那么调用 `/research` 时会触发该插件包中的技能。这是有意为之——正是由于你为插件包指定了名称，才使其被启用。
- **缺失的技能会被跳过，而不会导致任务失败**。如果某个插件包列出了 `skill-foo` 这一技能，而你并未安装它，该插件包仍会加载其他可正常使用的技能，并向智能体反馈一份清单，说明哪些技能被跳过了。
- **插件包在所有场景下均有效**——无论是交互式 CLI、TUI、控制台聊天，还是各类通道平台（Telegram、Discord、Slack 等）——因为任务调度与单个技能命令的处理都在同一位置集中进行。
- **插件包不会影响提示词缓存**。调用插件包时，系统会像使用 `/<skill-name>` 一样生成全新的用户消息，而不会对系统提示词进行任何修改。

### 何时应使用插件包而非手动安装每个技能

以下情况适合使用插件包：
- 对于重复性任务，你总是需要搭配相同的几项技能（如 `/backend-dev`、`/release-prep`、`/incident-response`）。
- 相较于连续输入多个 `/skill` 命令，使用插件包能让你的思维模型更简洁，只需记住一个名称即可。
- 你希望为整个团队统一配置“任务模板”，只需将插件包的 YAML 文件放入共享的 dotfiles 仓库中，并通过符号链接将其置于 `~/.hermes/skill-bundles/` 目录下即可。

需要明确的是，插件包仅是一个 YAML 别名——它并不会自动为你安装技能。这些技能本身必须已经存在（位于 `~/.hermes/skills/` 目录或外部技能目录中）。否则，调用插件包时，缺失的技能就会被直接跳过。

## 智能体管理的技能（skill_manage 工具）

智能体可以通过 `skill_manage` 工具自行创建、更新和删除技能。这相当于智能体的**程序化记忆**——当它找到一种复杂的处理流程后，会将该方法保存为技能，以便日后重复使用。

### 智能体会创建技能的场景

- 成功完成一项包含 5 次及以上工具调用的复杂任务后。
- 在遇到错误或僵局后找到了可行的解决方案时。
- 用户指出了其处理方法的不足并予以纠正后。
- 发现某种复杂的处理流程后。
...
:::tip
进行更新时，推荐使用 `patch` 操作——因为只需传递变更后的文本，相比 `edit` 更节省令牌。
:::

### 对智能体技能写入操作的审批控制（skills.write_approval）

默认情况下，智能体可以自由创建技能——包括在每轮对话结束后进行的[后台自我改进审核](/user-guide/features/memory#controlling-memory-writes-write_approval)过程中。如果你希望先对每一次技能创建操作进行审批（适用于那些无法准确判断自身学习成果的较小模型、安全要求较高的环境，或只是希望有人监督自我改进流程），可以启用写入审批机制：
