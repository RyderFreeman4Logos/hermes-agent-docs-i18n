...
在集成控制面板、浏览器界面或控制平面时，可使用此端点。通过它，相关应用无需了解 Hermes 的内部实现细节，即可判断当前运行的 Hermes 版本是否支持任务执行、流式处理、任务取消以及会话连续性功能。

### GET /health

健康检查接口。返回 `{"status": "ok"}`。对于需要 `/v1/` 前缀的 OpenAI 兼容客户端，也可通过 **GET /v1/health** 访问该接口。

### GET /health/detailed

更详细的健康检查接口，除了会显示健康状态外，还会报告当前活跃的会话、正在运行的智能体以及资源使用情况。非常适合用于监控与可观测性工具。

## Runs API（适用于流式处理的替代方案）

除了 `/v1/chat/completions` 和 `/v1/responses` 接口外，服务器还提供了一个 **runs** API，专为需要长时间会话的场景设计。通过该接口，客户端可以订阅进度事件，而无需自行处理流式数据传输。

### POST /v1/runs

创建一个新的智能体任务实例。接口会返回一个 `run_id`，可用于订阅后续的进度事件。
...
在终端状态（`completed`、`failed` 或 `cancelled`）出现后，系统会短暂保留相关状态，以便用于轮询及界面状态同步。

### GET /v1/runs/\{run_id\}/events

该接口以 Server-Sent Events 的形式推送任务执行过程中的工具调用进度、令牌变化以及生命周期事件。它专为那些希望在不丢失状态的情况下进行连接/断开操作的仪表板应用和客户端设计。

### POST /v1/runs/\{run_id\}/stop

用于中断正在运行的智能体轮次。该接口会立即返回 `{"status": "stopping"}`，同时 Hermes 会指示当前正在运行的智能体在下一个安全的中断点停止。

### POST /v1/runs/\{run_id\}/approval

用于处理那些需要人工决策才能继续的任务（例如，受审批策略限制的工具调用）。请求体中需包含审批结果；一旦该结果被记录下来，任务就会恢复执行。此接口在 `/v1/capabilities` 中被标记为 `run_approval` 功能，以便外部界面在弹出审批提示之前就能判断是否支持该功能。

## Jobs API（后台调度任务）

服务器提供了一个轻量级的 Jobs CRUD 接口，允许远程客户端管理已调度或正在后台运行的智能体任务。所有接口均需通过相同的令牌认证才能访问。
...
创建一个新的调度任务。请求体格式与 `hermes cron` 相同，包括提示语、调度时间、使用的技能、提供商配置以及交付目标。
...
更新现有任务的各项设置（如提示语、调度时间等）。支持部分更新，系统会自动合并这些更改。

### DELETE /api/jobs/\{job_id\}

删除某个任务，同时也会取消该任务下正在进行的任何运行。

### POST /api/jobs/\{job_id\}/pause

暂停某个任务而不将其删除。该任务的下次调度时间会被暂缓，直到被重新启动为止。
...
外部界面无需搭建专门的仪表板，即可通过 REST 接口管理 Hermes 会话。所有接口均受 `API_SERVER_KEY` 保护，位于 `/api/sessions/*` 路径下。
...
| `POST` | `/api/sessions/{id}/fork` | 通过 `SessionDB` 机制创建该会话的子会话（功能与 CLI 的 `/branch` 命令类似） |
...
`/v1/capabilities` 接口通过 `session_*` 功能标志及 `endpoints.session_*` 条目公开了所有相关接口信息，从而使外部界面能够判断是否支持该功能，并采取相应的处理方式。在 `chat` 和 `chat/stream` 类型的请求体中，还支持嵌入图片（即支持多模态内容）。
