...
`web` 这一附加组件会引入 FastAPI/Uvicorn；而 `pty` 组件则会引入 `ptyprocess`（用于 POSIX 环境）或 `pywinpty`（用于原生 Windows 环境——需注意，内置的 TUI 本身仍需依赖 WSL）。若您同时需要消息传递、语音功能等，使用 `pip install hermes-agent[all]` 即可一次性安装所有组件，这是最简便的方式。

如果在运行 `hermes dashboard` 时缺少相关依赖，该程序会提示您需要安装哪些组件。如果前端尚未构建且系统中存在 `npm`，则首次启动时会自动进行构建。

“聊天”标签页在每次启动 `hermes dashboard` 时都会出现——内置的浏览器聊天面板（通过 PTY/WebSocket 运行 TUI）始终可用，无需额外设置任何参数。
...
- **最近会话列表**——显示最近 20 个会话的详细信息，包括所使用的模型、消息数量、令牌使用情况以及对话预览内容

状态页面每 5 秒自动刷新一次。

### 聊天页面

“聊天”标签页会将完整的 Hermes TUI（与通过 `hermes --tui` 打开的界面相同）直接嵌入浏览器中。在终端 TUI 中能实现的所有功能——斜杠命令、模型选择器、工具调用卡片、Markdown 流式显示、澄清/授权/批准提示、主题定制等——在这里都能正常使用，因为仪表板实际上是在运行真实的 TUI 可执行文件，并通过 [xterm.js](https://xtermjs.org/) 及其 WebGL 渲染器来呈现 ANSI 输出，从而实现精确的单元格布局。

**工作原理如下：**

- `/api/pty` 会打开一个通过仪表板会话令牌进行身份验证的 WebSocket 连接
- 服务器会在 POSIX 伪终端背后启动 `hermes --tui` 进程
- 按键输入会被发送到伪终端，而 ANSI 输出则会流回浏览器
- xterm.js 的 WebGL 渲染器会将每个单元格渲染到整数像素网格上；鼠标跟踪（SGR 1006 标准）、宽字符（Unicode 11）以及绘图符号都能以原生方式显示
...
- Node.js（要求与 `hermes --tui` 相同；TUI 包会在首次启动时自动构建）
- `ptyprocess`——由 `pty` 附加组件自动安装（可通过 `pip install 'hermes-agent[web,pty]'` 安装，或使用 `[all]` 一次性安装两者）
- POSIX 内核（Linux、macOS 或 WSL2）。具体到“聊天”终端面板，需要 POSIX 伪终端——原生 Windows 版 Python 没有对应的机制，因此在原生 Windows 环境下，虽然仪表板的其他功能（如会话管理、任务处理、指标展示、配置编辑器等）仍可正常使用，但“聊天”标签页会显示提示，建议使用 WSL2 来启用该功能。

关闭浏览器标签页后，服务器端的伪终端会被及时释放。重新打开浏览器则会创建一个新的会话。

若希望 [Hermes Desktop](#connecting-hermes-desktop-to-a-remote-backend) 连接到另一台机器上运行的仪表板，而非其自带的本地后端，请参阅下文的远程后端相关内容。

### 将 Hermes Desktop 连接到远程后端

Hermes Desktop 通常会启动自己的本地后端，但它也可以通过 **设置 → 网关 → 远程网关** 的路径，连接到运行在远程机器（如虚拟机、家庭实验室设备等）上的仪表板。这也是出现“桌面端显示后端已准备就绪，但聊天功能无法使用”这一问题的常见原因，因为桌面端的就绪检查所要求的条件，比实际进行实时聊天连接所需的条件更为宽松。

:::info 前置条件：远程主机上必须正在运行 `hermes dashboard` 
桌面端所连接的“远程后端”实际上就是运行在远程机器上的 `hermes dashboard` 进程——也就是本文档所描述的那个服务器。在执行以下任何操作之前，该服务器都必须处于运行状态且可访问；桌面端仅会连接到该服务器，而不会自动为其启动服务。建议通过 `systemd`/`tmux` 等工具保持该服务器持续运行，这样即便用户登出或重启系统，服务也不会中断。至于 **网关**（Telegram/Discord/Slack 等平台），则是一个独立的长期运行的进程——如果您依赖消息通道功能，需要单独启动该网关；桌面端应用并不会连接到网关。
:::

桌面端的“远程后端已准备就绪”检测功能仅会访问 `GET /api/status` 这一公开接口——只要主机上存在任何正在运行的仪表板，该接口就会立即响应。而实时聊天连接则是通过另一个 WebSocket 连接到 `/api/ws`（以及 `/api/pty`）接口，这个 Socket 的使用还受到另外两项检查的约束，而状态检测功能并不会涉及这些检查：

1. **必须完成身份验证。** 当仪表板绑定到非回环地址时，会启用身份验证机制。建议使用用户名和密码来保护该机制（可通过内置的 [用户名/密码提供器](#usernamepassword-provider-no-oauth-idp) 实现）；桌面端只需登录一次，即可通过一次性令牌在 WebSocket 连接中重复使用该会话。如果未配置相应的提供器，非回环地址的仪表板将在启动时直接失败。
2. **绑定主机必须允许客户端连接，并且其 Host 头部信息必须匹配。** 回环地址绑定（`127.0.0.1`）仅接受本机客户端连接，因此无论凭据如何，远程机器在 Socket 层就会被拒绝。建议将仪表板绑定到非回环地址（`--host 0.0.0.0`），这样基于对等 IP 的验证机制才能允许远程客户端接入。在桌面端输入的远程 URL 必须能够通过仪表板绑定的同一主机访问——DNS 重定向验证机制要求 Host 头部信息必须保持一致。

#### 远程仪表板配置

请设置用户名和密码，然后让仪表板绑定到可访问的地址上运行。如果是通过 `systemd` 服务来管理仪表板，则需按照相应方式配置服务。
