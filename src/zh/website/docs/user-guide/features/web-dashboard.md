...
`web` 这一附加组件会引入 FastAPI/Uvicorn；而 `pty` 组件则会引入 `ptyprocess`（用于 POSIX 环境）或 `pywinpty`（用于原生 Windows 环境——需注意，内置的 TUI 本身仍需依赖 WSL）。若您同时需要消息传递、语音功能等，使用 `pip install hermes-agent[all]` 即可一次性安装所有附加组件，这是最简便的方式。

如果在运行 `hermes dashboard` 时缺少相关依赖，该程序会提示您需要安装哪些组件。如果前端尚未构建且系统中存在 `npm`，则首次启动时会自动进行构建。

每次启动 `hermes dashboard` 都会显示聊天标签页——通过 PTY/WebSocket 运行的内置浏览器聊天面板始终可用，无需任何额外参数。
...
- **活跃会话数**——过去 5 分钟内活跃的会话数量
- **最近会话列表**——显示最近 20 个会话，包括所用模型、消息数量、Token 使用情况以及对话预览内容

状态页面每 5 秒自动刷新一次。

### 聊天页面

**聊天**标签页会将完整的 Hermes TUI（与通过 `hermes --tui` 打开的界面相同）直接嵌入浏览器中。在终端 TUI 中能实现的所有功能——斜杠命令、模型选择器、工具调用卡片、Markdown 流式显示、澄清/授权/批准提示、主题定制等——在这里都能正常使用，因为仪表板实际运行的是真实的 TUI 可执行文件，并通过 [xterm.js](https://xtermjs.org/) 及其 WebGL 渲染器来呈现 ANSI 输出，从而实现像素级精确的界面布局。
...
- xterm.js 的 WebGL 渲染器会将每个界面单元绘制到整数像素网格上；鼠标追踪（SGR 1006 标准）、宽字符（Unicode 11）以及绘图符号都能实现原生渲染
- 调整浏览器窗口大小时，会通过 `@xterm/addon-fit` 插件相应地调整 TUI 的大小

**恢复现有会话：**在**会话**标签页中，点击任意会话旁边的播放图标（▶），即可跳转至 `/chat?resume=<id>` 地址，并以 `--resume` 参数启动 TUI，从而加载完整的对话历史记录。

**会话切换器（右侧栏）：**聊天标签页在终端旁设有一个细长的右侧栏，其中包含类似 ChatGPT 的对话列表，让您无需离开页面即可切换不同对话。该栏将模型选择器置于顶部，会话列表紧随其下；终端则占据屏幕大部分区域。列表显示当前活跃配置文件下的最近会话信息——包括标题（若无法显示则显示对话预览）、最后活跃时间、消息数量，以及非 CLI 会话的来源渠道。点击任意行即可在原处恢复该会话（终端会加载该对话的历史记录），当前活跃的会话会被高亮显示。**新建聊天**则会启动一个全新会话，同时刷新控件会重新加载会话列表。该右侧栏仅用于切换会话——删除、重命名、导出及批量清理操作仍需在**会话**标签页中进行。在屏幕较窄时，该栏会折叠为滑出式面板。

**前提条件：**

- Node.js（要求与 `hermes --tui` 相同；TUI 组件会在首次启动时自动构建）
- `ptyprocess`——由 `pty` 附加组件自动安装（可通过 `pip install 'hermes-agent[web,pty]'` 安装，或使用 `[all]` 一次性安装两者）
- POSIX 内核（Linux、macOS 或 WSL2）。/chat 终端面板需要 POSIX 伪终端——原生 Windows 版 Python 不具备此功能，因此在原生 Windows 环境下，虽然仪表板的其他功能（如会话管理、任务处理、指标展示、配置编辑器）仍可正常使用，但 /chat 标签页会显示提示，建议使用 WSL2 才能使用该功能。

关闭浏览器标签页后，服务器端的伪终端会被及时释放。重新打开浏览器则会启动一个全新会话。
...
Hermes Desktop 通常会启动自己的本地后端，但也可以通过**设置 → 网关 → 远程网关**选项连接到远程机器（如虚拟机、家庭实验室设备等）上运行的仪表板。这类问题往往是“桌面端显示后端已准备就绪，但聊天功能无法使用”的常见原因，因为桌面端的就绪检查所验证的内容远少于实际实时聊天连接所需的条件。

:::info 前提条件：远程主机上必须正在运行 `hermes dashboard` 
Desktop 所连接的“远程后端”实际上就是运行在远程机器上的 `hermes dashboard` 进程——也就是本文档所介绍的同一台服务器。在执行后续操作之前，该服务器必须处于运行状态且可访问；桌面端仅会连接到该服务器，而不会自动为其启动服务。建议通过 `systemd`/`tmux` 等工具保持该服务器持续运行，这样即便用户退出登录或重启电脑，服务也不会中断。而**网关**（Telegram/Discord/Slack 等）则是另一个独立的长期运行的进程——如果您依赖消息渠道功能，需单独启动该进程；桌面端应用并非连接到网关。
:::

桌面端的“远程后端已准备就绪”检测仅会访问 `GET /api/status` 这一公共接口——只要主机上运行有任何仪表板，该接口就会立即响应。而实时聊天连接则是通过另一个 WebSocket 连接到 `/api/ws`（以及 `/api/pty`）接口，这个连接还需要满足状态检测从未涉及的另外两项要求：

1. **必须完成身份验证。**当仪表板绑定到非回环地址时，会启用身份验证机制。可通过用户名和密码进行保护（可使用内置的[用户名/密码提供器](#usernamepassword-provider-no-oauth-idp)）；桌面端只需登录一次，即可通过一次性令牌为 WebSocket 连接复用该会话。如果没有配置相应的提供器，非回环地址的仪表板在启动时就会直接失败。
2. **绑定主机必须允许客户端连接，并且其 Host 头信息需匹配。**回环绑定（`127.0.0.1`）仅接受本地客户端连接，因此无论凭据如何，远程机器在套接字层面就会被拒绝。建议将仪表板绑定到非回环地址（`--host 0.0.0.0`），这样基于对等 IP 的验证机制就能允许远程客户端接入。在桌面端输入的远程地址，必须能够通过仪表板绑定的同一主机访问——DNS 重定向验证要求 Host 头信息必须一致。

#### 远程仪表板配置

设置用户名和密码后，将仪表板绑定到可访问的地址并运行它。对于基于 `systemd` 的服务：
...
- 当 `auth_required: false` 时，说明绑定地址为回环地址，或网关未启动。请尝试绑定非回环地址。  
- 当 `auth_required: true` 但 providers 列表中不存在 `"basic"` 时，表示用户名/密码环境变量未被加载。请先修复该问题。  
:::

如果 `/api/status` 显示网关已启用且使用了 `"basic"` 提供商，但桌面端在登录后仍无法连接，说明问题已超出基础配置范畴——请获取最新的 `desktop.log` 文件（路径：设置 → 网关 → 打开日志），并查看同一时段内控制台中的日志，重点查找 `/api/ws` 的返回码（4403 表示聊天 WS 被请求守卫拒绝，例如主机/对端不匹配；4401 表示 WS 令牌未通过身份验证）。

### 配置

提供基于表单的 `config.yaml` 编辑器。所有 150 多个配置字段都会从 `DEFAULT_CONFIG` 中自动检测出来，并按标签页分类展示：

![配置管理页面——左侧为分类筛选器，右侧为自动检测到的字段](/img/dashboard/admin-config.png)

- **模型** — 默认模型、提供商、基础 URL 以及推理设置  
- **终端** — 后端类型（本地/ Docker/ SSH/ 模态）、超时时间以及 Shell 预设选项  
- **显示** — 主题皮肤、工具进度显示、继续处理提示以及加载动画设置  
- **智能体** — 最大迭代次数、网关超时时间以及服务等级  
- **委托** — 子智能体数量限制以及推理耗力设置  
- **内存** — 提供商选择以及上下文注入设置  
- **审批** — 危险命令的审批模式（询问/自动通过/拒绝）  
- 其他更多配置——`config.yaml` 的每个部分都对应有相应的表单字段  

那些具有固定有效值的字段（如终端后端类型、主题皮肤、审批模式等）会以下拉菜单形式呈现；布尔值则表现为切换开关；其余字段均为文本输入框。  

**操作按钮：**  
- **保存** — 立即将更改写入 `config.yaml`  
- **恢复为默认值** — 将所有字段重置为默认值（需点击“保存”才会生效）  
- **导出** — 以 JSON 格式下载当前配置  
- **导入** — 上传 JSON 配置文件以替换现有设置  

:::提示  
配置更改会在下一次智能体会话或网关重启时生效。Web 控制台编辑的正是与 `hermes config set` 命令以及网关读取的文件相同的 `config.yaml`。  
:::

### API 密钥

用于管理存储 API 密钥和凭据的 `.env` 文件。这些密钥按类别分组：  
...
- **智能体设置** — 非敏感环境变量，如 `API_SERVER_ENABLED`  

每项密钥都会显示：  
- 当前是否已设置（会显示经过脱敏处理的值预览）  
- 该密钥的用途说明  
- 对应提供商的注册/密钥页面链接  
- 用于设置或更新值的输入框  
- 删除按钮  

高级或极少使用的密钥默认会被隐藏，需要通过切换开关来显示。  

### 会话

可浏览并查看所有智能体会话。每行信息包括会话标题、来源平台图标（CLI、Telegram、Discord、Slack、cron）、模型名称、消息数量、工具调用次数，以及该会话最后活跃的时间。正在运行的会话会显示为闪烁的徽章。  

- **搜索** — 使用 FTS5 对所有消息内容进行全文搜索。搜索结果会高亮显示相关片段，展开后自动滚动到第一个匹配的消息。  
- **统计信息** — 统计栏会显示总会话数、当前活跃的会话数、归档会话数、总消息数，以及按来源分类的详细数据。  
- **展开查看** — 点击某次会话即可查看其完整的消息历史记录。消息会根据发送方角色（用户、助手、系统、工具）以不同颜色标注，并以支持语法高亮的 Markdown 格式呈现。  
- **工具调用** — 包含工具调用的助手消息会以可折叠的块形式展示，其中包含函数名称及 JSON 参数。  
- **重命名** — 可直接在线设置或清除会话标题（使用铅笔图标）。  
- **导出** — 以 JSON 格式下载会话数据（包含元数据及完整消息历史）（使用下载图标）。  
- **清理旧会话** — “清理旧会话”按钮可用于删除超过 N 天未使用的已结束会话。  
- **删除** — 使用垃圾桶图标可移除某次会话及其所有消息历史记录。  

![会话管理页面——统计栏、清理功能以及每行对应的重命名/导出/删除按钮](/img/dashboard/admin-sessions.png)
...
- **显示行数** — 可选择要显示的行数（50、100、200 或 500 行）  
- **自动刷新** — 开启/关闭实时滚动功能，系统会每隔 5 秒检查一次是否有新日志行添加  
- **颜色编码** — 日志行会根据严重程度以不同颜色显示（错误为红色，警告为黄色，调试信息为浅色）  

### 分析统计

基于会话历史数据计算的使用量和成本分析结果。可选择时间范围（7 天、30 天或 90 天），查看以下内容：  

- **摘要卡片** — 总令牌数（输入/输出）、缓存命中率、预估或实际总成本，以及每日平均会话数  
- **每日令牌使用图表** — 堆叠条形图显示每日输入和输出令牌的使用情况，鼠标悬停时可查看详细分类及成本数据  
- **每日明细表** — 显示每天的日期、会话数、输入令牌数、输出令牌数、缓存命中率以及成本  
- **按模型分类的明细** — 表格形式展示所使用的每个模型、对应会话数、令牌使用量及预估成本  

### Cron 任务

可用于创建和管理定时执行的 Cron 作业，这些作业会按照预设的时间表触发智能体提示。  

- **创建** — 填写任务名称（可选）、提示内容、Cron 表达式（例如 `0 9 * * *`），以及消息发送目标（本地、Telegram、Discord、Slack 或电子邮件）  
- **任务列表** — 每个任务都会显示其名称、提示内容预览、调度表达式、状态徽章（启用/暂停/错误）、发送目标、上次运行时间以及下次运行时间  
- **暂停/继续** — 在启用和暂停状态之间切换任务  
- **编辑** — 打开预填好的模态窗口，用于修改任务的提示内容、调度时间、名称或发送目标  
- **立即触发** — 在非预定时间立即执行该任务  
- **删除** — 永久移除某个 Cron 作业  
...
- **配置文件卡片** — 每张卡片都会显示对应的模型/提供商、技能数量、网关状态、描述以及各类徽章（活跃、默认、别名）  
- **创建** — 需要输入名称，可选择是否从默认配置克隆、是否复制所有内容、是否不包含任何预装技能，还需填写描述和模型信息；专门的配置文件创建页面（`/profiles/new`）提供了完整的配置流程（包括模型、MCP 服务及技能的设置）  
- **管理技能与工具** — 可跳转到针对该配置文件的技能管理页面（会自动切换侧边栏中的配置文件选择器）  
- **设为当前活跃配置** — 可更改默认的活跃配置，使得**后续的 CLI/网关运行**都使用该配置（功能与 `hermes profile use` 相同）。此操作不会改变控制台管理的配置——那是由配置文件选择器负责的  
- **编辑模型/描述/SOUL** — 提供内联编辑器，可直接修改该配置文件的内容  
- **重命名/删除** — 仅适用于已命名的配置文件  

### 技能

可浏览、搜索及切换已安装的技能和工具集，同时还能从技能中心安装新技能。这些技能均从 `~/.hermes/skills/` 目录中加载，并按类别分组展示。  

- **搜索** — 可通过名称、描述或类别来筛选已安装的技能和工具集  
- **类别筛选** — 点击类别标签可缩小搜索范围（例如 MLOps、MCP、红队测试、AI 等）  
- **切换开关** — 通过开关按钮启用或禁用单个技能，更改会在下一次会话时生效  
- **工具集** — 另一个视图用于展示内置的工具集（如文件操作、网页浏览等），显示其启用/禁用状态、设置要求以及所含工具列表  
- **浏览技能中心** — 第三个视图可在所有来源中搜索技能中心的内容（功能与 `hermes skills search` 相同），可通过标识符一键安装任意技能，并实时显示安装日志；还提供“更新全部”按钮，用于刷新已安装的技能。  

![技能管理页面——浏览技能中心视图：搜索、安装及更新功能](/img/dashboard/admin-skills-hub.png)

### MCP 服务

无需使用 CLI 即可管理[MCP](/integrations/mcp)服务器。相关配置与 `hermes mcp` 命令所读取的 `config.yaml` 文件中的 `mcp_servers` 块内容相同。  

**您的 MCP 服务器：**  
- **添加** — 可注册 HTTP/SSE 服务器（提供 URL）或标准输入/输出服务器（指定命令及参数），对于标准输入/输出服务器还可设置可选的 `KEY=VALUE` 形式的环境变量  
- **启用/禁用** — 可在不删除服务器的情况下切换其开启或关闭状态。被禁用的服务器仍会保留在配置文件中，以便日后重新启用；更改会在下一次网关重启时生效  
- **测试** — 可连接到服务器，列出其提供的工具，然后断开连接——此功能可在智能体实际依赖该服务器之前验证连接是否正常  
- **删除** — 从配置文件中移除某个服务器  
- 在列表视图中，包含敏感信息的环境变量值会进行脱敏处理  

**目录列表**：可浏览经过 Nous 审核通过的 MCP 服务器（即预装的 `optional-mcps/` 目录中的服务器），并一键安装任意服务器。需要 API 密钥的服务器会在界面中直接提示用户输入密钥，这些密钥会被保存到 `.env` 文件中。此目录与 `hermes mcp catalog` / `hermes mcp install` 命令使用的目录相同。  

![MCP 管理页面——显示您的服务器及其启用/禁用开关，以及安装目录列表](/img/dashboard/admin-mcp.png)

### Webhook 订阅

用于管理动态[Webhook 订阅](/user-guide/messaging/webhooks)。首先必须在消息设置中启用 Webhook 功能，否则页面会给出相应提示。  

- **创建** — 需要输入名称、描述、事件筛选条件、消息发送目标，可选择是否使用直接发送模式，还需输入智能体提示内容。创建完成后，页面会显示对应的路由 URL 以及一次性使用的 HMAC 密钥，方便用户复制  
- **启用/禁用** — 可切换订阅状态的开启或关闭。被禁用的路由虽然仍保留在订阅文件中，但网关会拒绝接收来自这些路由的请求（返回 403 错误）。网关会自动热加载配置文件，因此更改会在下一个事件到达时生效——无需重启网关  
- **列表** — 每个订阅项都会显示其 URL、支持的事件类型以及消息发送目标  
- **删除** — 可移除某个订阅项  

![Webhook 管理页面——显示所有订阅项及其启用/禁用开关](/img/dashboard/admin-webhooks.png)

### 配对功能

无需使用 CLI 即可批准或撤销消息用户的配对权限——远程管理员可通过此功能将 Telegram/Discord 等平台的用户添加到已配对的网关中。该功能与 `hermes pairing` 完全兼容。  

- **待处理请求** — 每条记录都会显示平台类型、配对码、用户信息以及该用户距离上次配对的时间，旁边还有“批准”按钮  
- **已批准用户** — 每条记录会显示平台类型和用户信息，旁边有“撤销”按钮  
