...
针对“智能体保存了关于我的错误假设”这一问题的解决方案是：将 `write_approval: true` 设为值，这样一来，每次数据保存——尤其是那些在未收到提示时自动进行的后台保存——都必须在获得您的确认（是/否）之后才会被写入您的个人资料中。

## 后台审核通知（`display.memory_notifications`）

在每一轮对话结束后，智能体的后台自我优化功能可能会悄悄地保存某条记忆或更新某个技能。默认情况下，它会在聊天界面中显示简短的“💾 记忆已更新”提示，以便您知晓该操作已执行。您可以自行控制此类通知的详细程度：
