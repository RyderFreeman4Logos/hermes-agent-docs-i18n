...
针对“智能体保存了关于我的错误假设”这一问题，解决方案是设置 `write_approval: true`。这样一来，每次数据保存——尤其是那些在未收到用户提示时自动进行的后台操作——都必须在获得您的确认（是/否）之后，才会被写入您的个人资料中。

## 后台审核通知（`display.memory_notifications`）

在每个对话轮次结束后，智能体可能会在后台默默地保存某些记忆信息或更新相关技能。这就是 Hermes 所采用的“基于用户同意的学习循环”：通过反复的修正与经验积累，这些知识会被整合为简洁的记忆条目或流程化技能；而 `write_approval` 功能则允许在这些更改影响后续对话之前，先将其暂存以供审核。默认情况下，系统会在聊天界面显示简短的 “💾 记忆已更新” 提示，让您知晓该操作已完成。您也可以自行调整此类提示的显示频率。
...
| 值 | 行为 |
|-------|-----------|
| `off` | 不发送聊天通知。审查过程仍会执行并记录结果——只是您不会看到相关提示。 |
| `on`（默认值） | 显示通用提示，例如 `💾 内存已更新`、`💾 技能 ‘foo’ 已修复`。 |
| `verbose` | 显示变更内容的简要预览，例如 `💾 内存 ➕ 用户偏好简短回复`，或显示技能版本的 “旧” → “新” 对比片段。 |
...
默认情况下，审查会在您的**主聊天模型**上执行，通过重放对话来进行——由于对话已存储在提示词缓存中，因此只需进行低成本的缓存读取。如果您使用的是成本较高的主模型，也可以选择在成本更低的模型上运行审查：
...
当您将代理指向与主模型**不同**的模型时，审查过程会在该模型上执行，且成本会大幅降低（在基准测试中约为原来的1/3至1/5）。由于不同的模型无法复用主模型的提示词缓存，因此该分支会自动重放对话的精简**摘要**——即最新轮次的完整内容以及之前轮次的总结——而非完整的对话记录，从而最大限度地减少写入新缓存的数据量。在测试中，内存捕获结果与使用主模型时的完全一致，技能捕获结果也几乎相同。

若将其保持为`auto`状态（或设置为您的主模型），则一切照旧——审查过程将继续在主模型上运行，并重放完整的预热缓存内容。

## 控制技能写入功能（`skills.write_approval`）

技能功能同样采用相同的开启/关闭机制，但由于`SKILL.md`文件体积过大，无法在聊天窗口中完整显示，因此其审查用户界面有所不同：
