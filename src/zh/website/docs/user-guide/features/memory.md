...
| 值 | 行为 |
|-------|-----------|
| `off` | 不显示聊天通知。审核过程仍会进行，相关记录也会被写入——只是你不会看到对应的消息行。 |
| `on`（默认值） | 显示通用消息，例如 `💾 内存已更新`、`💾 技能 ‘foo’ 已修复`。 |
| `verbose` | 显示变更内容的简要预览，例如 `💾 内存 ➕ 用户偏好简洁回复`，或显示技能内容从“旧版本”到“新版本”的差异片段。 |

> 此设置仅适用于**网关**的聊天通知功能。审核过程本身以及向内存/技能存储区的写入操作均不受此设置影响。可通过 `display.platforms.<platform>.memory_notifications` 按平台单独配置该设置。

## 控制技能写入（`skills.write_approval`）

技能同样遵循相同的开启/关闭机制，但由于 `SKILL.md` 文件体积过大，无法在聊天窗口中完整显示，因此其审核用户体验有所不同：
