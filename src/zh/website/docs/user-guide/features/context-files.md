...
`SOUL.md` 用于控制智能体的性格特点、语气以及交流风格。详细信息请参阅 [性格设置](/user-guide/features/personality) 页面。
...
- Hermes 不会在工作目录中查找 `SOUL.md` 文件
- 若该文件为空，则不会将其中的任何内容添加到提示词中
- 若文件包含内容，系统会在扫描并截断后原样将其插入提示词中

## .cursorrules

Hermes 兼容 Cursor IDE 的 `.cursorrules` 文件以及 `.cursor/rules/*.mdc` 规则模块。如果这些文件位于项目根目录中，且未找到优先级更高的上下文文件（如 `.hermes.md`、`AGENTS.md` 或 `CLAUDE.md`），则会将它们作为项目上下文加载。

这意味着在使用 Hermes 时，您现有的 Cursor 约定会自动生效。
...
1. **扫描工作目录** — 按顺序查找 `.hermes.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`（首次匹配成功）
2. **读取内容** — 每个文件均以 UTF-8 编码格式读取
3. **安全扫描** — 检查内容中是否存在提示词注入风险
4. **截断处理** — 对长度超过 `context_file_max_chars` 字符限制（默认为 20,000 字符）的文件，会对其头部和尾部进行截断（头部保留 70%，尾部保留 20%，中间添加标记）
5. **整合内容** — 将所有处理后的部分汇总在 `# Project Context` 标题下
6. **插入提示词** — 将整合好的内容添加到系统提示词中

### 会话期间（动态发现）

`agent/subdirectory_hints.py` 中的 `SubdirectoryHintTracker` 会监控工具调用参数中的文件路径：

1. **提取路径** — 每次工具调用后，从参数（如 `path`、`workdir` 或 shell 命令）中提取文件路径
2. **遍历上级目录** — 依次检查当前目录及最多 5 个上级目录（已访问过的目录不再重复检查）
3. **加载对应文件** — 若找到 `AGENTS.md`、`CLAUDE.md` 或 `.cursorrules` 文件，即将其加载（每个目录中取首次匹配的文件）
4. **安全扫描** — 执行与启动时相同的提示词注入检测
5. **截断处理** — 每个文件的长度上限为 8,000 字符
6. **插入结果** — 将处理后的内容附加到工具响应中，从而使模型能自然地在上下文中看到这些信息
...
:::warning
该扫描工具可防范常见的注入攻击模式，但无法替代对共享代码库中上下文文件的仔细检查。对于非您创建的项目，请务必验证其中的 AGENTS.md 文件内容。
:::
...
| 限制项 | 数值 |
|-------|------|
| 每个文件的最大字符数 | `context_file_max_chars`（默认为 20,000，约 7,000 个标记） |
| 开头部分截断比例 | 70% |
| 结尾部分截断比例 | 20% |
...
5. **列明关键路径与端口**——智能体会依赖这些信息来执行终端命令
6. **随着项目发展及时更新**——过时的上下文信息比没有上下文更糟糕
:::
