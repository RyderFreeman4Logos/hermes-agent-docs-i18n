...
- 用户需要共享上下文存储空间及令牌成本
- 某人执行的长时间、高资源消耗的任务可能会占用其他所有人的上下文空间
...
- Alice 中断自己的正在运行请求时，仅会影响该频道中她的会话
- Bob 可以在同一频道继续对话，而不会继承 Alice 的历史记录或被其操作中断

当设置为 `group_sessions_per_user: false` 时：

- 整个房间会共享该频道/线程的同一个运行智能体名额
- 不同用户发送的后续消息可能会互相干扰，或排队等待处理

本指南将带你完成从在 Discord 开发者门户创建机器人到发送第一条消息的完整设置流程。

## 第1步：创建 Discord 应用程序

1. 访问 [Discord 开发者门户](https://discord.com/developers/applications)，使用你的 Discord 账户登录。
2. 点击右上角的 **New Application**。
3. 为你的应用程序输入名称（例如“Hermes Agent”），并同意开发者服务条款。
4. 点击 **Create**。

你将进入 **General Information** 页面。请记下 **Application ID**——稍后生成邀请链接时需要用到它。

## 第2步：创建机器人

1. 在左侧边栏中点击 **Bot**。
2. Discord 会自动为你的应用程序创建一个机器人用户。你会看到机器人的用户名，你可以对其进行自定义。
...
:::提示
你可以在该页面为机器人设置自定义头像和横幅。这些内容就是用户在 Discord 中看到的样子。
:::

:::信息[私有机器人的替代方案]
如果你希望让机器人保持私有状态（即 Public Bot 设为 OFF），则必须在第5步中使用 **Manual URL** 方法，而非“安装”选项卡。因为 Discord 提供的链接要求必须开启 Public Bot 功能。
:::

## 第3步：启用特权网关意图

这是整个设置过程中最关键的一步。如果未正确启用相应意图，你的机器人虽然能连接到 Discord，但**将无法读取消息内容**。

在 **Bot** 页面中，向下滚动至 **Privileged Gateway Intents**。你会看到三个开关：
...
- 如果未启用 **Message Content Intent**，机器人虽然能收到消息事件，但消息文本字段为空——机器人实际上无法看到你输入的内容。
- 如果未启用 **Server Members Intent**，机器人将无法解析允许通信用户的用户名，可能无法识别是谁在与其发送消息。

:::警告[这是 Discord 机器人无法正常工作的首要原因]
如果你的机器人已上线却从不回复消息，那几乎可以肯定是因为 **Message Content Intent** 未被启用。请返回 [开发者门户](https://discord.com/developers/applications)，选择你的应用程序 → Bot → Privileged Gateway Intents，确保 **Message Content Intent** 已切换为 ON 状态，然后点击 **Save Changes**。
:::

**关于服务器数量的限制：**
- 如果你的机器人仅在**100个服务器以下**运行，可以自由地开启或关闭这些意图。
- 如果机器人已在**100个或更多服务器**中运行，Discord 要求你提交验证申请才能使用特权意图。对于个人使用而言，这通常不是问题。

点击页面底部的 **Save Changes**。

## 第4步：获取机器人令牌

机器人令牌是 Hermes Agent 用于以你的机器人身份登录的凭证。仍在 **Bot** 页面上：

1. 在 **Token** 区域下方，点击 **Reset Token**。
2. 如果你的 Discord 账户启用了双重认证，请输入相应的 2FA 码。
3. Discord 会显示新的令牌。**请立即复制它。**

:::警告[令牌仅显示一次]
该令牌只会显示一次。一旦丢失，你需要重新生成新的令牌。切勿公开分享你的令牌，也不要将其提交到 Git 中——拥有该令牌的人就可以完全控制你的机器人。
:::

请将令牌保存在安全的地方（例如密码管理器中）。第8步时需要用到它。

## 第5步：生成邀请链接

你需要一个 OAuth2 链接，以便将机器人邀请到你的服务器。有两种实现方式：

### 方案A：使用“安装”选项卡（推荐）

:::注意[需要开启公共机器人]
此方法要求在第2步中将 **Public Bot** 设置为 **ON**。如果你将其设置为 OFF，请改用下面的手动 URL 方法。
:::
...
请将 `YOUR_APP_ID` 替换为步骤1中获取的应用程序ID。
...
| 推荐配置 | `309237763136` | 上述所有权限外加嵌入链接、在主题串中发送消息、添加反应、创建公开主题串 |
...
:::info
要在Discord服务器中邀请机器人，您需要拥有**管理服务器**权限。如果下拉菜单中没有显示您的服务器，请让服务器管理员使用邀请链接。
:::

授权完成后，机器人将出现在您服务器的成员列表中（在启动Hermes网关之前，它将显示为离线状态）。

## 第7步：查找您的Discord用户ID

Hermes Agent会利用您的Discord用户ID来控制谁可以与机器人交互。具体操作如下：

1. 打开Discord（桌面版或网页版）。
2. 进入**设置** → **高级设置**，将**开发者模式**切换为**开启**状态。
3. 关闭设置窗口。
4. 右键点击您的用户名（可在消息、成员列表或个人资料中找到）→ **复制用户ID**。

您的用户ID是一个长数字，例如 `284102345871466496`。

:::tip
开发者模式还允许您以相同方式复制**频道ID**和**服务器ID**——右键点击频道或服务器名称，然后选择“复制ID”。如果您想手动设置主频道，就需要频道ID。
:::
...
- 属于某个范围（私信或服务器频道）的 `allow_admin_from` 列表中的用户，可以通过实时命令注册表运行**所有**已注册的斜杠命令——包括内置命令和插件注册的命令。
- 不在 `allow_admin_from` 列表中的用户，仅能运行 `user_allowed_commands` 中列出的命令，以及始终允许使用的命令 `/help` 和 `/whoami`。
- 普通聊天内容（非斜杠命令消息）不受影响。非管理员用户仍可正常与智能体交流，只是无法触发任意命令。
- **向后兼容性**：如果某个范围未设置 `allow_admin_from`，则该范围的斜杠命令限制功能将被禁用。现有系统无需更改即可继续正常运行。
- 私信中的管理员身份并不等同于服务器频道中的管理员身份。每个范围都有独立的管理员列表。

可使用 `/whoami` 命令查看当前所处的作用范围、您的权限等级（管理员/普通用户/无限制），以及您可以运行的斜杠命令。

### 仅限管理员使用执行审批按钮

默认情况下，任何被允许与智能体交流的用户——包括通过 `hermes pairing approve` 配对成功的用户——都可以点击危险命令提示框中的“批准”/“拒绝”按钮。这是与普通聊天访问控制一致的默认行为。若要仅限制管理员才能批准危险命令，请在 Discord 平台的 `extra` 块中设置 `require_admin_for_exec_approval`：
...
- **默认关闭**——执行审批按钮仍属于用户权限范围，任何已通过身份验证的用户均可进行审批。现有安装不会受到影响。
- **开启时**——操作者必须同时通过常规身份验证检查，并且其名称需列在 `allow_admin_from` 中（该键与斜杠命令分离机制所使用的键相同）。风险较低的组件视图（模型选择器、澄清请求、提示词更新功能）仍属于用户权限范围。
- **配置错误时自动关闭**——即使该开关处于开启状态，但如果 `allow_admin_from` 为空，则*任何人*都无法进行审批，系统还会记录警告信息，这样就能及时发现配置错误，而不会导致用户被悄悄限制访问。

## 交互式模型选择器

在 Discord 频道中发送不带参数的 `/model` 命令，即可打开基于下拉菜单的模型选择器：

1. **提供方选择**——一个下拉列表，显示可用的提供方（最多25个）。
2. **模型选择**——另一个下拉列表，展示所选提供方对应的模型（最多25个）。

该选择器在120秒后超时。只有经过授权的用户（即列入 `DISCORD_ALLOWED_USERS` 的用户）才能使用它。如果您已知模型名称，可直接输入 `/model <名称>`。

## 技能的原生斜杠命令

Hermes 会自动将已安装的技能注册为**Discord 原生应用命令**。这意味着这些技能会与内置命令一同出现在 Discord 的自动补全 `/` 菜单中。

- 每个技能都会对应一个 Discord 斜杠命令（例如 `/code-review`、 `/ascii-art`）。
- 这些技能可接受可选的 `args` 字符串参数。
- Discord 对每个机器人的应用命令数量设有100个的限制——如果您的技能数量超过此限制，多余的技能将会被跳过，并在日志中留下警告信息。
- 技能会在机器人启动时与 `/model`、 `/reset` 和 `/background` 等内置命令一同被注册。

无需额外配置——通过 `hermes skills install` 安装的任何技能，都将在下一次网关重启时自动注册为 Discord 斜杠命令。
...
**备注：**  
- 确认音效最多每轮触发一次，且仅当机器人处于语音频道且混音器处于激活状态时才会播放。该功能会使用您配置的文本转语音服务提供商。  
- `ambient_path` 可接受任何 `ffmpeg` 能够解码的文件，系统会对其进行无缝循环播放。若留空，则会使用内置的合成背景音（无需额外资源）。  
- 所有设置均保存在 `config.yaml` 文件中（而非 `.env`），这些为行为配置而非敏感信息。  
- 当 `voice_fx.enabled` 设定为 `false` 时，语音播放将直接使用原始的单次播放路径，其余设置不会发生变化。  

## 论坛频道  

Discord 论坛频道（类型 15）不支持直接消息发送——论坛中的每条内容都必须以主题帖形式存在。Hermes 会自动检测论坛频道，每当需要向其中发送内容时都会创建一个新的主题帖，因此文本回复、文本转语音、图片、语音消息以及文件附件均无需机器人进行特殊处理即可正常使用。  

- **主题帖名称**源自消息的第一行内容（去掉 Markdown 标题前缀后，长度限制为 100 字符）。如果消息仅包含附件，则以附件的文件名为默认主题帖名称。  
- **附件**会随新创建的主题帖一同发送——无需单独上传步骤，也不会出现部分发送的情况。  
- **一次发送，一个主题帖**：每次向论坛发送内容都会生成一个新的主题帖。因此，对同一论坛的后续发送将会产生多个独立的主题帖。  
- **三层检测机制**：首先查询频道目录缓存，其次检查进程本地的探测缓存，最后作为最后手段通过实时请求 `GET /channels/{id}` 进行检测（该检测结果会被缓存，整个进程运行期间有效）。  

通过刷新目录（在支持该功能的平台上使用 `/channels refresh` 命令，或重启网关），即可将机器人启动后新建的论坛频道信息添加到缓存中。  
...
**解决方法**：前往 [开发者门户](https://discord.com/developers/applications) → 选择您的应用 → 进入“机器人”选项卡 → “特权网关意图” → 启用 **消息内容意图** → 点击“保存更改”。随后重启网关。  

### 启动时出现“不允许的意图”错误  

**原因**：您的代码请求了在开发者门户中未被启用的意图。  

**解决方法**：在机器人设置中启用全部三种特权网关意图（在线状态、服务器成员、消息内容），然后重启机器人。  

### 机器人无法查看特定频道中的消息  

**原因**：机器人的角色没有该频道的查看权限。  

**解决方法**：在 Discord 中进入该频道的设置 → “权限”选项，为机器人的角色添加权限，确保已勾选 **查看频道** 和 **读取消息历史记录**。  

### 出现 403 禁止访问错误  

**原因**：机器人缺少必要的权限。  

**解决方法**：使用第 5 步中的链接重新邀请机器人，并授予正确的权限；或者直接在“服务器设置” → “角色”中手动调整机器人的角色权限。  

### 机器人处于离线状态  

**原因**：Hermes 网关未运行，或令牌不正确。  

**解决方法**：检查 `hermes gateway` 是否正在运行。确认 `.env` 文件中的 `DISCORD_BOT_TOKEN` 值是否正确。如果您最近重置了令牌，请及时更新。  

### 出现“用户未被允许”/机器人忽略您的情况  

**原因**：您的用户 ID 不在 `DISCORD_ALLOWED_USERS` 列表中。  

**解决方法**：在 `~/.hermes/.env` 文件中将您的用户 ID 添加到 `DISCORD_ALLOWED_USERS` 中，然后重启网关。  

### 同一频道中的用户意外共享上下文信息  

**原因**：`group_sessions_per_user` 功能未被启用，或相关平台无法为该上下文中的消息提供用户 ID。  

**解决方法**：在 `~/.hermes/config.yaml` 中设置该参数，然后重启网关即可。
