...
**备注：**  
- 确认音效最多每轮触发一次，且仅当机器人处于语音频道且混音器处于启用状态时才会播放。该功能会使用您配置的文本转语音服务提供商。  
- `ambient_path` 可接受任何 `ffmpeg` 能够解码的文件格式，系统会对其进行无缝循环播放。若留空，则会使用内置的合成背景音（无需额外资源）。  
- 所有设置均保存在 `config.yaml` 文件中（而非 `.env`），这些属于行为配置而非敏感信息。  
- 当 `voice_fx.enabled` 设置为 `false` 时，语音播放将直接使用原始的单次播放路径，其余设置不会发生变化。  

## 论坛频道  

Discord 论坛频道（类型 15）不支持直接发送消息——所有帖子都必须以主题帖的形式存在。Hermes 会自动检测论坛频道，每当需要向其中发送内容时都会创建一个新的主题帖，因此无需对机器人进行特殊处理，`send_message`、文本转语音、图片、语音消息及文件附件均可正常使用。  

- **主题帖名称**源自消息的第一行内容（去除 Markdown 标题前缀后，长度限制为 100 字符）。如果消息仅包含附件，则以附件文件名为默认主题帖名称。  
- **附件**会随新创建的主题帖一同发送——无需单独上传，也不会出现部分发送的情况。  
- **一次发送，一个主题帖**：每次向论坛发送消息都会生成一个新的主题帖。因此，对同一论坛的多次发送将会产生多个独立的主题帖。  
- **三层检测机制**：首先检查频道目录缓存，其次是进程本地的探测缓存，最后作为兜底方案通过实时请求 `GET /channels/{id}` 进行检测（该检测结果会被缓存，整个进程运行期间有效）。  

若刷新频道目录（在支持该功能的平台上使用 `/channels refresh` 命令，或重启网关），即可将机器人启动后新增的论坛频道信息同步到缓存中。  
...
**解决方法**：在机器人设置中启用全部三种特权网关意图（在线状态、服务器成员、消息内容），之后重启机器人。  
...
**原因**：`group_sessions_per_user` 功能被禁用，或相关平台无法为该上下文中的消息提供用户 ID。  
