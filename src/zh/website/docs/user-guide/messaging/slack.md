...
4. 直接跳至**步骤 6：将应用安装到工作空间**。该清单已自动处理了权限范围、事件和斜杠命令的配置。
...
随后您将进入应用的**基本信息**页面，接着按照以下步骤 2–6 操作即可。

---

## 步骤 2：配置机器人令牌的权限范围

在侧边栏中导航至**功能 → OAuth 和权限**。向下滚动到**权限范围 → 机器人令牌权限范围**，并添加以下项：

| 权限范围 | 用途 |
|---------|------|
| `chat:write` | 以机器人身份发送消息 |
| `app_mentions:read` | 检测在频道中被@提及的情况 |
| `channels:history` | 阅读机器人所在公共频道的消息 |
| `channels:read` | 列出并获取公共频道的信息 |
| `groups:history` | 阅读机器人被邀请加入的私密频道的消息 |
| `im:history` | 阅读直接消息的历史记录 |
| `im:read` | 查看基本的直接消息信息 |
| `im:write` | 打开并管理直接消息 |
| `users:read` | 查询用户信息 |
| `files:read` | 阅读并下载附件，包括语音笔记/音频文件 |
| `files:write` | 上传文件（图片、音频、文档） |

:::注意 缺少权限范围 = 缺少相应功能
如果没有 `channels:history` 和 `groups:history`，机器人**将无法接收频道中的消息**——它仅能在直接消息中运行。若没有 `files:read`，Hermes 虽可进行聊天，但**无法可靠地读取用户上传的附件**。这些是最常被遗漏的权限范围。
:::
...
| `message.channels` | **是** | 机器人接收其被添加到的**公共**频道的消息 |
| `message.groups` | **推荐** | 机器人接收其被邀请加入的**私密**频道的消息 |
| `app_mention` | **是** | 当机器人被@提及时，可避免 Bolt SDK 出现错误 |

4. 点击页面底部的**保存更改**

:::危险 缺少事件订阅是最常见的配置问题
如果机器人能在直接消息中正常工作，但**在频道中无法发送消息**，那几乎可以肯定是因为您忘了添加 `message.channels`（针对公共频道）和/或 `message.groups`（针对私密频道）。如果没有这些事件，Slack 根本不会将频道中的消息传递给机器人。
:::
...
2. 点击**安装到工作空间**
3. 仔细检查权限设置后点击**允许**
4. 经过授权后，您将看到一个以 `xoxb-` 开头的**机器人用户 OAuth 令牌**
...
成员 ID 的格式类似 `U01ABC2DEF3`。您至少需要自己的成员 ID。
...
在那些 Slack 的默认“机器人会记住该对话线程”功能令用户感到困惑的繁忙工作空间中，可将此选项设置为 `true`——例如，在一个漫长的技术支持对话线程中，机器人最初提供了帮助，而您希望它在未被再次主动提及时保持沉默。私信及正在进行的互动会话则不受影响。  
:::

:::信息  
Slack 支持这两种模式：默认情况下需要使用 `@mention` 才能开始对话，但您也可以通过 `SLACK_FREE_RESPONSE_CHANNELS`（以逗号分隔的频道 ID）或 `config.yaml` 中的 `slack.free_response_channels` 来指定无需回复的特定频道。一旦机器人在某个对话线程中建立了活跃会话，后续的回复就无需再使用提及方式。在私信中，机器人始终会在无需被提及的情况下作出回应。  
:::

### 频道白名单（`allowed_channels`）  
将机器人的响应范围限制在特定的 Slack 频道内——当机器人被邀请加入众多频道，但仅需在少数几个频道中回应时，此功能非常有用。一旦设置了此选项，来自列表之外频道的消息将会**被直接忽略**，即便提到了该机器人也是如此。  
**私信不受此过滤规则限制**，因此授权用户始终可以通过私信与机器人取得联系。
...
**备注：**  
- 绑定是通过频道 ID 进行匹配的。对于已绑定频道中的线程消息，该线程会继承父频道的绑定设置。  
- 技能仅在会话开始时（新建会话或自动重置后）加载。若需更改绑定设置，需运行 `/new` 命令或等待会话自动重置，更改才能生效。  
- 可结合 `channel_prompts` 使用，以便在技能指令的基础上为每个频道设置特定的语气或限制条件。  
...
| 机器人在私信中能响应，但在频道中无法响应 | **最常见的问题。** 需在事件订阅中添加 `message.channels` 和 `message.groups`，重新安装应用，然后使用 `/invite @Hermes Agent` 将机器人邀请至该频道 |
| 机器人不回复频道中的@提及消息 | 1) 确认已订阅 `message.channels` 事件。2) 机器人必须已被邀请进入该频道。3) 确保已添加 `channels:history` 权限范围。4) 更改权限范围或事件后需重新安装应用 |
| 机器人忽略私密频道中的消息 | 需同时订阅 `message.groups` 事件并添加 `groups:history` 权限范围，之后重新安装应用并使用 `/invite` 命令邀请机器人 |
| 在私信中出现“已禁用向此应用发送消息”的提示 | 在应用主页设置中启用**消息标签页**（参见第5步） |
| 出现“not_authed”或“invalid_auth”错误 | 重新生成机器人令牌和应用令牌，并更新 `.env` 文件 |
| 机器人有响应，但无法在频道中发布内容 | 使用 `/invite @Hermes Agent` 命令将机器人邀请至该频道 |
| 机器人可以聊天，但无法读取上传的图片/文件 | 需添加 `files:read` 权限，然后**重新安装**应用。当 Slack 返回权限/认证失败时，Hermes 现已在聊天界面中显示附件访问相关的诊断信息 |
| 出现 `missing_scope` 错误 | 在 OAuth 和权限设置中添加所需的权限范围，然后**重新安装**应用 |
| Socket 连接频繁断开 | 检查网络连接；虽然 Bolt 会自动重连，但不稳定的连接会导致延迟 |
| 更改了权限范围或事件后仍无变化 | 每次更改权限范围或事件订阅后，都必须将应用**重新安装**到工作空间中 |

### 快速检查清单

如果机器人无法在频道中正常工作，请确认以下**所有**项目：  

1. ✅ 已订阅 `message.channels` 事件（适用于公共频道）  
2. ✅ 已订阅 `message.groups` 事件（适用于私密频道）  
3. ✅ 已订阅 `app_mention` 事件  
4. ✅ 公共频道已添加 `channels:history` 权限范围  
5. ✅ 私密频道已添加 `groups:history` 权限范围  
6. ✅ 添加权限范围或事件后已**重新安装**应用  
7. ✅ 已使用 `/invite @Hermes Agent` 命令将机器人邀请至该频道  
8. ✅ 消息中已对机器人进行**@提及**  
...
:::warning
**务必设置 `SLACK_ALLOWED_USERS`**，并填入授权用户的成员 ID。出于安全考虑，若未设置此参数，网关将默认**拒绝所有消息**。切勿分享您的机器人令牌——应将其视同密码对待。
:::
