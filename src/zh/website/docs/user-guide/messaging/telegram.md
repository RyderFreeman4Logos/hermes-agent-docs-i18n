...
## 第 3 步：隐私模式（群组中使用时极为重要）

Telegram 机器人默认启用**隐私模式**。这正是人们在群组中使用机器人时最容易产生困惑的根源。
...
:::warning
更改隐私设置后，**必须将机器人从群组中移除后再重新添加**。Telegram 会缓存机器人加入群组时的隐私状态，只有将其移除并重新添加后，该状态才会更新。
:::

:::tip
另一种无需关闭隐私模式的方法是将机器人提升为**群组管理员**。无论隐私模式如何设置，管理员机器人始终能接收所有消息，这样就不必手动切换全局隐私模式了。
:::

### 查看群组聊天内容而不自动回复

对于类似 OpenClaw/Yuanbao 的群组行为，可配置 Telegram 使机器人能够**查看**普通群组消息，但仅在**被直接触发时**才进行回复：
...
**使用 `edit`（默认值）在私信中显示的效果**——网关会先发送常规的预览消息，随后通过 `editMessageText` 功能逐步更新该消息，从而避免出现 Telegram 的草稿预览折叠或回退问题。

**使用 `auto` 或 `draft` 在私信中显示的效果**——Telegram 会展示一个逐字符更新的动画式草稿预览。当回复完成后，它会以普通消息的形式发送，且客户端上的草稿预览会自动消失。由于草稿没有消息 ID，因此最终的内容才会保留在聊天记录中。

**群组、超级群组及论坛主题呢？** Telegram 仅允许在私信中使用 `sendMessageDraft` 功能。对于其他类型的聊天场景，网关会自动采用基于编辑的方案，用户体验与之前一致。

**如果草稿预览出现故障会怎样？** 无论是因为短暂的网络错误、服务器端拒绝响应，还是 Python-telegram-bot 版本过旧等原因导致故障，系统都会将后续的回复切换回基于编辑的方案。下一次回复将会重新尝试发送。

## 渲染：富文本消息、表格及链接预览

**富文本消息（Bot API 10.1）**——最终回复会通过 Telegram 原生的 [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage) 函数发送，同时使用代理的**原始 Markdown 格式**。这样一来，表格、任务列表、标题、嵌套引文、可折叠的 `<details>` 区块、脚注/参考文献、数学公式、下划线、上标/下标、标记文本以及链接都能以原生格式呈现，无需经过客户端端的简化处理。在私信中，实时流式预览也会使用 `sendRichMessageDraft`，因此动画式草稿预览与最终的富文本消息效果一致。

当内容超过 32,768 字节的富文本限制时，系统会自动跳过富文本渲染路径；而如果遇到 Telegram 的拒绝响应（如旧版 Python-telegram-bot 不支持的接口、解析错误、块或列尺寸过大等），系统也会**无缝切换**到 MarkdownV2 渲染路径——这样您的消息就不会丢失。不过，短暂的网络错误不会被自动重发（因此不会出现重复的最终消息）。

**MarkdownV2 作为备用方案**——当某条消息无法使用富文本渲染路径时，Hermes 会将 Markdown 格式转换为 MarkdownV2。由于 MarkdownV2 不支持原生表格语法，因此管道式表格会被进行标准化处理：

- **小型表格**会转换为**行组项目符号列表**——每行内容都会变成列标题下的可读项目符号列表，适用于包含 2–4 列且单元格内容较短的表格。
- **较大或较宽的表格**则会转换为带有对齐列的**代码块格式**，从而避免内容折叠。

对于这种 API 备用方案，无需进行任何配置——适配器会自动为每条消息选择最合适的渲染方式。如果某个 Telegram 客户端虽然能够接收消息，但无法渲染富文本消息（例如某些将富文本显示为不透明媒体块的客户端），则可以取消该功能并强制使用 MarkdownV2 渲染路径：
...
默认情况下，富文本消息功能是启用的（`rich_messages: true`）。此设置旨在确保与客户端渲染方式兼容；一旦 Telegram 拒绝处理富文本 API 请求，Hermes 会自动切换到备用方案。如果您希望在保持富文本消息功能的同时，仅采用传统的“始终以代码块形式显示表格”的方式，可以在 `config.yaml` 中将 `telegram.pretty_tables` 设置为 `false` 以禁用表格格式化功能（默认值为 `true`）。

**链接预览功能**。Telegram 会自动为机器人消息中的网址生成链接预览。如果您希望禁用此功能（例如避免出现过长的 `/tools` 输出，或减少包含大量链接的机器人回复等）：
