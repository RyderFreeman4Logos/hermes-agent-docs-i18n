...
**使用 `edit`（默认值）在私信中显示的效果**——网关会先发送常规的预览消息，再通过 `editMessageText` 功能逐步更新内容，从而避免出现 Telegram 的草稿预览崩溃或回退问题。

**使用 `auto` 或 `draft` 在私信中显示的效果**——Telegram 会展示一个逐字符更新的动画式草稿预览。当回复完成后，它会以普通消息的形式发送，且客户端上的草稿预览会自动消失。由于草稿没有消息 ID，因此最终的内容才会保留在聊天记录中。

**群组、超级群组及论坛主题呢？** Telegram 仅允许在私信中使用 `sendMessageDraft` 功能。对于其他类型的聊天场景，网关会自动切换为基于编辑的发送方式——用户体验与之前保持一致。

**如果草稿预览出现故障怎么办？** 无论是因为临时网络错误、服务器端拒绝响应，还是 `python-telegram-bot` 版本过旧导致的故障，系统都会立即将后续的发送流程切换回基于编辑的方式。下一次响应将会重新尝试发送。

## 渲染：富文本消息、表格及链接预览

**富文本消息（Bot API 10.1）**。一旦启用此功能，最终回复将使用 Telegram 原生的 [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage) 函数发送，并结合代理的**原始 Markdown 格式**，因此表格、任务列表、标题、嵌套引文、可折叠的 `<details>` 区块、脚注/参考文献、数学公式、下划线、上标/下标、高亮文本以及链接都能以原生格式呈现——无需经过客户端的简化处理。在私信中，实时流式预览也会使用 `sendRichMessageDraft`，因此动画式的草稿预览与最终的富文本消息效果一致。

当内容超过 32,768 字节的富文本限制时，系统会自动跳过富文本渲染路径；而如果遇到 Telegram 端的拒绝响应（如旧版 `python-telegram-bot` 不支持对应接口、解析错误或块/列尺寸过大），系统也会**无缝切换**为 MarkdownV2 渲染方式——确保您的消息不会丢失。对于临时性或网络错误，系统不会自动重新发送消息（从而避免出现重复的最终消息）。

**MarkdownV2 作为备用方案**。当某条消息无法使用富文本渲染路径时，Hermes 会将该 Markdown 内容转换为 MarkdownV2 格式。由于 MarkdownV2 不支持原生表格语法，因此管道式表格会被进行标准化处理：

- **小型表格**会被转换成**行组项目符号列表**——每行内容都会以列标题下的可读项目符号形式呈现。这种方式适用于 2–4 列且单元格内容较短的表格。
- **较大或较宽的表格**则会转换为带有对齐列的**代码块格式**，从而避免内容折叠问题。

默认情况下富文本消息功能是关闭的，因为部分 Telegram 客户端虽然能接收 Bot API 的消息数据，但渲染效果较差。如需在能够良好显示富文本消息的客户端中使用该功能，可手动开启。
