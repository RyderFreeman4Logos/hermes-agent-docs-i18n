...
- 简短描述是对整个机器人的**全局**说明（所有用户均可查看），而非针对单次对话。用户会在机器人资料页上看到该描述，而不会显示在正在进行的对话中的实时徽章中。
- 只有通过**正常**的网关关闭方式（如 `/stop`、`disconnect`）才会将状态设置为“离线”。若发生程序崩溃，则会保留最后记录的状态——这正是资料页文本指示器所固有的局限性。
- 该功能默认处于关闭状态，因为它会修改机器人的全局资料信息。

## 第3步：隐私模式（群组中使用时至关重要）

Telegram机器人具有**默认开启的隐私模式**。这是在群组中使用机器人时最容易引发混淆的问题来源。
...
:::warning
更改隐私设置后，**必须将机器人从相关群组中移除后再重新添加**。Telegram会在机器人加入群组时缓存其隐私状态，只有将其移除并重新添加后，状态才会更新。
:::

:::tip
另一种替代关闭隐私模式的方法是将机器人提升为**群组管理员**。管理员机器人无论隐私模式如何设置，始终能够接收所有消息，这样就不必再切换全局隐私模式了。
:::

### 查看群组聊天内容而不自动回复

对于类似OpenClaw/Yuanbao的群组交互行为，可配置Telegram让机器人能够**查看**普通群组消息，但仅在**直接触发**时才进行回复：
...
**使用 `edit`（默认值）在私聊中显示的效果**——网关会先发送常规的预览消息，再通过 `editMessageText` 逐步更新内容，从而避免 Telegram 的草稿预览折叠或回滚问题。

**使用 `auto` 或 `draft` 在私聊中显示的效果**——Telegram 会展示一个逐字符更新的动画式草稿预览。当回复完成后，它会以普通消息的形式发送，而客户端上的草稿预览也会自动消失。由于草稿没有消息 ID，因此最终内容才会保留在聊天记录中。

**群组、超级群组及论坛主题呢？** Telegram 仅允许在私聊中使用 `sendMessageDraft` 功能。对于其他类型的聊天，网关会透明地切换到基于编辑的发送方式——用户体验与之前一致。

**如果草稿预览出现故障会怎样？** 无论是因为临时网络错误、服务器端拒绝处理，还是 `python-telegram-bot` 版本过旧导致的故障，系统都会自动将后续响应切换回基于编辑的发送方式。下一次响应将会重新尝试发送。

## 渲染：富文本消息、表格及链接预览

**富文本消息（Bot API 10.1）**。对于那些包含传统 MarkdownV2 方式无法正确处理的元素——如表格、任务列表、可折叠的 `<details>` 标签以及数学公式——系统会使用代理的**原始 Markdown 格式**，通过 Telegram 原生的 [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage) 函数进行发送，从而实现原生渲染，无需在客户端进行格式简化。在流式回复过程中，最终内容是通过 `editMessageText` 的 `rich_message` 参数**直接编辑现有预览**来生成的——不会生成第二条消息，也不会删除原有内容，因此不会在回合结束时出现重复发送的闪烁现象。在私聊中，实时流式预览同样使用 `sendRichMessageDraft`，因此动画草稿与最终的富文本消息保持一致。而普通回复（纯文本、加粗/斜体文字、简单列表）则仍采用 MarkdownV2 方式处理，以确保不同客户端上的字体大小和间距一致。

当内容超过 32,768 字符的富文本限制时，系统会自动跳过富文本渲染路径；若遇到 Telegram 的拒绝响应（如旧版 `python-telegram-bot` 不支持的接口、解析错误或块/列尺寸过大），系统也会**透明地回退到 MarkdownV2 路径**——您的消息绝不会丢失。对于临时性或网络错误，系统不会自动重新发送消息（因此不会出现重复的最终消息）。

**MarkdownV2 回退机制**。当某条消息无法使用富文本渲染路径时，Hermes 会将 Markdown 格式转换为 MarkdownV2。由于 MarkdownV2 不支持原生表格语法，管道格式的表格会被进行规范化处理：

- **小型表格**会转换为**行组项目符号列表**——每行内容都会变成列标题下的可读项目符号列表，适用于 2–4 列且单元格内容较短的表格。
- **较大或较宽的表格**则会回退为带有对齐列的**代码块格式**，从而避免内容折叠。

富文本消息功能是**默认开启**的。不过某些 Telegram 客户端虽然能接收 Bot API 的消息数据，但渲染效果较差；如需关闭此功能并强制所有回复都使用传统的 MarkdownV2 方式，可进行相应设置：
...
此设置旨在确保与客户端渲染方式的兼容性；当 Telegram 拒绝处理富格式 API 调用时，Hermes 会自动回退到默认模式。如果您希望在启用富格式消息的同时仍保持传统的“始终以代码块显示表格”的行为，可通过在 `config.yaml` 中将 `telegram.pretty_tables` 设置为 `false` 来禁用表格的标准化处理（默认值为 `true`）。
