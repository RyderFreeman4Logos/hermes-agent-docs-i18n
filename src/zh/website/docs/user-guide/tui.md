...
其键绑定与[经典 CLI](cli.md#keybindings)完全一致。唯一的行为差异如下：

- **鼠标拖动**：会以统一的选中背景高亮显示文本。
- **`Cmd+V` / `Ctrl+V`**：首先尝试常规文本粘贴，若失败则尝试从 OSC52 或系统剪贴板读取内容；当剪贴板或粘贴的内容为图片时，最终会以图片形式插入。
- **`/terminal-setup`**：会在本地安装 VS Code、Cursor 和 Windsurf 的终端键绑定，从而在 macOS 上实现更流畅的 `Cmd+Enter` 操作以及更好的撤销/重做功能。
- **斜杠命令自动补全**：会以带有说明的浮动面板形式显示，而非内联下拉菜单。
- **`Ctrl+X`**：会打开实时会话切换器。当有排队中的消息被高亮显示（即在智能体仍在运行时发送的消息）时，该命令仍会删除该排队消息。**`Esc`**：则取消编辑操作，仅取消高亮显示而不会删除内容。
- **`Ctrl+G` / `Ctrl+X Ctrl+E`**：会在 `$EDITOR` 中打开当前输入缓冲区，便于输入多行内容或较长的提示语；保存后退出时，这些内容会作为新的提示语发送出去。

## 斜杠命令

所有斜杠命令均保持原有功能不变。其中部分命令由 TUI 所管理——它们会生成更丰富的输出，或以覆盖层形式显示，而非内联面板：
...
| `/agents`（别名 `/tasks`） | 可视化监控覆盖层——展示实时的子智能体树结构，提供终止/暂停控制功能，同时显示各分支的成本、令牌使用量及文件统计信息，以及逐步骤操作历史记录 |
| `/reload` | 重新读取 `~/.hermes/.env` 文件并将其加载到正在运行的 TUI 进程中，这样新添加的 API 密钥无需重启即可生效 |
| `/mouse [on\|off\|toggle\|wheel\|buttons\|all]` | 允许在运行时选择鼠标跟踪预设（该设置也会保存到 `config.yaml` 文件中的 `display.mouse_tracking` 字段）。`wheel`（1000+1006）模式可保持滚轮滚动功能，同时避免因悬停事件导致 tmux 在提示行上方频繁显示“剪贴板中无图片”的提示；`buttons` 模式可添加拖动选择功能；默认值为 `all`，即采用基于悬停的 UI。 |

其他所有斜杠命令（包括已安装的技能、快速命令以及个性设置切换功能）的运行方式都与经典 CLI 完全相同。详情请参阅[斜杠命令参考文档](../reference/slash-commands.md)。

## 实时会话切换器

当您希望一个终端充当多个 TUI 会话的调度器时，可使用实时会话切换器。它仅列出当前在该 TUI 进程中正在运行的会话；已关闭的会话仍会以保存的文本形式存在，可通过 `/resume` 命令或 `hermes --tui --resume <id-or-title>` 重新打开。

可通过以下任意方式打开该切换器：

- 在 TUI 中按下 `Ctrl+X`。
- 使用 `/sessions` 或 `/switch` 命令。
- 使用 `/sessions new` 立即创建一个新的实时会话。
- 点击状态行中的 “N 个实时会话” 显示数字。

<img alt="Hermes TUI 会话调度器，显示一个正在运行的会话及一个“新建”按钮" src="/img/docs/tui-session-orchestrator/session-orchestrator.png" />

<video controls muted loop playsInline src="/img/docs/tui-session-orchestrator/session-orchestrator-demo.mp4" title="Hermes TUI 会话调度器演示" />

在切换器内部：

- 使用 `↑` / `↓` 键移动选择项；点击鼠标也可选中对应行。
- 按下 `Enter` 键可切换到选中的实时会话。
- 按下 `Ctrl+D` 键可关闭选中的实时会话。
- 按下 `Ctrl+N` 键可启动一个空白的实时会话。
- 按下 `Ctrl+R` 键可刷新实时会话列表。
- 按下 `Esc` 键可关闭切换器。
- 选择 `+new`，输入提示语后按 `Enter` 即可启动一个新的实时会话。如果希望仅为该新会话选择特定模型，可先按 `Tab` 键。
