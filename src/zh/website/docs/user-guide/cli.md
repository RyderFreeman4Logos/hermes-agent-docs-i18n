...
| 元素 | 描述 |
|---------|-------------|
| 模型名称 | 当前使用的模型（若超过26个字符则会被截断） |
| 令牌数量 | 已使用的上下文令牌数 / 最大上下文窗口大小 |
| 上下文条 | 带有颜色编码阈值指示的可视化填充条 |
| 成本 | 会话预估费用（对于未知或免费模型则显示为 `n/a`） |
| 🗜️ N | **上下文压缩次数** — 表示当前会话已被自动压缩的次数。首次发生压缩时即显示该数值。 |
| ▶ N | **正在运行的后台任务数** — 表示当前会话中仍有多少个 `/background` 提示正在处理中。只要存在至少一个正在运行的任务，就会显示此数值。 |
| 时长 | 会话已持续的时间 |
| ⚠ YOLO | **YOLO模式警告** — 每当 `HERMES_YOLO_MODE` 开启时（即在启动时使用 `hermes --yolo` 命令，或是在会话进行中切换为 `/yolo` 模式）都会显示此警告。该提示与顶部的横幅警告功能类似，旨在提醒用户当前处于自动批准模式。 |

该条状图的宽度会根据终端宽度自动调整：在≥76列的终端上显示完整布局，在52–75列的终端上显示紧凑版，在小于52列的终端上则仅显示最简版内容（模型名称、时长，以及处于YOLO模式时的对应标识）。
...
如需查看包含各类别费用明细（输入令牌与输出令牌费用）的详细信息，请使用 `/usage` 命令。

### 会话恢复显示功能

当恢复之前的会话时（使用 `hermes -c` 或 `hermes --resume <id>` 命令），在顶部横幅与输入提示之间会出现一个“历史对话”面板，简要展示对话记录的概要。详细信息及相关配置请参阅 [会话 — 会话恢复时的对话概览](sessions.md#conversation-recap-on-resume)。
...
|-----|------|
| `Enter` | 发送消息 |
| `Alt+Enter`、`Ctrl+J` 或 `Shift+Enter` | 换行（多行输入）。`Shift+Enter` 需要终端能够将其与 `Enter` 键区分开来 — 具体要求见下文。在 Windows Terminal 中，`Alt+Enter` 键会被终端用于切换全屏模式；此时请使用 `Ctrl+Enter` 或 `Ctrl+J` 代替。 |
| `Alt+V` | 在终端支持的情况下，从剪贴板粘贴图片 |
| `Ctrl+V` | 粘贴文本，并在合适时机附加剪贴板中的图片 |
| `Ctrl+B` | 在启用语音模式时开始/停止录音（语音模式对应的记录键为 `voice.record_key`，默认值为 `ctrl+b`） |
| `Ctrl+G` | 在 `$EDITOR`（如 vim/nvim/nano/VS Code 等编辑器）中打开当前的输入缓冲区。保存并退出后，即可将编辑后的文本作为下一条提示发送出去 — 这对于较长、包含多段文字的提示尤为实用。 |
| `Ctrl+X Ctrl+E` | 适用于外部编辑器的 Emacs 风格替代快捷键，功能与 `Ctrl+G` 相同。 |
| `Ctrl+C` | 中断代理运行（在2秒内再次按下可强制退出） |
| `Ctrl+D` | 退出当前会话 |
| `Ctrl+Z` | 将 Hermes 暂停到后台运行（仅适用于 Unix 系统）。可在终端中输入 `fg` 命令来恢复其运行状态。 |
| `Tab` | 接受自动建议内容（即虚线显示的提示文本），或完成斜杠命令的自动补全 |

**多行粘贴预览功能。** 当您粘贴多行文本时，CLI 会显示一个简短的单行预览信息（例如 `[已粘贴：47行，1,842个字符 — 按Enter键发送]`），而不会将全部内容直接显示在滚动历史中。实际上最终发送的内容仍是完整的多行文本，此功能仅用于优化显示效果。

**最终回复中的 Markdown 格式去除。** CLI 会从代理的最终回复中移除较为复杂的 Markdown 标记以及 `**粗体**` / `*斜体*` 格式标记，从而使回复以易于在终端中阅读的纯文本形式呈现，而非原始的 Markdown 代码。代码块和列表格式则会被保留。此功能不会影响网关平台或工具生成的回复 — 它们仍会保留 Markdown 格式以便在对应平台上直接渲染。
...
| `/background <prompt>` | 在独立的后台会话中运行指定的提示内容 |
| `/skin` | 查看或切换当前启用的 CLI 界面主题 |
| `/voice on` | 启用 CLI 语音模式（按 `Ctrl+B` 键开始录音） |
| `/voice tts` | 切换 Hermes 回复的语音播放功能 |
| `/reasoning high` | 提高推理强度 |
| `/title My Session` | 为当前会话命名 |
| `/status` | 显示会话相关信息 — 包括模型名称、用户配置文件信息、已使用令牌数、会话时长 — 后还会显示一个本地的 **会话概览** 区块（列出最近的对话轮次数、最常使用的工具、处理过的文件，以及最新的用户提示和助手回复内容）。该功能完全在本地计算完成，不会调用大型语言模型。 |
| `/sessions` | 在传统的 CLI 界面中直接打开交互式会话选择器（与 TUI 使用的界面相同）。可通过输入文字进行筛选，使用方向键导航，按 Enter 键选择要恢复的会话。 |

如需查看所有内置的 CLI 命令及消息相关命令的完整列表，请参阅 [斜杠命令参考文档](../reference/slash-commands.md)。

关于设置、服务提供方、静音调节以及消息发送/Discord 语音功能的使用方法，可参阅 [语音模式](features/voice-mode.md) 文档。

:::tip
所有命令均不区分大小写 — `/HELP` 与 `/help` 的功能完全相同。已安装的技能也会自动转换为斜杠命令。
:::

## 快捷指令

您可以定义自定义指令，这些指令能够立即执行Shell命令，而无需调用大型语言模型。此类指令既可在CLI环境中使用，也适用于消息平台（如Telegram、Discord等）。
