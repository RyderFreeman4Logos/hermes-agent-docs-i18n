...
请关闭列出的进程后重新运行。如果您确信这些并发进程不会造成干扰（这种情况较为罕见——通常仅在杀毒程序错误识别文件时才有用），可传递 `--force` 参数来跳过检查。即便如此，更新工具仍会以指数退避策略尝试重命名 `.exe` 文件；而对于顽固的锁定情况，则会通过 `MoveFileEx(MOVEFILE_DELAY_UNTIL_REBOOT)` 函数将替换操作安排在下次重启时执行，从而确保更新能够顺利完成。

另一项独立的保护机制会禁止在由 Python 解释器运行的任何进程（包括桌面应用程序的后端、网关以及 Python REPL）存在时修改虚拟环境。这些进程会锁定原生扩展文件（`.pyd`），而如果在访问被拒时依赖同步过程中断，就会导致安装程序卡在两个版本之间无法进展。此保护机制**不会**被 `--force` 参数绕过；如果您确定检测到的占用进程为误报，可使用 `hermes update --force-venv` 这一专用命令。
