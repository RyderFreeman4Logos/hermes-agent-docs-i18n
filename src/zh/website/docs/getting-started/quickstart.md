...
该命令可一次性完成登录操作，将Nous设置为您的服务提供商，并启用工具网关。
:::
...
| 服务提供商 | 类型 | 设置方式 |
|----------|------|----------|
| **Nous Portal** | 基于订阅、无需额外配置 | 通过 `hermes model` 执行OAuth登录 |
| **OpenAI Codex** | 使用ChatGPT OAuth机制及Codex模型 | 通过 `hermes model` 执行设备代码认证 |
| **Anthropic** | 直接使用Claude模型——Max套餐加额外使用额度（通过OAuth），或使用按令牌计费的API密钥 | 通过 `hermes model` 执行OAuth登录（需拥有Max套餐及额外额度），或输入Anthropic API密钥 |
| **OpenRouter** | 支持多服务提供商，可路由至多种模型 | 输入您的API密钥 |
...
| **GMI Cloud** | 支持多模型的直接API访问 | 设置 `GMI_API_KEY` |
| **MiniMax (OAuth)** | 通过浏览器OAuth访问Minimax前沿模型——无需API密钥（`hermes_cli/models.py`中的模型名称在不同版本中可能会发生变化） | 通过 `hermes model` 选择MiniMax (OAuth) |
| **MiniMax** | 国际版的Minimax接口 | 设置 `MINIMAX_API_KEY` |
| **MiniMax China** | 中国地区的Minimax接口 | 设置 `MINIMAX_CN_API_KEY` |
| **Alibaba Cloud** | 通过DashScope访问Qwen模型 | 设置 `DASHSCOPE_API_KEY`（Qwen编程套餐也支持 `ALIBABA_CODING_PLAN_API_KEY`） |
| **Hugging Face** | 通过统一路由器访问20多种开源模型（包括Qwen、DeepSeek、Kimi等） | 设置 `HF_TOKEN` |
| **AWS Bedrock** | 通过原生Converse API访问Claude、Nova、Llama、DeepSeek模型 | 使用IAM角色或执行 `aws configure`操作（详见[指南](../guides/aws-bedrock.md)） |
| **Azure Foundry** | 由Azure AI Foundry提供的模型 | 设置 `AZURE_FOUNDRY_API_KEY` 和 `AZURE_FOUNDRY_BASE_URL` |
| **Google AI Studio** | 通过直接API访问Gemini模型 | 设置 `GOOGLE_API_KEY` / `GEMINI_API_KEY` |
| **Google Gemini (OAuth)** | 通过 `google-gemini-cli`的OAuth流程访问Gemini——无需密钥 | 通过 `hermes model` 选择Google Gemini (OAuth) |
| **xAI** | 通过直接API访问Grok模型 | 设置 `XAI_API_KEY` |
| **xAI Grok OAuth** | SuperGrok/Premium+套餐用户，无需API密钥 | 通过 `hermes model` 选择xAI Grok OAuth |
| **NovitaAI** | 多模型API网关 | 设置 `NOVITA_API_KEY` |
| **StepFun** | Step套餐提供的模型 | 设置 `STEPFUN_API_KEY` |
...
| **LM Studio** | 本地桌面应用，提供兼容OpenAI的API接口 | 设置 `LM_API_KEY`（如非默认设置，则还需设置 `LM_BASE_URL`） |
| **Qwen OAuth** | 通过Qwen Portal的浏览器OAuth机制登录——无需API密钥 | 通过 `hermes model` 选择Qwen OAuth |
| **Kilo Code** | 由KiloCode提供的模型 | 设置 `KILOCODE_API_KEY` |
| **OpenCode Zen** | 支持按需使用精选模型 | 设置 `OPENCODE_ZEN_API_KEY` |
...
| **GitHub Copilot** | GitHub Copilot订阅服务（支持GPT-5.x、Claude、Gemini等模型） | 通过 `hermes model` 执行OAuth登录，或使用 `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP智能体后端（会生成本地的 `copilot` CLI工具） | 需先使用 `hermes model` 操作（需提前安装 `copilot` CLI并完成登录） |
| **自定义接口** | VLLM、SGLang、Ollama或任何兼容OpenAI的API | 设置基础URL及API密钥 |

对于大多数首次使用的用户：请选择一个服务提供商，除非有特殊原因，否则建议直接使用默认设置。完整的供应商列表以及相关环境变量和设置步骤，请参阅[服务提供商](../integrations/providers.md)页面。

:::注意 最小上下文长度：64K tokens
Hermes Agent要求模型具备至少**64,000个tokens**的上下文容量。上下文长度过短的模型无法为多步骤工具调用流程保留足够的工作内存，从而会在启动时被拒绝。大多数托管模型（如Claude、GPT、Gemini、Qwen、DeepSeek）都轻松满足这一要求。如果您使用的是本地模型，请将其上下文大小设置为至少64K（例如，对于llama.cpp可使用 `--ctx-size 65536`，对于Ollama则可使用 `-c 65536`）。
:::

:::提示
您可以通过 `hermes model`随时更换服务提供商——无需绑定特定平台。如需查看所有支持的服务提供商及其详细设置信息，请参阅[AI服务提供商](../integrations/providers.md)。
:::
