...
- **快速设置（Nous Portal）**——支持免费OAuth登录，无需API密钥；可同时设置模型及工具网关相关功能。这是推荐的快捷路径。
- **完整设置**——需手动配置每个服务提供商、工具及各项选项（需自行准备密钥）。
- **空白起步模式**——除运行智能体所需的最低配置外，其余所有功能均处于关闭状态：即服务提供商与模型、文件操作工具集以及终端工具集。该模式下不支持网页访问、浏览器功能、代码执行、视觉处理、内存管理、任务委托、定时任务、技能功能、插件或MCP服务器，同时压缩功能、检查点机制、智能路由及内存捕获功能也会被禁用。在完成最低基础配置后，可选择两种路径之一：要么从完全关闭所有功能的状态开始（立即获得最简版智能体），要么逐步启用各项配置（选择性开启工具、技能、插件、MCP及消息功能）。当您希望拥有一个经过严格控制的极简智能体，并且只打算启用真正需要的功能时，可选择此模式。
...
| **OpenAI Codex** | 基于ChatGPT的OAuth机制，使用Codex模型 | 通过 `hermes model` 进行设备代码认证 |
| **Anthropic** | 直接使用Claude模型——可选择Max套餐及额外使用额度（通过OAuth登录），或使用按令牌计费的API密钥 | 通过 `hermes model` 进行OAuth登录（需拥有Max套餐及额外额度），或输入Anthropic API密钥 |
| **OpenRouter** | 支持跨多种模型进行多服务提供商路由 | 输入对应API密钥 |
| **Fireworks AI** | 直接调用兼容OpenAI的模型API | 设置 `FIREWORKS_API_KEY` |
...
| **MiniMax (OAuth)** | 通过浏览器OAuth调用Minimax前沿模型——无需API密钥（`hermes_cli/models.py` 中的模型名称在不同版本中可能会变化） | 通过 `hermes model` 选择Minimax（OAuth）模式 |
| **MiniMax** | 国际版本的Minimax接口 | 设置 `MINIMAX_API_KEY` |
| **MiniMax China** | 中国地区的Minimax接口 | 设置 `MINIMAX_CN_API_KEY` |
| **Alibaba Cloud** | 通过DashScope调用Qwen系列模型 | 设置 `DASHSCOPE_API_KEY`（Qwen编程套餐也支持 `ALIBABA_CODING_PLAN_API_KEY`） |
| **Hugging Face** | 通过统一路由器访问20多种开源模型（包括Qwen、DeepSeek、Kimi等） | 设置 `HF_TOKEN` |
| **AWS Bedrock** | 通过原生Converse API调用Claude、Nova、Llama、DeepSeek等模型 | 需配置IAM角色或使用 `aws configure` 命令（详见[指南](../guides/aws-bedrock.md)） |
| **Azure Foundry** | 由Azure AI Foundry提供的模型 | 设置 `AZURE_FOUNDRY_API_KEY` 及 `AZURE_FOUNDRY_BASE_URL` |
| **Google AI Studio** | 通过直接API调用Gemini系列模型 | 设置 `GOOGLE_API_KEY` / `GEMINI_API_KEY` |
| **xAI** | 通过直接API调用Grok系列模型 | 设置 `XAI_API_KEY` |
...
| **Qwen OAuth** | 通过Qwen Portal的浏览器OAuth机制登录——无需API密钥 | 通过 `hermes model` 选择Qwen OAuth模式 |
...
| **DeepSeek** | 直接调用DeepSeek的API接口 | 设置 `DEEPSEEK_API_KEY` |
| **NVIDIA NIM** | 通过build.nvidia.com或本地NIM环境调用Nemotron模型 | 设置 `NVIDIA_API_KEY`（可选：设置 `NVIDIA_BASE_URL`） |
...
| **GitHub Copilot ACP** | Copilot ACP智能体后端（会生成本地的 `copilot` CLI工具） | 需先通过 `hermes model` 配置（需已安装 `copilot` CLI并完成登录） |
| **自定义接口** | VLLM、SGLang、Ollama或任何兼容OpenAI的API | 需设置基础URL及API密钥 |

对于大多数首次使用用户而言：只需选择一种服务提供商，除非有特殊需求，否则直接接受默认设置即可。完整的供应商列表以及相关的环境变量和配置步骤详见[服务提供商](../integrations/providers.md)页面。
:::caution 最小上下文长度：64K个标记
Hermes Agent要求模型具备至少**64,000个标记**的上下文容量。上下文长度过短的模型无法为多步骤工具调用流程保留足够的工作内存，因此在启动时会被拒绝。大多数托管模型（如Claude、GPT、Gemini、Qwen、DeepSeek）都轻松满足这一要求。如果您使用的是本地模型，则需将其上下文大小设置为至少64K（例如，对于lama.cpp可使用 `--ctx-size 65536`，对于Ollama则可使用 `-c 65536`）。
:::

:::tip
您随时可以通过 `hermes model` 命令更换服务提供商——不存在绑定限制。如需查看所有支持的服务提供商列表及详细配置信息，请参阅[AI服务提供商](../integrations/providers.md)。
:::
