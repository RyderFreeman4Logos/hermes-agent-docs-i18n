...
网页搜索提供者插件用于注册能够处理 `web_search`、`web_extract` 以及（可选的）深度爬取请求的后端服务。所有内置提供者——Firecrawl、SearXNG、Tavily、Exa、Parallel、Brave Search（免费版）、xAI 和 DDGS——均以插件形式存在于 `plugins/web/<name>/` 目录下。你可以通过在该目录旁创建新文件夹来添加自定义插件，或覆盖现有的内置插件。

:::提示
网页搜索是 Hermes 支持的多种**后端插件**之一。其他类型的插件及其对应的命名规则包括：[图像生成提供者插件](/developer-guide/image-gen-provider-plugin)、[视频生成提供者插件](/developer-guide/video-gen-provider-plugin)、[内存提供者插件](/developer-guide/memory-provider-plugin)、[上下文引擎插件](/developer-guide/context-engine-plugin)以及[模型提供者插件](/developer-guide/model-provider-plugin)。常规的工具/钩子/CLI 插件则位于 [构建 Hermes 插件](/guides/build-a-hermes-plugin) 文档中。
:::

## 发现机制的工作原理

Hermes 会在三个位置扫描网页搜索后端：

1. **内置插件** — `<repo>/plugins/web/<name>/`（通过 `kind: backend` 自动加载，始终可用）
2. **用户自定义插件** — `~/.hermes/plugins/web/<name>/`（需通过 `plugins.enabled` 或 `hermes plugins enable <name>` 手动启用）
3. **Pip 安装的插件** — 包含 `hermes_agent.plugins` 入口点的包

每个插件中的 `register(ctx)` 函数都会调用 `ctx.register_web_search_provider(...)`，从而将该插件的实例添加到 `agent/web_search_registry.py` 中的注册表中。针对各项功能的默认提供者将由配置文件决定：
...
如果上述配置键均未设置，Hermes 会自动从环境中存在的 API 密钥或 URL 中检测合适的后端。`hermes tools` 工具可指导用户完成选择过程。
...
| `requires_env` | 在执行 `hermes plugins install` 时触发交互式凭据输入（详细格式请参见[构建 Hermes 插件](/guides/build-a-hermes-plugin#gate-on-environment-variables)） |

## ABC 参考

完整接口定义位于 `agent/web_search_provider.py` 文件中。您可以重写的方法如下：

| 成员 | 是否必填 | 默认值 | 用途 |
|---|---|---|---|
| `name` | ✅ | — | 用于 `web.*_backend` 配置中的稳定标识符 |
| `display_name` | — | `name` | 在 `hermes tools` 中显示的标签 |
| `is_available()` | ✅ | — | 可用性检测机制——基于环境变量及可选依赖项 |
...
单个类即可同时支持多种功能——Firecrawl、Tavily、Exa 和 Parallel 均同时具备搜索与提取功能。Brave Search 和 DDGS 仅支持搜索功能；SearXNG 也仅支持搜索功能，但文档中提供了“与我搭配提取提供者”的使用方案。

## 响应格式

工具封装层期望接收固定结构的响应数据，从而无需在不同后端之间进行转换。
...
如果您的提供者仅支持一种功能，请将其他标志保持默认值（`False`），这样注册表就会跳过该工具——这样当用户仅使用该工具进行搜索，却要求智能体执行提取操作时，就不会出现“提供者 X 失败”这类误导性错误。
...
5. 将响应数据序列化为 JSON 后传回给大语言模型。

错误会以工具结果的形式呈现，由大语言模型决定如何解释这些错误。如果未注册任何提供者（或所有可用提供者均不满足功能要求），该工具会返回一条指向 `hermes tools` 的有用错误信息。

## 懒加载可选依赖项

如果您的提供者封装了第三方 SDK（例如 DDGS 所使用的 `ddgs` 包），请勿在模块顶层直接 `import` 该 SDK。应在 `is_available()` 或 `search()` 方法中使用 `tools.lazy_deps.ensure(...)` 函数——Hermes 会在首次使用时根据 `security.allow_lazy_installs` 的设置来安装该包。有关安全机制的详细信息，请参阅 [构建 Hermes 插件 → 懒加载](/guides/build-a-hermes-plugin#lazy-install-optional-python-dependencies)。

## 参考实现示例

- **`plugins/web/brave_free/`** —— 一个体积小、通过 API 密钥验证、仅支持搜索功能的 HTTP 提供者，是不错的入门模板。
- **`plugins/web/ddgs/`** —— 无需密钥的提供者，其 SDK 会实现懒加载。对于那些封装了 Python 包的后端来说，这是一种实用的实现方式。
- **`plugins/web/firecrawl/`** —— 全功能提供者，支持搜索、提取及爬取操作，并具备多种格式处理模式。
- **`plugins/web/searxng/`** —— 自托管型后端，通过 URL 配置，无需身份验证。
- **`plugins/web/xai/`** —— 基于 Grok 服务器端 `web_search` 工具的大语言模型驱动搜索方案。该示例展示了如何在不新增环境变量的情况下，复用现有的 OAuth/环境变量认证机制（通过 `tools/xai_http.py` 实现），以及如何编写符合“无网络环境”要求的简易 `is_available()` 函数。
...
`my_backend_web_package` 必须提供一个顶层 `register` 函数。有关完整的设置步骤，请参阅通用插件指南中的[通过 pip 分发](/guides/build-a-hermes-plugin#distribute-via-pip)部分。

## 相关页面

- [网络搜索](/user-guide/features/web-search) — 面向用户的功能文档及针对不同后端的配置方式
- [插件概览](/user-guide/features/plugins) — 一览所有插件类型
- [构建 Hermes 插件](/guides/build-a-hermes-plugin) — 工具、钩子函数及斜杠命令的通用指南
