...
图像生成提供者插件用于注册后端服务，以处理所有 `image_generate` 工具调用——无论是 DALL·E、gpt-image、Grok、Flux、Imagen、Stable Diffusion、FAL、Replicate，还是本地的 ComfyUI 工作流，乃至其他任何图像生成工具。所有内置提供者（OpenAI、OpenAI-Codex、xAI、FAL、Krea、DeepInfra、OpenRouter、Meta Model API）均以插件形式提供。您可以通过将目录放入 `plugins/image_gen/<名称>/` 中来添加新插件，或覆盖已有的内置插件。

:::提示
图像生成插件是 Hermes 支持的多种**后端插件**之一。其他具有更专用功能的插件还包括[内存提供者插件](/developer-guide/memory-provider-plugin)、[上下文引擎插件](/developer-guide/context-engine-plugin)以及[模型提供者插件](/developer-guide/model-provider-plugin)。常规的工具/钩子/CLI 插件则位于[构建 Hermes 插件](/developer-guide/plugins)相关文档中。
:::
...
1. **内置版本**——`<repo>/plugins/image_gen/<名称>/`（带有 `kind: backend` 标识，会自动加载且始终可用）
2. **用户自定义版本**——`~/.hermes/plugins/image_gen/<名称>/`（需通过 `plugins.enabled` 参数启用）
3. **Pip 安装版本**——声明了 `hermes_agent.plugins` 入口点的包
...
`image_generate` 工具封装会向注册表查询当前激活的提供者，并将请求发送至该提供者处。如果未注册任何提供者，该工具会显示一条指向 `hermes tools` 的有用错误提示。
...
至此，一个打包好的插件便已生成完成。位于 `~/.hermes/plugins/image_gen/<name>/` 目录下的用户自定义插件需被添加到 `config.yaml` 文件中的 `plugins.enabled` 列表中（或执行命令 `hermes plugins enable <name>`）。

## ImageGenProvider ABC 接口

该插件需继承自 `agent.image_gen_provider.ImageGenProvider` 类。其中仅要求必须实现 `name` 属性和 `generate()` 方法——其余所有字段均设有合理的默认值：
...
`kind: backend` 用于将插件路由至 image-gen 注册路径。`requires_env` 选项会在执行 `hermes plugins install` 时被询问。

## ABC 参考

完整接口定义位于 `agent/image_gen_provider.py` 中。您通常需要重写的方法如下：
...
| `default_model()` | — | `list_models()` 中的第一个元素 | 当未配置模型时的默认值 |
| `get_setup_schema()` | — | 最简版本 | 用于提供选择器元数据及环境变量提示 |
| `generate(prompt, aspect_ratio, **kwargs)` | ✅ | — | 实际的生成调用函数 |
...
部分后端会返回图像的 URL（如 fal、Replicate），而其他后端则返回 base64 编码的图像数据（如 OpenAI 的 gpt-image-2）。对于 base64 格式的输出，可使用 `save_b64_image()` 函数——该函数会将图像保存至 `$HERMES_HOME/cache/images/<前缀>_<时间戳>_<唯一标识符>.<扩展名>` 路径下，并返回该路径的绝对路径。随后可在 `success_response()` 函数中通过 `image=` 参数传入该路径（以字符串形式）。无论是 Telegram 的图片消息还是 Discord 的附件，都能识别这两种格式的地址与绝对路径。

## 用户自定义插件

用户可将自定义插件放置在 `~/.hermes/plugins/image_gen/<名称>/` 目录下，该插件需具备与内置插件相同的 `name` 属性，随后通过 `hermes plugins enable <名称>` 命令启用它。由于插件注册机制遵循“最后写入者胜出”的原则，用户自定义的版本将会替代内置版本。此方法非常适合将 `openai` 插件指向私有代理服务器，或替换为自定义的模型目录。
...
或者通过交互方式操作：运行 `hermes tools` → 选择“图像生成” → 选取 `my-backend` → 如有提示则输入 API 密钥。

## 参考实现

- **`plugins/image_gen/openai/__init__.py`** — 提供低/中/高三个等级的 gpt-image-2 模型，这些模型实际上共享同一个 API 模型，仅通过不同的 `quality` 参数来区分等级。这是单个后端下结合 config.yaml 配置优先级链实现分级模型的典型示例。
- **`plugins/image_gen/xai/__init__.py`** — 通过 xAI 技术调用 Grok Imagine。其输出形式有所不同（以 URL 形式返回，且目录结构更为简单）。
- **`plugins/image_gen/openai-codex/__init__.py`** — 基于 OpenAI SDK 的 Codex 风格响应 API 变体，仅使用了不同的路由基础地址。
...
`my_backend_imggen_package` 必须提供一个顶层 `register` 函数。如需了解完整的设置流程，请参阅通用插件指南中的[通过 pip 分发](/developer-guide/plugins#distribute-via-pip)部分。
