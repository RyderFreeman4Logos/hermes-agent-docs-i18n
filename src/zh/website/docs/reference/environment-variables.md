...
| `HERMES_ACCEPT_HOOKS` | 在无终端提示的情况下，自动批准 `config.yaml` 中声明的所有未见过的全局 shell hook。相当于 `--accept-hooks` 或 `hooks_auto_accept: true`。 |
| `HERMES_IGNORE_USER_CONFIG` | 跳过 `~/.hermes/config.yaml` 文件，使用内置默认设置（但 `.env` 文件中的凭证仍会被加载）。相当于 `--ignore-user-config`。 |
| `HERMES_IGNORE_RULES` | 跳过自动注入的 `AGENTS.md`、`SOUL.md`、`.cursorrules` 文件、内存内容以及预加载的技能。相当于 `--ignore-rules`。 |
| `HERMES_SAFE_MODE` | 故障排查模式：禁用所有自定义设置——跳过插件发现和 MCP 服务器加载流程。该模式会由 `--safe-mode` 参数自动启用（同时也会设置上述两个参数）。 |
...
| `HERMES_API_TIMEOUT` | LLM API 调用的超时时间（秒为单位）（默认值：`1800`） |
| `HERMES_API_CALL_STALE_TIMEOUT` | 非流式调用的超时时间（秒为单位）（默认值：`90`）。若未设置，则对本地提供商自动禁用该功能；对于上下文规模极大的场景，该超时时间可能会相应增加。也可通过 `config.yaml` 中的 `providers.<id>.stale_timeout_seconds` 或 `providers.<id>.models.<model>.stale_timeout_seconds` 参数进行配置。 |
| `HERMES_STREAM_READ_TIMEOUT` | 流式套接字读取的超时时间（秒为单位）（默认值：`120`）。对于本地提供商，该值会自动调整为 `HERMES_API_TIMEOUT`。如果在长时间代码生成过程中出现本地 LLM 超时情况，可适当增加此值。 |
| `HERMES_STREAM_STALE_TIMEOUT` | 检测流式数据过时的超时时间（秒为单位）（默认值：`180`）。对于本地提供商，该功能会自动禁用。若在此时间内没有新的数据块到达，系统将主动断开连接。 |
| `HERMES_STREAM_RETRIES` | 遇到短暂网络故障时，在流式通信过程中尝试重新连接的次数（默认值：`3`） |
| `HERMES_AGENT_TIMEOUT` | 运行中的智能体在网关端处于非活跃状态时的超时时间（秒为单位）（默认值：`1800`，即 30 分钟）。每次调用工具或发送流式 token 后，该计时器都会重置。将其设置为 `0` 可禁用此功能。 |
| `HERMES_AGENT_TIMEOUT_WARNING` | 网关端：在智能体处于非活跃状态达到指定时间后发送警告信息（默认值：`HERMES_AGENT_TIMEOUT` 的 75%）。 |
| `HERMES_AGENT_NOTIFY_INTERVAL` | 网关端：对于长时间运行的智能体轮次，进度通知之间的间隔时间（秒为单位）。 |
| `HERMES_CHECKPOINT_TIMEOUT` | 创建文件系统检查点的超时时间（秒为单位）（默认值：`30`） |
| `HERMES_EXEC_ASK` | 在网关模式下是否启用执行操作前的审批提示（`true`/`false`） |
| `HERMES_ENABLE_PROJECT_PLUGINS` | 是否允许智能体加载器及仪表板 Web 服务器自动发现位于 `./.hermes/plugins/` 目录中的项目本地插件。该参数接受以下任意真值形式：`1` / `true` / `yes` / `on`（不区分大小写）。其余所有值——包括 `0`、`false`、`no`、`off` 以及空字符串——均视为**禁用**状态（默认值）。注意：自 GHSA-5qr3-c538-wm9j (#29156) 及 #43719 版本起，仪表板 Web 服务器不再自动导入项目或用户安装的 Python `api` 文件——这些文件仅能通过静态 JS/CSS 扩展界面功能，而后端路由则保留给预打包的插件使用。 |
| `HERMES_PLUGINS_DEBUG` | 设置为 `1`/`true` 可在标准错误流中输出详细的插件发现过程日志，内容包括扫描的目录、解析的清单文件、跳过某些文件的理由，以及解析或 `register()` 操作失败时的完整堆栈跟踪信息。该功能主要面向插件开发者。 |
| `HERMES_BACKGROUND_NOTIFICATIONS` | 网关端的后台进程通知模式：`all`（默认值）、`result`、`error`、`off` |
| `HERMES_EPHEMERAL_SYSTEM_PROMPT` | 在 API 调用时注入的临时系统提示语，不会被保存到会话中 |
...
| `HERMES_WRITE_SAFE_ROOT` | 可选目录前缀，用于限制 `write_file`/`patch` 操作的写入范围；超出该范围的路径将需要额外审批。 |
| `HERMES_DISABLE_LAZY_INSTALLS` | 官方 Docker 镜像中自动设置的内部桥接变量，旨在防止在不可变的 `/opt/hermes` 目录树中安装运行时依赖项。在用户配置文件中，对应的设置项为 `config.yaml` 中的 `security.allow_lazy_installs: false`；请勿在 `.env` 文件中设置此参数。 |
| `HERMES_DISABLE_FILE_STATE_GUARD` | 设置为 `1` 可关闭 `patch`/`write_file` 操作中的“自上次读取后文件已被修改”检测功能。 |
| `HERMES_CORE_TOOLS` | 以逗号分隔的形式用于覆盖默认的核心工具列表（高级用法，极少需要）。 |
...
| `HERMES_DUMP_REQUESTS` | 是否将 API 请求的负载内容输出到日志文件中（`true`/`false`） |
| `HERMES_dump_REQUEST_STDOUT` | 将 API 请求的负载内容直接输出到标准输出流，而非日志文件。 |
| `HERMES_OAUTH_TRACE` | 设置为 `1` 可记录 OAuth 令牌的交换及刷新尝试过程，同时会包含经过脱敏处理的计时信息。 |
| `HERMES_OAUTH_FILE` | 用于指定存储 OAuth 凭证文件的路径（默认值：`~/.hermes/auth.json`）。 |
| `HERMES_AGENT_HELP_GUIDANCE` | 为自定义部署场景的系统提示语添加额外的指导文本。 |
| `HERMES_AGENT_LOGO` | 用于覆盖 CLI 启动时显示的 ASCII 标识徽标。 |
| `DELEGATION_MAX_CONCURRENT_CHILDREN` | 每批 `delegate_task` 请求可并发运行的子智能体最大数量（默认值：`3`，下限为 1，无上限）。也可通过 `config.yaml` 中的 `delegation.max_concurrent_children` 参数进行设置——配置文件中的值具有优先级。 |

## 接口相关参数

| 变量 | 描述 |
|------|------|
| `HERMES_TUI` | 当设置为 `1` 时，将启动 [TUI](../user-guide/tui.md) 界面，而非传统的命令行界面。相当于传递了 `--tui` 参数。 |
| `HERMES_TUI_DIR` | 预构建的 `ui-tui/` 目录路径（该目录必须包含 `dist/entry.js` 文件以及完整的 `node_modules` 包）。发行版及 Nix 环境会使用此路径来跳过首次启动时的 `npm install` 操作。 |
| `HERMES_TUI_RESUME` | 启动时根据标识符恢复特定的 TUI 会话。当该参数被设置后，执行 `hermes --tui` 命令时会跳过创建新会话的步骤，直接继续使用指定的会话——这在连接中断或终端崩溃后重新连接时非常有用。 |
| `HERMES_TUI_THEME` | 强制指定 TUI 的颜色主题：`light`、`dark`，或直接输入 6 位十六进制值表示背景色（例如 `ffffff` 或 `1a1a2e`）。若未设置该参数，Hermes 会通过查询 `COLORFGBG` 及终端背景色来自动选择主题；而该变量则可在那些未设置 `COLORFGBG` 的终端（如 Ghostty、Warp、iTerm2 等）上强制指定主题。 |
| `HERMES_INFERENCE_MODEL` | 在不修改 `config.yaml` 的情况下，强制为 `hermes -z` / `hermes chat` 命令指定使用的模型。该参数需与 `--provider` 参数配合使用。非常适合那些需要为每次调用自动切换默认模型的脚本化调用场景（如自动化扫描工具、CI 环境、批量处理脚本等）。 |
...
| `HERMES_SESSION_ID` | **会自动被嵌入到 Hermes 启动的每一个工具子进程**中，包括 `terminal`、`execute_code`、持久化终端、Docker/Singularity 后端以及委托运行的子智能体。智能体会将当前会话 ID 设置为此值；从工具中调用的用户脚本可以读取该值，从而将其输出结果、性能数据或副作用与对应的 Hermes 会话关联起来。**不建议手动设置此值**——从父终端覆盖设置的值仅在非智能体运行模式下有效，且一旦智能体开始新的会话，该覆盖值就会被立即替换。 |

## 上下文压缩（仅适用于 `config.yaml`）

上下文压缩功能完全通过 `config.yaml` 文件进行配置，不存在对应的环境变量。阈值相关设置位于 `compression:` 块中，而摘要生成模型/提供者则配置在 `auxiliary.compression:` 下。
