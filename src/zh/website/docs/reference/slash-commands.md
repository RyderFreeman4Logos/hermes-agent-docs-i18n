...
| `/skills` | 从在线注册表中搜索、安装、查看或管理技能。同时也可通过以下命令查看技能审批流程的状态：`/skills pending`、`/skills diff <id>`、`/skills approve <id>`、`/skills reject <id>`、`/skills approval on\|off`。详情请参阅 [智能体技能写入的审批机制](/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval)。 |
...
| `/usage` | 显示令牌使用情况、费用明细、会话时长；若当前使用的服务提供商支持，还会显示实时从该提供商 API 获取的**账户限额**信息，包括剩余配额/积分/套餐使用量 |
| `/insights` | 显示过去 30 天内的使用情况分析数据 |
| `/platforms`（别名：`/gateway`） | 显示网关/消息平台的运行状态（仅提供 CLI 摘要视图） |
| `/platform <list\|pause\|resume> [name]` | 操作正在运行的网关平台。`/platform list` 可列出所有适配器及其状态（运行中、因故障暂停、手动暂停）；`/platform pause <name>` 会停止向该适配器发送新消息，但不会卸载它；`/platform resume <name>` 可重新启用该适配器。当适配器因重复出现可重试的故障（网络问题、速率限制、5xx 错误等）导致断路器触发时，网关会自动暂停该适配器——一旦上游服务恢复正常，可使用 `/platform resume <name>` 解除暂停状态。该命令可在任何可访问网关的界面中使用（CLI 会话、Telegram、Discord 等） |
| `/paste` | 附加剪贴板中的内容 |
| `/copy [number]` | 将助手的最后一条回复复制到剪贴板（输入数字可指定从最后数第 N 条回复）。仅支持 CLI 使用 |
| `/image <path>` | 附加本地图片文件，以便在后续提示中使用 |
| `/debug` | 上传调试报告（包含系统信息及日志），并生成可分享的链接。该功能在消息交互中也可使用 |
...
| `/skills ...` | 从注册表及官方可选技能目录中搜索、浏览、查看、安装、审计、发布及配置技能 |
...
用户自定义别名会优先于内置的简写名称，因此将别名命名为 `sonnet`、`kimi`、`opus` 等即可覆盖内置名称。别名名称不区分大小写。
...
| 命令 | 描述 |
|------|------|
| `/start` | 平台协议命令。许多聊天平台（如 Telegram、Discord 等）会在用户首次与机器人对话时自动发送 `/start`。Hermes 会默默响应此请求——不会返回机器人回复，也不会消耗会话次数——因此初次交互无需浪费轮次。你也可以主动发送该命令以确认网关可访问。 |
| `/new` | 开始新的对话。 |
| `/reset` | 重置对话历史记录。 |
| `/status` | 显示会话信息，随后是本地的**会话概要**板块（包括最近的轮次数、最常使用的工具、处理过的文件，以及最新的提示词和回复）。 |
| `/stop` | 终止所有正在运行的后台进程，并中断当前正在工作的机器人。 |
| `/model [provider:model]` | 显示或更改所使用的模型。支持切换提供商（如 `/model zai:glm-5`）、自定义端点（如 `/model custom:model`）、带名称的自定义提供商（如 `/model custom:local:qwen`）、自动检测模式（如 `/model custom`），以及用户自定义的别名（如 `/model fav`、`/model grok`——详见[自定义模型别名](#custom-model-aliases)）。使用 `--global` 可将更改永久保存到 `config.yaml` 中。**注意：** `/model` 仅能在已配置的提供商之间切换。若要添加新提供商或设置 API 密钥，需在终端（聊天会话之外）中使用 `hermes model` 命令。 |
| `/codex-runtime [auto\|codex_app_server\|on\|off]` | 切换可选的[Codex 应用服务器运行时模式](../user-guide/features/codex-app-server-runtime)。该设置会保存到 `config.yaml` 中的 `model.openai_runtime` 字段，并清除缓存中的机器人实例，以便下一条消息能使用新的运行时模式。更改会在下一次会话中生效。 |
...
| `/compress [here [N] \| focus topic]` | 手动压缩对话上下文。`/compress here [N]` 会保留最近 N 条对话内容（默认为 2 条）的原貌，并对其余内容进行总结。设置“聚焦主题”可进一步限定完整总结中需保留的内容范围。 |
| `/topic [off\|help\|session-id]` | **仅适用于 Telegram 私信。**用于管理用户自定义的多会话主题模式。`/topic` 可启用该模式或查看其状态；`/topic off` 可禁用该模式并清除所有绑定；`/topic help` 可查看使用说明；在主题模式下输入 `/topic <session-id>` 可恢复之前的会话。详见[多会话私信模式](/user-guide/messaging/telegram#multi-session-dm-mode-topic)。 |
| `/title [name]` | 设置或显示会话标题。 |
| `/resume [name]` | 恢复之前命名的会话。 |
| `/usage` | 显示令牌使用情况、预估成本构成（输入/输出）、上下文窗口状态、会话持续时间，以及——若当前提供商支持——包含从提供商 API 实时获取的剩余配额/积分的**账户限制**板块。 |
...
| `/queue <prompt>`（别名：`/q`）| 将某个提示词排队，等待在当前轮次之后处理，而不会中断当前正在进行的操作。 |
| `/steer <prompt>` | 在下一次工具调用之后插入一条消息，且不会中断当前流程——模型会在下一次迭代时处理该消息，而非视为新的轮次。 |
| `/goal <text>` | 设置一个长期目标，Hermes 会在多轮对话中持续努力实现该目标——这相当于我们实现的“Ralph 循环”。每次轮次结束后，会有一个判断模型进行检查；若目标尚未达成，Hermes 会自动继续尝试，直到目标达成、你手动暂停/清除该目标，或达到轮次预算上限（默认为 20 轮）。子命令包括：`/goal status`、`/goal pause`、`/goal resume`、`/goal clear`。在机器人运行过程中可安全地使用这些子命令来查看状态、暂停或清除目标；若要设置新目标，则需先执行 `/stop`。详见[持久目标](/user-guide/features/goals)。 |
| `/footer [on\|off\|status]` | 切换最终回复中是否显示运行时元数据页脚（包含模型名称、上下文占比及当前工作目录等信息）。 |
| `/curator [status\|run\|pin\|archive]` | 用于控制后台技能的维护操作。 |
| `/memory [pending\|approve\|reject\|approval]` | 查看由写入审批门控机制（`memory.write_approval`）暂存的待处理内存写入内容——可直接在聊天中批准或拒绝它们——同时可通过 `/memory approval on\|off` 切换该门控的开启状态。详见[控制内存写入](/user-guide/features/memory#controlling-memory-writes-write_approval)。 |
| `/skills [pending\|approve\|reject\|diff\|approval]` | 查看由写入审批门控机制（`skills.write_approval`）暂存的待处理**技能**写入内容。每条待处理写入会以一行简短摘要的形式显示；在聊天界面中，`/skills diff <id>` 的输出会被截断——完整差异信息可在 CLI 或 `~/.hermes/pending/skills/<id>.json` 文件中查看。该功能仅在门控开启或仍有待处理写入时显示；搜索/安装功能仍仅通过 CLI 实现。 |
| `/kanban <action>` | 通过聊天界面操作多角色、多项目的协作看板——其参数与 CLI 完全一致。由于无需经过正在运行的机器人，因此可在当前轮次中执行诸如 `/kanban unblock t_abc`、`/kanban comment t_abc "…"`、`/kanban list --mine`、`/kanban boards switch <slug>` 等操作。`/kanban create …` 会自动将发起聊天的窗口订阅到新任务的终端事件中。详见[看板斜杠命令](/user-guide/features/kanban#kanban-slash-command)。 |
| `/reload-mcp`（别名：`/reload_mcp`）| 根据配置重新加载 MCP 服务器。 |
| `/yolo` | 切换 YOLO 模式——跳过所有危险命令的审批提示。 |
| `/commands [page]` | 浏览所有命令和技能（支持分页）。 |
| `/approve [session\|always]` | 批准并执行某个待处理的危险命令。`session` 仅针对当前会话进行批准；`always` 会将该命令加入永久允许列表。 |
| `/deny` | 拒绝某个待处理的危险命令。 |
| `/update` | 将 Hermes Agent 更新到最新版本。 |
| `/restart` | 在终止所有正在运行的任务后，优雅地重启网关。当网关重新上线后，会向请求者的聊天窗口或线程发送确认消息。 |
...
- `/skin`、`/snapshot`、`/gquota`、`/reload`、`/tools`、`/toolsets`、`/browser`、`/config`、`/cron`、`/platforms`、`/paste`、`/image`、`/statusbar`、`/plugins`、`/busy`、`/indicator`、`/redraw`、`/clear`、`/history`、`/save`、`/copy`、`/handoff` 以及 `/quit` 均为**仅适用于 CLI**的命令。 |
- `/skills` 命令**仅用于在 CLI 中搜索、浏览或安装技能**；其写入审批相关的子命令（`pending`、`approve`、`reject`、`diff`、`approval`）在 `skills.write_approval` 开启时也可在消息平台中使用。`/memory` 命令则在这两种场景下均可用。 |
- `/verbose` 默认**仅适用于 CLI**，但若在 `config.yaml` 中设置 `display.tool_progress_command: true`，则可在消息平台上启用该功能。启用后，它会循环切换 `display.tool_progress` 的显示模式，并将设置保存到配置文件中。 |
- `/sethome`、`/update`、`/restart`、`/approve`、`/deny`、`/topic` 以及 `/commands` 均为**仅适用于消息传递平台**的命令。 |
- `/status`、`/version`、`/background`、`/queue`、`/steer`、`/voice`、`/reload-mcp`、`/reload-skills`、`/rollback`、`/debug`、`/fast`、`/footer`、`/curator`、`/kanban`、`/sessions` 以及 `/yolo` 命令在**CLI 和消息网关**上均可使用。 |
- `/voice join`、`/voice channel` 以及 `/voice leave` 命令仅在 Discord 上有效。 |
- 在 TUI 界面中，`/sessions` 会显示当前 TUI 进程中的活跃会话。若需查看已保存或已关闭的对话记录，可使用 `/resume [name]` 或 `hermes --tui --resume <id-or-title>` 命令。 |

## 破坏性命令的确认提示

在运行那些会丢弃未保存会话状态的斜杠命令之前，CLI 会先显示确认提示。目前支持的破坏性命令如下：

| 命令 | 会销毁的内容 |
|------|--------------|
| `/clear` | 清空屏幕并启动全新会话——当前的会话 ID 及内存中的历史记录都会被清除。 |
| `/new` / `/reset` | 启动全新会话（新的会话 ID + 空的历史记录）。 |
| `/undo` | 从历史记录中删除上一次用户与助手之间的对话内容。 |
| `/exit --delete` / `/quit --delete` | 退出程序，并**永久删除**当前会话的 SQLite 历史记录及磁盘上的对话记录文件。 |

对于上述每条命令，CLI 都会弹出三选一的模态窗口：**仅本次批准**（仅当前次操作继续执行）、**始终批准**（本次操作继续执行，并将 `approvals.destructive_slash_confirm` 的值设为 `false`，从而使后续的破坏性命令无需再次提示即可运行），或**取消**。

**直接跳过**：可在命令后追加 `now`、`--yes` 或 `-y`，从而跳过模态窗口，仅执行单次操作——例如 `/reset now`、`/new --yes my-session`、`/clear -y`、`/undo -y`。当终端上无法正确显示模态窗口时（参见[问题 #30768](https://github.com/NousResearch/hermes-agent/issues/30768)，尤其是 Windows PowerShell 环境），或需要在脚本中调用 CLI 时，此方法非常有用。

若要全局禁用这些确认提示，可在 `~/.hermes/config.yaml` 中将 `approvals.destructive_slash_confirm` 的值设为 `false`；如需重新启用，则将其改回 `true`。相关背景信息请参见[安全性——破坏性斜杠命令的确认机制](../user-guide/security.md#dangerous-command-approval)。
