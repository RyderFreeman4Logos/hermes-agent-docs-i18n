...
通过相互关联的Markdown文件来构建并维护一个持久且不断丰富的知识库。
该方案基于[Andrej Karpathy的LLM维基架构](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)设计。

与传统RAG系统（每次查询都需要从头重新检索知识）不同，该维基只需构建一次知识库即可保持内容最新。其中的交叉引用早已设置完成，矛盾之处也会被及时标记出来，所有收录的内容都会被综合呈现。

**分工协作：** 由人类负责筛选资料并指导分析方向；而智能体则负责总结内容、建立交叉引用、整理文件并确保整体一致性。

## 该技能的触发场景

当用户出现以下情况时，可使用此技能：
- 要求创建、搭建或启动一个维基或知识库
- 要求将新资料导入、添加到现有维基中或对其进行处理
- 提出问题且配置路径下已存在相关维基
...
**位置：** 通过`WIKI_PATH`环境变量设置（例如在`${HERMES_HOME:-~/.hermes}/.env`文件中）。
...
该维基实际上只是一个存储 Markdown 文件的目录——你可以在 Obsidian、VS Code 或任何编辑器中打开它。无需数据库，也不需要任何特殊工具。 
...
**第一层——原始数据源：**不可更改。智能体仅能读取这些数据，绝不能对其进行修改。
**第二层——维基页面：**由智能体管理的 Markdown 文件。这些文件由智能体创建、更新并建立相互引用关系。
**第三层——架构规范：**`SCHEMA.md` 文件用于定义数据结构、编写规范以及标签分类体系。

## 恢复现有的维基页面（非常重要——请在每次会话前执行）

如果用户已有现有维基页面，**在采取任何操作之前务必先了解其结构**：

① **阅读 `SCHEMA.md`** — 了解相关领域、编写规范以及标签分类体系。
② **阅读 `index.md`** — 了解现有的页面及其内容概要。
...
对于规模较大的维基（100页以上），在创建新内容之前，还应先针对当前主题快速执行`search_files`操作。
