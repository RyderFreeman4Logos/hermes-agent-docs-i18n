...
| **排版逻辑** | 从左上角到右下角（Z形排列）或分列式 |
| **文字要求** | 标题需在3米外仍可清晰阅读，正文则在1米外可读。不可使用完整段落，仅用项目符号 |
| **图表要求** | 使用更高分辨率的论文原有图表，重点结果可放大展示 |
...
| 报告类型 | 时长 | 内容要点 |
|----------|------|----------|
| **精选展示** | 5分钟 | 阐述问题、研究方法及一项关键结果。需严格控制在5分钟内完成 |
| **口头报告** | 15-20分钟 | 完整阐述：问题背景、研究方法、关键结果、消融实验及局限性分析 |
| **研讨会报告** | 10-15分钟 | 需根据研讨会受众调整内容，可能需要补充更多背景信息 |

**幻灯片设计规则**：
- 每张幻灯片仅呈现一个核心观点
- 尽量减少文字——详细内容需口头讲解而非展示在幻灯片中
- 通过动画逐步展示关键图表，帮助听众逐步理解
- 结尾添加一张“要点总结”幻灯片（用一句话概括论文贡献）
- 预留备用幻灯片，以应对可能出现的提问
...
- **Twitter/X系列推文**：5-8条推文。首条先介绍核心结果，而非研究方法。需包含图1及关键结果图表
- **博客文章**：字数在800-1500字之间。面向机器学习实践者撰写，而非审稿人。无需过多理论推导，重点阐述直观结论与实际应用价值
...
研讨会论文与短篇论文（如ACL短篇论文、Findings系列论文）遵循相同的流程，但存在不同的约束条件与期望标准。
...
|----------|------------|--------------|
| **页数限制** | 通常为4-6页 | 7-9页 |
| **评审标准** | 对完整性要求相对较低 | 需做到内容完整、论述详尽 |
| **评审流程** | 一般为单盲评审或简化版评审 | 双盲评审，流程更为严格 |
| **评价重点** | 新颖思路、初步结果或领域观点性文章 | 完整的实证研究，且需有强基准对比 |
| **arXiv提交时间** | 可随时提交 | 需注意时机（参见arXiv提交策略） |
| **贡献要求** | 具有新方向、有趣的负面结果或阶段性研究成果 | 需有重大进展，并有充分证据支撑 |

**何时选择投稿研讨会论文**：
- 拥有处于早期阶段的想法，希望在正式撰写完整论文前获得反馈
- 得到负面结果，但内容不足以支撑8页以上的篇幅
- 想针对某个热点话题发表观点性文章或立场声明
- 进行重复实验研究或可复现性报告撰写
...
| 类型 | 页数 | 预期内容 |
|------|------|----------|
| **长篇论文** | 8页 | 完整的研究内容，包含强基准对比及消融实验 |
| **短篇论文** | 4页 | 突出一个清晰的研究观点，并提供相应证据支撑 |
| **Findings系列论文** | 8页 | 研究质量优秀，但略未达到主流会议的要求 |

**短篇论文写作策略**：聚焦一个核心主张并予以充分论证。切勿试图将长篇论文的内容压缩到4页内——应另起一篇更具针对性的论文。

---

## 非实证机器学习类的论文类型

上述主要流程适用于实证机器学习论文。其他类型的论文则需要不同的结构与证据标准。关于各类论文的详细指导，请参阅[references/paper-types.md](references/paper-types.md)文档。

### 理论类论文

**标准结构**：引言 → 基础知识（定义、符号说明）→ 主要结果（定理）→ 证明概要 → 讨论 → 完整证明（附录）

**与实证论文的主要区别**：
- 论文贡献表现为定理、界或反例结果，而非实验数据
- 方法部分被“基础知识”与“主要结果”取代
- 证明是核心证据，而非实验数据（尽管对理论的实证验证也是可取的）
- 文本中提供证明概要，完整证明放在附录中，这是常见做法
- 实验部分为可选内容，但若能验证理论预测，则能显著增强论文说服力

**证明写作原则**：
- 以正式方式陈述定理，并明确列出所有假设条件
- 在给出正式证明前先阐述直观思路（“核心思想是……”）
- 证明概要应在0.5-1页内讲清核心逻辑
- 使用`\begin{proof}...\end{proof}`环境编写证明
- 为每个假设编号，并在定理中引用：“在假设1-3的前提下，……”

### 综述/教程类论文

**标准结构**：引言 → 分类/框架介绍 → 详细内容阐述 → 存在的开放问题 → 结论

**主要区别**：
- 论文贡献在于对现有研究的系统整理、综合分析以及开放问题的识别，而非新方法的提出
- 需在给定范围内做到内容全面（审稿人会检查是否有遗漏的参考文献）
- 需有清晰的分类体系或结构框架
| 价值来源 | 说明 |
|----------|------|
| 理解不同研究之间的关联 | 单篇论文往往难以展现这些联系 |
| 提供系统性视角 | 帮助读者把握领域全貌 |

| 最佳投稿期刊/会议 | 说明 |
|------------------|------|
| TMLR的综述专栏 | 专门收录高质量综述文章 |
| JMLR | 机器学习领域顶级期刊，接受综述类论文 |
| Foundations and Trends in ML | 专注于机器学习领域前沿发展的系列丛书 |
| ACM Computing Surveys | 计算机科学领域权威综述期刊 |

### 基准测试类论文

**标准结构**：引言 → 任务定义 → 数据集构建 → 基准方法评估 → 结果分析 → 该基准的预期用途与局限性

**主要区别**：
- 论文贡献在于基准测试工具本身——它必须填补真实的评估空白
| 要求 | 说明 |
|------|------|
| 数据集文档 | 是强制要求，而非可选内容（参见Step 5.11中的数据集模板） | 
| 基准难度 | 需证明该基准具有足够挑战性，避免基准方法能轻松达到理想分数 |
| 测量有效性 | 需证明该基准确实能衡量其所宣称的指标（即结构效度） | 

| 最佳投稿期刊/会议 | 说明 |
|------------------|------|
| NeurIPS的“数据集与基准测试”专栏 | 机器学习领域顶级会议，专门收录此类论文 |
| ACL的“资源类论文”栏目 | 也接受此类实用工具类论文 |
| LREC-COLING | 计算语言学领域相关会议 |

### 观点性论文

**标准结构**：引言 → 背景介绍 → 核心论点 → 支持证据 → 反驳观点 → 后续影响分析
...
| 最佳投稿期刊/会议 | 说明 |
|------------------|------|
| ICML的“观点类论文”专栏 | 机器学习领域顶级会议，接受此类文章 |
| 各类研讨会 | 也是发表观点性文章的常见平台 |
| TMLR | 同样适合此类主题的文章 |
...
该技能专为Hermes智能体设计，能够利用Hermes提供的工具、任务委派、调度及记忆功能，全程协助完成研究工作。

### 相关技能

可结合其他Hermes技能，针对不同研究阶段使用本技能：
...
| **qmd** | 第1阶段（文献调研）：通过混合BM25+向量搜索方式，检索本地知识库中的笔记、会议记录及文档资料 | 需先执行`skill_manage("install", "qmd")`进行安装 |
| **diagramming** | 第4-5阶段：用于创建基于Excalidraw的图表及系统架构图 | `skill_view("diagramming")` |
| **data-science** | 第4阶段（数据分析）：提供Jupyter实时内核，支持交互式分析与可视化操作 | `skill_view("data-science")` |
...
| **`process`** | 用于管理后台运行的实验任务：可执行`process("start", ...)`启动任务，`process("poll", pid)`轮询任务状态，`process("log", pid)`查看任务日志，`process("kill", pid)`终止任务 |
| **`execute_code`** | 用于运行Python代码，实现引文验证、统计分析及数据聚合等功能。该工具可通过RPC方式调用其他外部程序 |
| **`read_file`** / **`write_file`** / **`patch`** | 用于编辑论文内容、实验脚本及结果文件。对于大型.tex文件，可使用`patch`功能进行精准修改 |
| **`web_search`** | 用于文献检索，例如可输入“transformer attention mechanism 2024”进行搜索 |
| **`web_extract`** | 用于获取论文内容、验证引文信息，例如可通过该工具提取`https://arxiv.org/abs/2303.17651`的完整内容 |
| **`delegate_task`** | 用于实现分模块并行撰写——可为每个章节单独创建子智能体，同时也可用于并行进行引文验证任务 |
| **`todo`** | 用于跨会话跟踪任务状态。在每个研究阶段结束后需更新该任务列表 |
| **`memory`** | 用于在多次会话之间保留关键决策信息，如论文贡献方向、目标会议选择、审稿人反馈等 |
| **`cronjob`** | 用于安排实验监控、截止日期提醒以及自动检查arXiv上相关论文更新的情况 |
| **`clarify`** | 当遇到瓶颈时（如会议选择、论文贡献定位等），可向用户提出针对性问题以获取指导 |
