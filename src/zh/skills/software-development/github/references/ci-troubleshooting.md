...
4. 检查是否存在 `ModuleNotFoundError` —— 通常是由于 CI 环境中缺少依赖导致的。

**常见解决方案：**
- 更新断言内容，使其符合新的预期行为。
- 在 `requirements.txt` 或 `pyproject.toml` 中添加缺失的依赖项。
- 修复偶发故障的测试用例（例如增加重试机制、模拟外部服务响应、解决竞态条件问题）。
...
2. 对比本地环境与 CI 环境中的 Python/Node 版本差异。
...
2. 检查是否已配置机密信息（缺少 `GITHUB_TOKEN` 或自定义机密）。

**常见解决方案：**
- 在工作流的 YAML 文件中添加 `permissions:` 块。
- 验证机密信息是否存在：可使用 `gh secret list` 命令或查看仓库设置。
- 对于 fork 的拉取请求：部分机密信息按设计原则不可使用。
