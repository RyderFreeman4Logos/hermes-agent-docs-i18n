# GitHub REST API 快速参考指南

基础网址：`https://api.github.com`

所有请求均需添加以下头部信息：`-H "Authorization: token $GITHUB_TOKEN"`

可使用 `gh-env.sh` 辅助工具自动设置 `$GITHUB_TOKEN`、`$\GH_OWNER` 和 `$GH_REPO` 的值：
```bash
...
| 请求审核人 | POST | `/repos/{owner}/{repo}/pulls/{number}/requested_reviewers` |
| 创建代码审查 | POST | `/repos/{owner}/{repo}/pulls/{number}/reviews` |
| 添加内联评论 | POST | `/repos/{owner}/{repo}/pulls/{number}/comments` |
...
| 列出所有标签 | GET | `/repos/{owner}/{repo}/labels` |
| 搜索问题 | GET | `/search/issues?q={query}+repo:{owner}/{repo}` |

注意：Issues API 也会返回 PR 相关信息。解析数据时，请使用 `"pull_request" not in item` 进行过滤。
...
| 获取特定运行记录 | GET | `/repos/{owner}/{repo}/actions/runs/{run_id}` |
...
| 触发任务分发 | POST | `/repos/{owner}/{repo}/actions/workflows/{id}/dispatches` |
| 获取提交状态 | GET | `/repos/{owner}/{repo}/commits/{sha}/status` |
| 检查提交对应的运行记录 | GET | `/repos/{owner}/{repo}/commits/{sha}/check-runs` |

## 版本发布

| 操作 | 方法 | 接口地址 |
|------|------|----------|
| 列出版本发布记录 | GET | `/repos/{owner}/{repo}/releases` |
| 创建版本发布 | POST | `/repos/{owner}/{repo}/releases` |
| 获取特定版本发布记录 | GET | `/repos/{owner}/{repo}/releases/{id}` |
| 删除版本发布记录 | DELETE | `/repos/{owner}/{repo}/releases/{id}` |
...
| 操作 | 方法 | 端点 |
|------|------|------|
| 列出机密信息 | GET | `/repos/{owner}/{repo}/actions/secrets` |
| 获取公钥 | GET | `/repos/{owner}/{repo}/actions/secrets/public-key` |
| 设置机密信息 | PUT | `/repos/{owner}/{repo}/actions/secrets/{name}` |
| 删除机密信息 | DELETE | `/repos/{owner}/{repo}/actions/secrets/{name}` |
...
- `?page=2` 用于翻到下一页
| 可通过 `Link` 请求头中的 `rel="next"` 字段获取下一页地址 |
