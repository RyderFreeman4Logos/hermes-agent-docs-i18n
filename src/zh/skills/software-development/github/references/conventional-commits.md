...
| 类型 | 适用场景 | 示例 |
|------|----------|-------|
| `feat` | 新功能或新能力添加 | `feat(auth): 添加 OAuth2 登录流程` |
...
| `ci` | CI/CD 配置变更 | `ci: 在测试矩阵中加入 Python 3.12 版本` |
...
| `build` | 构建系统或外部依赖变更 | `build: 从 setuptools 更改为 hatch` |
| `revert` | 撤销之前的提交 | `revert: 撤销 "feat(auth): 添加 OAuth2 登录流程" 提交` |
...
可在类型后添加 `!` 符号，或在文档底部标注 `BREAKING CHANGE:`：
