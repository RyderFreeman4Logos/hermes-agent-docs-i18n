...
在代码正式提交之前，通过自动化流程进行验证。该流程包括静态扫描、基于基线设定的质量检查点、独立的审查子智能体，以及自动修复机制。

**核心原则：** 任何智能体都不得自行验证自己的工作成果。通过全新的上下文视角，能够发现你可能遗漏的问题。

## 适用场景

- 在实现功能或修复错误后，在执行 `git commit` 或 `git push` 之前
- 当用户要求“提交”、“推送”、“发布”、“完成”、“验证”或“合并前审查”时
- 在 Git 仓库中对多个文件进行修改并完成任务之后
- 在子智能体驱动的开发流程（两阶段审查）中的每项任务完成后

**无需使用的情况：** 仅涉及文档修改、纯配置调整，或用户明确要求“跳过验证”时。

**本技能与 github-code-review 的区别：** 本技能用于在提交前验证**你自己的代码更改**；而 `github-code-review` 则用于在 GitHub 上审查**他人提交的 Pull Request**，并附上内联评论。
