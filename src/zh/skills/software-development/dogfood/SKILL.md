...
用户需提供以下信息：
1. **目标网址**——测试的入口地址
2. **测试范围**——需要重点检测的区域/功能（或选择“全站”以进行全面测试）
...
2. 通过**生成快照**来了解 DOM 结构：
   ```
   browser_snapshot()
   ```

3. 检查控制台中的 JavaScript 错误：
   ```
   browser_console(clear=true)
   ```
每次页面跳转以及发生任何重要交互后都应执行此操作。隐性的 JavaScript 错误属于极具价值的发现。

4. **截取带注释的屏幕截图**，以便直观地查看页面内容并识别其中的交互元素：
   ```
   browser_vision(question="Describe the page layout, identify any visual issues, broken elements, or accessibility concerns", annotate=true)
   ```
`annotate=true` 参数会在交互元素上叠加编号为 `[N]` 的标签。每个 `[N]` 都会对应一个引用 `@eN`，以便后续在浏览器中执行命令。
...
| 工具 | 用途 |
|------|------|
| `browser_navigate` | 导航至指定网址 |
| `browser_snapshot` | 获取DOM文本快照（包括无障碍树结构） |
...
| `browser_vision` | 截图并配合AI进行分析；如需标注元素，可使用 `annotate=true` 参数 |
| `browser_console` | 获取JavaScript控制台的输出和错误信息 |

## 实用建议

- **在导航完成后以及进行重要操作后，务必检查 `browser_console()` 的输出。** 那些不会引发明显错误的JavaScript错误往往是极具价值的发现。
- 当需要确定交互元素的位置，或快照中的引用信息不清晰时，可使用 `browser_vision` 并设置 `annotate=true` 参数。
- **同时使用有效和无效的输入数据进行测试**——表单验证相关的缺陷十分常见。
- **滚动查看长页面**——页面下方的内容可能存在渲染问题。
- **对导航流程进行端到端测试**——完整点击完成多步骤操作。
- **通过截图检查响应式表现**，留意其中出现的布局问题。
- **不要忽略边缘情况**：空状态、过长的文本、特殊字符、快速连续点击等。
- 在向用户展示截图时，请使用 `MEDIA:<screenshot_path>` 格式，这样他们就能在报告中直接查看相关证据。
