...
**重要提示：** `uTD2DInfos` 需要输入纹理数据。若使用独立着色器，则应选用 `uTDOutputInfo`。
...
注意：MAT 使用 `vdat`/`pdat`，而 TOP 则使用 `vertexdat`/`pixeldat`。
...
这三种工具均采用计算着色器。已嵌入的DAT文件遵循以下命名规则：
