...
即使不使用 `loadFont()`，Google Fonts 也能在 `text()` 函数中正常工作，但无法用于 `textToPoints()` 函数。对于粒子文字效果，必须使用 OTF/TTF 格式的字体文件并通过 `loadFont()` 进行加载。
...
## textToPoints() — 将文本转换为粒子

用于将文本轮廓转换成点数组。此功能需要先加载字体（可通过 `loadFont()` 方法加载 OTF/TTF 格式字体）。
