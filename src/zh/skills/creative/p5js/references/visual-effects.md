...
缩放因子（如 0.005 等）至关重要：
- `0.001` — 效果极为平滑，适合表现大型特征
- `0.005` — 平滑度适中，适合表现中等大小的特征
- `0.01` — 标准的生成艺术缩放比例
- `0.05` — 细节丰富，适合表现小型特征
- `0.1` — 几乎随机分布，呈现颗粒感
...
通过多层噪声叠加来打造自然逼真的纹理效果。每增加一层，就会在更小的尺度上增添细节。
...
一种无散度噪声场。遵循漩涡噪声引导的粒子既不会聚拢也不会分散，而是以平滑的旋动轨迹流动。
...
将该场景下采样至较小的缓冲区中，对其应用模糊处理，并以叠加方式呈现。此效果可在高亮度区域周围生成柔和的光晕，是标准的生成艺术中的光晕技术。
...
**参数调整：**
- 下采样比例（标准值为1/4，1/8可让效果更柔和，1/2则会使效果更紧密）
- 模糊半径（通常为4-8，数值越大，光晕范围越广）
- 色调透明度（范围为40-120，用于控制光晕强度）
- 为提升性能，可每隔N帧更新一次光晕效果：`if (frameCount % 2 === 0) { ... }`

**常见错误：** 在执行ADD操作后忘记调用`blendMode(BLEND)`——此后绘制的所有内容都将采用叠加模式。

### 粒子轨迹的亮度问题

通过`createGraphics()`结合半透明渐变矩形来积累粒子轨迹，是实现粒子拖尾效果的标准方法，但**实际生成的轨迹往往比预期要暗**。因为渐变矩形的透明度值会在每一帧中以乘法方式累积。
...
**隐患所在：** 您为较长的轨迹设置了 alpha 值为 5，但由于粒子在达到足够密度之前就会逐渐消失，因此 alpha 值为 30 的粒子轨迹根本不可见。此时可采取以下方案之一：
- 将轨迹的 alpha 值提升至 80-150（而非人们通常认为的 20-40）；
- 降低消失效果的 alpha 值，但相应地接受更短的轨迹长度；
- 对粒子轨迹使用叠加混合模式：亮色粒子会逐渐变亮并累积，而暗色粒子则保持黑暗状态。
...
能够实现自然且均匀的分布——在放置元素时，其效果优于纯粹的随机方式。
...
呈现手绘般的自然美学效果，支持水彩、炭笔、钢笔及记号笔等绘画风格。需使用 **p5.js 2.x + WEBGL** 环境。
...
内置画笔：`2B`、`HB`、`2H`、`cpencil`、`pen`、`rotring`、`spray`、`marker`、`charcoal`、`hatch_brush`。
内置矢量场：`hand`、`curved`、`zigzag`、`waves`、`seabed`、`spiral`、`columns`。
...
无论实际渲染速度如何，都能以固定的帧率录制画布内容。对于复杂的生成式艺术创作而言，这一功能不可或缺。
