...
用于生成视觉图案的效果构建模块。在 v2 版本中，这些模块被应用于**直接返回像素画布的场景函数**内部。以下构建模块会对网格坐标数组进行处理，并生成 `(字符, 颜色)` 或数值/色调字段，随后由场景函数通过 `_render_vf()` 函数将其渲染到画布上。

> **另请参阅：** architecture.md · composition.md · scenes.md · shaders.md · troubleshooting.md

## 设计理念

效果是创意的核心。无需在每个项目中逐字复制现有效果——应将其视为**构建模块**，通过组合、修改和创新来打造全新的效果。每个项目都应具有独特的风格。

核心原则：
- **叠加多个效果**，而非使用单一的庞大函数
- **对所有参数进行量化设置**——色调、速度、密度、振幅等都应作为函数参数
- **根据内容动态响应**——音频/视频特征应至少能调节每个效果的 2-3 个参数
- **按不同片段调整效果**——切勿在整个视频中始终使用相同的效果配置
- **创造针对特定项目的效果**——下方的目录仅作为参考词汇表，并非固定列表
...
> **注意：** v1版本中的`eff_rings`、`eff_rays`、`eff_spiral`、`eff_glow`、`eff_tunnel`、`eff_vortex`、`eff_freq_waves`、`eff_interference`、`eff_aurora`以及`eff_ripple`这些函数已被下述的`vf_*`值字段生成器所取代（通过 `_render_vf()` 函数调用）。`vf_*`版本可与多网格合成流程无缝集成，因此所有新场景都建议优先使用该版本。
...
基于分离、对齐与凝聚这三条简单规则，从而产生出复杂的群体行为。
...
这类粒子会跟随数值场的梯度移动。任何 `vf_*` 函数都会变成一条承载粒子的“河流”：
...
真正的创造力源自**组合**。该系统包含三个层级：

### 第一层级：字符级叠加

将多个效果以 `(字符, 颜色)` 的形式进行叠加：
...
在渲染到画布后，可使用类似Photoshop的混合模式进行混合处理：
...
在 v2 协议中，效果节点被用于场景函数**内部**。场景函数本身会返回一个画布，而效果节点则负责生成中间格式的 `(字符, 颜色)` 数据，这些数据随后会通过网格的 `.render()` 方法或 `_render_vf()` 方法被渲染到画布上。
...
这类生成器可创建范围在 [0,1] 之间的浮点型数组 `(rows, cols)`，它们代表了最原始的视觉图案。所有此类生成器的函数签名均为 `(g, f, t, S, **params) -> float32 array`。
...
这类场能够生成与基于正弦波的场截然不同的纹理——具有有机感、无重复规律，且不存在明显的轴向对齐特征。它们正是高端生成艺术创作的基础。
...
这类场通过使用持久状态 `S`，逐帧演化出各种图案。它们能够生成无状态数学模型无法实现的复杂结构。
...
有符号距离场能够生成数学上极为精确的形状。与具有有机感且边缘模糊的正弦场不同，SDF能够提供清晰的几何边界，并且其边缘柔和度也是可调控的。结合域变形技术后，它们还能实现“熔化几何”效果。

所有的SDF图元都会返回一个**有符号距离值**（内部为负，外部为正）。可通过`sdf_render()`函数将其转换为数值场。
...
这类生成器可输出范围为 [0,1] 的 float32 类型色相数组，且能够与任意数值字段独立组合使用。每个生成器都相当于一个工厂函数，会返回一个签名为 `(g, f, t, S) -> float32 array` 的闭包函数；对于需要固定色相的情况，也可直接使用普通浮点数值。
...
在效果计算**之前**应用的 UV 空间变换。任何 `vf_*` 函数都可以通过对其所处理的网格坐标进行变换，从而实现旋转、缩放、平铺或变形等操作。
...
转换功能用于修改值字段所获取的坐标数据。需将转换操作包裹在 `vf_*` 调用之中：
...
用于实现平滑、可控的参数随时间变化的功能，可替代默认的静态参数或直接响应原始音频的模式。

### 缓动函数

标准的动画缓动曲线。所有函数均以 [0,1] 范围内的 `t` 值作为输入，并返回 [0,1] 范围内的数值：
...
可在特定时间点定义参数值，并通过平滑过渡函数在这些值之间进行插值：
...
在 `(x, y, t)` 点采样的 3D 噪声——其模式会随时间平滑演变，不会出现逐帧不连续的情况：
...
为获得最丰富的视觉效果，可针对每个区域调整**数值字段组合**、**色调字段**、**调色板**、**混合模式**、**反馈配置**以及**着色器链**。由于拥有12种数值字段 × 8种色调字段 × 14种调色板 × 20种混合模式 × 7种反馈变换方式 × 38种着色器，其组合数量实际上几乎是无限的。

---

## 效果组合——创意指南

上述列表仅是基础元素，以下将介绍如何将这些元素巧妙组合，打造出富有设计感的视觉效果。

### 通过分层营造深度感
每个场景都应至少包含两个网格密度不同的图层：
- **背景层**（sm或xs）：使用密集且低饱和度的纹理，避免画面呈现纯黑色。可在亮度较低时（bri=0.15-0.25）采用fBM、平滑噪声或域变形效果。
- **内容层**（md）：即主要视觉元素——环状结构、沃罗诺伊图、螺旋形、隧道等，需设置为全亮度。
- **强调层**（lg或xl）：用于添加稀疏的高光效果——粒子、文字模板、发光脉冲等，通过屏幕混合方式叠加在顶层。

### 经典的效果组合方案
| 组合 | 混合模式 | 效果原理 |
|------|---------|---------|
| fBM + 沃罗诺伊边缘 | `screen` | 有机质感填充单元格，边缘则增添结构感 |
| 域变形 + 等离体 | `difference` | 创造出迷幻的有机干涉效果 |
| 隧道 + 漩涡 | `screen` | 营造深度透视感的同时体现旋转能量 |
| 螺旋 + 干涉纹 | `exclusion` | 产生由不同空间频率带来的莫尔纹图案 |
| 反应-扩散算法 + 火焰 | `add` | 结合有机的生命感基底与动态的前景元素 |
| SDF几何体 + 域变形 | `screen` | 使简洁的几何形状漂浮在有机纹理之中 |

### 将效果用作遮罩
通过`mask_from_vf()`函数，任何数值字段均可作为其他效果的遮罩：
- 用沃罗诺伊单元格遮罩火焰效果（仅单元格内部可见火焰）
- 用fBM遮罩纯色图层（形成有机色的云状图案）
- 用SDF形状遮罩反应-扩散场
- 通过动画化的渐变过渡，让一个效果逐渐显现于另一个效果之上

### 创造全新效果
针对每个项目，至少设计一种目录中不存在的独特效果：
- **结合两个vf_*函数并运用数学运算**：`np.clip(vf_fbm(...) * vf_rings(...), 0, 1)`
- **在计算前应用坐标变换**：`vf_plasma(twisted_grid, ...)`
- **利用一个字段来调节另一个字段的参数**：`vf_spiral(..., tightness=2 + vf_fbm(...) * 5)`
- **叠加时间偏移**：分别在时间`t`和`t - 0.5`渲染同一字段，再通过差值混合生成运动轨迹
- **通过SDF边界反射数值字段**，打造万花筒般的几何图案 |
