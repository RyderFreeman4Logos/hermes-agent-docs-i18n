...
2. 安装VS Code扩展：James Yu开发的**LaTeX Workshop**
   - 打开VS Code → 进入“扩展”菜单（Cmd/Ctrl+Shift+X）→ 搜索“LaTeX Workshop”→ 点击安装

**使用方法：**
- 在VS Code中打开任意`.tex`文件
- 保存文件（Cmd/Ctrl+S）→ 系统会自动将其编译为PDF
- 点击绿色播放按钮或使用`Cmd/Ctrl+Alt+B`来执行编译
- 查看PDF：点击“查看LaTeX PDF”图标或使用`Cmd/Ctrl+Alt+V`
- 并排查看文档：先按`Cmd/Ctrl+Alt+V`，再拖动标签页

**相关设置**（需添加到VS Code的`settings.json`文件中）：
```json
{
...
| **Sublime Text** | LaTeXTools | 流行且维护良好 |
| **Vim/Neovim** | VimTeX | 功能强大，依赖键盘操作 |
| **Emacs** | AUCTeX | 功能完备的 LaTeX 开发环境 |
| **Texstudio** | 内置功能 | 专用的 LaTeX IDE |
...
2. **参考文献格式错误**：请使用提供的`.bst`文件，并通过`\bibliographystyle{}`指令指定格式
