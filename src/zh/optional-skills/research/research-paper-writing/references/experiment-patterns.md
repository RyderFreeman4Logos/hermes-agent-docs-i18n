...
这些模式与最佳实践源于使用 Hermes Agent 进行大规模研究实验的实践经验，涵盖了实验基础设施、评估方案、监控以及故障恢复等方面。
...
每个实验脚本都应在完成每一项任务后保存结果，以便在程序重启时跳过已处理过的任务：
...
该模式能够确保重复运行既安全又高效。如果某个流程在处理到第 47/150 步时发生崩溃，重新启动时将跳过前 46 步。

**2. 输出结果保留**

需保存所有中间输出，而不仅仅是最终结果。这样一来，无需再次运行即可进行事后分析：
...
将生成、评估和可视化操作分别放在不同的脚本中：

| 脚本名称 | 功能说明 |
|--------|---------|
| `run_experiment.py` | 核心方法执行 |
| `run_baselines.py` | 在相同计算条件下进行基线对比 |
| `run_eval.py` | 盲评/专家评审组评估 |
...
- 采用**奇数位数的评审员团队**（3位、5位或7位）以解决平局问题
- 采取**保守型平局处理机制**：由原结果/基准结果胜出（从而避免误报）
- CoT评审员的效率约为非CoT评审员的40%（1名CoT评审员相当于3名普通评审员）

### 代码/目标评估

针对具有真实答案可供评估的任务（如代码、数学题、事实类问题）：
...
比较不同方法时，需确保其使用的计算资源相同。如果某方法需要执行 N 次 API 调用，其他基准方法也会被分配 N 次调用机会：
...
| N 种方法最佳选择 | 6 次调用 | 进行 6 次独立生成，最终在公开测试中选出最优结果 |

### 人工评估设计

许多机器学习/自然语言处理论文都需要进行人工评估，尤其是那些涉及主观任务的场景（如文本生成、摘要生成、对话系统、创意写作）。设计不合理的人工评估往往是论文被拒的原因之一。

#### 需要人工评估的场景

| 任务类型 | 是否需要 | 备注 |
|--------|----------|------|
| 开放式文本生成 | 是 | 仅依靠大语言模型作为评估者并不足以让论文在 ACL/EMNLP 等会议上被接受 |
| 摘要生成 | 通常需要 | 至少需要对部分输出结果进行人工评估 |
| 对话系统 | 是 | 需要通过用户测试或人工标注来进行评估 |
...
| 维度 | 定义 | 评分标准 |
|-----------|------|----------|
| 流畅性 | 语法正确性及表达自然度 | 1-5 分李克特量表 |
| 相关性 | 是否针对任务要求作答 | 1-5 分李克特量表 |
| 事实准确性 | 所述事实是否正确 | 二元制或 1-5 分 |
| 连贯性 | 逻辑清晰度与一致性 | 1-5 分李克特量表 |
| 信息量 | 是否提供有用信息 | 1-5 分李克特量表 |
| 整体偏好 | 哪种输出更优？ | A/B/平局（两两对比） |

**两两对比法**（相较于绝对评分更为可靠）：
- 将两种输出并排展示（随机调整左右顺序）
- 提问：“哪个更好？A / B / 平局”
- 这种方法更具区分度，且不易受标注者评分标准变化的影响

#### 标注者间一致性评估

务必报告一致性指标。若未提供，审核人员会认为您的标注结果不可靠。
...
| Cohen's kappa | 2名评估人员，分类数据 | 必须为2人 | 名义/有序 |
| Fleiss' kappa | 3名及以上评估人员，分类数据 | 3名及以上 | 名义 |
| Pearson/Spearman | 连续得分数据 | 2人 | 区间/比率 |

#### 流量众包平台

| 平台 | 最佳适用场景 | 成本 | 质量水平 |
|------|--------------|------|----------|
| **Prolific** | 学术研究，高质量需求 | 每小时8-15美元 | 高 —— 拥有优质的学术参与者群体 |
| **MTurk** | 大规模任务，需快速完成 | 每小时2-10美元 | 不稳定 —— 需根据评估人员资质判断 |
| **Surge AI** | 专注于自然语言处理领域的标注任务 | 高级套餐价格 | 高 —— 由经过培训的标注人员完成 |
| **专业标注人员** | 领域特定任务（如医疗、法律领域） | 成本最高 | 质量最高 —— 但处理速度较慢 |

**伦理要求**：
- 需说明报酬标准（至少需达到当地最低工资标准）
- 如有必要，需描述评估人员的基本信息
- 若所在机构有要求，需获得机构审查委员会或相关伦理机构的批准
- ACL相关会议明确要求提供报酬证明文件
...
3. **统计已完成与预期数量**——“已完成45/150个问题”比“存在部分结果”更具参考价值。
4. **以结构化表格呈现报告**——务必将关键指标放入表格中。
5. **明确核心分析目标**——每个实验在完成后都应能回答一个具体的分析问题。
6. **无变化时静默处理**——若未发生任何变化，则抑制通知推送。
7. **完成即提交**——每个已完成的任务批次都应附带描述性信息后进行提交。
...
| **并行处理变慢** | 每次实验的耗时增加一倍 | 将并行实验数量控制在2-3个以内 |
| **安全扫描拦截** | 命令被安全机制阻止执行 | 改用`execute_code`函数，而非通过管道传递终端命令 |
| **任务委托失败** | `delegate_task`函数返回错误 | 回退为直接执行任务 |
| **复杂问题超时** | 进程卡住且无日志进度更新 | 终止进程，跳过该问题，并在结果中记录说明 |
| **数据集路径错误** | 出现文件未找到的错误 | 在启动任务前确认路径正确性 |

### 重试命名规范

在重新运行失败的实验时，可通过添加后缀来标识不同的尝试轮次：
...
受限任务用于检测方法是否遵循范围边界。设计时应遵循以下原则：

- **固定事实**：“仅使用这N个数据点，不得添加其他内容”
- **固定交付物格式**：明确指定格式类型（如报告、事后分析、备忘录——而非“改进此内容”）
- **固定结构**：“按此顺序排列这些章节，不得增减”
- **固定修改项**：“仅解决这N个具体问题，不得涉及其他内容”

**切勿将字数作为范围约束条件**。字数限制会导致错误的“收敛”现象——输出被拒的原因往往是长度而非质量。应约束的是内容范围（包含什么），而非长度。

### 示例：合理与不合理的约束对比

| 不合理的约束 | 原因 | 合理的约束 |
|--------------|------|------------|
| “最多500字” | 评审者会以字数过多为由拒绝 | “恰好4个章节，每个章节包含3个编号项” |
| “要简洁” | 过于模糊 | “每条限制要求都必须对应某个具体的基础事实” |
| “改进此内容” | 范围无限制 | “按照此固定结构撰写一篇600字的事件事后分析报告” |
| “让它更好” | 没有明确标准 | “仅解决这3个评审方的意见” |
...
| 使用场景 | figsize | 备注 |
|----------|---------|-------|
| 单栏 | `(3.5, 2.5)` | 适用于双栏布局中的单栏区域 |
| 双栏 | `(7.0, 3.0)` | 覆盖整个页面宽度 |
| 正方形（热力图、混淆矩阵） | `(3.5, 3.5)` | 单栏显示 |
| 高型单栏（多行数据） | `(3.5, 5.0)` | 建议谨慎使用 |

### 适合色盲人群的配色方案（Okabe-Ito方案）

所有论文中的图表均建议使用此配色方案。它能够被所有常见类型的色觉缺陷者准确区分：
...
此外，还应当通过**标记符号与线条样式**来区分不同线路，而不仅仅依靠颜色：
```python
STYLES = [
...
- **始终以 PDF 格式保存**：`fig.savefig('fig.pdf')` —— 向量图形，无论放大多少倍都清晰锐利
...
| 消融实验分析 | 叠加/分组柱状图 | 需突出显示被移除的组件 |
| 趋势/收敛情况 | 迭代过程中的折线图 | 需展示每次迭代中的最优结果 |
| 各任务的表现差异 | 热力图或分组柱状图 | 需显示不同任务间的差异程度 |
