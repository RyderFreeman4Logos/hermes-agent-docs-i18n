...
| API | 覆盖范围 | 速率限制 | 最佳适用场景 |
|-----|----------|-------------|----------|
| **Semantic Scholar** | 2.14亿篇论文 | 免费密钥：1次/秒 | 机器学习/AI相关论文、引文图谱 |
| **CrossRef** | 1.4亿多个DOI | 支持邮件通知的温和限流机制 | DOI查询、BibTeX格式文献检索 |
| **arXiv** | 预印本资源 | 3秒延迟 | 机器学习领域预印本、PDF文档获取 |
| **OpenAlex** | 2.4亿多篇学术作品 | 每日10万次请求，10次/秒 | MAG的开放源代码替代方案 |
...
Google Scholar 并未提供官方 API。通过抓取数据的行为违反了其服务条款。仅当 Semantic Scholar 的覆盖范围不足时，方可考虑使用 SerpApi（费用为每月 75 至 275 美元）。
...
以统一的关键字格式，将经过验证的条目添加到您的 .bib 文件中：
...
**建议**：在提交会议论文时，建议搭配 BibTeX 使用 natbib——所有主流会议模板（NeurIPS、ICML、ICLR、ACL、AAAI、COLM）均预置了 natbib 及对应的 `.bst` 文件。对于由您自行控制模板的期刊文章或个人项目，可选择搭配 Biber 的 BibLaTeX。
...
**问题：Semantic Scholar 无结果返回**
- 尝试使用更具体的关键词
- 检查作者姓名的拼写
- 对完整短语使用引号

**问题：DOI 无法转换为 BibTeX 格式**
- DOI 可能已注册但未与 CrossRef 关联
...
- [ ] 引用类型正确（@inproceedings 或 @article）
- [ ] 作者姓名完整且格式正确
- [ ] 年份和会议信息已核实
- [ ] 引用键的格式一致
...
- `habanero`（用于 CrossRef）：https://github.com/sckott/habanero
