...
本指南旨在帮助作者撰写非标准类型的论文：理论论文、综述/教程论文、基准/数据集论文以及立场论文。不同类型的论文在结构、证据要求以及目标发表平台方面各有特点。
...
- [可复现性与重复实验论文](#reproducibility-and-replication-papers)
...
- 与已知界进行对比：“相比[先前研究]中 O(·) 的界，该结果提升了……倍”

### 证明概要

在理论论文的正文中，证明概要是最为重要的部分。审稿人会通过它来判断作者是否具备真正的洞察力，还是仅进行了机械式的推导。
...
**有缺陷的证明草稿**：仅用略有不同的符号重新表述定理，或直接声称“该证明采用了标准方法”。 
...
| 误区 | 问题 | 解决方案 |
|---------|------|----------|
| 假设过于强硬 | 使研究成果显得过于简单 | 明确哪些假设是必要的；证明下界 |
| 未与现有界限进行对比 | 审稿人无法评估贡献价值 | 添加界限对比表 |
| 证明概要只是完整证明的简化版 | 无法体现核心思想 | 重点阐述1-2个关键观点，将具体推导过程放在附录中 |
| 无实验验证 | 审稿人会对实际应用价值产生质疑 | 增加合成实验来验证预测结果 |
| 符号使用不一致 | 令审稿人感到困惑 | 在“预备知识”部分列出符号说明表 |
| 存在简单证明时却采用过于复杂的证明方式 | 审稿人会怀疑其中存在错误 | 相比通用性，应更注重清晰性 |

### 理论论文的发表平台

| 发表平台 | 理论类论文接受率 | 备注 |
|---------|------------------|------|
| **NeurIPS** | 中等 | 偏好具有实际应用价值的理论研究 |
| **ICML** | 高 | 拥有强大的理论研究板块 |
| **ICLR** | 中等 | 更青睐经过实证验证的理论研究 |
| **COLT** | 高 | 以理论研究为主的会议 |
| **ALT** | 高 | 面向算法学习理论的会议 |
| **STOC/FOCS** | 适用于具有计算机科学特色的研究成果 | 若研究内容主要为组合数学或算法相关 |
| **JMLR** | 高 | 无页数限制，适合篇幅较长的证明 |
...
- 某一子领域已发展成熟，足以通过综合分析产生价值  
- 您发现了单篇论文未能揭示的各研究成果之间的关联  
- 该领域的初学者缺乏合适的入门途径  
- 自上次调研以来，该领域的格局发生了显著变化  

**警告**：开展此类调研需要深厚的专业素养。即便调研者知识面再广，若非该领域专家，也难免会忽略细节并错误解读相关研究成果。  
...
分类体系是调研报告的核心价值所在，应满足以下要求：

- **具有实际意义**：各类类别应对应真实的方法学差异，而非随意划分
- **覆盖全面**：所有相关论文都应有归属之处
- **尽可能互斥**：每篇论文仅属于一个主要类别
...
- 按训练要求划分：仅通过提示词引导、微调模型、基于强化学习的模型优化
- 按推理类型划分：数学推理、常识推理、逻辑推理、因果推理

### 撰写标准

- **引用所有相关论文**——作者需确认自己的研究是否被纳入评估
- **保持客观公正**——避免因个人偏好而否定某些方法
- **进行综合分析而非简单罗列**——提炼出各种方法间的规律、权衡因素及未解问题
- **附上对比表格**——即使是定性对比（如功能/特性清单形式也可）
- **提交前及时更新内容**——检查自开始撰写以来arXiv上新增的相关论文

### 调研发布渠道

| 发表平台 | 备注 |
|-------|-------|
| **TMLR**（综述专题） | 专门用于提交综述文章，无页数限制 |
| **JMLR** | 长篇格式，备受业内推崇 |
| **Foundations and Trends in ML** | 需邀请投稿，但也可自行申请 |
| **ACM Computing Surveys** | 目标受众为广泛的计算机科学领域人士 |
| **arXiv**（独立发表） | 无需同行评审，但若文章质量优异则能获得较高关注度 |
| **会议教程** | 可在 NeurIPS/ICML/ACL 等会议上以教程形式展示，或整理成论文发表 |
...
- 现有基准测试无法衡量您认为重要的指标
- 出现了新的技术能力，但目前尚无标准评估方法
- 现有基准测试已趋于饱和（所有方法的得分均超过95%）
- 您希望为某个碎片化的子领域建立统一的评估标准
...
| 评估标准 | 审稿人检查内容 |
|-----------|----------------|
| **评估的创新性** | 该指标是否衡量了现有基准测试未涉及的内容？ |
| **结构效度** | 该基准测试能否真正反映其所宣称的能力？ |
| **难度校准** | 难度既不能过低（导致结果饱和），也不能过高（导致性能出现随机性） |
| **标注质量** | 一致性指标、标注人员的资质以及相关指导方针 |
| **文档完整性** | 数据表、许可证及维护计划 |
| **可复现性** | 其他人能否轻松使用该基准测试？ |
...
| 发表平台 | 备注 |
|-------|------|
| **NeurIPS 数据集与基准测试** | 专门设置的板块，最适合发布此类内容 |
...
- **首先提出最有力的论点** —— 不要在第一段就含糊其辞  
- **诚实地回应反对意见** —— 优秀的立场论文会针对最有力的质疑，而非最弱的  
- **提供证据** —— 没有证据的立场论文不过是主观评论  
- **具体明确** —— “该领域应当采取X措施”比“还需要更多工作”更佳  
- **避免歪曲现有研究** —— 公平地描述对立观点  
...
| 会议 | 备注 |
|------|------|
| **ICML**（立场论文专题） | 专为立场论文设立的专题环节 |
| **NeurIPS**（研讨会论文） | 研讨会通常欢迎立场类论文 |
...
| **ReScience** | 专注于研究结果复现的学术期刊 |
| **TMLR** | 接收包含分析内容的复现研究论文 |
| **研讨会** | 各大型会议中关于可复现性的专题研讨会 |
