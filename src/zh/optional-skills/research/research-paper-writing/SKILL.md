...
这是一套端到端的流程，用于撰写可直接投稿的机器学习/AI研究论文，目标期刊包括**NeurIPS、ICML、ICLR、ACL、AAAI和COLM**。该技能涵盖了整个研究生命周期：实验设计、执行、监控、分析、论文撰写、审稿、修改以及投稿。

这并非**线性流程**，而是一个迭代循环。实验结果会触发新的实验，审稿意见则会促使进一步分析。智能体必须能够处理这些反馈循环。
```
┌─────────────────────────────────────────────────────────────┐
...
- 通过补充实验或修改内容来**回应审稿意见**；
- 在不同会议格式之间**转换论文**；
- 撰写**非实证类论文**——如理论研究、综述、基准测试或立场文件（详见[超越实证机器学习的论文类型](#paper-types-beyond-empirical-ml)）；
...
| “收敛需要满足特定约束条件” | 受限条件与无限制条件对比 | 收敛速度比较 |

**规则**：如果某个实验无法与任何论点对应，就不要进行该实验。

### 第2.2步：设计基线方法

强大的基线方法是区分被接受论文与被拒论文的关键。审稿人往往会询问：“他们是否与X进行了对比？”
...
许多自然语言处理、人机交互及对齐领域的论文都要求以人工评估作为主要或补充性证据。在开展自动化实验之前，应先完成人工评估的设计——因为人工评估往往需要更长的准备时间（如伦理委员会审批、标注员招募等）。

**何时需要进行人工评估：**
- 自动化指标无法反映您关注的关键要素（流畅性、实用性、安全性）
- 您的研究成果侧重于面向用户的品质（可读性、用户偏好、信任度）
- 自然语言处理领域的学术会议（如ACL、EMNLP）要求在生成任务中进行人工评估
...
| **评分方式** | 莱克特量表（1-5分）、成对比较、排序 | 对于大型语言模型生成的文本，成对比较比莱克特量表更可靠 |
| **样本量** | 每位标注员的评估数量及总评估项数 | 建议通过功效分析确定样本量，至少100个评估项，3名以上标注员 |
| **一致性指标** | 科恩卡帕值、克里彭多夫阿尔法值、ICC系数 | 当标注员超过2人时使用克里彭多夫阿尔法值；同时需报告原始一致性数据 |
| **平台选择** | Prolific、MTurk、内部团队 | 追求高质量评估可选择Prolific；大规模评估可使用MTurk；需要领域专业知识的任务可选用内部团队 |
...
- **置信区间**：关键结果需给出 95% 置信区间
- **成对检验**：用于比较两种方法时采用 McNemar 检验
- **效应量**：为体现实际意义，需计算 Cohen’s d 或 Cohen’s h 值

关于 McNemar 检验、自举法置信区间以及 Cohen’s h 的完整实现方式，请参阅 [references/experiment-patterns.md](references/experiment-patterns.md) 文档。
...
3. **哪些实验失败了？** 失败的实验往往最具参考价值。如实报告失败情况能够提升论文质量。
4. **还需要进行哪些后续实验？** 研究结果通常会引发新的问题。

#### 处理阴性或无效结果

当假设不成立或研究结果无明确结论时，你有三种处理方式：
...
| 您的应用场景 | 推荐策略 | 原因 |
|---------------|----------|-----|
| 中等层级模型 + 有约束的任务 | **Autoreason** | 此类场景最适用。模型的生成能力与自我评估能力之间的差距最大，基准方法能有效优化那些表现较差的模型输出。 |
| 中等层级模型 + 无约束的任务 | 带有范围限制的 **Autoreason** | 通过设定固定事实、结构或输出要求，来限定优化空间。 |
| 最先进层级模型 + 有约束的任务 | **Autoreason** | 即使使用最先进的模型，在有约束任务中也能解决2/3的问题。 |
| 最先进层级模型 + 无约束的任务 | **Critique-and-revise** 或 **Single pass** | 此类场景下Autoreason效果最差，因为模型自身的自我评估能力已经足够优秀。 |
| 具体技术任务（如系统设计） | **Critique-and-revise** | 直接的查找与修正循环效率更高。 |
| 填写模板类任务（仅需一种正确结构） | **Single pass** 或 **Conservative** | 决策空间极小，重复迭代无法带来额外价值。 |
| 包含测试用例的代码 | **Autoreason (code variant)** | 先对代码失败的原因进行结构化分析，再着手修复。其问题解决率为62%，而其他方法仅为43%。 |
| 非常弱的模型（如Llama 8B级别） | **Single pass** | 这类模型能力太弱，无法生成多种候选方案，应优先提升代码的生成质量。 |
...
这一差距是结构性的，而非暂时的。随着成本下降，如今的顶尖方案明日便可能沦为中等水平，但最优解始终存在，只是位置会发生变化。

### Autoreason 循环（概要）

每次迭代都会从全新的独立智能体中生成三个候选方案：

1. **批评者** → 查找现有方案A的问题（不提供修复方案）
2. **作者B** → 根据批评意见对方案A进行修改
3. **整合器** → 合并方案A和B（标签随机分配）
4. **评审小组** → 3名盲评的链式推理评审员通过博尔达计数法对A、B及AB组合进行评分
5. **收敛判定** → 若方案A在连续k=2次迭代中获胜，则流程结束

**关键参数：**
- k=2次迭代即视为收敛（k=1则收敛过早，k=3则成本过高且无法提升质量）
- 始终使用3名链式推理评审员（可加快3倍收敛速度）
- 作者的随机温度值为0.8，评审员的为0.3
- 保守的平局处理规则：现有方案在平局中获胜
- 每个角色均为全新的独立智能体，彼此之间没有共享上下文

### 在论文润色中的应用

通过autoreason对论文本身进行优化时：
- **为批评者提供真实依据**：包括实际的实验数据、结果JSON文件及统计输出。若缺乏这些信息，模型可能会编造虚假的消融研究及伪置信区间。
- **至少使用3名评审员**：出现故障的评审解析器不会仅带来噪声，反而会完全阻碍流程的平衡发展。
- **明确修改范围**：应指定“针对这些具体缺陷进行改进”，而非笼统地要求“提升论文质量”。

### 失效模式

| 失败类型 | 检测方式 | 解决方案 |
|---------|-----------|----------|
| 无法收敛（A从未获胜） | 在20次以上迭代中A的胜率始终低于15% | 为任务添加范围约束 |
| 合成偏差 | 单词数量无限制增长 | 对结构及输出结果设置限制 |
| 单次迭代性能下降 | 基线得分高于迭代后输出结果 | 改为单次迭代；模型可能过弱 |
| 过拟合（代码场景） | 公开测试通过率高，但私有测试通过率低 | 除测试反馈外，还需结合结构化分析 |
| 评分工具异常 | 解析失败导致评分小组人数少于3人 | 继续之前需先修复解析器 |

如需完整的提示词、Borda评分规则、模型选择指南、范围约束设计模式以及计算预算参考信息，请参阅[references/autoreason-methodology.md](references/autoreason-methodology.md)。

---

## 第5阶段：论文初稿撰写

完整的初稿撰写流程（各章节顺序、LaTeX框架搭建、图表规范、摘要与引言中的公式格式、相关研究部分的撰写方式）均记载在`references/phase5-paper-drafting.md`中——进入该阶段后请使用`read_file`函数加载该文件。同时可结合`references/writing-guide.md`中的散文写作风格规则使用。
...
每位审稿人在查看元评审意见后均可进一步完善自己的评审内容。可设置提前终止条件：若审稿人回复“我已完成评审”（即无需任何修改），则停止迭代。

**用于评审的模型选择**：即便论文最初是用性能较低的模型撰写的，也建议使用当前最强的模型来进行评审。审稿模型应与写作模型分开选择。

**少样本校准**：如果条件允许，可提供1-2篇来自目标期刊的真实已发表评审意见作为示例。这能显著提升评分的准确性。相关示例请参阅[references/reviewer-guidelines.md](references/reviewer-guidelines.md)。

### 步骤6.1b：视觉审阅环节（VLM）

仅依靠文本审阅会遗漏一类问题：图表质量、排版问题以及视觉一致性方面的问题。如果您拥有具备视觉处理能力的模型，可以对编译后的PDF文件进行独立的**视觉审阅**：
...
各会议均设有必填的核查清单。请务必仔细填写——若清单不完整，论文可能会被直接拒收。

详情请参阅 [references/checklists.md](references/checklists.md)，其中包含：
- NeurIPS会议的16项论文核查清单
...
双盲评审要求审稿人无法知晓论文的作者身份。请逐一检查以下所有项目：
...
上述主要流程针对的是实证性机器学习论文。其他类型的论文则需要不同的结构与证据标准。有关各类论文的详细指导，请参阅[references/paper-types.md](references/paper-types.md)。
...
**与实证性论文的主要区别**：
- 研究贡献体现为定理、界或反例结果，而非实验数据
- “方法”部分被“基础知识”和“主要结果”取代
- 证据为证明而非实验结果（尽管对理论的实证验证也是可接受的）
- 正文提供证明概要，完整证明放在附录中是标准做法
...
| 论文结构安排 | 当存在多种可行的结构方案时 |
| 实验优先级排序 | 当待处理的实验数量超过可用时间时 |
| 论文提交准备情况 | 最终提交之前 |

**无需询问的内容**（请主动做出选择或明确标注）：
- 用词选择、章节顺序
- 应突出展示哪些具体结果
...
了解审稿人的关注点有助于更有针对性地准备论文：

| 评估标准 | 审稿人检查内容 |
|----------|----------------|
| **质量** | 技术层面的合理性、论点的充分支撑、合理的基准对比 |
| **清晰度** | 表达清晰、专家可复现、符号使用一致 |
| **重要性** | 对社区的影响、是否推动了领域理解的发展 |
| **原创性** | 是否带来新见解（无需采用全新方法） |

**评分标准（NeurIPS 6分制）：**
- 6分：强烈推荐录用 —— 具有开创性且毫无瑕疵
- 5分：推荐录用 —— 技术扎实，影响力高
- 4分：勉强推荐录用 —— 整体良好，但评估不够充分
- 3分：勉强拒绝 —— 缺点大于优点
- 2分：拒绝 —— 存在技术缺陷
- 1分：强烈拒绝 —— 为已有成果或存在伦理问题

详细指南、常见顾虑及反驳策略请参阅 [references/reviewer-guidelines.md](references/reviewer-guidelines.md)。
...
| 文档路径 | 内容说明 |
|----------|----------|
| [references/writing-guide.md](references/writing-guide.md) | Gopen与Swan的7项原则、Perez的实用技巧、Lipton的用词指南、Steinhardt的精准度要求以及图表设计规范 |
| [references/citation-workflow.md](references/citation-workflow.md) | 引文相关API、Python代码、CitationManager类以及BibTeX文件管理方法 |
| [references/checklists.md](references/checklists.md) | NeurIPS要求的16项检查项、ICML、ICLR、ACL的提交要求，以及通用预提交检查清单 |
| [references/reviewer-guidelines.md](references/reviewer-guidelines.md) | 评估标准、评分规则、常见问题处理方式以及反驳意见模板 |
| [references/sources.md](references/sources.md) | 所有写作指南、会议规范及API的完整参考文献列表 |
| [references/experiment-patterns.md](references/experiment-patterns.md) | 实验设计模式、评估方案、监控方法以及错误恢复策略 |
| [references/autoreason-methodology.md](references/autoreason-methodology.md) | Autoreason循环机制、策略选择方法、模型使用指南、提示词编写技巧、范围限制要求以及Borda评分法 |
| [references/human-evaluation.md](references/human-evaluation.md) | 人工评估设计流程、标注规范、一致性度量指标、众包质量控制方法以及IRB审批相关指导 |
| [references/paper-types.md](references/paper-types.md) | 理论论文（证明书写、定理结构）、综述论文、基准测试论文以及立场声明类论文 |

### LaTeX模板

`templates/` 目录中提供了针对 **NeurIPS 2025**、**ICML 2026**、**ICLR 2026**、**ACL**、**AAAI 2026** 及 **COLM 2025** 等会议的模板。

有关编译说明，请参阅 [templates/README.md](templates/README.md)。
...
**会议相关规范：** [NeurIPS](https://neurips.cc/Conferences/2025/PaperInformation/StyleFiles) | [ICML](https://icml.cc/Conferences/2025/AuthorInstructions) | [ICLR](https://iclr.cc/Conferences/2026/AuthorGuide) | [ACL](https://github.com/acl-org/acl-style-files)
