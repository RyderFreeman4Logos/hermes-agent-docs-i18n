...
**注意：此功能不可用于读取用户的个人邮件**（此类需求请使用himalaya或Gmail）。AgentMail旨在为智能体提供独立的身份标识和专属收件箱。
...
### 2. 配置MCP服务器
在 `~/.hermes/config.yaml` 文件中添加相关配置（请直接粘贴您的实际API密钥——MCP环境变量不会从.env文件中读取）：
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. 创建专用收件箱：
   ...
   - 代理将获得地址：`hermes-agent@agentmail.to`
2. 发送邮件：
...
2. 使用该收件箱ID调用`list_threads`查看对话记录
3. 使用`get_thread`读取特定线程及其邮件内容
...
## 常见问题
- 免费套餐仅支持3个收件箱以及每月3,000封邮件发送量
- 免费套餐下的邮件均来自`@agentmail.to`域名（付费套餐则支持自定义域名）
- MCP服务器需使用Node.js 18及以上版本运行（可通过`npx -y agentmail-mcp`命令安装）
- 必须先安装Python的`mcp`包：`pip install mcp`
- 若需接收实时邮件（通过Webhook），则需要一台公网服务器；个人使用建议改用通过cronjob定时调用`list_threads`函数的方式
...
- AgentMail MCP 代码仓库：https://github.com/agentmail-to/agentmail-mcp
