...
| `infsh app run <app> --input '<json>'` | 使用内联 JSON 数据运行应用 |
| `infsh app run <app> --input <file> --no-wait` | 运行后不等待任务完成 |
| `infsh app sample <app>` | 显示示例输入数据 |
| `infsh app sample <app> --save <file>` | 将示例数据保存到文件 |

## 任务命令

| 命令 | 描述 |
|---------|-------------|
| `infsh task get <task-id>` | 获取任务状态与结果 |
| `infsh task get <task-id> --json` | 以 JSON 格式获取任务信息 |
| `infsh task get <task-id> --save <file>` | 将任务结果保存至文件 |
