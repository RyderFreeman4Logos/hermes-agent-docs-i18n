# 用于 Accelerate 的自定义插件

## 概述

Accelerate 支持创建**自定义插件**，从而在内置训练策略（DDP、FSDP、DeepSpeed）的基础上进一步扩展分布式训练功能。
...
**应用场景**：在 H100 GPU 上实现超快速训练

## 自定义 DeepSpeed 配置

### 结合 CPU 协同处理的 ZeRO-3 模式
...
- Accelerate 插件文档：https://huggingface.co/docs/accelerate/package_reference/kwargs
- DeepSpeed 配置指南：https://www.deepspeed.ai/docs/config-json/
- FSDP 使用手册：https://pytorch.org/docs/stable/fsdp.html
- 自定义训练循环指南：https://huggingface.co/docs/accelerate/usage_guides/training_tpu
