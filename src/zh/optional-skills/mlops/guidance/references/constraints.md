...
本指南介绍了 Guidance 中的正则表达式约束、基于语法的生成方式以及令牌修复技术。
...
**正则表达式与免费生成模式对比：**
- 简单正则表达式（数字）：速度约为免费生成模式的1.2倍
- 复杂正则表达式（邮箱地址）：速度约为免费生成模式的1.5倍
- 基于语法的生成方式：速度约为免费生成模式的2倍

**但优势在于：**
- 输出结果100%有效（而免费生成模式经验证后有效率仅约70%）
- 无需重复尝试机制
- 对于结构化输出，整体端到端处理速度更快

**优化建议：**
- 仅对关键字段使用正则表达式
- 对于规模较小的固定集合，优先使用`select()`函数（速度最快）
- 尽可能使用`stop`序列控制生成长度（比设置最大词数更高效）
- 通过复用函数来缓存已编译的语法规则

## 相关资源

- **Token Healing相关论文**：https://arxiv.org/abs/2306.17648
- **使用指南文档**：https://guidance.readthedocs.io
- **GitHub项目页面**：https://github.com/guidance-ai/guidance
