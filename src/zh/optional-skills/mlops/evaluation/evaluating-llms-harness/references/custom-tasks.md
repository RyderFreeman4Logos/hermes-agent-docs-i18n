...
自定义任务允许您使用自己的数据集和指标来评估模型。这些任务通过 YAML 配置文件进行定义，对于复杂的逻辑处理，还可搭配可选的 Python 工具。
...
**返回结果为空**：请核实 `doc_to_text` 和 `doc_to_target` 模板配置是否正确。

**指标相关错误**：请确保指标名称准确无误（需使用 exact_match，而非 not exact-match）。

**过滤功能问题**：可通过 `--log_samples` 参数测试过滤规则的有效性。
