...
| 提供商 | 模型类型 | 请求类型 | logprobs支持 |
|--------|----------|-----------|--------------|
| OpenAI（补全任务） | `openai-completions` | 全部类型 | ✅ 支持 |
| OpenAI（聊天任务） | `openai-chat-completions` | 仅`generate_until`类型 | ❌ 不支持 |
| Anthropic（补全任务） | `anthropic-completions` | 全部类型 | ❌ 不支持 |
| Anthropic（聊天任务） | `anthropic-chat` | 仅`generate_until`类型 | ❌ 不支持 |
| 本地（兼容OpenAI） | `local-completions` | 视服务器配置而定 | 不固定 |

**注意**：不支持logprobs的模型仅能用于生成任务评估，无法用于困惑度或对数似然度测试。
...
OpenAI 按 token 收费。在运行之前请先估算相关成本：
...
**节省成本的建议**：
- 测试时请使用 `--limit N` 参数
- 先尝试使用 `gpt-3.5-turbo`，再选用 `gpt-4`
- 将 `max_gen_toks` 设置为最低必要值
...
- Claude 3.5 Sonnet：每 100 万输入字符费用为 3.00 美元，每 100 万输出字符费用为 15.00 美元
- Claude 3 Opus：每 100 万输入字符费用为 15.00 美元，每 100 万输出字符费用为 75.00 美元
- Claude 3 Haiku：每 100 万输入字符费用为 0.25 美元，每 100 万输出字符费用为 1.25 美元
...
如需创建自定义 API 接口，可继承 `TemplateAPI` 类：
