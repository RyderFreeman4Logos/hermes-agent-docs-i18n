...
本指南介绍如何利用数据并行、张量并行和流水线并行技术在多块 GPU 上开展评估。

## 概述

分布式评估可通过以下方式加速基准测试：
- **数据并行**：将评估样本分配到不同 GPU 上处理（每块 GPU 都拥有完整的模型副本）
- **张量并行**：将模型权重拆分到多个 GPU 上（适用于大型模型）
- **流水线并行**：将模型层拆分到不同 GPU 上（适用于超大型模型）

**适用场景**：
- 数据并行：模型可容纳在单块 GPU 中，且希望加快评估速度
- 张量/流水线并行：模型过大，无法仅依赖单块 GPU 处理

## HuggingFace 模型 (`hf`)

### 数据并行（推荐方案）

每块 GPU 都加载完整的模型副本，并分别处理部分评估数据。
...
**配备8块GPU时**：700亿参数模型（140GB）/ 8 = 每块GPU占用17.5GB ✅
...
- `max_memory_per_gpu`：每块 GPU 的最大内存限制（例如：`"40GB"`）
- `max_cpu_memory`：用于任务卸载的 CPU 最大内存限额
...
**结果**：得益于数据并行技术，性能提升2倍；通过张量并行技术，可支持700亿参数规模的模型。  
...
**内存占用**：700亿参数的模型分布在4块GPU上，因此每块GPU的占用内存约为35GB。
...
**结果**：70B模型可正常运行（准确率=4），速度提升2倍（精度损失=2）
...
**注意**：SGLang 正在逐步废弃数据并行功能，建议改用张量并行。
...
### 70B 模型评估（MMLU，5-shot 测试）

| 方法 | GPU 数量 | 计算时间 | 每张 GPU 内存占用 | 备注 |
|------|----------|----------|------------------|-------|
| HuggingFace（无并行处理） | 1 | 8 小时 | 140GB（内存溢出） | 内存不足 |
| HuggingFace（线程数=8） | 8 | 2 小时 | 17.5GB | 速度较慢，但可运行 |
| HuggingFace（精度=8） | 8 | 1 小时 | 140GB（内存溢出） | 内存不足 |
| vLLM（线程数=4） | 4 | 30 分钟 | 35GB | 速度极快！ |
| vLLM（线程数=4，精度=2） | 8 | 15 分钟 | 35GB | 速度最快 |

### 7B 模型评估（多任务测试）

| 方法 | GPU 数量 | 计算时间 | 加速比 |
|------|----------|----------|--------|
| HuggingFace（单线程） | 1 | 4 小时 | 1× |
...
**示例**：
- 70亿参数 FP16格式：7 × 2 × 1.2 = 168GB ✅ 可适配A100 40GB显存
- 130亿参数 FP16格式：13 × 2 × 1.2 = 31.2GB ✅ 可适配A100 40GB显存
...
- 在4块GPU上运行70B模型：168GB ÷ 4 = 每块GPU 42GB ✅
- 在8块GPU上运行70B模型：168GB ÷ 8 = 每块GPU 21GB ✅
