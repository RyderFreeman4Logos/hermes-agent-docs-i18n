...
本指南全面介绍了 lm-evaluation-harness 中的 60 多项评估任务，包括它们的测试目标以及结果解读方法。
...
**衡量内容**：涵盖57个领域的广泛知识（STEM、人文科学、社会科学、法律等）。

**任务变体**：
- `mmlu`：原始的57领域基准测试
- `mmlu_pro`：难度更高的版本，包含侧重逻辑推理的题目
- `mmlu_prox`：多语言扩展版本
...
**测评内容**：针对小学水平应用题的数学推理能力。

**任务变体**：
- `gsm8k`：基础任务
- `gsm8k_cot`：结合思维链提示的任务
- `gsm_plus`：包含干扰项的对抗性任务
...
**评估内容**：根据文档字符串生成 Python 代码的能力（功能正确性）。

**任务类型**：
- `humaneval`：标准基准测试
- `humaneval_instruct`：适用于经过指令微调的模型
...
**测试内容**：包含23项极具挑战性的推理任务，此前各类模型均无法在这些任务上超越人类水平。
...
- **所有任务的表现都仅处于随机水平**：说明模型未得到正确训练
- **生成类任务的准确率为0%**：很可能是格式或解析问题所致
- **不同运行结果差异极大**：请检查种子值/采样设置
- **在所有指标上均优于GPT-4**：很可能是数据被污染

## 最佳实践建议

1. **务必说明少样本训练的设置**：如0样本、5样本等
2. **运行多次并使用不同种子值**：最终报告平均值及标准差
3. **检查数据是否被污染**：在训练数据中查找基准测试案例
4. **与已公开的基准模型进行对比**：以此验证你的实验环境
5. **完整列出所有超参数**：包括模型类型、批量大小、最大token数、温度参数等
