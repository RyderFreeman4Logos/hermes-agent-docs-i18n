...
请优先使用仅基于 URL 的工作流。在查找 GGUF 文件、选择量化级别或构建 `llama-server` 命令时，无需依赖 `hf` 库或 API 客户端。 
...
可使用以下参数：
- 使用 `search=<term>` 搜索特定模型系列，例如 `Qwen`、`Gemma`、`Phi` 或 `Mistral`
- 如果用户的硬件条件有限，可使用 `num_parameters=min:0,max:24B` 等参数进行限制
- 若用户希望找到当前最受欢迎的仓库，可使用 `sort=trending` 参数

如果用户尚未选定模型系列，请勿直接从随机生成的 GGUF 仓库开始搜索。应先进行搜索，再筛选候选列表。
...
2. 从获取的页面文本或HTML中提取“Hardware compatibility”部分，包括：
   - 量化标签
   - 文件大小
   - 位深分类
3. 提取片段中出现的任何其他启动参数，例如`--jinja`。

当“HF local-app”片段存在时，应以该片段为权威依据。

请直接读取URL内容，而非依赖浏览器渲染的界面。如果获取的页面源代码未显示“Hardware compatibility”部分，则说明该部分不可见，此时应转而使用树形API，并参考`quantization.md`中的通用指南。
...
仅在 API 接口失败或用户希望查看网页版时，才将 `https://huggingface.co/<repo>/tree/main` 作为人工备选方案使用。

## 4. 构建命令

推荐的操作顺序如下：

1. 从本地应用页面复制完整的 Hugging Face 代码片段
2. 如果页面提供了明确的量化标签，则可使用简写方式选择相应文件
...
4. 若希望通过命令行界面而非服务器来使用，可选用：

```bash
llama-cli -hf <repo>:<QUANT>
```

当仓库使用了自定义标签或非标准命名方式，可能导致`:<QUANT>`含义不明时，请使用“精确文件”格式。
...
- 仓库特定的量化标签非常重要。除非页面本身已有相应修改，否则切勿将 `UD-Q4_K_M` 直接改为 `Q4_K_M`。
- `mmproj` 文件是多模态模型的投影器权重，而非主语言模型检查点。
- 若由于用户未配置硬件配置文件，或获取的页面源代码未显示相关内容而导致 HF 硬件兼容性面板缺失，仍应使用树形 API，并参考 `quantization.md` 中的通用量化指南。
- 如果该仓库已存在 GGUF 格式的模型文件，则无需直接进入转换流程。
