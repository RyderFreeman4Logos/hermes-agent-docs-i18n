...
在查阅通用表格之前，请先使用以下方式打开模型仓库：

```text
https://huggingface.co/<repo>?local-app=llama.cpp
```

请优先使用从获取的 `?local-app=llama.cpp` 页面文本或 HTML 中“硬件兼容性”部分所显示的精确数量标签和尺寸。随后在以下位置确认对应的文件名：

```text
https://huggingface.co/api/models/<repo>/tree/main?recursive=true
```

请先使用中心页面进行查询，只有当仓库页面未提供明确推荐时，才可采用下述通用方法。
...
| Q8_0 | 5.9584（+0.03%） | 7.0 GB | 25 tok/s | 几乎无损失 |
| **Q6_K** | 5.9642（+0.13%） | 5.5 GB | 30 tok/s | 最佳质量与体积平衡 |
| **Q5_K_M** | 5.9796（+0.39%） | 4.8 GB | 35 tok/s | 性能与质量均衡 |
| **Q4_K_M** | 6.0565（+1.68%） | 4.1 GB | 40 tok/s | **推荐选择** |
| Q4_K_S | 6.1125（+2.62%） | 3.9 GB | 42 tok/s | 速度更快，但质量较低 |
| Q3_K_M | 6.3184（+6.07%） | 3.3 GB | 45 tok/s | 仅适用于小型模型 |
| Q2_K | 6.8673（+15.3%） | 2.7 GB | 50 tok/s | 不推荐使用 |
...
**针对70亿参数模型的使用建议**：为确保在普通硬件上正常运行，建议选择Q3_K_M或Q4_K_S格式。
