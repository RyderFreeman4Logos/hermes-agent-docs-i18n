...
| **FP8** | 8位（50%） | <0.5% | 最快 | 仅限 H100 GPU |
| **SqueezeLLM** | 3-4位（75-80%） | 2-3% | 中等 | 极端压缩需求 |

**推荐方案**：
- **生产环境**：针对70B级模型使用AWQ
- **H100 GPU**：为获得最佳速度请使用FP8
- **最大兼容性**：选择GPTQ
- **极端压缩需求**：使用SqueezeLLM

## AWQ 的配置与使用

**AWQ**（感知激活值的权重量化）在4位量化时能实现最佳的精度。
...
**FP8**（8位浮点格式）能够在H100显卡上实现最佳运行速度，同时将精度损失降至最低。
...
| 量化格式 | 精度 | 内存占用 | 运行速度 | 是否适合实际应用 |
|----------|------|----------|----------|------------------|
| FP16（基准值） | 100% | 140GB | 1.0倍 | ✅（若有足够内存） |
| FP8 | 99.5% | 70GB | 1.8倍 | ✅（仅支持H100显卡） |
| AWQ 4位 | 99.0% | 35GB | 1.5倍 | ✅（70B模型最佳选择） |
| GPTQ 4位 | 98.5% | 35GB | 1.5倍 | ✅（兼容性良好） |
| SqueezeLLM 3位 | 96.0% | 26GB | 1.3倍 | ⚠️（需验证精度） |
...
**FP8格式**：
- 使用H100/H800显卡
- 在尽可能减少精度损失的前提下追求最高运行速度
- 用于实际生产环境部署

**AWQ 4位格式**：
- 需要将70B模型部署在40GB内存的GPU上
- 用于实际生产环境
- 可接受1%以内的精度损失

**GPTQ 4位格式**：
- 需要支持多种不同型号的模型
- 不使用H100显卡（此时应选择FP8格式）
- 可接受1-2%的精度损失

**测试策略**：

1. **建立基准**：在评估集上测量FP16格式的模型精度
2. **进行量化**：生成量化后的模型版本
3. **性能评估**：在相同任务下对比量化前后模型的表现
4. **做出决策**：若精度下降幅度低于阈值（通常为1-2%），则该模型可用
