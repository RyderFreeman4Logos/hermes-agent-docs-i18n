...
适用于部署生产级大语言模型 API、优化推理延迟与吞吐量，以及为 GPU 内存有限的场景提供模型服务。该框架支持 OpenAI 兼容的接口、量化技术（GPTQ/AWQ/FP8）以及张量并行处理功能。

## 快速入门

通过基于分块 KV 缓存的 PagedAttention 机制以及混合预填充请求与解码请求的连续批处理技术，vLLM 的吞吐量可比传统 Transformer 框架提升 24 倍。
...
**性能优化**：关于 PagedAttention 调优、连续批处理机制及基准测试结果的内容，可查看 [references/optimization.md](references/optimization.md)。

**量化指南**：涉及 AWQ/GPTQ/FP8 的配置方法、模型准备流程以及精度对比等信息，请参考 [references/quantization.md](references/quantization.md)。

**故障排查**：详细的错误信息、调试步骤及性能诊断方法，详见 [references/troubleshooting.md](references/troubleshooting.md)。
