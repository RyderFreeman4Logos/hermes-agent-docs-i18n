...
本指南介绍如何使用 Meta AI 的 Segment Anything Model 实现零样本图像分割。
...
**核心优势：**
- **零样本分割**：无需微调即可处理任意图像领域
- **灵活的标注方式**：支持点标注、边界框标注或现有掩码
- **自动分割功能**：可自动生成所有物体掩码
- **高精度表现**：基于 1100 万张图像中的 11 亿个掩码进行训练
- **多种模型尺寸可选**：ViT-B（速度最快）、ViT-L、ViT-H（精度最高）
- **支持 ONNX 导出**：可在浏览器和边缘设备上部署

**如需其他替代方案，可考虑：**
- **YOLO/Detectron2**：用于需要类别识别的实时物体检测
- **Mask2Former**：用于需要类别分类的语义/全景分割
- **GroundingDINO + SAM**：用于基于文本提示的分割任务
- **SAM 2**：用于视频分割任务
...
| 边界遮罩质量差 | 尝试不同的提示词，同时使用框与点信息 |
| 边缘伪影 | 采用 stability_score 进行过滤 |
| 小物体被遗漏 | 提高 points_per_side 的数值 |

## 参考资料

- **[高级用法](references/advanced-usage.md)** - 批量处理、微调及集成方式
