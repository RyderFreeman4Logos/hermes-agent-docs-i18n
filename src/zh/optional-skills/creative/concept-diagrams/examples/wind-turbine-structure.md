...
这是一幅展示现代风力发电机所有主要部件的实体/结构剖面图，范围从地下基础一直延伸到叶片尖端。

## 使用的关键设计元素

- **地下部分**：土层、带有钢筋加固网格的深层混凝土基础以及扩展基脚
- **剖面视图**：显示塔筒壁厚，内部组件清晰可见
- **锥形塔筒**：通过特定线条设计，呈现出向顶部逐渐变细的真实塔筒轮廓
- **内部通道**：塔筒内的梯子及电梯井
- **电缆布线**：从机舱向下穿过塔筒直至变压器的电力电缆
- **机舱剖面**：外壳内部的所有组件——齿轮箱、发电机、制动装置以及偏航系统均一目了然
- **转子组件**：叶片根部带有变桨电机的轮毂，以及三片具有渐变填充结构的复合材料叶片
- **地面标识**：清晰区分地面以上与以下区域
- **部件颜色编码**：不同系统类型采用不同颜色标识（蓝色=发电机，金色=齿轮箱，红色=制动装置，绿色=偏航系统，紫色=变桨系统）
- **图例栏**：快速查询各颜色所代表的含义
