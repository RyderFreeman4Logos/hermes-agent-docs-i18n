...
该图通过矩形之外的多种 SVG 图形元素——如路径、多边形和椭圆——来呈现飞机的侧视图，从而实现更为逼真的视觉效果。

## 主要使用图形元素

- **路径元素**：利用二次贝塞尔曲线绘制出带有机头的弯曲机身
- **多边形元素**：锥形机翼、三角形稳定面以及控制面
- **椭圆元素**：发动机（圆柱体）和起落架轮子（圆形）
- **线条元素**：起落架支柱以及用于标注的引导线
- **虚线**：内部结构（油箱）以及可活动的控制面（方向舵、升降舵）
- **分层构图**：在机身轮廓内绘制客舱区域
- **带标签的引导线**：将标签与其所对应的部件相连
...
| 实际形态 | SVG元素 | 示例 |
|-----------|----------|------|
| 弧形结构 | 包含Q（二次曲线）或C（三次曲线）的`<path>`元素 | 机身、鼻锥 |
| 锥形/棱角结构 | `<polygon>`元素 | 翼部、稳定翼 |
| 圆柱形结构 | `<ellipse>`元素 | 发动机、轮子、油箱 |
| 线性结构 | `<line>`元素 | 支架、塔架、齿轮轴 |
| 内部分区 | 父形状内的`<rect>`元素 | 舱室划分 |
| 虚线边界 | 任意形状上的`stroke-dasharray`属性 | 油箱、控制面 |

## 布局注意事项

- **视图框尺寸**：680×400（更宽的宽高比更适合侧视图展示）
- **分层绘制**：先绘制外部结构，再在其上方添加内部细节
- **引线标注**：使用`.leader`类（虚线样式）将标签与对应组件连接
- **文字大小**：组件标签使用`.tl`类（10像素），分区标签使用`.ts`类（12像素）
- **语义化配色**：按系统分类着色（结构部分=蓝色，推进系统=珊瑚色，燃料系统=琥珀色等）
