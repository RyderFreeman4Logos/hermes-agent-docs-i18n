...
涵盖常见创意编程任务的完整网络配方。每种模式都会展示操作符链、用于构建该网络的MCP工具调用方式以及关键参数设置。

## 基于音频的视觉效果

### 模式1：音频频谱 -> 噪声位移

通过音频控制噪声参数，从而生成具有音乐响应特性的自然纹理。
...
这是一个功能完备的完整方案。它首先播放 MP3 文件，接着进行 FFT 分析，将得到的频谱数据作为纹理输入到 GLSL 着色器中——其中内部分形结构会对低音作出反应，而外部分形则会对高音作出响应。
...
**测试中的关键发现：**
- `spectrum_tex`（CHOP To TOP）可生成256x2大小的纹理——X轴代表频率，第一个通道的Y轴值为0.25
- 在`vec2(0.05, 0.0)`位置采样可获得低频音，而在`vec2(0.65, 0.0)`位置采样则可获得高频音
- 根据像素距离（`d * 0.5`）进行采样，可使内部分形对低频音产生反应，外部分形则对高频音产生反应
- 在`fract()`缩放功能中加入`bass * 0.3`的参数，能让分形随低频鼓点节奏律动
...
- 复合运算方式更改：屏幕叠加（发光效果）、相加（增亮）、相乘（变暗）
- 在反馈循环中加入HSV色调调节功能，以实现色彩动态变化
- 添加模糊效果，营造梦幻般的柔和感
- 用GLSL TOP算法替代噪声函数，从而生成自定义的种子图案

### 模式5：实例化（类粒子系统）

基于CHOP数据或DAT文件控制，渲染数千个几何体副本，每个副本都具有独特的位置、旋转和缩放参数。
...
将输出内容录制到文件中。**H.264/H.265 格式需要商业许可证**——在非商业用途中请使用 Motion JPEG（`mjpa`）格式。
...
**常见陷阱：**
- 在同一脚本中同时设置 `par.file` 和 `par.record = True` 可能会导致竞态条件——请使用 `run("...", delayFrames=2)` 来解决。
- 过度频繁调用 `TOP.save()` 总是会捕获到同一帧——处理动画时请使用 MovieFileOut。
- 更多详细信息请参阅 `pitfalls.md` 中的第 25-27 条。

### 模式 8b：TD → 外部处理流程（FFmpeg / Python / 后处理）

将 TD 生成的可视化内容导出，以便在其他工具中使用（如 ffmpeg、Python、ASCII 艺术等）。当需要将 TD 的输出结果与外部处理流程结合使用时（如 ASCII 转换、Python 渲染链、机器学习推理等），这就是标准的工作流程。
...
**重要注意事项：**
- 在时间码录制阶段请使用 ProRes 格式，以避免在合成过程中出现画质损失
- 应以目标输出帧率进行提取（而非时间码的渲染帧率）
- 对于需要音频同步的内容，需使用 Python（scipy FFT）单独分析音频文件，提取每帧的特征值（均方根值、频谱带信息、拍频等），以此来调整合成参数
- 在开始录制之前，请务必确认时间码帧率大于 0（参见错误案例 #37、#38）
...
### 模式 9b：音频响应型 GLSL 分形（经过验证的方案）

音频频谱会通过频谱纹理输入直接驱动 GLSL 分形着色器。低频会使内部分形线条变得更粗，中频则会改变其旋转方式，而高频则能照亮外部边缘。**在使用这些方案中的任何参数名称之前，请务必先执行探索步骤（SKILL.md 第 0 步）——不同版本的 Hermes Agent 中这些参数名称可能会有所差异。**
...
- 内层分形迭代采样较低的x值 → 对低音作出响应
- 外层迭代采样较高的x值 → 对高音作出响应
- 对`fract()`结果应用`bass * 0.3`的缩放系数 → 使分形缩放效果随低音节奏脉动
- 对正弦频率应用`bass * 4.0`的系数 → 使线条密度随低音节奏脉动
- 对旋转速度应用`mids * 0.3`的系数 → 在人声或中频段落使螺旋结构旋转得更快
- 对环状元素的透明度应用`highs * 0.5`的系数 → 在外环产生高频闪光效果

**输出录制方式：** 使用`MovieFileOut TOP`并搭配`mjpa`编码格式（H.264格式需要商业许可证）。相关注意事项请参见第25-27条。
...
对于需要在多帧之间保持状态的特效（如粒子效果、流体效果、元胞自动机效果），可使用支持多遍处理的 GLSL Multi TOP，或采用反馈 TOP 循环来实现。
...
常用 MIDI 映射规则：
- CC 通道（旋钮/推子）：连续值范围为 0-127，可映射为浮点参数
- 音符开启/关闭：二进制触发信号，可用于将音量包络映射至 Trigger CHOP 功能
- 拨奏力度：用于控制强度/亮度
