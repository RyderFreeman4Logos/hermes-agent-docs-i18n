...
| 容器COMP + `overTOP` | 完全控制、不同尺寸的面板 | 需要更多配置，但灵活性极高 |
| GLSL合成器 | 过程化/树状结构风格 | 功能最强大，但实现更为复杂 |

---

## layoutTOP

内置网格合成器——构建规则平铺网格的最快捷方式。
...
将每个元素分别作为独立的 `containerCOMP` 来构建，然后使用 `overTOP` 方法进行组合。  
...
**提示：** 如果面板尺寸不同，请在每个 `overTOP` 参数前添加 `resolutionTOP`。
...
**核心原则：** 各个组件之间互不知情，由组合器负责将它们整合在一起。音频总线虽被所有组件引用，但实际上是独立存在的。
