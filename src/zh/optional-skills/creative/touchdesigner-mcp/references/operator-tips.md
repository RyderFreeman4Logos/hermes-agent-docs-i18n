...
**重要规则：**
- 类名：必须使用 `wireframeMAT`，而非带大写后缀的 `wireframeMat`
- 几何体 SOP/POP 需放置于几何体组件内部
- 材质路径应为 `geo.par.material`，而非 `geo.par.mat`
- 渲染几何体时需设置 `render.par.geometry = geo.path`（字符串路径形式）
- 若需清晰显示线框结构，请将 `wireframeMAT.par.wireframemode` 设定为 `'topology'`；若需显示三角形边缘，则设为 `'tesselated'`
- 备选方案：可直接使用 `renderTOP.par.overridemat` 代替针对单个几何体的材质设置
...
| 出现异常的初始模式 | 初始状态设置错误 | 使用常量 TOP（黑色） |
...
### 应用场景
- **波浪模拟** — R代表高度，G代表速度，初始状态为黑色
- **元胞自动机** — 白色表示存活，黑色表示死亡，初始状态为随机噪声
