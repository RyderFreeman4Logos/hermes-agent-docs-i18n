...
在 TD 099 中，`geometryCOMP` 能够渲染 **POPs**，但无法渲染 SOPs。位于 geometry COMP 内部的 `boxSOP` 将不会被显示——且不会产生任何错误。
...
| `sphereSOP` | `spherePOP` | `radx/y/z`, `freq`, `type`（测地线/网格/共享极点/四面体） |
...
`spherePOP.par.type` 的可选值包括：`geodesic`、`grid`、`sharedpoles`、`tetrahedron`。若需处理柏拉图立体多面体，应选择 `tetrahedron`。
