...
外部控制器输入与输出——包括 MIDI 硬件、TouchOSC 移动端界面，以及通过网络进行的 OSC 路由。

对于基于音频生成的 MIDI 模式（通过频谱分析触发音轨），请参阅 `audio-reactive.md`。
...
- `ch1c74` — 第1个通道，74级副音
- `ch1n60` — 第1个通道，60音（中央C）——对应值为力度值 0-127
...
`midiinCHOP` 输出的音符在按住时具有对应的速度值，松开时则为 0。可使用 `triggerCHOP` 将按住的音符转换为脉冲信号：
...
在无法提前知晓控制器 CC 布局时，可使用该模式创建可重复使用的学习规则：

1. 在对应节点后添加 `midiinCHOP` 和 `selectCHOP`。
2. 用户转动控制器旋钮。
3. 通过 `td_read_chop` 函数读取 `midiinCHOP` 的数据，确定哪个通道的值为非零——该通道即为当前激活的 CC。
4. 将 `selectCHOP.par.channames` 设置为该通道名称。
...
针对注释事件，应使用 `event` 模式，并通过 `pulseCHOP` 或 `triggerCHOP` 功能来发送该值。

---

## OSC输入 — 网络控制

OSC是MIDI更为灵活的变体。它被广泛用于以下场景：
- TouchOSC / Lemur移动控制台
- 展示控制系统（QLab、Watchout）
- 应用程序间同步（通过Max for Live连接Ableton，以及Resolume等）
...
**常见注意事项：** TD 仅会在该地址收到第一条消息后才会创建通道。因此，在设置过程中，请从控制器发送一条“hello”消息，或手动预先指定通道名称。

### 用于原始事件的 OSC In DAT 格式

当需要完整访问消息内容时（包括多个带类型的参数，以及包含括号或正则表达式的地址），请使用 `oscinDAT` 格式。
...
对于一次性发送的字符串或结构化消息，可使用 `oscoutDAT` 并调用 `.sendOSC(address, args)` 方法：
...
通过手机或平板电脑实现实时视频艺术控制的常用设置步骤：

1. **配置 TouchOSC 布局** — 为每个控制项分配对应的 OSC 地址，例如 `/vj/master`、`/vj/scene/1` 等。
2. **确定设备的局域网 IP 地址** — TouchOSC 需要指向该地址。
3. **TD 在 `oscinCHOP.par.port = 8000`（或其他端口）上监听数据**。
4. **通过表达式将通道映射到参数**：
...
局域网内的 OSC 功能可即插即用。对于多 TD 实例的同步（例如投影集群）：

- 一个 TD 作为**主节点**，通过 OSC 广播 `/sync/...` 消息；
- 其他工作 TD 运行 `oscinCHOP`，并在同一端口上监听；
- 主节点的 `oscoutCHOP.par.netaddress` 应设置为 UDP **广播地址**（例如 `192.168.1.255`），以此向所有对等设备发送消息。

若需要在广域网环境中保证稳定性，建议使用带有外部中继的 `webserverDAT` 或 `websocketDAT`——因为 UDP 的数据丢失往往难以被察觉。

---

## 常见问题与陷阱

1. **MIDI设备索引**——设备`0`指的是TD最先枚举到的那个设备。如果重新排序，其编号也可能会发生变化。尽可能通过设备名称来指定具体设备。
2. **OSC通道名称**——TD会在第一条消息到达时才会创建通道。新通道的出现会导致之前已生成的依赖项失效，从而引发一帧的延迟。
3. **OSC排队模式**——设置`par.queued = True`可将处理任务推迟到每帧单独批处理时再进行。虽然这样能降低延迟，但同一帧内到达的多条消息会被合并为最后一个值。该选项适用于触发器类控件，而连续型旋钮则建议设置为开启状态。
4. **MIDI时钟与传输同步**——`midiinCHOP`功能可在可用时报告时钟信号。若您的TD版本支持，可使用`midisyncCHOP`功能；否则也可通过计算时钟脉冲（每个四分音符有24个脉冲）来获取BPM值。
5. **延迟问题**——有线MIDI的延迟约为1-3毫秒；而WiFi OSC的延迟则在10-30毫秒之间，且存在抖动现象。对于需要精确同步的场景，建议使用有线连接。
6. **端口冲突**——在大多数操作系统中，仅有一个进程能够绑定某个UDP端口。如果`oscinCHOP`检测不到任何数据流，建议检查是否有其他应用程序（如Max、Ableton等）正在占用该端口。
...
| 目标 | 操作链路 |
|---|---|
| 旋钮 → 霓虹亮度 | `midiinCHOP` → 控制 `bloom.par.threshold` 的表达式 |
| 音符 → 场景切换 | `midiinCHOP` → `triggerCHOP` → `selectCHOP` → 驱动 `switchTOP.par.index` |
| 手机滑块 → 主音量推子 | TouchOSC `/master` → `oscinCHOP` → 控制输出端 `level.par.opacity` 的表达式 |
| 节拍器 → Resolume 场景触发 | `oscoutCHOP` 的通道 `composition/layers/1/clips/1/connect` → Resolume 在 7000 端口监听该信号 |
| 多投影仪同步 | 主节拍器的 `oscoutCHOP` 信号广播 → 各个工作节点的 `oscinCHOP` |
