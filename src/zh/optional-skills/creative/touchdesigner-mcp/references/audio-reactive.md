...
用于根据音频驱动视觉效果的多种模式——频谱分析、节拍检测、包络跟踪。
...
请使用 `audiofilterCHOP` 功能来实现频段分离（请勿使用按通道索引指定的 `selectCHOP`）：
...
- `mathCHOP` 中不存在 `par.clamp` 参数——请使用独立的 `limitCHOP` 参数
...
## 重要提示：请勿使用 Lag CHOP 进行频谱平滑处理

在时间切片模式下，Lag CHOP 会将 256 个样本的频谱扩展至 1600–2400 个样本，并将所有数值平均到接近零的水平（约 1e-06）。这样一来，着色器就无法获取任何可用数据。建议直接使用 `mathCHOP(gain=8)`，或通过带有反馈纹理的时间插值技术在 GLSL 中实现平滑处理。

验证结果：
- 未使用 Lag CHOP 时：低频段数值为 5.0–5.4（信号强劲，可正常使用）
- 使用 Lag CHOP 时：所有频段数值均为 0.000001（信号完全消失）
