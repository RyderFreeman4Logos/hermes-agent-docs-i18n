...
该技能中的 `comfyui_setup.sh` 脚本会自动选择最佳可用方案。首次运行时可能会询问是否启用分析功能，如需以非交互方式禁用该功能：
```bash
...
用于将原始的工作流 JSON 文件提交给正在运行的服务器。该功能**功能有限**——不支持参数注入，也无法下载结构化输出。如需为智能体使用此功能，请改用 `scripts/run_workflow.py`。
...
将 `uv-compile` 设为默认选项：`comfy manager uv-compile-default true`
...
存储内容包括：默认工作区、最近使用的工作区、后台服务器进程ID、API令牌、管理器GUI模式以及启动附加参数。
