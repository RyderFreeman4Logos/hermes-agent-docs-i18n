...
对于 `/api/prompt` 接口以及该技能中的所有脚本，均必须使用**API 格式**。
网页界面还会生成一种用于可视化编辑的“编辑器格式”，但该格式**无法**直接提交。
...
此类格式包含用于表示可视化图的 `nodes[]` 和 `links[]` 数组。如需转换，可在 ComfyUI 的网页界面中选择 **Workflow → Export (API)**（新版界面）或“Save (API Format)”按钮（旧版界面）。
...
链接为长度为2的数组，格式为`[upstream_node_id, output_slot]`。该技能的参数注入器会拒绝用字面值覆盖此类链接（此时会记录警告并跳过该操作）。
...
为区分正样本与负样本，该技能会通过Reroute/Primitive节点追溯到源节点`CLIPTextEncode`，并查询`KSampler.negative`字段。若无法获取相关信息，则会依据`_meta.title`中的关键词（如“negative”、“neg”、“anti”）进行判断。
...
ComfyUI 会扫描提示词文本，查找 `embedding:NAME` 这一语法结构。该技能的 `_common.iter_embedding_refs()` 函数会将这些嵌入模型作为依赖项提取出来。

```text
"a beautiful cat, embedding:goodvibes:1.2, embedding:art-style"
```

`extract_schema.py`与`check_deps.py`会将这些问题体现为`embedding_dependencies`/`missing_embeddings`。 
...
`scripts/extract_schema.py` 负责自动识别哪些节点 ID/字段对应着哪些面向用户的参数。该脚本会返回一个 `parameters` 字典，`run_workflow.py` 会读取该字典，并从 `--args` 参数中注入相应值。

## 可控参数的识别方法（启发式规则）

对于未知的工作流：

1. **提示词文本** — 任何形式的 `CLIPTextEncode.text`。可通过从 `KSampler.positive` / `.negative` 回溯连接关系来明确具体内容（切勿仅依赖元标题）。
2. **种子值** — `KSampler.seed` / `KSamplerAdvanced.noise_seed` / `RandomNoise.noise_seed`。
3. **尺寸** — `Empty*LatentImage.width/height`（数值必须为 8 的倍数）。
4. **步数 / CFG 值** — `KSampler.steps`、`KSampler.cfg`。通常步数为 20–50，CFG 值为 5–15（Flux 模型使用引导参数而非 CFG 值）。
5. **模型 / 检查点文件** — `CheckpointLoaderSimple.ckpt_name`。文件名必须与已安装的文件完全一致。
6. **LoRA 参数** — `LoraLoader.lora_name`、`.strength_model`。
7. **用于 img2img / inpaint 模式的图像** — `LoadImage.image`，即上传后的服务器端文件名。
8. **去噪强度** — `KSampler.denoise`，取值范围为 0.0–1.0：1.0 表示忽略输入图像，0.0 表示原样传递。对于 img2img 模式，最佳值为 0.4–0.7。

## 输出节点

输出结果由以下类型的节点生成。该技能所定义的 `OUTPUT_NODES` 集合还会扩展包含常见的社区插件包中的节点。

| 节点 | 输出键 | 内容 |
|------|-----------|---------|
| `SaveImage` | `images` | `{filename, subfolder, type}` 格式的列表 |
| `PreviewImage` | `images` | 临时预览版本（不会被保存） |
| `VHS_VideoCombine` | `gifs`（旧版本）或 `videos`/`video`（新云版本） | 视频文件引用 |
...
部分保存的 JSON 文件会将工作流封装在 `"prompt"` 键下（与该 `/api/prompt` 接口的数据结构一致）。该技能内置的 `_common.unwrap_workflow()` 函数可处理此类情况——可直接传入以下任意格式：

- 原始 API 格式：`{"3": {...}, "4": {...}}`
- 封装格式：`{"prompt": {"3": {...}}, "client_id": "..."}`

对于采用编辑器格式的输入，该函数会返回明确的错误信息并提示用户重新导出。
