...
- `official-cli.md` — 详尽列出所有 `comfy ...` 命令及其参数说明
- `rest-api.md` — REST 与 WebSocket 接口信息（本地及云端），以及数据载荷结构规范
- `workflow-format.md` — API 格式的 JSON 规则、常见节点类型及参数映射方式
- `template-integrity.md` — 将 `comfyui-workflow-templates` 从编辑器格式转换为 API 格式：路径重定向处理、带点的动态输入键名（如 `values.a`、`resize_type.width`）、云端使用注意事项（302 重定向、免费套餐单次仅可运行 1 个任务、1080p 分辨率对应的显存限制），以及兼容 Discord 的 ffmpeg 合成功能。该文档由 [@purzbeats](https://github.com/purzbeats) 撰写。每当从官方模板开始使用时，建议查阅此文档。
...
- 用户希望串联多个生成步骤（如 txt2img → upscale → face restore）。  
- 用户需要使用 ControlNet、 inpainting、img2img 或其他高级处理流程。  
- 用户希望管理 ComfyUI 队列、检查模型或安装自定义节点。  
...
| `marginal` | SD1.5 模型可用；SDXL 模型运行较吃力；Flux/视频模型基本不可用 | 轻量级工作流可本地运行，否则请选择**方案A：云端部署** |
| `cloud`    | 无可用 GPU，显存<6 GB，Apple Silicon 芯片统一内存<16 GB，使用 Intel Mac 或 Rosetta Python 环境 | 除非用户明确要求本地安装，否则应**切换至云端部署** |

该脚本还会显示 `wsl: true`（通过 NVIDIA passthrough 技术在 WSL2 环境中运行）以及 `rosetta: true`（在 Apple Silicon 芯片上使用 x86_64 版 Python——此类系统需重新安装为 ARM64 版本）。

如果判定结果为 `cloud`，但用户仍希望本地安装，则不应默许操作。应原样显示 `notes` 数组中的提示信息，并询问用户是选择(a)切换至云端，还是(b)强行进行本地安装（在现代模型上这会导致内存不足或运行速度极慢）。

### 选择安装方案

首先进行硬件检测。当用户已告知其硬件配置时，可参考下表选择相应的方案作为备选。
