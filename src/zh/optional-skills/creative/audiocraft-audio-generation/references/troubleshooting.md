...
**错误提示**：在模型加载过程中出现 `torch.cuda.OutOfMemoryError` 错误
...
| 错误信息 | 原因 | 解决方案 |
|---------|------|----------|
| `CUDA out of memory` | 模型过大 | 使用更小的模型，缩短运行时长 |
...
| `KeyError: 'melody'` | 用于生成旋律的模型不正确 | 使用 musicgen-melody 模型 |
