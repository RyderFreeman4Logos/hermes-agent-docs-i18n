...
> **重要提示：** 所有位于形状和箭头上的文本标签均需使用容器绑定机制（`containerId` + `boundElements`）来设置。  
> 请勿使用并不存在的 `"label"` 属性——该属性将被直接忽略，从而导致对应的形状显示为空白。  

---

## 示例 1：两个相互连接且带有标签的框

这是一个包含两个框以及它们之间一条箭头的最简流程图示例。
...
- **切勿使用 `"label"` 属性**——这是最常见的错误。该属性并非 Excalidraw 文件格式的一部分，系统会直接忽略它，从而导致生成的图形为空白且没有可见文字。请始终按照上述示例所示，使用容器绑定方式（`containerId` + `boundElements`）。
- **每个被绑定的文本都需要双向关联**——既需要图形包含 `boundElements: [{"id": "t_xxx", "type": "text"}]`，文本也需要设置 `containerId: "shape_id"`。缺少任意一项，绑定功能都无法正常工作。
- **所有文本元素都需设置 `originalText` 和 `autoResize: true`**——Excalidraw 会利用这些参数来实现文本的自动换行。
- **所有文本元素都需设置 `fontFamily: 1`**——如果不设置此参数，文本可能无法以预期的手写风格显示。
- **当 y 坐标相近时元素会发生重叠**——务必检查文本、矩形框和标签是否叠在一起。
- **箭头标签需要预留空间**——像“ATP + NADPH”这样的长标签会超出短箭头的显示范围。建议缩短标签长度或加宽箭头。
- **标题需相对于图表居中**——先估算出图表的总体宽度，再将标题文本置于其中心位置。
- **装饰元素应最后绘制**——那些可爱的插图（如太阳、星星、图标）应放在数组的最后，这样它们才能显示在所有内容之上。
