...
若要创建深色主题的图表，需将一个巨大的深色背景矩形作为数组中的**第一个元素**。该矩形的尺寸应足够大，能够覆盖整个视图区域：
...
建议使用主颜色方案中的标准主色调——这些颜色在深色背景下仍具有足够的亮度：
- 蓝色 `#4a9eed`、琥珀色 `#f59e0b`、绿色 `#22c55e`、红色 `#ef4444`、紫色 `#8b5cf6`

若需要较为柔和的形状边框，可使用 `#555555`。

## 示例：深色模式下的带标签矩形

请使用容器绑定功能（而非无效的 `"label"` 属性）来实现。在深色背景上，将文本的 `strokeColor` 设置为 `"#e5e5e5"`，以确保文字可见。
...
注意：对于显示在深色背景上的独立文本元素，务必明确设置 `"strokeColor": "#e5e5e5"`。默认值 `#1e1e1e` 在深色背景下是不可见的。
